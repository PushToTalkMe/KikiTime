function Np(e, t) {
  for (var n = 0; n < t.length; n++) {
    const r = t[n];
    if (typeof r != "string" && !Array.isArray(r)) {
      for (const a in r)
        if (a !== "default" && !(a in e)) {
          const i = Object.getOwnPropertyDescriptor(r, a);
          i &&
            Object.defineProperty(
              e,
              a,
              i.get ? i : { enumerable: !0, get: () => r[a] }
            );
        }
    }
  }
  return Object.freeze(
    Object.defineProperty(e, Symbol.toStringTag, { value: "Module" })
  );
}
(function () {
  const t = document.createElement("link").relList;
  if (t && t.supports && t.supports("modulepreload")) return;
  for (const a of document.querySelectorAll('link[rel="modulepreload"]')) r(a);
  new MutationObserver((a) => {
    for (const i of a)
      if (i.type === "childList")
        for (const o of i.addedNodes)
          o.tagName === "LINK" && o.rel === "modulepreload" && r(o);
  }).observe(document, { childList: !0, subtree: !0 });
  function n(a) {
    const i = {};
    return (
      a.integrity && (i.integrity = a.integrity),
      a.referrerpolicy && (i.referrerPolicy = a.referrerpolicy),
      a.crossorigin === "use-credentials"
        ? (i.credentials = "include")
        : a.crossorigin === "anonymous"
        ? (i.credentials = "omit")
        : (i.credentials = "same-origin"),
      i
    );
  }
  function r(a) {
    if (a.ep) return;
    a.ep = !0;
    const i = n(a);
    fetch(a.href, i);
  }
})();
function Op(e) {
  return e && e.__esModule && Object.prototype.hasOwnProperty.call(e, "default")
    ? e.default
    : e;
}
function Rp(e) {
  if (e.__esModule) return e;
  var t = e.default;
  if (typeof t == "function") {
    var n = function r() {
      if (this instanceof r) {
        var a = [null];
        a.push.apply(a, arguments);
        var i = Function.bind.apply(t, a);
        return new i();
      }
      return t.apply(this, arguments);
    };
    n.prototype = t.prototype;
  } else n = {};
  return (
    Object.defineProperty(n, "__esModule", { value: !0 }),
    Object.keys(e).forEach(function (r) {
      var a = Object.getOwnPropertyDescriptor(e, r);
      Object.defineProperty(
        n,
        r,
        a.get
          ? a
          : {
              enumerable: !0,
              get: function () {
                return e[r];
              },
            }
      );
    }),
    n
  );
}
var lr = {},
  Lp = {
    get exports() {
      return lr;
    },
    set exports(e) {
      lr = e;
    },
  },
  Qa = {},
  S = {},
  Mp = {
    get exports() {
      return S;
    },
    set exports(e) {
      S = e;
    },
  },
  D = {};
/**
 * @license React
 * react.production.min.js
 *
 * Copyright (c) Facebook, Inc. and its affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */ var Nr = Symbol.for("react.element"),
  Dp = Symbol.for("react.portal"),
  Ip = Symbol.for("react.fragment"),
  zp = Symbol.for("react.strict_mode"),
  Up = Symbol.for("react.profiler"),
  Ap = Symbol.for("react.provider"),
  Fp = Symbol.for("react.context"),
  Bp = Symbol.for("react.forward_ref"),
  Wp = Symbol.for("react.suspense"),
  Hp = Symbol.for("react.memo"),
  bp = Symbol.for("react.lazy"),
  yu = Symbol.iterator;
function Vp(e) {
  return e === null || typeof e != "object"
    ? null
    : ((e = (yu && e[yu]) || e["@@iterator"]),
      typeof e == "function" ? e : null);
}
var qs = {
    isMounted: function () {
      return !1;
    },
    enqueueForceUpdate: function () {},
    enqueueReplaceState: function () {},
    enqueueSetState: function () {},
  },
  Zs = Object.assign,
  ec = {};
function Mn(e, t, n) {
  (this.props = e),
    (this.context = t),
    (this.refs = ec),
    (this.updater = n || qs);
}
Mn.prototype.isReactComponent = {};
Mn.prototype.setState = function (e, t) {
  if (typeof e != "object" && typeof e != "function" && e != null)
    throw Error(
      "setState(...): takes an object of state variables to update or a function which returns an object of state variables."
    );
  this.updater.enqueueSetState(this, e, t, "setState");
};
Mn.prototype.forceUpdate = function (e) {
  this.updater.enqueueForceUpdate(this, e, "forceUpdate");
};
function tc() {}
tc.prototype = Mn.prototype;
function vl(e, t, n) {
  (this.props = e),
    (this.context = t),
    (this.refs = ec),
    (this.updater = n || qs);
}
var hl = (vl.prototype = new tc());
hl.constructor = vl;
Zs(hl, Mn.prototype);
hl.isPureReactComponent = !0;
var wu = Array.isArray,
  nc = Object.prototype.hasOwnProperty,
  _l = { current: null },
  rc = { key: !0, ref: !0, __self: !0, __source: !0 };
function ac(e, t, n) {
  var r,
    a = {},
    i = null,
    o = null;
  if (t != null)
    for (r in (t.ref !== void 0 && (o = t.ref),
    t.key !== void 0 && (i = "" + t.key),
    t))
      nc.call(t, r) && !rc.hasOwnProperty(r) && (a[r] = t[r]);
  var l = arguments.length - 2;
  if (l === 1) a.children = n;
  else if (1 < l) {
    for (var u = Array(l), s = 0; s < l; s++) u[s] = arguments[s + 2];
    a.children = u;
  }
  if (e && e.defaultProps)
    for (r in ((l = e.defaultProps), l)) a[r] === void 0 && (a[r] = l[r]);
  return {
    $$typeof: Nr,
    type: e,
    key: i,
    ref: o,
    props: a,
    _owner: _l.current,
  };
}
function Yp(e, t) {
  return {
    $$typeof: Nr,
    type: e.type,
    key: t,
    ref: e.ref,
    props: e.props,
    _owner: e._owner,
  };
}
function ml(e) {
  return typeof e == "object" && e !== null && e.$$typeof === Nr;
}
function Gp(e) {
  var t = { "=": "=0", ":": "=2" };
  return (
    "$" +
    e.replace(/[=:]/g, function (n) {
      return t[n];
    })
  );
}
var Su = /\/+/g;
function Di(e, t) {
  return typeof e == "object" && e !== null && e.key != null
    ? Gp("" + e.key)
    : t.toString(36);
}
function ra(e, t, n, r, a) {
  var i = typeof e;
  (i === "undefined" || i === "boolean") && (e = null);
  var o = !1;
  if (e === null) o = !0;
  else
    switch (i) {
      case "string":
      case "number":
        o = !0;
        break;
      case "object":
        switch (e.$$typeof) {
          case Nr:
          case Dp:
            o = !0;
        }
    }
  if (o)
    return (
      (o = e),
      (a = a(o)),
      (e = r === "" ? "." + Di(o, 0) : r),
      wu(a)
        ? ((n = ""),
          e != null && (n = e.replace(Su, "$&/") + "/"),
          ra(a, t, n, "", function (s) {
            return s;
          }))
        : a != null &&
          (ml(a) &&
            (a = Yp(
              a,
              n +
                (!a.key || (o && o.key === a.key)
                  ? ""
                  : ("" + a.key).replace(Su, "$&/") + "/") +
                e
            )),
          t.push(a)),
      1
    );
  if (((o = 0), (r = r === "" ? "." : r + ":"), wu(e)))
    for (var l = 0; l < e.length; l++) {
      i = e[l];
      var u = r + Di(i, l);
      o += ra(i, t, n, u, a);
    }
  else if (((u = Vp(e)), typeof u == "function"))
    for (e = u.call(e), l = 0; !(i = e.next()).done; )
      (i = i.value), (u = r + Di(i, l++)), (o += ra(i, t, n, u, a));
  else if (i === "object")
    throw (
      ((t = String(e)),
      Error(
        "Objects are not valid as a React child (found: " +
          (t === "[object Object]"
            ? "object with keys {" + Object.keys(e).join(", ") + "}"
            : t) +
          "). If you meant to render a collection of children, use an array instead."
      ))
    );
  return o;
}
function Br(e, t, n) {
  if (e == null) return e;
  var r = [],
    a = 0;
  return (
    ra(e, r, "", "", function (i) {
      return t.call(n, i, a++);
    }),
    r
  );
}
function Qp(e) {
  if (e._status === -1) {
    var t = e._result;
    (t = t()),
      t.then(
        function (n) {
          (e._status === 0 || e._status === -1) &&
            ((e._status = 1), (e._result = n));
        },
        function (n) {
          (e._status === 0 || e._status === -1) &&
            ((e._status = 2), (e._result = n));
        }
      ),
      e._status === -1 && ((e._status = 0), (e._result = t));
  }
  if (e._status === 1) return e._result.default;
  throw e._result;
}
var _e = { current: null },
  aa = { transition: null },
  $p = {
    ReactCurrentDispatcher: _e,
    ReactCurrentBatchConfig: aa,
    ReactCurrentOwner: _l,
  };
D.Children = {
  map: Br,
  forEach: function (e, t, n) {
    Br(
      e,
      function () {
        t.apply(this, arguments);
      },
      n
    );
  },
  count: function (e) {
    var t = 0;
    return (
      Br(e, function () {
        t++;
      }),
      t
    );
  },
  toArray: function (e) {
    return (
      Br(e, function (t) {
        return t;
      }) || []
    );
  },
  only: function (e) {
    if (!ml(e))
      throw Error(
        "React.Children.only expected to receive a single React element child."
      );
    return e;
  },
};
D.Component = Mn;
D.Fragment = Ip;
D.Profiler = Up;
D.PureComponent = vl;
D.StrictMode = zp;
D.Suspense = Wp;
D.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED = $p;
D.cloneElement = function (e, t, n) {
  if (e == null)
    throw Error(
      "React.cloneElement(...): The argument must be a React element, but you passed " +
        e +
        "."
    );
  var r = Zs({}, e.props),
    a = e.key,
    i = e.ref,
    o = e._owner;
  if (t != null) {
    if (
      (t.ref !== void 0 && ((i = t.ref), (o = _l.current)),
      t.key !== void 0 && (a = "" + t.key),
      e.type && e.type.defaultProps)
    )
      var l = e.type.defaultProps;
    for (u in t)
      nc.call(t, u) &&
        !rc.hasOwnProperty(u) &&
        (r[u] = t[u] === void 0 && l !== void 0 ? l[u] : t[u]);
  }
  var u = arguments.length - 2;
  if (u === 1) r.children = n;
  else if (1 < u) {
    l = Array(u);
    for (var s = 0; s < u; s++) l[s] = arguments[s + 2];
    r.children = l;
  }
  return { $$typeof: Nr, type: e.type, key: a, ref: i, props: r, _owner: o };
};
D.createContext = function (e) {
  return (
    (e = {
      $$typeof: Fp,
      _currentValue: e,
      _currentValue2: e,
      _threadCount: 0,
      Provider: null,
      Consumer: null,
      _defaultValue: null,
      _globalName: null,
    }),
    (e.Provider = { $$typeof: Ap, _context: e }),
    (e.Consumer = e)
  );
};
D.createElement = ac;
D.createFactory = function (e) {
  var t = ac.bind(null, e);
  return (t.type = e), t;
};
D.createRef = function () {
  return { current: null };
};
D.forwardRef = function (e) {
  return { $$typeof: Bp, render: e };
};
D.isValidElement = ml;
D.lazy = function (e) {
  return { $$typeof: bp, _payload: { _status: -1, _result: e }, _init: Qp };
};
D.memo = function (e, t) {
  return { $$typeof: Hp, type: e, compare: t === void 0 ? null : t };
};
D.startTransition = function (e) {
  var t = aa.transition;
  aa.transition = {};
  try {
    e();
  } finally {
    aa.transition = t;
  }
};
D.unstable_act = function () {
  throw Error("act(...) is not supported in production builds of React.");
};
D.useCallback = function (e, t) {
  return _e.current.useCallback(e, t);
};
D.useContext = function (e) {
  return _e.current.useContext(e);
};
D.useDebugValue = function () {};
D.useDeferredValue = function (e) {
  return _e.current.useDeferredValue(e);
};
D.useEffect = function (e, t) {
  return _e.current.useEffect(e, t);
};
D.useId = function () {
  return _e.current.useId();
};
D.useImperativeHandle = function (e, t, n) {
  return _e.current.useImperativeHandle(e, t, n);
};
D.useInsertionEffect = function (e, t) {
  return _e.current.useInsertionEffect(e, t);
};
D.useLayoutEffect = function (e, t) {
  return _e.current.useLayoutEffect(e, t);
};
D.useMemo = function (e, t) {
  return _e.current.useMemo(e, t);
};
D.useReducer = function (e, t, n) {
  return _e.current.useReducer(e, t, n);
};
D.useRef = function (e) {
  return _e.current.useRef(e);
};
D.useState = function (e) {
  return _e.current.useState(e);
};
D.useSyncExternalStore = function (e, t, n) {
  return _e.current.useSyncExternalStore(e, t, n);
};
D.useTransition = function () {
  return _e.current.useTransition();
};
D.version = "18.2.0";
(function (e) {
  e.exports = D;
})(Mp);
const yl = Op(S),
  fo = Np({ __proto__: null, default: yl }, [S]);
/**
 * @license React
 * react-jsx-runtime.production.min.js
 *
 * Copyright (c) Facebook, Inc. and its affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */ var Kp = S,
  Xp = Symbol.for("react.element"),
  Jp = Symbol.for("react.fragment"),
  qp = Object.prototype.hasOwnProperty,
  Zp = Kp.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED.ReactCurrentOwner,
  ef = { key: !0, ref: !0, __self: !0, __source: !0 };
function ic(e, t, n) {
  var r,
    a = {},
    i = null,
    o = null;
  n !== void 0 && (i = "" + n),
    t.key !== void 0 && (i = "" + t.key),
    t.ref !== void 0 && (o = t.ref);
  for (r in t) qp.call(t, r) && !ef.hasOwnProperty(r) && (a[r] = t[r]);
  if (e && e.defaultProps)
    for (r in ((t = e.defaultProps), t)) a[r] === void 0 && (a[r] = t[r]);
  return {
    $$typeof: Xp,
    type: e,
    key: i,
    ref: o,
    props: a,
    _owner: Zp.current,
  };
}
Qa.Fragment = Jp;
Qa.jsx = ic;
Qa.jsxs = ic;
(function (e) {
  e.exports = Qa;
})(Lp);
const tf = lr.Fragment,
  w = lr.jsx,
  O = lr.jsxs;
var go = {},
  _a = {},
  nf = {
    get exports() {
      return _a;
    },
    set exports(e) {
      _a = e;
    },
  },
  Oe = {},
  vo = {},
  rf = {
    get exports() {
      return vo;
    },
    set exports(e) {
      vo = e;
    },
  },
  oc = {};
/**
 * @license React
 * scheduler.production.min.js
 *
 * Copyright (c) Facebook, Inc. and its affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */ (function (e) {
  function t(T, R) {
    var M = T.length;
    T.push(R);
    e: for (; 0 < M; ) {
      var K = (M - 1) >>> 1,
        te = T[K];
      if (0 < a(te, R)) (T[K] = R), (T[M] = te), (M = K);
      else break e;
    }
  }
  function n(T) {
    return T.length === 0 ? null : T[0];
  }
  function r(T) {
    if (T.length === 0) return null;
    var R = T[0],
      M = T.pop();
    if (M !== R) {
      T[0] = M;
      e: for (var K = 0, te = T.length, Ar = te >>> 1; K < Ar; ) {
        var Ut = 2 * (K + 1) - 1,
          Mi = T[Ut],
          At = Ut + 1,
          Fr = T[At];
        if (0 > a(Mi, M))
          At < te && 0 > a(Fr, Mi)
            ? ((T[K] = Fr), (T[At] = M), (K = At))
            : ((T[K] = Mi), (T[Ut] = M), (K = Ut));
        else if (At < te && 0 > a(Fr, M)) (T[K] = Fr), (T[At] = M), (K = At);
        else break e;
      }
    }
    return R;
  }
  function a(T, R) {
    var M = T.sortIndex - R.sortIndex;
    return M !== 0 ? M : T.id - R.id;
  }
  if (typeof performance == "object" && typeof performance.now == "function") {
    var i = performance;
    e.unstable_now = function () {
      return i.now();
    };
  } else {
    var o = Date,
      l = o.now();
    e.unstable_now = function () {
      return o.now() - l;
    };
  }
  var u = [],
    s = [],
    p = 1,
    f = null,
    v = 3,
    _ = !1,
    m = !1,
    h = !1,
    E = typeof setTimeout == "function" ? setTimeout : null,
    d = typeof clearTimeout == "function" ? clearTimeout : null,
    c = typeof setImmediate < "u" ? setImmediate : null;
  typeof navigator < "u" &&
    navigator.scheduling !== void 0 &&
    navigator.scheduling.isInputPending !== void 0 &&
    navigator.scheduling.isInputPending.bind(navigator.scheduling);
  function g(T) {
    for (var R = n(s); R !== null; ) {
      if (R.callback === null) r(s);
      else if (R.startTime <= T)
        r(s), (R.sortIndex = R.expirationTime), t(u, R);
      else break;
      R = n(s);
    }
  }
  function y(T) {
    if (((h = !1), g(T), !m))
      if (n(u) !== null) (m = !0), Me(j);
      else {
        var R = n(s);
        R !== null && ct(y, R.startTime - T);
      }
  }
  function j(T, R) {
    (m = !1), h && ((h = !1), d(N), (N = -1)), (_ = !0);
    var M = v;
    try {
      for (
        g(R), f = n(u);
        f !== null && (!(f.expirationTime > R) || (T && !Ce()));

      ) {
        var K = f.callback;
        if (typeof K == "function") {
          (f.callback = null), (v = f.priorityLevel);
          var te = K(f.expirationTime <= R);
          (R = e.unstable_now()),
            typeof te == "function" ? (f.callback = te) : f === n(u) && r(u),
            g(R);
        } else r(u);
        f = n(u);
      }
      if (f !== null) var Ar = !0;
      else {
        var Ut = n(s);
        Ut !== null && ct(y, Ut.startTime - R), (Ar = !1);
      }
      return Ar;
    } finally {
      (f = null), (v = M), (_ = !1);
    }
  }
  var x = !1,
    P = null,
    N = -1,
    W = 5,
    L = -1;
  function Ce() {
    return !(e.unstable_now() - L < W);
  }
  function It() {
    if (P !== null) {
      var T = e.unstable_now();
      L = T;
      var R = !0;
      try {
        R = P(!0, T);
      } finally {
        R ? zt() : ((x = !1), (P = null));
      }
    } else x = !1;
  }
  var zt;
  if (typeof c == "function")
    zt = function () {
      c(It);
    };
  else if (typeof MessageChannel < "u") {
    var Ur = new MessageChannel(),
      ge = Ur.port2;
    (Ur.port1.onmessage = It),
      (zt = function () {
        ge.postMessage(null);
      });
  } else
    zt = function () {
      E(It, 0);
    };
  function Me(T) {
    (P = T), x || ((x = !0), zt());
  }
  function ct(T, R) {
    N = E(function () {
      T(e.unstable_now());
    }, R);
  }
  (e.unstable_IdlePriority = 5),
    (e.unstable_ImmediatePriority = 1),
    (e.unstable_LowPriority = 4),
    (e.unstable_NormalPriority = 3),
    (e.unstable_Profiling = null),
    (e.unstable_UserBlockingPriority = 2),
    (e.unstable_cancelCallback = function (T) {
      T.callback = null;
    }),
    (e.unstable_continueExecution = function () {
      m || _ || ((m = !0), Me(j));
    }),
    (e.unstable_forceFrameRate = function (T) {
      0 > T || 125 < T
        ? console.error(
            "forceFrameRate takes a positive int between 0 and 125, forcing frame rates higher than 125 fps is not supported"
          )
        : (W = 0 < T ? Math.floor(1e3 / T) : 5);
    }),
    (e.unstable_getCurrentPriorityLevel = function () {
      return v;
    }),
    (e.unstable_getFirstCallbackNode = function () {
      return n(u);
    }),
    (e.unstable_next = function (T) {
      switch (v) {
        case 1:
        case 2:
        case 3:
          var R = 3;
          break;
        default:
          R = v;
      }
      var M = v;
      v = R;
      try {
        return T();
      } finally {
        v = M;
      }
    }),
    (e.unstable_pauseExecution = function () {}),
    (e.unstable_requestPaint = function () {}),
    (e.unstable_runWithPriority = function (T, R) {
      switch (T) {
        case 1:
        case 2:
        case 3:
        case 4:
        case 5:
          break;
        default:
          T = 3;
      }
      var M = v;
      v = T;
      try {
        return R();
      } finally {
        v = M;
      }
    }),
    (e.unstable_scheduleCallback = function (T, R, M) {
      var K = e.unstable_now();
      switch (
        (typeof M == "object" && M !== null
          ? ((M = M.delay), (M = typeof M == "number" && 0 < M ? K + M : K))
          : (M = K),
        T)
      ) {
        case 1:
          var te = -1;
          break;
        case 2:
          te = 250;
          break;
        case 5:
          te = 1073741823;
          break;
        case 4:
          te = 1e4;
          break;
        default:
          te = 5e3;
      }
      return (
        (te = M + te),
        (T = {
          id: p++,
          callback: R,
          priorityLevel: T,
          startTime: M,
          expirationTime: te,
          sortIndex: -1,
        }),
        M > K
          ? ((T.sortIndex = M),
            t(s, T),
            n(u) === null &&
              T === n(s) &&
              (h ? (d(N), (N = -1)) : (h = !0), ct(y, M - K)))
          : ((T.sortIndex = te), t(u, T), m || _ || ((m = !0), Me(j))),
        T
      );
    }),
    (e.unstable_shouldYield = Ce),
    (e.unstable_wrapCallback = function (T) {
      var R = v;
      return function () {
        var M = v;
        v = R;
        try {
          return T.apply(this, arguments);
        } finally {
          v = M;
        }
      };
    });
})(oc);
(function (e) {
  e.exports = oc;
})(rf);
/**
 * @license React
 * react-dom.production.min.js
 *
 * Copyright (c) Facebook, Inc. and its affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */ var lc = S,
  Ne = vo;
function k(e) {
  for (
    var t = "https://reactjs.org/docs/error-decoder.html?invariant=" + e, n = 1;
    n < arguments.length;
    n++
  )
    t += "&args[]=" + encodeURIComponent(arguments[n]);
  return (
    "Minified React error #" +
    e +
    "; visit " +
    t +
    " for the full message or use the non-minified dev environment for full errors and additional helpful warnings."
  );
}
var uc = new Set(),
  ur = {};
function tn(e, t) {
  Cn(e, t), Cn(e + "Capture", t);
}
function Cn(e, t) {
  for (ur[e] = t, e = 0; e < t.length; e++) uc.add(t[e]);
}
var it = !(
    typeof window > "u" ||
    typeof window.document > "u" ||
    typeof window.document.createElement > "u"
  ),
  ho = Object.prototype.hasOwnProperty,
  af =
    /^[:A-Z_a-z\u00C0-\u00D6\u00D8-\u00F6\u00F8-\u02FF\u0370-\u037D\u037F-\u1FFF\u200C-\u200D\u2070-\u218F\u2C00-\u2FEF\u3001-\uD7FF\uF900-\uFDCF\uFDF0-\uFFFD][:A-Z_a-z\u00C0-\u00D6\u00D8-\u00F6\u00F8-\u02FF\u0370-\u037D\u037F-\u1FFF\u200C-\u200D\u2070-\u218F\u2C00-\u2FEF\u3001-\uD7FF\uF900-\uFDCF\uFDF0-\uFFFD\-.0-9\u00B7\u0300-\u036F\u203F-\u2040]*$/,
  ku = {},
  Eu = {};
function of(e) {
  return ho.call(Eu, e)
    ? !0
    : ho.call(ku, e)
    ? !1
    : af.test(e)
    ? (Eu[e] = !0)
    : ((ku[e] = !0), !1);
}
function lf(e, t, n, r) {
  if (n !== null && n.type === 0) return !1;
  switch (typeof t) {
    case "function":
    case "symbol":
      return !0;
    case "boolean":
      return r
        ? !1
        : n !== null
        ? !n.acceptsBooleans
        : ((e = e.toLowerCase().slice(0, 5)), e !== "data-" && e !== "aria-");
    default:
      return !1;
  }
}
function uf(e, t, n, r) {
  if (t === null || typeof t > "u" || lf(e, t, n, r)) return !0;
  if (r) return !1;
  if (n !== null)
    switch (n.type) {
      case 3:
        return !t;
      case 4:
        return t === !1;
      case 5:
        return isNaN(t);
      case 6:
        return isNaN(t) || 1 > t;
    }
  return !1;
}
function me(e, t, n, r, a, i, o) {
  (this.acceptsBooleans = t === 2 || t === 3 || t === 4),
    (this.attributeName = r),
    (this.attributeNamespace = a),
    (this.mustUseProperty = n),
    (this.propertyName = e),
    (this.type = t),
    (this.sanitizeURL = i),
    (this.removeEmptyString = o);
}
var ue = {};
"children dangerouslySetInnerHTML defaultValue defaultChecked innerHTML suppressContentEditableWarning suppressHydrationWarning style"
  .split(" ")
  .forEach(function (e) {
    ue[e] = new me(e, 0, !1, e, null, !1, !1);
  });
[
  ["acceptCharset", "accept-charset"],
  ["className", "class"],
  ["htmlFor", "for"],
  ["httpEquiv", "http-equiv"],
].forEach(function (e) {
  var t = e[0];
  ue[t] = new me(t, 1, !1, e[1], null, !1, !1);
});
["contentEditable", "draggable", "spellCheck", "value"].forEach(function (e) {
  ue[e] = new me(e, 2, !1, e.toLowerCase(), null, !1, !1);
});
[
  "autoReverse",
  "externalResourcesRequired",
  "focusable",
  "preserveAlpha",
].forEach(function (e) {
  ue[e] = new me(e, 2, !1, e, null, !1, !1);
});
"allowFullScreen async autoFocus autoPlay controls default defer disabled disablePictureInPicture disableRemotePlayback formNoValidate hidden loop noModule noValidate open playsInline readOnly required reversed scoped seamless itemScope"
  .split(" ")
  .forEach(function (e) {
    ue[e] = new me(e, 3, !1, e.toLowerCase(), null, !1, !1);
  });
["checked", "multiple", "muted", "selected"].forEach(function (e) {
  ue[e] = new me(e, 3, !0, e, null, !1, !1);
});
["capture", "download"].forEach(function (e) {
  ue[e] = new me(e, 4, !1, e, null, !1, !1);
});
["cols", "rows", "size", "span"].forEach(function (e) {
  ue[e] = new me(e, 6, !1, e, null, !1, !1);
});
["rowSpan", "start"].forEach(function (e) {
  ue[e] = new me(e, 5, !1, e.toLowerCase(), null, !1, !1);
});
var wl = /[\-:]([a-z])/g;
function Sl(e) {
  return e[1].toUpperCase();
}
"accent-height alignment-baseline arabic-form baseline-shift cap-height clip-path clip-rule color-interpolation color-interpolation-filters color-profile color-rendering dominant-baseline enable-background fill-opacity fill-rule flood-color flood-opacity font-family font-size font-size-adjust font-stretch font-style font-variant font-weight glyph-name glyph-orientation-horizontal glyph-orientation-vertical horiz-adv-x horiz-origin-x image-rendering letter-spacing lighting-color marker-end marker-mid marker-start overline-position overline-thickness paint-order panose-1 pointer-events rendering-intent shape-rendering stop-color stop-opacity strikethrough-position strikethrough-thickness stroke-dasharray stroke-dashoffset stroke-linecap stroke-linejoin stroke-miterlimit stroke-opacity stroke-width text-anchor text-decoration text-rendering underline-position underline-thickness unicode-bidi unicode-range units-per-em v-alphabetic v-hanging v-ideographic v-mathematical vector-effect vert-adv-y vert-origin-x vert-origin-y word-spacing writing-mode xmlns:xlink x-height"
  .split(" ")
  .forEach(function (e) {
    var t = e.replace(wl, Sl);
    ue[t] = new me(t, 1, !1, e, null, !1, !1);
  });
"xlink:actuate xlink:arcrole xlink:role xlink:show xlink:title xlink:type"
  .split(" ")
  .forEach(function (e) {
    var t = e.replace(wl, Sl);
    ue[t] = new me(t, 1, !1, e, "http://www.w3.org/1999/xlink", !1, !1);
  });
["xml:base", "xml:lang", "xml:space"].forEach(function (e) {
  var t = e.replace(wl, Sl);
  ue[t] = new me(t, 1, !1, e, "http://www.w3.org/XML/1998/namespace", !1, !1);
});
["tabIndex", "crossOrigin"].forEach(function (e) {
  ue[e] = new me(e, 1, !1, e.toLowerCase(), null, !1, !1);
});
ue.xlinkHref = new me(
  "xlinkHref",
  1,
  !1,
  "xlink:href",
  "http://www.w3.org/1999/xlink",
  !0,
  !1
);
["src", "href", "action", "formAction"].forEach(function (e) {
  ue[e] = new me(e, 1, !1, e.toLowerCase(), null, !0, !0);
});
function kl(e, t, n, r) {
  var a = ue.hasOwnProperty(t) ? ue[t] : null;
  (a !== null
    ? a.type !== 0
    : r ||
      !(2 < t.length) ||
      (t[0] !== "o" && t[0] !== "O") ||
      (t[1] !== "n" && t[1] !== "N")) &&
    (uf(t, n, a, r) && (n = null),
    r || a === null
      ? of(t) && (n === null ? e.removeAttribute(t) : e.setAttribute(t, "" + n))
      : a.mustUseProperty
      ? (e[a.propertyName] = n === null ? (a.type === 3 ? !1 : "") : n)
      : ((t = a.attributeName),
        (r = a.attributeNamespace),
        n === null
          ? e.removeAttribute(t)
          : ((a = a.type),
            (n = a === 3 || (a === 4 && n === !0) ? "" : "" + n),
            r ? e.setAttributeNS(r, t, n) : e.setAttribute(t, n))));
}
var st = lc.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED,
  Wr = Symbol.for("react.element"),
  ln = Symbol.for("react.portal"),
  un = Symbol.for("react.fragment"),
  El = Symbol.for("react.strict_mode"),
  _o = Symbol.for("react.profiler"),
  sc = Symbol.for("react.provider"),
  cc = Symbol.for("react.context"),
  jl = Symbol.for("react.forward_ref"),
  mo = Symbol.for("react.suspense"),
  yo = Symbol.for("react.suspense_list"),
  Cl = Symbol.for("react.memo"),
  ft = Symbol.for("react.lazy"),
  dc = Symbol.for("react.offscreen"),
  ju = Symbol.iterator;
function An(e) {
  return e === null || typeof e != "object"
    ? null
    : ((e = (ju && e[ju]) || e["@@iterator"]),
      typeof e == "function" ? e : null);
}
var Q = Object.assign,
  Ii;
function $n(e) {
  if (Ii === void 0)
    try {
      throw Error();
    } catch (n) {
      var t = n.stack.trim().match(/\n( *(at )?)/);
      Ii = (t && t[1]) || "";
    }
  return (
    `
` +
    Ii +
    e
  );
}
var zi = !1;
function Ui(e, t) {
  if (!e || zi) return "";
  zi = !0;
  var n = Error.prepareStackTrace;
  Error.prepareStackTrace = void 0;
  try {
    if (t)
      if (
        ((t = function () {
          throw Error();
        }),
        Object.defineProperty(t.prototype, "props", {
          set: function () {
            throw Error();
          },
        }),
        typeof Reflect == "object" && Reflect.construct)
      ) {
        try {
          Reflect.construct(t, []);
        } catch (s) {
          var r = s;
        }
        Reflect.construct(e, [], t);
      } else {
        try {
          t.call();
        } catch (s) {
          r = s;
        }
        e.call(t.prototype);
      }
    else {
      try {
        throw Error();
      } catch (s) {
        r = s;
      }
      e();
    }
  } catch (s) {
    if (s && r && typeof s.stack == "string") {
      for (
        var a = s.stack.split(`
`),
          i = r.stack.split(`
`),
          o = a.length - 1,
          l = i.length - 1;
        1 <= o && 0 <= l && a[o] !== i[l];

      )
        l--;
      for (; 1 <= o && 0 <= l; o--, l--)
        if (a[o] !== i[l]) {
          if (o !== 1 || l !== 1)
            do
              if ((o--, l--, 0 > l || a[o] !== i[l])) {
                var u =
                  `
` + a[o].replace(" at new ", " at ");
                return (
                  e.displayName &&
                    u.includes("<anonymous>") &&
                    (u = u.replace("<anonymous>", e.displayName)),
                  u
                );
              }
            while (1 <= o && 0 <= l);
          break;
        }
    }
  } finally {
    (zi = !1), (Error.prepareStackTrace = n);
  }
  return (e = e ? e.displayName || e.name : "") ? $n(e) : "";
}
function sf(e) {
  switch (e.tag) {
    case 5:
      return $n(e.type);
    case 16:
      return $n("Lazy");
    case 13:
      return $n("Suspense");
    case 19:
      return $n("SuspenseList");
    case 0:
    case 2:
    case 15:
      return (e = Ui(e.type, !1)), e;
    case 11:
      return (e = Ui(e.type.render, !1)), e;
    case 1:
      return (e = Ui(e.type, !0)), e;
    default:
      return "";
  }
}
function wo(e) {
  if (e == null) return null;
  if (typeof e == "function") return e.displayName || e.name || null;
  if (typeof e == "string") return e;
  switch (e) {
    case un:
      return "Fragment";
    case ln:
      return "Portal";
    case _o:
      return "Profiler";
    case El:
      return "StrictMode";
    case mo:
      return "Suspense";
    case yo:
      return "SuspenseList";
  }
  if (typeof e == "object")
    switch (e.$$typeof) {
      case cc:
        return (e.displayName || "Context") + ".Consumer";
      case sc:
        return (e._context.displayName || "Context") + ".Provider";
      case jl:
        var t = e.render;
        return (
          (e = e.displayName),
          e ||
            ((e = t.displayName || t.name || ""),
            (e = e !== "" ? "ForwardRef(" + e + ")" : "ForwardRef")),
          e
        );
      case Cl:
        return (
          (t = e.displayName || null), t !== null ? t : wo(e.type) || "Memo"
        );
      case ft:
        (t = e._payload), (e = e._init);
        try {
          return wo(e(t));
        } catch {}
    }
  return null;
}
function cf(e) {
  var t = e.type;
  switch (e.tag) {
    case 24:
      return "Cache";
    case 9:
      return (t.displayName || "Context") + ".Consumer";
    case 10:
      return (t._context.displayName || "Context") + ".Provider";
    case 18:
      return "DehydratedFragment";
    case 11:
      return (
        (e = t.render),
        (e = e.displayName || e.name || ""),
        t.displayName || (e !== "" ? "ForwardRef(" + e + ")" : "ForwardRef")
      );
    case 7:
      return "Fragment";
    case 5:
      return t;
    case 4:
      return "Portal";
    case 3:
      return "Root";
    case 6:
      return "Text";
    case 16:
      return wo(t);
    case 8:
      return t === El ? "StrictMode" : "Mode";
    case 22:
      return "Offscreen";
    case 12:
      return "Profiler";
    case 21:
      return "Scope";
    case 13:
      return "Suspense";
    case 19:
      return "SuspenseList";
    case 25:
      return "TracingMarker";
    case 1:
    case 0:
    case 17:
    case 2:
    case 14:
    case 15:
      if (typeof t == "function") return t.displayName || t.name || null;
      if (typeof t == "string") return t;
  }
  return null;
}
function Nt(e) {
  switch (typeof e) {
    case "boolean":
    case "number":
    case "string":
    case "undefined":
      return e;
    case "object":
      return e;
    default:
      return "";
  }
}
function pc(e) {
  var t = e.type;
  return (
    (e = e.nodeName) &&
    e.toLowerCase() === "input" &&
    (t === "checkbox" || t === "radio")
  );
}
function df(e) {
  var t = pc(e) ? "checked" : "value",
    n = Object.getOwnPropertyDescriptor(e.constructor.prototype, t),
    r = "" + e[t];
  if (
    !e.hasOwnProperty(t) &&
    typeof n < "u" &&
    typeof n.get == "function" &&
    typeof n.set == "function"
  ) {
    var a = n.get,
      i = n.set;
    return (
      Object.defineProperty(e, t, {
        configurable: !0,
        get: function () {
          return a.call(this);
        },
        set: function (o) {
          (r = "" + o), i.call(this, o);
        },
      }),
      Object.defineProperty(e, t, { enumerable: n.enumerable }),
      {
        getValue: function () {
          return r;
        },
        setValue: function (o) {
          r = "" + o;
        },
        stopTracking: function () {
          (e._valueTracker = null), delete e[t];
        },
      }
    );
  }
}
function Hr(e) {
  e._valueTracker || (e._valueTracker = df(e));
}
function fc(e) {
  if (!e) return !1;
  var t = e._valueTracker;
  if (!t) return !0;
  var n = t.getValue(),
    r = "";
  return (
    e && (r = pc(e) ? (e.checked ? "true" : "false") : e.value),
    (e = r),
    e !== n ? (t.setValue(e), !0) : !1
  );
}
function ma(e) {
  if (((e = e || (typeof document < "u" ? document : void 0)), typeof e > "u"))
    return null;
  try {
    return e.activeElement || e.body;
  } catch {
    return e.body;
  }
}
function So(e, t) {
  var n = t.checked;
  return Q({}, t, {
    defaultChecked: void 0,
    defaultValue: void 0,
    value: void 0,
    checked: n ?? e._wrapperState.initialChecked,
  });
}
function Cu(e, t) {
  var n = t.defaultValue == null ? "" : t.defaultValue,
    r = t.checked != null ? t.checked : t.defaultChecked;
  (n = Nt(t.value != null ? t.value : n)),
    (e._wrapperState = {
      initialChecked: r,
      initialValue: n,
      controlled:
        t.type === "checkbox" || t.type === "radio"
          ? t.checked != null
          : t.value != null,
    });
}
function gc(e, t) {
  (t = t.checked), t != null && kl(e, "checked", t, !1);
}
function ko(e, t) {
  gc(e, t);
  var n = Nt(t.value),
    r = t.type;
  if (n != null)
    r === "number"
      ? ((n === 0 && e.value === "") || e.value != n) && (e.value = "" + n)
      : e.value !== "" + n && (e.value = "" + n);
  else if (r === "submit" || r === "reset") {
    e.removeAttribute("value");
    return;
  }
  t.hasOwnProperty("value")
    ? Eo(e, t.type, n)
    : t.hasOwnProperty("defaultValue") && Eo(e, t.type, Nt(t.defaultValue)),
    t.checked == null &&
      t.defaultChecked != null &&
      (e.defaultChecked = !!t.defaultChecked);
}
function xu(e, t, n) {
  if (t.hasOwnProperty("value") || t.hasOwnProperty("defaultValue")) {
    var r = t.type;
    if (
      !(
        (r !== "submit" && r !== "reset") ||
        (t.value !== void 0 && t.value !== null)
      )
    )
      return;
    (t = "" + e._wrapperState.initialValue),
      n || t === e.value || (e.value = t),
      (e.defaultValue = t);
  }
  (n = e.name),
    n !== "" && (e.name = ""),
    (e.defaultChecked = !!e._wrapperState.initialChecked),
    n !== "" && (e.name = n);
}
function Eo(e, t, n) {
  (t !== "number" || ma(e.ownerDocument) !== e) &&
    (n == null
      ? (e.defaultValue = "" + e._wrapperState.initialValue)
      : e.defaultValue !== "" + n && (e.defaultValue = "" + n));
}
var Kn = Array.isArray;
function yn(e, t, n, r) {
  if (((e = e.options), t)) {
    t = {};
    for (var a = 0; a < n.length; a++) t["$" + n[a]] = !0;
    for (n = 0; n < e.length; n++)
      (a = t.hasOwnProperty("$" + e[n].value)),
        e[n].selected !== a && (e[n].selected = a),
        a && r && (e[n].defaultSelected = !0);
  } else {
    for (n = "" + Nt(n), t = null, a = 0; a < e.length; a++) {
      if (e[a].value === n) {
        (e[a].selected = !0), r && (e[a].defaultSelected = !0);
        return;
      }
      t !== null || e[a].disabled || (t = e[a]);
    }
    t !== null && (t.selected = !0);
  }
}
function jo(e, t) {
  if (t.dangerouslySetInnerHTML != null) throw Error(k(91));
  return Q({}, t, {
    value: void 0,
    defaultValue: void 0,
    children: "" + e._wrapperState.initialValue,
  });
}
function Tu(e, t) {
  var n = t.value;
  if (n == null) {
    if (((n = t.children), (t = t.defaultValue), n != null)) {
      if (t != null) throw Error(k(92));
      if (Kn(n)) {
        if (1 < n.length) throw Error(k(93));
        n = n[0];
      }
      t = n;
    }
    t == null && (t = ""), (n = t);
  }
  e._wrapperState = { initialValue: Nt(n) };
}
function vc(e, t) {
  var n = Nt(t.value),
    r = Nt(t.defaultValue);
  n != null &&
    ((n = "" + n),
    n !== e.value && (e.value = n),
    t.defaultValue == null && e.defaultValue !== n && (e.defaultValue = n)),
    r != null && (e.defaultValue = "" + r);
}
function Pu(e) {
  var t = e.textContent;
  t === e._wrapperState.initialValue && t !== "" && t !== null && (e.value = t);
}
function hc(e) {
  switch (e) {
    case "svg":
      return "http://www.w3.org/2000/svg";
    case "math":
      return "http://www.w3.org/1998/Math/MathML";
    default:
      return "http://www.w3.org/1999/xhtml";
  }
}
function Co(e, t) {
  return e == null || e === "http://www.w3.org/1999/xhtml"
    ? hc(t)
    : e === "http://www.w3.org/2000/svg" && t === "foreignObject"
    ? "http://www.w3.org/1999/xhtml"
    : e;
}
var br,
  _c = (function (e) {
    return typeof MSApp < "u" && MSApp.execUnsafeLocalFunction
      ? function (t, n, r, a) {
          MSApp.execUnsafeLocalFunction(function () {
            return e(t, n, r, a);
          });
        }
      : e;
  })(function (e, t) {
    if (e.namespaceURI !== "http://www.w3.org/2000/svg" || "innerHTML" in e)
      e.innerHTML = t;
    else {
      for (
        br = br || document.createElement("div"),
          br.innerHTML = "<svg>" + t.valueOf().toString() + "</svg>",
          t = br.firstChild;
        e.firstChild;

      )
        e.removeChild(e.firstChild);
      for (; t.firstChild; ) e.appendChild(t.firstChild);
    }
  });
function sr(e, t) {
  if (t) {
    var n = e.firstChild;
    if (n && n === e.lastChild && n.nodeType === 3) {
      n.nodeValue = t;
      return;
    }
  }
  e.textContent = t;
}
var qn = {
    animationIterationCount: !0,
    aspectRatio: !0,
    borderImageOutset: !0,
    borderImageSlice: !0,
    borderImageWidth: !0,
    boxFlex: !0,
    boxFlexGroup: !0,
    boxOrdinalGroup: !0,
    columnCount: !0,
    columns: !0,
    flex: !0,
    flexGrow: !0,
    flexPositive: !0,
    flexShrink: !0,
    flexNegative: !0,
    flexOrder: !0,
    gridArea: !0,
    gridRow: !0,
    gridRowEnd: !0,
    gridRowSpan: !0,
    gridRowStart: !0,
    gridColumn: !0,
    gridColumnEnd: !0,
    gridColumnSpan: !0,
    gridColumnStart: !0,
    fontWeight: !0,
    lineClamp: !0,
    lineHeight: !0,
    opacity: !0,
    order: !0,
    orphans: !0,
    tabSize: !0,
    widows: !0,
    zIndex: !0,
    zoom: !0,
    fillOpacity: !0,
    floodOpacity: !0,
    stopOpacity: !0,
    strokeDasharray: !0,
    strokeDashoffset: !0,
    strokeMiterlimit: !0,
    strokeOpacity: !0,
    strokeWidth: !0,
  },
  pf = ["Webkit", "ms", "Moz", "O"];
Object.keys(qn).forEach(function (e) {
  pf.forEach(function (t) {
    (t = t + e.charAt(0).toUpperCase() + e.substring(1)), (qn[t] = qn[e]);
  });
});
function mc(e, t, n) {
  return t == null || typeof t == "boolean" || t === ""
    ? ""
    : n || typeof t != "number" || t === 0 || (qn.hasOwnProperty(e) && qn[e])
    ? ("" + t).trim()
    : t + "px";
}
function yc(e, t) {
  e = e.style;
  for (var n in t)
    if (t.hasOwnProperty(n)) {
      var r = n.indexOf("--") === 0,
        a = mc(n, t[n], r);
      n === "float" && (n = "cssFloat"), r ? e.setProperty(n, a) : (e[n] = a);
    }
}
var ff = Q(
  { menuitem: !0 },
  {
    area: !0,
    base: !0,
    br: !0,
    col: !0,
    embed: !0,
    hr: !0,
    img: !0,
    input: !0,
    keygen: !0,
    link: !0,
    meta: !0,
    param: !0,
    source: !0,
    track: !0,
    wbr: !0,
  }
);
function xo(e, t) {
  if (t) {
    if (ff[e] && (t.children != null || t.dangerouslySetInnerHTML != null))
      throw Error(k(137, e));
    if (t.dangerouslySetInnerHTML != null) {
      if (t.children != null) throw Error(k(60));
      if (
        typeof t.dangerouslySetInnerHTML != "object" ||
        !("__html" in t.dangerouslySetInnerHTML)
      )
        throw Error(k(61));
    }
    if (t.style != null && typeof t.style != "object") throw Error(k(62));
  }
}
function To(e, t) {
  if (e.indexOf("-") === -1) return typeof t.is == "string";
  switch (e) {
    case "annotation-xml":
    case "color-profile":
    case "font-face":
    case "font-face-src":
    case "font-face-uri":
    case "font-face-format":
    case "font-face-name":
    case "missing-glyph":
      return !1;
    default:
      return !0;
  }
}
var Po = null;
function xl(e) {
  return (
    (e = e.target || e.srcElement || window),
    e.correspondingUseElement && (e = e.correspondingUseElement),
    e.nodeType === 3 ? e.parentNode : e
  );
}
var No = null,
  wn = null,
  Sn = null;
function Nu(e) {
  if ((e = Lr(e))) {
    if (typeof No != "function") throw Error(k(280));
    var t = e.stateNode;
    t && ((t = qa(t)), No(e.stateNode, e.type, t));
  }
}
function wc(e) {
  wn ? (Sn ? Sn.push(e) : (Sn = [e])) : (wn = e);
}
function Sc() {
  if (wn) {
    var e = wn,
      t = Sn;
    if (((Sn = wn = null), Nu(e), t)) for (e = 0; e < t.length; e++) Nu(t[e]);
  }
}
function kc(e, t) {
  return e(t);
}
function Ec() {}
var Ai = !1;
function jc(e, t, n) {
  if (Ai) return e(t, n);
  Ai = !0;
  try {
    return kc(e, t, n);
  } finally {
    (Ai = !1), (wn !== null || Sn !== null) && (Ec(), Sc());
  }
}
function cr(e, t) {
  var n = e.stateNode;
  if (n === null) return null;
  var r = qa(n);
  if (r === null) return null;
  n = r[t];
  e: switch (t) {
    case "onClick":
    case "onClickCapture":
    case "onDoubleClick":
    case "onDoubleClickCapture":
    case "onMouseDown":
    case "onMouseDownCapture":
    case "onMouseMove":
    case "onMouseMoveCapture":
    case "onMouseUp":
    case "onMouseUpCapture":
    case "onMouseEnter":
      (r = !r.disabled) ||
        ((e = e.type),
        (r = !(
          e === "button" ||
          e === "input" ||
          e === "select" ||
          e === "textarea"
        ))),
        (e = !r);
      break e;
    default:
      e = !1;
  }
  if (e) return null;
  if (n && typeof n != "function") throw Error(k(231, t, typeof n));
  return n;
}
var Oo = !1;
if (it)
  try {
    var Fn = {};
    Object.defineProperty(Fn, "passive", {
      get: function () {
        Oo = !0;
      },
    }),
      window.addEventListener("test", Fn, Fn),
      window.removeEventListener("test", Fn, Fn);
  } catch {
    Oo = !1;
  }
function gf(e, t, n, r, a, i, o, l, u) {
  var s = Array.prototype.slice.call(arguments, 3);
  try {
    t.apply(n, s);
  } catch (p) {
    this.onError(p);
  }
}
var Zn = !1,
  ya = null,
  wa = !1,
  Ro = null,
  vf = {
    onError: function (e) {
      (Zn = !0), (ya = e);
    },
  };
function hf(e, t, n, r, a, i, o, l, u) {
  (Zn = !1), (ya = null), gf.apply(vf, arguments);
}
function _f(e, t, n, r, a, i, o, l, u) {
  if ((hf.apply(this, arguments), Zn)) {
    if (Zn) {
      var s = ya;
      (Zn = !1), (ya = null);
    } else throw Error(k(198));
    wa || ((wa = !0), (Ro = s));
  }
}
function nn(e) {
  var t = e,
    n = e;
  if (e.alternate) for (; t.return; ) t = t.return;
  else {
    e = t;
    do (t = e), t.flags & 4098 && (n = t.return), (e = t.return);
    while (e);
  }
  return t.tag === 3 ? n : null;
}
function Cc(e) {
  if (e.tag === 13) {
    var t = e.memoizedState;
    if (
      (t === null && ((e = e.alternate), e !== null && (t = e.memoizedState)),
      t !== null)
    )
      return t.dehydrated;
  }
  return null;
}
function Ou(e) {
  if (nn(e) !== e) throw Error(k(188));
}
function mf(e) {
  var t = e.alternate;
  if (!t) {
    if (((t = nn(e)), t === null)) throw Error(k(188));
    return t !== e ? null : e;
  }
  for (var n = e, r = t; ; ) {
    var a = n.return;
    if (a === null) break;
    var i = a.alternate;
    if (i === null) {
      if (((r = a.return), r !== null)) {
        n = r;
        continue;
      }
      break;
    }
    if (a.child === i.child) {
      for (i = a.child; i; ) {
        if (i === n) return Ou(a), e;
        if (i === r) return Ou(a), t;
        i = i.sibling;
      }
      throw Error(k(188));
    }
    if (n.return !== r.return) (n = a), (r = i);
    else {
      for (var o = !1, l = a.child; l; ) {
        if (l === n) {
          (o = !0), (n = a), (r = i);
          break;
        }
        if (l === r) {
          (o = !0), (r = a), (n = i);
          break;
        }
        l = l.sibling;
      }
      if (!o) {
        for (l = i.child; l; ) {
          if (l === n) {
            (o = !0), (n = i), (r = a);
            break;
          }
          if (l === r) {
            (o = !0), (r = i), (n = a);
            break;
          }
          l = l.sibling;
        }
        if (!o) throw Error(k(189));
      }
    }
    if (n.alternate !== r) throw Error(k(190));
  }
  if (n.tag !== 3) throw Error(k(188));
  return n.stateNode.current === n ? e : t;
}
function xc(e) {
  return (e = mf(e)), e !== null ? Tc(e) : null;
}
function Tc(e) {
  if (e.tag === 5 || e.tag === 6) return e;
  for (e = e.child; e !== null; ) {
    var t = Tc(e);
    if (t !== null) return t;
    e = e.sibling;
  }
  return null;
}
var Pc = Ne.unstable_scheduleCallback,
  Ru = Ne.unstable_cancelCallback,
  yf = Ne.unstable_shouldYield,
  wf = Ne.unstable_requestPaint,
  X = Ne.unstable_now,
  Sf = Ne.unstable_getCurrentPriorityLevel,
  Tl = Ne.unstable_ImmediatePriority,
  Nc = Ne.unstable_UserBlockingPriority,
  Sa = Ne.unstable_NormalPriority,
  kf = Ne.unstable_LowPriority,
  Oc = Ne.unstable_IdlePriority,
  $a = null,
  Je = null;
function Ef(e) {
  if (Je && typeof Je.onCommitFiberRoot == "function")
    try {
      Je.onCommitFiberRoot($a, e, void 0, (e.current.flags & 128) === 128);
    } catch {}
}
var Ye = Math.clz32 ? Math.clz32 : xf,
  jf = Math.log,
  Cf = Math.LN2;
function xf(e) {
  return (e >>>= 0), e === 0 ? 32 : (31 - ((jf(e) / Cf) | 0)) | 0;
}
var Vr = 64,
  Yr = 4194304;
function Xn(e) {
  switch (e & -e) {
    case 1:
      return 1;
    case 2:
      return 2;
    case 4:
      return 4;
    case 8:
      return 8;
    case 16:
      return 16;
    case 32:
      return 32;
    case 64:
    case 128:
    case 256:
    case 512:
    case 1024:
    case 2048:
    case 4096:
    case 8192:
    case 16384:
    case 32768:
    case 65536:
    case 131072:
    case 262144:
    case 524288:
    case 1048576:
    case 2097152:
      return e & 4194240;
    case 4194304:
    case 8388608:
    case 16777216:
    case 33554432:
    case 67108864:
      return e & 130023424;
    case 134217728:
      return 134217728;
    case 268435456:
      return 268435456;
    case 536870912:
      return 536870912;
    case 1073741824:
      return 1073741824;
    default:
      return e;
  }
}
function ka(e, t) {
  var n = e.pendingLanes;
  if (n === 0) return 0;
  var r = 0,
    a = e.suspendedLanes,
    i = e.pingedLanes,
    o = n & 268435455;
  if (o !== 0) {
    var l = o & ~a;
    l !== 0 ? (r = Xn(l)) : ((i &= o), i !== 0 && (r = Xn(i)));
  } else (o = n & ~a), o !== 0 ? (r = Xn(o)) : i !== 0 && (r = Xn(i));
  if (r === 0) return 0;
  if (
    t !== 0 &&
    t !== r &&
    !(t & a) &&
    ((a = r & -r), (i = t & -t), a >= i || (a === 16 && (i & 4194240) !== 0))
  )
    return t;
  if ((r & 4 && (r |= n & 16), (t = e.entangledLanes), t !== 0))
    for (e = e.entanglements, t &= r; 0 < t; )
      (n = 31 - Ye(t)), (a = 1 << n), (r |= e[n]), (t &= ~a);
  return r;
}
function Tf(e, t) {
  switch (e) {
    case 1:
    case 2:
    case 4:
      return t + 250;
    case 8:
    case 16:
    case 32:
    case 64:
    case 128:
    case 256:
    case 512:
    case 1024:
    case 2048:
    case 4096:
    case 8192:
    case 16384:
    case 32768:
    case 65536:
    case 131072:
    case 262144:
    case 524288:
    case 1048576:
    case 2097152:
      return t + 5e3;
    case 4194304:
    case 8388608:
    case 16777216:
    case 33554432:
    case 67108864:
      return -1;
    case 134217728:
    case 268435456:
    case 536870912:
    case 1073741824:
      return -1;
    default:
      return -1;
  }
}
function Pf(e, t) {
  for (
    var n = e.suspendedLanes,
      r = e.pingedLanes,
      a = e.expirationTimes,
      i = e.pendingLanes;
    0 < i;

  ) {
    var o = 31 - Ye(i),
      l = 1 << o,
      u = a[o];
    u === -1
      ? (!(l & n) || l & r) && (a[o] = Tf(l, t))
      : u <= t && (e.expiredLanes |= l),
      (i &= ~l);
  }
}
function Lo(e) {
  return (
    (e = e.pendingLanes & -1073741825),
    e !== 0 ? e : e & 1073741824 ? 1073741824 : 0
  );
}
function Rc() {
  var e = Vr;
  return (Vr <<= 1), !(Vr & 4194240) && (Vr = 64), e;
}
function Fi(e) {
  for (var t = [], n = 0; 31 > n; n++) t.push(e);
  return t;
}
function Or(e, t, n) {
  (e.pendingLanes |= t),
    t !== 536870912 && ((e.suspendedLanes = 0), (e.pingedLanes = 0)),
    (e = e.eventTimes),
    (t = 31 - Ye(t)),
    (e[t] = n);
}
function Nf(e, t) {
  var n = e.pendingLanes & ~t;
  (e.pendingLanes = t),
    (e.suspendedLanes = 0),
    (e.pingedLanes = 0),
    (e.expiredLanes &= t),
    (e.mutableReadLanes &= t),
    (e.entangledLanes &= t),
    (t = e.entanglements);
  var r = e.eventTimes;
  for (e = e.expirationTimes; 0 < n; ) {
    var a = 31 - Ye(n),
      i = 1 << a;
    (t[a] = 0), (r[a] = -1), (e[a] = -1), (n &= ~i);
  }
}
function Pl(e, t) {
  var n = (e.entangledLanes |= t);
  for (e = e.entanglements; n; ) {
    var r = 31 - Ye(n),
      a = 1 << r;
    (a & t) | (e[r] & t) && (e[r] |= t), (n &= ~a);
  }
}
var U = 0;
function Lc(e) {
  return (e &= -e), 1 < e ? (4 < e ? (e & 268435455 ? 16 : 536870912) : 4) : 1;
}
var Mc,
  Nl,
  Dc,
  Ic,
  zc,
  Mo = !1,
  Gr = [],
  wt = null,
  St = null,
  kt = null,
  dr = new Map(),
  pr = new Map(),
  vt = [],
  Of =
    "mousedown mouseup touchcancel touchend touchstart auxclick dblclick pointercancel pointerdown pointerup dragend dragstart drop compositionend compositionstart keydown keypress keyup input textInput copy cut paste click change contextmenu reset submit".split(
      " "
    );
function Lu(e, t) {
  switch (e) {
    case "focusin":
    case "focusout":
      wt = null;
      break;
    case "dragenter":
    case "dragleave":
      St = null;
      break;
    case "mouseover":
    case "mouseout":
      kt = null;
      break;
    case "pointerover":
    case "pointerout":
      dr.delete(t.pointerId);
      break;
    case "gotpointercapture":
    case "lostpointercapture":
      pr.delete(t.pointerId);
  }
}
function Bn(e, t, n, r, a, i) {
  return e === null || e.nativeEvent !== i
    ? ((e = {
        blockedOn: t,
        domEventName: n,
        eventSystemFlags: r,
        nativeEvent: i,
        targetContainers: [a],
      }),
      t !== null && ((t = Lr(t)), t !== null && Nl(t)),
      e)
    : ((e.eventSystemFlags |= r),
      (t = e.targetContainers),
      a !== null && t.indexOf(a) === -1 && t.push(a),
      e);
}
function Rf(e, t, n, r, a) {
  switch (t) {
    case "focusin":
      return (wt = Bn(wt, e, t, n, r, a)), !0;
    case "dragenter":
      return (St = Bn(St, e, t, n, r, a)), !0;
    case "mouseover":
      return (kt = Bn(kt, e, t, n, r, a)), !0;
    case "pointerover":
      var i = a.pointerId;
      return dr.set(i, Bn(dr.get(i) || null, e, t, n, r, a)), !0;
    case "gotpointercapture":
      return (
        (i = a.pointerId), pr.set(i, Bn(pr.get(i) || null, e, t, n, r, a)), !0
      );
  }
  return !1;
}
function Uc(e) {
  var t = Vt(e.target);
  if (t !== null) {
    var n = nn(t);
    if (n !== null) {
      if (((t = n.tag), t === 13)) {
        if (((t = Cc(n)), t !== null)) {
          (e.blockedOn = t),
            zc(e.priority, function () {
              Dc(n);
            });
          return;
        }
      } else if (t === 3 && n.stateNode.current.memoizedState.isDehydrated) {
        e.blockedOn = n.tag === 3 ? n.stateNode.containerInfo : null;
        return;
      }
    }
  }
  e.blockedOn = null;
}
function ia(e) {
  if (e.blockedOn !== null) return !1;
  for (var t = e.targetContainers; 0 < t.length; ) {
    var n = Do(e.domEventName, e.eventSystemFlags, t[0], e.nativeEvent);
    if (n === null) {
      n = e.nativeEvent;
      var r = new n.constructor(n.type, n);
      (Po = r), n.target.dispatchEvent(r), (Po = null);
    } else return (t = Lr(n)), t !== null && Nl(t), (e.blockedOn = n), !1;
    t.shift();
  }
  return !0;
}
function Mu(e, t, n) {
  ia(e) && n.delete(t);
}
function Lf() {
  (Mo = !1),
    wt !== null && ia(wt) && (wt = null),
    St !== null && ia(St) && (St = null),
    kt !== null && ia(kt) && (kt = null),
    dr.forEach(Mu),
    pr.forEach(Mu);
}
function Wn(e, t) {
  e.blockedOn === t &&
    ((e.blockedOn = null),
    Mo ||
      ((Mo = !0),
      Ne.unstable_scheduleCallback(Ne.unstable_NormalPriority, Lf)));
}
function fr(e) {
  function t(a) {
    return Wn(a, e);
  }
  if (0 < Gr.length) {
    Wn(Gr[0], e);
    for (var n = 1; n < Gr.length; n++) {
      var r = Gr[n];
      r.blockedOn === e && (r.blockedOn = null);
    }
  }
  for (
    wt !== null && Wn(wt, e),
      St !== null && Wn(St, e),
      kt !== null && Wn(kt, e),
      dr.forEach(t),
      pr.forEach(t),
      n = 0;
    n < vt.length;
    n++
  )
    (r = vt[n]), r.blockedOn === e && (r.blockedOn = null);
  for (; 0 < vt.length && ((n = vt[0]), n.blockedOn === null); )
    Uc(n), n.blockedOn === null && vt.shift();
}
var kn = st.ReactCurrentBatchConfig,
  Ea = !0;
function Mf(e, t, n, r) {
  var a = U,
    i = kn.transition;
  kn.transition = null;
  try {
    (U = 1), Ol(e, t, n, r);
  } finally {
    (U = a), (kn.transition = i);
  }
}
function Df(e, t, n, r) {
  var a = U,
    i = kn.transition;
  kn.transition = null;
  try {
    (U = 4), Ol(e, t, n, r);
  } finally {
    (U = a), (kn.transition = i);
  }
}
function Ol(e, t, n, r) {
  if (Ea) {
    var a = Do(e, t, n, r);
    if (a === null) Ki(e, t, r, ja, n), Lu(e, r);
    else if (Rf(a, e, t, n, r)) r.stopPropagation();
    else if ((Lu(e, r), t & 4 && -1 < Of.indexOf(e))) {
      for (; a !== null; ) {
        var i = Lr(a);
        if (
          (i !== null && Mc(i),
          (i = Do(e, t, n, r)),
          i === null && Ki(e, t, r, ja, n),
          i === a)
        )
          break;
        a = i;
      }
      a !== null && r.stopPropagation();
    } else Ki(e, t, r, null, n);
  }
}
var ja = null;
function Do(e, t, n, r) {
  if (((ja = null), (e = xl(r)), (e = Vt(e)), e !== null))
    if (((t = nn(e)), t === null)) e = null;
    else if (((n = t.tag), n === 13)) {
      if (((e = Cc(t)), e !== null)) return e;
      e = null;
    } else if (n === 3) {
      if (t.stateNode.current.memoizedState.isDehydrated)
        return t.tag === 3 ? t.stateNode.containerInfo : null;
      e = null;
    } else t !== e && (e = null);
  return (ja = e), null;
}
function Ac(e) {
  switch (e) {
    case "cancel":
    case "click":
    case "close":
    case "contextmenu":
    case "copy":
    case "cut":
    case "auxclick":
    case "dblclick":
    case "dragend":
    case "dragstart":
    case "drop":
    case "focusin":
    case "focusout":
    case "input":
    case "invalid":
    case "keydown":
    case "keypress":
    case "keyup":
    case "mousedown":
    case "mouseup":
    case "paste":
    case "pause":
    case "play":
    case "pointercancel":
    case "pointerdown":
    case "pointerup":
    case "ratechange":
    case "reset":
    case "resize":
    case "seeked":
    case "submit":
    case "touchcancel":
    case "touchend":
    case "touchstart":
    case "volumechange":
    case "change":
    case "selectionchange":
    case "textInput":
    case "compositionstart":
    case "compositionend":
    case "compositionupdate":
    case "beforeblur":
    case "afterblur":
    case "beforeinput":
    case "blur":
    case "fullscreenchange":
    case "focus":
    case "hashchange":
    case "popstate":
    case "select":
    case "selectstart":
      return 1;
    case "drag":
    case "dragenter":
    case "dragexit":
    case "dragleave":
    case "dragover":
    case "mousemove":
    case "mouseout":
    case "mouseover":
    case "pointermove":
    case "pointerout":
    case "pointerover":
    case "scroll":
    case "toggle":
    case "touchmove":
    case "wheel":
    case "mouseenter":
    case "mouseleave":
    case "pointerenter":
    case "pointerleave":
      return 4;
    case "message":
      switch (Sf()) {
        case Tl:
          return 1;
        case Nc:
          return 4;
        case Sa:
        case kf:
          return 16;
        case Oc:
          return 536870912;
        default:
          return 16;
      }
    default:
      return 16;
  }
}
var _t = null,
  Rl = null,
  oa = null;
function Fc() {
  if (oa) return oa;
  var e,
    t = Rl,
    n = t.length,
    r,
    a = "value" in _t ? _t.value : _t.textContent,
    i = a.length;
  for (e = 0; e < n && t[e] === a[e]; e++);
  var o = n - e;
  for (r = 1; r <= o && t[n - r] === a[i - r]; r++);
  return (oa = a.slice(e, 1 < r ? 1 - r : void 0));
}
function la(e) {
  var t = e.keyCode;
  return (
    "charCode" in e
      ? ((e = e.charCode), e === 0 && t === 13 && (e = 13))
      : (e = t),
    e === 10 && (e = 13),
    32 <= e || e === 13 ? e : 0
  );
}
function Qr() {
  return !0;
}
function Du() {
  return !1;
}
function Re(e) {
  function t(n, r, a, i, o) {
    (this._reactName = n),
      (this._targetInst = a),
      (this.type = r),
      (this.nativeEvent = i),
      (this.target = o),
      (this.currentTarget = null);
    for (var l in e)
      e.hasOwnProperty(l) && ((n = e[l]), (this[l] = n ? n(i) : i[l]));
    return (
      (this.isDefaultPrevented = (
        i.defaultPrevented != null ? i.defaultPrevented : i.returnValue === !1
      )
        ? Qr
        : Du),
      (this.isPropagationStopped = Du),
      this
    );
  }
  return (
    Q(t.prototype, {
      preventDefault: function () {
        this.defaultPrevented = !0;
        var n = this.nativeEvent;
        n &&
          (n.preventDefault
            ? n.preventDefault()
            : typeof n.returnValue != "unknown" && (n.returnValue = !1),
          (this.isDefaultPrevented = Qr));
      },
      stopPropagation: function () {
        var n = this.nativeEvent;
        n &&
          (n.stopPropagation
            ? n.stopPropagation()
            : typeof n.cancelBubble != "unknown" && (n.cancelBubble = !0),
          (this.isPropagationStopped = Qr));
      },
      persist: function () {},
      isPersistent: Qr,
    }),
    t
  );
}
var Dn = {
    eventPhase: 0,
    bubbles: 0,
    cancelable: 0,
    timeStamp: function (e) {
      return e.timeStamp || Date.now();
    },
    defaultPrevented: 0,
    isTrusted: 0,
  },
  Ll = Re(Dn),
  Rr = Q({}, Dn, { view: 0, detail: 0 }),
  If = Re(Rr),
  Bi,
  Wi,
  Hn,
  Ka = Q({}, Rr, {
    screenX: 0,
    screenY: 0,
    clientX: 0,
    clientY: 0,
    pageX: 0,
    pageY: 0,
    ctrlKey: 0,
    shiftKey: 0,
    altKey: 0,
    metaKey: 0,
    getModifierState: Ml,
    button: 0,
    buttons: 0,
    relatedTarget: function (e) {
      return e.relatedTarget === void 0
        ? e.fromElement === e.srcElement
          ? e.toElement
          : e.fromElement
        : e.relatedTarget;
    },
    movementX: function (e) {
      return "movementX" in e
        ? e.movementX
        : (e !== Hn &&
            (Hn && e.type === "mousemove"
              ? ((Bi = e.screenX - Hn.screenX), (Wi = e.screenY - Hn.screenY))
              : (Wi = Bi = 0),
            (Hn = e)),
          Bi);
    },
    movementY: function (e) {
      return "movementY" in e ? e.movementY : Wi;
    },
  }),
  Iu = Re(Ka),
  zf = Q({}, Ka, { dataTransfer: 0 }),
  Uf = Re(zf),
  Af = Q({}, Rr, { relatedTarget: 0 }),
  Hi = Re(Af),
  Ff = Q({}, Dn, { animationName: 0, elapsedTime: 0, pseudoElement: 0 }),
  Bf = Re(Ff),
  Wf = Q({}, Dn, {
    clipboardData: function (e) {
      return "clipboardData" in e ? e.clipboardData : window.clipboardData;
    },
  }),
  Hf = Re(Wf),
  bf = Q({}, Dn, { data: 0 }),
  zu = Re(bf),
  Vf = {
    Esc: "Escape",
    Spacebar: " ",
    Left: "ArrowLeft",
    Up: "ArrowUp",
    Right: "ArrowRight",
    Down: "ArrowDown",
    Del: "Delete",
    Win: "OS",
    Menu: "ContextMenu",
    Apps: "ContextMenu",
    Scroll: "ScrollLock",
    MozPrintableKey: "Unidentified",
  },
  Yf = {
    8: "Backspace",
    9: "Tab",
    12: "Clear",
    13: "Enter",
    16: "Shift",
    17: "Control",
    18: "Alt",
    19: "Pause",
    20: "CapsLock",
    27: "Escape",
    32: " ",
    33: "PageUp",
    34: "PageDown",
    35: "End",
    36: "Home",
    37: "ArrowLeft",
    38: "ArrowUp",
    39: "ArrowRight",
    40: "ArrowDown",
    45: "Insert",
    46: "Delete",
    112: "F1",
    113: "F2",
    114: "F3",
    115: "F4",
    116: "F5",
    117: "F6",
    118: "F7",
    119: "F8",
    120: "F9",
    121: "F10",
    122: "F11",
    123: "F12",
    144: "NumLock",
    145: "ScrollLock",
    224: "Meta",
  },
  Gf = {
    Alt: "altKey",
    Control: "ctrlKey",
    Meta: "metaKey",
    Shift: "shiftKey",
  };
function Qf(e) {
  var t = this.nativeEvent;
  return t.getModifierState ? t.getModifierState(e) : (e = Gf[e]) ? !!t[e] : !1;
}
function Ml() {
  return Qf;
}
var $f = Q({}, Rr, {
    key: function (e) {
      if (e.key) {
        var t = Vf[e.key] || e.key;
        if (t !== "Unidentified") return t;
      }
      return e.type === "keypress"
        ? ((e = la(e)), e === 13 ? "Enter" : String.fromCharCode(e))
        : e.type === "keydown" || e.type === "keyup"
        ? Yf[e.keyCode] || "Unidentified"
        : "";
    },
    code: 0,
    location: 0,
    ctrlKey: 0,
    shiftKey: 0,
    altKey: 0,
    metaKey: 0,
    repeat: 0,
    locale: 0,
    getModifierState: Ml,
    charCode: function (e) {
      return e.type === "keypress" ? la(e) : 0;
    },
    keyCode: function (e) {
      return e.type === "keydown" || e.type === "keyup" ? e.keyCode : 0;
    },
    which: function (e) {
      return e.type === "keypress"
        ? la(e)
        : e.type === "keydown" || e.type === "keyup"
        ? e.keyCode
        : 0;
    },
  }),
  Kf = Re($f),
  Xf = Q({}, Ka, {
    pointerId: 0,
    width: 0,
    height: 0,
    pressure: 0,
    tangentialPressure: 0,
    tiltX: 0,
    tiltY: 0,
    twist: 0,
    pointerType: 0,
    isPrimary: 0,
  }),
  Uu = Re(Xf),
  Jf = Q({}, Rr, {
    touches: 0,
    targetTouches: 0,
    changedTouches: 0,
    altKey: 0,
    metaKey: 0,
    ctrlKey: 0,
    shiftKey: 0,
    getModifierState: Ml,
  }),
  qf = Re(Jf),
  Zf = Q({}, Dn, { propertyName: 0, elapsedTime: 0, pseudoElement: 0 }),
  eg = Re(Zf),
  tg = Q({}, Ka, {
    deltaX: function (e) {
      return "deltaX" in e ? e.deltaX : "wheelDeltaX" in e ? -e.wheelDeltaX : 0;
    },
    deltaY: function (e) {
      return "deltaY" in e
        ? e.deltaY
        : "wheelDeltaY" in e
        ? -e.wheelDeltaY
        : "wheelDelta" in e
        ? -e.wheelDelta
        : 0;
    },
    deltaZ: 0,
    deltaMode: 0,
  }),
  ng = Re(tg),
  rg = [9, 13, 27, 32],
  Dl = it && "CompositionEvent" in window,
  er = null;
it && "documentMode" in document && (er = document.documentMode);
var ag = it && "TextEvent" in window && !er,
  Bc = it && (!Dl || (er && 8 < er && 11 >= er)),
  Au = String.fromCharCode(32),
  Fu = !1;
function Wc(e, t) {
  switch (e) {
    case "keyup":
      return rg.indexOf(t.keyCode) !== -1;
    case "keydown":
      return t.keyCode !== 229;
    case "keypress":
    case "mousedown":
    case "focusout":
      return !0;
    default:
      return !1;
  }
}
function Hc(e) {
  return (e = e.detail), typeof e == "object" && "data" in e ? e.data : null;
}
var sn = !1;
function ig(e, t) {
  switch (e) {
    case "compositionend":
      return Hc(t);
    case "keypress":
      return t.which !== 32 ? null : ((Fu = !0), Au);
    case "textInput":
      return (e = t.data), e === Au && Fu ? null : e;
    default:
      return null;
  }
}
function og(e, t) {
  if (sn)
    return e === "compositionend" || (!Dl && Wc(e, t))
      ? ((e = Fc()), (oa = Rl = _t = null), (sn = !1), e)
      : null;
  switch (e) {
    case "paste":
      return null;
    case "keypress":
      if (!(t.ctrlKey || t.altKey || t.metaKey) || (t.ctrlKey && t.altKey)) {
        if (t.char && 1 < t.char.length) return t.char;
        if (t.which) return String.fromCharCode(t.which);
      }
      return null;
    case "compositionend":
      return Bc && t.locale !== "ko" ? null : t.data;
    default:
      return null;
  }
}
var lg = {
  color: !0,
  date: !0,
  datetime: !0,
  "datetime-local": !0,
  email: !0,
  month: !0,
  number: !0,
  password: !0,
  range: !0,
  search: !0,
  tel: !0,
  text: !0,
  time: !0,
  url: !0,
  week: !0,
};
function Bu(e) {
  var t = e && e.nodeName && e.nodeName.toLowerCase();
  return t === "input" ? !!lg[e.type] : t === "textarea";
}
function bc(e, t, n, r) {
  wc(r),
    (t = Ca(t, "onChange")),
    0 < t.length &&
      ((n = new Ll("onChange", "change", null, n, r)),
      e.push({ event: n, listeners: t }));
}
var tr = null,
  gr = null;
function ug(e) {
  ed(e, 0);
}
function Xa(e) {
  var t = pn(e);
  if (fc(t)) return e;
}
function sg(e, t) {
  if (e === "change") return t;
}
var Vc = !1;
if (it) {
  var bi;
  if (it) {
    var Vi = "oninput" in document;
    if (!Vi) {
      var Wu = document.createElement("div");
      Wu.setAttribute("oninput", "return;"),
        (Vi = typeof Wu.oninput == "function");
    }
    bi = Vi;
  } else bi = !1;
  Vc = bi && (!document.documentMode || 9 < document.documentMode);
}
function Hu() {
  tr && (tr.detachEvent("onpropertychange", Yc), (gr = tr = null));
}
function Yc(e) {
  if (e.propertyName === "value" && Xa(gr)) {
    var t = [];
    bc(t, gr, e, xl(e)), jc(ug, t);
  }
}
function cg(e, t, n) {
  e === "focusin"
    ? (Hu(), (tr = t), (gr = n), tr.attachEvent("onpropertychange", Yc))
    : e === "focusout" && Hu();
}
function dg(e) {
  if (e === "selectionchange" || e === "keyup" || e === "keydown")
    return Xa(gr);
}
function pg(e, t) {
  if (e === "click") return Xa(t);
}
function fg(e, t) {
  if (e === "input" || e === "change") return Xa(t);
}
function gg(e, t) {
  return (e === t && (e !== 0 || 1 / e === 1 / t)) || (e !== e && t !== t);
}
var Qe = typeof Object.is == "function" ? Object.is : gg;
function vr(e, t) {
  if (Qe(e, t)) return !0;
  if (typeof e != "object" || e === null || typeof t != "object" || t === null)
    return !1;
  var n = Object.keys(e),
    r = Object.keys(t);
  if (n.length !== r.length) return !1;
  for (r = 0; r < n.length; r++) {
    var a = n[r];
    if (!ho.call(t, a) || !Qe(e[a], t[a])) return !1;
  }
  return !0;
}
function bu(e) {
  for (; e && e.firstChild; ) e = e.firstChild;
  return e;
}
function Vu(e, t) {
  var n = bu(e);
  e = 0;
  for (var r; n; ) {
    if (n.nodeType === 3) {
      if (((r = e + n.textContent.length), e <= t && r >= t))
        return { node: n, offset: t - e };
      e = r;
    }
    e: {
      for (; n; ) {
        if (n.nextSibling) {
          n = n.nextSibling;
          break e;
        }
        n = n.parentNode;
      }
      n = void 0;
    }
    n = bu(n);
  }
}
function Gc(e, t) {
  return e && t
    ? e === t
      ? !0
      : e && e.nodeType === 3
      ? !1
      : t && t.nodeType === 3
      ? Gc(e, t.parentNode)
      : "contains" in e
      ? e.contains(t)
      : e.compareDocumentPosition
      ? !!(e.compareDocumentPosition(t) & 16)
      : !1
    : !1;
}
function Qc() {
  for (var e = window, t = ma(); t instanceof e.HTMLIFrameElement; ) {
    try {
      var n = typeof t.contentWindow.location.href == "string";
    } catch {
      n = !1;
    }
    if (n) e = t.contentWindow;
    else break;
    t = ma(e.document);
  }
  return t;
}
function Il(e) {
  var t = e && e.nodeName && e.nodeName.toLowerCase();
  return (
    t &&
    ((t === "input" &&
      (e.type === "text" ||
        e.type === "search" ||
        e.type === "tel" ||
        e.type === "url" ||
        e.type === "password")) ||
      t === "textarea" ||
      e.contentEditable === "true")
  );
}
function vg(e) {
  var t = Qc(),
    n = e.focusedElem,
    r = e.selectionRange;
  if (
    t !== n &&
    n &&
    n.ownerDocument &&
    Gc(n.ownerDocument.documentElement, n)
  ) {
    if (r !== null && Il(n)) {
      if (
        ((t = r.start),
        (e = r.end),
        e === void 0 && (e = t),
        "selectionStart" in n)
      )
        (n.selectionStart = t), (n.selectionEnd = Math.min(e, n.value.length));
      else if (
        ((e = ((t = n.ownerDocument || document) && t.defaultView) || window),
        e.getSelection)
      ) {
        e = e.getSelection();
        var a = n.textContent.length,
          i = Math.min(r.start, a);
        (r = r.end === void 0 ? i : Math.min(r.end, a)),
          !e.extend && i > r && ((a = r), (r = i), (i = a)),
          (a = Vu(n, i));
        var o = Vu(n, r);
        a &&
          o &&
          (e.rangeCount !== 1 ||
            e.anchorNode !== a.node ||
            e.anchorOffset !== a.offset ||
            e.focusNode !== o.node ||
            e.focusOffset !== o.offset) &&
          ((t = t.createRange()),
          t.setStart(a.node, a.offset),
          e.removeAllRanges(),
          i > r
            ? (e.addRange(t), e.extend(o.node, o.offset))
            : (t.setEnd(o.node, o.offset), e.addRange(t)));
      }
    }
    for (t = [], e = n; (e = e.parentNode); )
      e.nodeType === 1 &&
        t.push({ element: e, left: e.scrollLeft, top: e.scrollTop });
    for (typeof n.focus == "function" && n.focus(), n = 0; n < t.length; n++)
      (e = t[n]),
        (e.element.scrollLeft = e.left),
        (e.element.scrollTop = e.top);
  }
}
var hg = it && "documentMode" in document && 11 >= document.documentMode,
  cn = null,
  Io = null,
  nr = null,
  zo = !1;
function Yu(e, t, n) {
  var r = n.window === n ? n.document : n.nodeType === 9 ? n : n.ownerDocument;
  zo ||
    cn == null ||
    cn !== ma(r) ||
    ((r = cn),
    "selectionStart" in r && Il(r)
      ? (r = { start: r.selectionStart, end: r.selectionEnd })
      : ((r = (
          (r.ownerDocument && r.ownerDocument.defaultView) ||
          window
        ).getSelection()),
        (r = {
          anchorNode: r.anchorNode,
          anchorOffset: r.anchorOffset,
          focusNode: r.focusNode,
          focusOffset: r.focusOffset,
        })),
    (nr && vr(nr, r)) ||
      ((nr = r),
      (r = Ca(Io, "onSelect")),
      0 < r.length &&
        ((t = new Ll("onSelect", "select", null, t, n)),
        e.push({ event: t, listeners: r }),
        (t.target = cn))));
}
function $r(e, t) {
  var n = {};
  return (
    (n[e.toLowerCase()] = t.toLowerCase()),
    (n["Webkit" + e] = "webkit" + t),
    (n["Moz" + e] = "moz" + t),
    n
  );
}
var dn = {
    animationend: $r("Animation", "AnimationEnd"),
    animationiteration: $r("Animation", "AnimationIteration"),
    animationstart: $r("Animation", "AnimationStart"),
    transitionend: $r("Transition", "TransitionEnd"),
  },
  Yi = {},
  $c = {};
it &&
  (($c = document.createElement("div").style),
  "AnimationEvent" in window ||
    (delete dn.animationend.animation,
    delete dn.animationiteration.animation,
    delete dn.animationstart.animation),
  "TransitionEvent" in window || delete dn.transitionend.transition);
function Ja(e) {
  if (Yi[e]) return Yi[e];
  if (!dn[e]) return e;
  var t = dn[e],
    n;
  for (n in t) if (t.hasOwnProperty(n) && n in $c) return (Yi[e] = t[n]);
  return e;
}
var Kc = Ja("animationend"),
  Xc = Ja("animationiteration"),
  Jc = Ja("animationstart"),
  qc = Ja("transitionend"),
  Zc = new Map(),
  Gu =
    "abort auxClick cancel canPlay canPlayThrough click close contextMenu copy cut drag dragEnd dragEnter dragExit dragLeave dragOver dragStart drop durationChange emptied encrypted ended error gotPointerCapture input invalid keyDown keyPress keyUp load loadedData loadedMetadata loadStart lostPointerCapture mouseDown mouseMove mouseOut mouseOver mouseUp paste pause play playing pointerCancel pointerDown pointerMove pointerOut pointerOver pointerUp progress rateChange reset resize seeked seeking stalled submit suspend timeUpdate touchCancel touchEnd touchStart volumeChange scroll toggle touchMove waiting wheel".split(
      " "
    );
function Lt(e, t) {
  Zc.set(e, t), tn(t, [e]);
}
for (var Gi = 0; Gi < Gu.length; Gi++) {
  var Qi = Gu[Gi],
    _g = Qi.toLowerCase(),
    mg = Qi[0].toUpperCase() + Qi.slice(1);
  Lt(_g, "on" + mg);
}
Lt(Kc, "onAnimationEnd");
Lt(Xc, "onAnimationIteration");
Lt(Jc, "onAnimationStart");
Lt("dblclick", "onDoubleClick");
Lt("focusin", "onFocus");
Lt("focusout", "onBlur");
Lt(qc, "onTransitionEnd");
Cn("onMouseEnter", ["mouseout", "mouseover"]);
Cn("onMouseLeave", ["mouseout", "mouseover"]);
Cn("onPointerEnter", ["pointerout", "pointerover"]);
Cn("onPointerLeave", ["pointerout", "pointerover"]);
tn(
  "onChange",
  "change click focusin focusout input keydown keyup selectionchange".split(" ")
);
tn(
  "onSelect",
  "focusout contextmenu dragend focusin keydown keyup mousedown mouseup selectionchange".split(
    " "
  )
);
tn("onBeforeInput", ["compositionend", "keypress", "textInput", "paste"]);
tn(
  "onCompositionEnd",
  "compositionend focusout keydown keypress keyup mousedown".split(" ")
);
tn(
  "onCompositionStart",
  "compositionstart focusout keydown keypress keyup mousedown".split(" ")
);
tn(
  "onCompositionUpdate",
  "compositionupdate focusout keydown keypress keyup mousedown".split(" ")
);
var Jn =
    "abort canplay canplaythrough durationchange emptied encrypted ended error loadeddata loadedmetadata loadstart pause play playing progress ratechange resize seeked seeking stalled suspend timeupdate volumechange waiting".split(
      " "
    ),
  yg = new Set("cancel close invalid load scroll toggle".split(" ").concat(Jn));
function Qu(e, t, n) {
  var r = e.type || "unknown-event";
  (e.currentTarget = n), _f(r, t, void 0, e), (e.currentTarget = null);
}
function ed(e, t) {
  t = (t & 4) !== 0;
  for (var n = 0; n < e.length; n++) {
    var r = e[n],
      a = r.event;
    r = r.listeners;
    e: {
      var i = void 0;
      if (t)
        for (var o = r.length - 1; 0 <= o; o--) {
          var l = r[o],
            u = l.instance,
            s = l.currentTarget;
          if (((l = l.listener), u !== i && a.isPropagationStopped())) break e;
          Qu(a, l, s), (i = u);
        }
      else
        for (o = 0; o < r.length; o++) {
          if (
            ((l = r[o]),
            (u = l.instance),
            (s = l.currentTarget),
            (l = l.listener),
            u !== i && a.isPropagationStopped())
          )
            break e;
          Qu(a, l, s), (i = u);
        }
    }
  }
  if (wa) throw ((e = Ro), (wa = !1), (Ro = null), e);
}
function H(e, t) {
  var n = t[Wo];
  n === void 0 && (n = t[Wo] = new Set());
  var r = e + "__bubble";
  n.has(r) || (td(t, e, 2, !1), n.add(r));
}
function $i(e, t, n) {
  var r = 0;
  t && (r |= 4), td(n, e, r, t);
}
var Kr = "_reactListening" + Math.random().toString(36).slice(2);
function hr(e) {
  if (!e[Kr]) {
    (e[Kr] = !0),
      uc.forEach(function (n) {
        n !== "selectionchange" && (yg.has(n) || $i(n, !1, e), $i(n, !0, e));
      });
    var t = e.nodeType === 9 ? e : e.ownerDocument;
    t === null || t[Kr] || ((t[Kr] = !0), $i("selectionchange", !1, t));
  }
}
function td(e, t, n, r) {
  switch (Ac(t)) {
    case 1:
      var a = Mf;
      break;
    case 4:
      a = Df;
      break;
    default:
      a = Ol;
  }
  (n = a.bind(null, t, n, e)),
    (a = void 0),
    !Oo ||
      (t !== "touchstart" && t !== "touchmove" && t !== "wheel") ||
      (a = !0),
    r
      ? a !== void 0
        ? e.addEventListener(t, n, { capture: !0, passive: a })
        : e.addEventListener(t, n, !0)
      : a !== void 0
      ? e.addEventListener(t, n, { passive: a })
      : e.addEventListener(t, n, !1);
}
function Ki(e, t, n, r, a) {
  var i = r;
  if (!(t & 1) && !(t & 2) && r !== null)
    e: for (;;) {
      if (r === null) return;
      var o = r.tag;
      if (o === 3 || o === 4) {
        var l = r.stateNode.containerInfo;
        if (l === a || (l.nodeType === 8 && l.parentNode === a)) break;
        if (o === 4)
          for (o = r.return; o !== null; ) {
            var u = o.tag;
            if (
              (u === 3 || u === 4) &&
              ((u = o.stateNode.containerInfo),
              u === a || (u.nodeType === 8 && u.parentNode === a))
            )
              return;
            o = o.return;
          }
        for (; l !== null; ) {
          if (((o = Vt(l)), o === null)) return;
          if (((u = o.tag), u === 5 || u === 6)) {
            r = i = o;
            continue e;
          }
          l = l.parentNode;
        }
      }
      r = r.return;
    }
  jc(function () {
    var s = i,
      p = xl(n),
      f = [];
    e: {
      var v = Zc.get(e);
      if (v !== void 0) {
        var _ = Ll,
          m = e;
        switch (e) {
          case "keypress":
            if (la(n) === 0) break e;
          case "keydown":
          case "keyup":
            _ = Kf;
            break;
          case "focusin":
            (m = "focus"), (_ = Hi);
            break;
          case "focusout":
            (m = "blur"), (_ = Hi);
            break;
          case "beforeblur":
          case "afterblur":
            _ = Hi;
            break;
          case "click":
            if (n.button === 2) break e;
          case "auxclick":
          case "dblclick":
          case "mousedown":
          case "mousemove":
          case "mouseup":
          case "mouseout":
          case "mouseover":
          case "contextmenu":
            _ = Iu;
            break;
          case "drag":
          case "dragend":
          case "dragenter":
          case "dragexit":
          case "dragleave":
          case "dragover":
          case "dragstart":
          case "drop":
            _ = Uf;
            break;
          case "touchcancel":
          case "touchend":
          case "touchmove":
          case "touchstart":
            _ = qf;
            break;
          case Kc:
          case Xc:
          case Jc:
            _ = Bf;
            break;
          case qc:
            _ = eg;
            break;
          case "scroll":
            _ = If;
            break;
          case "wheel":
            _ = ng;
            break;
          case "copy":
          case "cut":
          case "paste":
            _ = Hf;
            break;
          case "gotpointercapture":
          case "lostpointercapture":
          case "pointercancel":
          case "pointerdown":
          case "pointermove":
          case "pointerout":
          case "pointerover":
          case "pointerup":
            _ = Uu;
        }
        var h = (t & 4) !== 0,
          E = !h && e === "scroll",
          d = h ? (v !== null ? v + "Capture" : null) : v;
        h = [];
        for (var c = s, g; c !== null; ) {
          g = c;
          var y = g.stateNode;
          if (
            (g.tag === 5 &&
              y !== null &&
              ((g = y),
              d !== null && ((y = cr(c, d)), y != null && h.push(_r(c, y, g)))),
            E)
          )
            break;
          c = c.return;
        }
        0 < h.length &&
          ((v = new _(v, m, null, n, p)), f.push({ event: v, listeners: h }));
      }
    }
    if (!(t & 7)) {
      e: {
        if (
          ((v = e === "mouseover" || e === "pointerover"),
          (_ = e === "mouseout" || e === "pointerout"),
          v &&
            n !== Po &&
            (m = n.relatedTarget || n.fromElement) &&
            (Vt(m) || m[ot]))
        )
          break e;
        if (
          (_ || v) &&
          ((v =
            p.window === p
              ? p
              : (v = p.ownerDocument)
              ? v.defaultView || v.parentWindow
              : window),
          _
            ? ((m = n.relatedTarget || n.toElement),
              (_ = s),
              (m = m ? Vt(m) : null),
              m !== null &&
                ((E = nn(m)), m !== E || (m.tag !== 5 && m.tag !== 6)) &&
                (m = null))
            : ((_ = null), (m = s)),
          _ !== m)
        ) {
          if (
            ((h = Iu),
            (y = "onMouseLeave"),
            (d = "onMouseEnter"),
            (c = "mouse"),
            (e === "pointerout" || e === "pointerover") &&
              ((h = Uu),
              (y = "onPointerLeave"),
              (d = "onPointerEnter"),
              (c = "pointer")),
            (E = _ == null ? v : pn(_)),
            (g = m == null ? v : pn(m)),
            (v = new h(y, c + "leave", _, n, p)),
            (v.target = E),
            (v.relatedTarget = g),
            (y = null),
            Vt(p) === s &&
              ((h = new h(d, c + "enter", m, n, p)),
              (h.target = g),
              (h.relatedTarget = E),
              (y = h)),
            (E = y),
            _ && m)
          )
            t: {
              for (h = _, d = m, c = 0, g = h; g; g = an(g)) c++;
              for (g = 0, y = d; y; y = an(y)) g++;
              for (; 0 < c - g; ) (h = an(h)), c--;
              for (; 0 < g - c; ) (d = an(d)), g--;
              for (; c--; ) {
                if (h === d || (d !== null && h === d.alternate)) break t;
                (h = an(h)), (d = an(d));
              }
              h = null;
            }
          else h = null;
          _ !== null && $u(f, v, _, h, !1),
            m !== null && E !== null && $u(f, E, m, h, !0);
        }
      }
      e: {
        if (
          ((v = s ? pn(s) : window),
          (_ = v.nodeName && v.nodeName.toLowerCase()),
          _ === "select" || (_ === "input" && v.type === "file"))
        )
          var j = sg;
        else if (Bu(v))
          if (Vc) j = fg;
          else {
            j = dg;
            var x = cg;
          }
        else
          (_ = v.nodeName) &&
            _.toLowerCase() === "input" &&
            (v.type === "checkbox" || v.type === "radio") &&
            (j = pg);
        if (j && (j = j(e, s))) {
          bc(f, j, n, p);
          break e;
        }
        x && x(e, v, s),
          e === "focusout" &&
            (x = v._wrapperState) &&
            x.controlled &&
            v.type === "number" &&
            Eo(v, "number", v.value);
      }
      switch (((x = s ? pn(s) : window), e)) {
        case "focusin":
          (Bu(x) || x.contentEditable === "true") &&
            ((cn = x), (Io = s), (nr = null));
          break;
        case "focusout":
          nr = Io = cn = null;
          break;
        case "mousedown":
          zo = !0;
          break;
        case "contextmenu":
        case "mouseup":
        case "dragend":
          (zo = !1), Yu(f, n, p);
          break;
        case "selectionchange":
          if (hg) break;
        case "keydown":
        case "keyup":
          Yu(f, n, p);
      }
      var P;
      if (Dl)
        e: {
          switch (e) {
            case "compositionstart":
              var N = "onCompositionStart";
              break e;
            case "compositionend":
              N = "onCompositionEnd";
              break e;
            case "compositionupdate":
              N = "onCompositionUpdate";
              break e;
          }
          N = void 0;
        }
      else
        sn
          ? Wc(e, n) && (N = "onCompositionEnd")
          : e === "keydown" && n.keyCode === 229 && (N = "onCompositionStart");
      N &&
        (Bc &&
          n.locale !== "ko" &&
          (sn || N !== "onCompositionStart"
            ? N === "onCompositionEnd" && sn && (P = Fc())
            : ((_t = p),
              (Rl = "value" in _t ? _t.value : _t.textContent),
              (sn = !0))),
        (x = Ca(s, N)),
        0 < x.length &&
          ((N = new zu(N, e, null, n, p)),
          f.push({ event: N, listeners: x }),
          P ? (N.data = P) : ((P = Hc(n)), P !== null && (N.data = P)))),
        (P = ag ? ig(e, n) : og(e, n)) &&
          ((s = Ca(s, "onBeforeInput")),
          0 < s.length &&
            ((p = new zu("onBeforeInput", "beforeinput", null, n, p)),
            f.push({ event: p, listeners: s }),
            (p.data = P)));
    }
    ed(f, t);
  });
}
function _r(e, t, n) {
  return { instance: e, listener: t, currentTarget: n };
}
function Ca(e, t) {
  for (var n = t + "Capture", r = []; e !== null; ) {
    var a = e,
      i = a.stateNode;
    a.tag === 5 &&
      i !== null &&
      ((a = i),
      (i = cr(e, n)),
      i != null && r.unshift(_r(e, i, a)),
      (i = cr(e, t)),
      i != null && r.push(_r(e, i, a))),
      (e = e.return);
  }
  return r;
}
function an(e) {
  if (e === null) return null;
  do e = e.return;
  while (e && e.tag !== 5);
  return e || null;
}
function $u(e, t, n, r, a) {
  for (var i = t._reactName, o = []; n !== null && n !== r; ) {
    var l = n,
      u = l.alternate,
      s = l.stateNode;
    if (u !== null && u === r) break;
    l.tag === 5 &&
      s !== null &&
      ((l = s),
      a
        ? ((u = cr(n, i)), u != null && o.unshift(_r(n, u, l)))
        : a || ((u = cr(n, i)), u != null && o.push(_r(n, u, l)))),
      (n = n.return);
  }
  o.length !== 0 && e.push({ event: t, listeners: o });
}
var wg = /\r\n?/g,
  Sg = /\u0000|\uFFFD/g;
function Ku(e) {
  return (typeof e == "string" ? e : "" + e)
    .replace(
      wg,
      `
`
    )
    .replace(Sg, "");
}
function Xr(e, t, n) {
  if (((t = Ku(t)), Ku(e) !== t && n)) throw Error(k(425));
}
function xa() {}
var Uo = null,
  Ao = null;
function Fo(e, t) {
  return (
    e === "textarea" ||
    e === "noscript" ||
    typeof t.children == "string" ||
    typeof t.children == "number" ||
    (typeof t.dangerouslySetInnerHTML == "object" &&
      t.dangerouslySetInnerHTML !== null &&
      t.dangerouslySetInnerHTML.__html != null)
  );
}
var Bo = typeof setTimeout == "function" ? setTimeout : void 0,
  kg = typeof clearTimeout == "function" ? clearTimeout : void 0,
  Xu = typeof Promise == "function" ? Promise : void 0,
  Eg =
    typeof queueMicrotask == "function"
      ? queueMicrotask
      : typeof Xu < "u"
      ? function (e) {
          return Xu.resolve(null).then(e).catch(jg);
        }
      : Bo;
function jg(e) {
  setTimeout(function () {
    throw e;
  });
}
function Xi(e, t) {
  var n = t,
    r = 0;
  do {
    var a = n.nextSibling;
    if ((e.removeChild(n), a && a.nodeType === 8))
      if (((n = a.data), n === "/$")) {
        if (r === 0) {
          e.removeChild(a), fr(t);
          return;
        }
        r--;
      } else (n !== "$" && n !== "$?" && n !== "$!") || r++;
    n = a;
  } while (n);
  fr(t);
}
function Et(e) {
  for (; e != null; e = e.nextSibling) {
    var t = e.nodeType;
    if (t === 1 || t === 3) break;
    if (t === 8) {
      if (((t = e.data), t === "$" || t === "$!" || t === "$?")) break;
      if (t === "/$") return null;
    }
  }
  return e;
}
function Ju(e) {
  e = e.previousSibling;
  for (var t = 0; e; ) {
    if (e.nodeType === 8) {
      var n = e.data;
      if (n === "$" || n === "$!" || n === "$?") {
        if (t === 0) return e;
        t--;
      } else n === "/$" && t++;
    }
    e = e.previousSibling;
  }
  return null;
}
var In = Math.random().toString(36).slice(2),
  Xe = "__reactFiber$" + In,
  mr = "__reactProps$" + In,
  ot = "__reactContainer$" + In,
  Wo = "__reactEvents$" + In,
  Cg = "__reactListeners$" + In,
  xg = "__reactHandles$" + In;
function Vt(e) {
  var t = e[Xe];
  if (t) return t;
  for (var n = e.parentNode; n; ) {
    if ((t = n[ot] || n[Xe])) {
      if (
        ((n = t.alternate),
        t.child !== null || (n !== null && n.child !== null))
      )
        for (e = Ju(e); e !== null; ) {
          if ((n = e[Xe])) return n;
          e = Ju(e);
        }
      return t;
    }
    (e = n), (n = e.parentNode);
  }
  return null;
}
function Lr(e) {
  return (
    (e = e[Xe] || e[ot]),
    !e || (e.tag !== 5 && e.tag !== 6 && e.tag !== 13 && e.tag !== 3) ? null : e
  );
}
function pn(e) {
  if (e.tag === 5 || e.tag === 6) return e.stateNode;
  throw Error(k(33));
}
function qa(e) {
  return e[mr] || null;
}
var Ho = [],
  fn = -1;
function Mt(e) {
  return { current: e };
}
function b(e) {
  0 > fn || ((e.current = Ho[fn]), (Ho[fn] = null), fn--);
}
function B(e, t) {
  fn++, (Ho[fn] = e.current), (e.current = t);
}
var Ot = {},
  fe = Mt(Ot),
  ke = Mt(!1),
  Kt = Ot;
function xn(e, t) {
  var n = e.type.contextTypes;
  if (!n) return Ot;
  var r = e.stateNode;
  if (r && r.__reactInternalMemoizedUnmaskedChildContext === t)
    return r.__reactInternalMemoizedMaskedChildContext;
  var a = {},
    i;
  for (i in n) a[i] = t[i];
  return (
    r &&
      ((e = e.stateNode),
      (e.__reactInternalMemoizedUnmaskedChildContext = t),
      (e.__reactInternalMemoizedMaskedChildContext = a)),
    a
  );
}
function Ee(e) {
  return (e = e.childContextTypes), e != null;
}
function Ta() {
  b(ke), b(fe);
}
function qu(e, t, n) {
  if (fe.current !== Ot) throw Error(k(168));
  B(fe, t), B(ke, n);
}
function nd(e, t, n) {
  var r = e.stateNode;
  if (((t = t.childContextTypes), typeof r.getChildContext != "function"))
    return n;
  r = r.getChildContext();
  for (var a in r) if (!(a in t)) throw Error(k(108, cf(e) || "Unknown", a));
  return Q({}, n, r);
}
function Pa(e) {
  return (
    (e =
      ((e = e.stateNode) && e.__reactInternalMemoizedMergedChildContext) || Ot),
    (Kt = fe.current),
    B(fe, e),
    B(ke, ke.current),
    !0
  );
}
function Zu(e, t, n) {
  var r = e.stateNode;
  if (!r) throw Error(k(169));
  n
    ? ((e = nd(e, t, Kt)),
      (r.__reactInternalMemoizedMergedChildContext = e),
      b(ke),
      b(fe),
      B(fe, e))
    : b(ke),
    B(ke, n);
}
var tt = null,
  Za = !1,
  Ji = !1;
function rd(e) {
  tt === null ? (tt = [e]) : tt.push(e);
}
function Tg(e) {
  (Za = !0), rd(e);
}
function Dt() {
  if (!Ji && tt !== null) {
    Ji = !0;
    var e = 0,
      t = U;
    try {
      var n = tt;
      for (U = 1; e < n.length; e++) {
        var r = n[e];
        do r = r(!0);
        while (r !== null);
      }
      (tt = null), (Za = !1);
    } catch (a) {
      throw (tt !== null && (tt = tt.slice(e + 1)), Pc(Tl, Dt), a);
    } finally {
      (U = t), (Ji = !1);
    }
  }
  return null;
}
var gn = [],
  vn = 0,
  Na = null,
  Oa = 0,
  De = [],
  Ie = 0,
  Xt = null,
  nt = 1,
  rt = "";
function Bt(e, t) {
  (gn[vn++] = Oa), (gn[vn++] = Na), (Na = e), (Oa = t);
}
function ad(e, t, n) {
  (De[Ie++] = nt), (De[Ie++] = rt), (De[Ie++] = Xt), (Xt = e);
  var r = nt;
  e = rt;
  var a = 32 - Ye(r) - 1;
  (r &= ~(1 << a)), (n += 1);
  var i = 32 - Ye(t) + a;
  if (30 < i) {
    var o = a - (a % 5);
    (i = (r & ((1 << o) - 1)).toString(32)),
      (r >>= o),
      (a -= o),
      (nt = (1 << (32 - Ye(t) + a)) | (n << a) | r),
      (rt = i + e);
  } else (nt = (1 << i) | (n << a) | r), (rt = e);
}
function zl(e) {
  e.return !== null && (Bt(e, 1), ad(e, 1, 0));
}
function Ul(e) {
  for (; e === Na; )
    (Na = gn[--vn]), (gn[vn] = null), (Oa = gn[--vn]), (gn[vn] = null);
  for (; e === Xt; )
    (Xt = De[--Ie]),
      (De[Ie] = null),
      (rt = De[--Ie]),
      (De[Ie] = null),
      (nt = De[--Ie]),
      (De[Ie] = null);
}
var Pe = null,
  Te = null,
  V = !1,
  Ve = null;
function id(e, t) {
  var n = ze(5, null, null, 0);
  (n.elementType = "DELETED"),
    (n.stateNode = t),
    (n.return = e),
    (t = e.deletions),
    t === null ? ((e.deletions = [n]), (e.flags |= 16)) : t.push(n);
}
function es(e, t) {
  switch (e.tag) {
    case 5:
      var n = e.type;
      return (
        (t =
          t.nodeType !== 1 || n.toLowerCase() !== t.nodeName.toLowerCase()
            ? null
            : t),
        t !== null
          ? ((e.stateNode = t), (Pe = e), (Te = Et(t.firstChild)), !0)
          : !1
      );
    case 6:
      return (
        (t = e.pendingProps === "" || t.nodeType !== 3 ? null : t),
        t !== null ? ((e.stateNode = t), (Pe = e), (Te = null), !0) : !1
      );
    case 13:
      return (
        (t = t.nodeType !== 8 ? null : t),
        t !== null
          ? ((n = Xt !== null ? { id: nt, overflow: rt } : null),
            (e.memoizedState = {
              dehydrated: t,
              treeContext: n,
              retryLane: 1073741824,
            }),
            (n = ze(18, null, null, 0)),
            (n.stateNode = t),
            (n.return = e),
            (e.child = n),
            (Pe = e),
            (Te = null),
            !0)
          : !1
      );
    default:
      return !1;
  }
}
function bo(e) {
  return (e.mode & 1) !== 0 && (e.flags & 128) === 0;
}
function Vo(e) {
  if (V) {
    var t = Te;
    if (t) {
      var n = t;
      if (!es(e, t)) {
        if (bo(e)) throw Error(k(418));
        t = Et(n.nextSibling);
        var r = Pe;
        t && es(e, t)
          ? id(r, n)
          : ((e.flags = (e.flags & -4097) | 2), (V = !1), (Pe = e));
      }
    } else {
      if (bo(e)) throw Error(k(418));
      (e.flags = (e.flags & -4097) | 2), (V = !1), (Pe = e);
    }
  }
}
function ts(e) {
  for (e = e.return; e !== null && e.tag !== 5 && e.tag !== 3 && e.tag !== 13; )
    e = e.return;
  Pe = e;
}
function Jr(e) {
  if (e !== Pe) return !1;
  if (!V) return ts(e), (V = !0), !1;
  var t;
  if (
    ((t = e.tag !== 3) &&
      !(t = e.tag !== 5) &&
      ((t = e.type),
      (t = t !== "head" && t !== "body" && !Fo(e.type, e.memoizedProps))),
    t && (t = Te))
  ) {
    if (bo(e)) throw (od(), Error(k(418)));
    for (; t; ) id(e, t), (t = Et(t.nextSibling));
  }
  if ((ts(e), e.tag === 13)) {
    if (((e = e.memoizedState), (e = e !== null ? e.dehydrated : null), !e))
      throw Error(k(317));
    e: {
      for (e = e.nextSibling, t = 0; e; ) {
        if (e.nodeType === 8) {
          var n = e.data;
          if (n === "/$") {
            if (t === 0) {
              Te = Et(e.nextSibling);
              break e;
            }
            t--;
          } else (n !== "$" && n !== "$!" && n !== "$?") || t++;
        }
        e = e.nextSibling;
      }
      Te = null;
    }
  } else Te = Pe ? Et(e.stateNode.nextSibling) : null;
  return !0;
}
function od() {
  for (var e = Te; e; ) e = Et(e.nextSibling);
}
function Tn() {
  (Te = Pe = null), (V = !1);
}
function Al(e) {
  Ve === null ? (Ve = [e]) : Ve.push(e);
}
var Pg = st.ReactCurrentBatchConfig;
function He(e, t) {
  if (e && e.defaultProps) {
    (t = Q({}, t)), (e = e.defaultProps);
    for (var n in e) t[n] === void 0 && (t[n] = e[n]);
    return t;
  }
  return t;
}
var Ra = Mt(null),
  La = null,
  hn = null,
  Fl = null;
function Bl() {
  Fl = hn = La = null;
}
function Wl(e) {
  var t = Ra.current;
  b(Ra), (e._currentValue = t);
}
function Yo(e, t, n) {
  for (; e !== null; ) {
    var r = e.alternate;
    if (
      ((e.childLanes & t) !== t
        ? ((e.childLanes |= t), r !== null && (r.childLanes |= t))
        : r !== null && (r.childLanes & t) !== t && (r.childLanes |= t),
      e === n)
    )
      break;
    e = e.return;
  }
}
function En(e, t) {
  (La = e),
    (Fl = hn = null),
    (e = e.dependencies),
    e !== null &&
      e.firstContext !== null &&
      (e.lanes & t && (Se = !0), (e.firstContext = null));
}
function Ae(e) {
  var t = e._currentValue;
  if (Fl !== e)
    if (((e = { context: e, memoizedValue: t, next: null }), hn === null)) {
      if (La === null) throw Error(k(308));
      (hn = e), (La.dependencies = { lanes: 0, firstContext: e });
    } else hn = hn.next = e;
  return t;
}
var Yt = null;
function Hl(e) {
  Yt === null ? (Yt = [e]) : Yt.push(e);
}
function ld(e, t, n, r) {
  var a = t.interleaved;
  return (
    a === null ? ((n.next = n), Hl(t)) : ((n.next = a.next), (a.next = n)),
    (t.interleaved = n),
    lt(e, r)
  );
}
function lt(e, t) {
  e.lanes |= t;
  var n = e.alternate;
  for (n !== null && (n.lanes |= t), n = e, e = e.return; e !== null; )
    (e.childLanes |= t),
      (n = e.alternate),
      n !== null && (n.childLanes |= t),
      (n = e),
      (e = e.return);
  return n.tag === 3 ? n.stateNode : null;
}
var gt = !1;
function bl(e) {
  e.updateQueue = {
    baseState: e.memoizedState,
    firstBaseUpdate: null,
    lastBaseUpdate: null,
    shared: { pending: null, interleaved: null, lanes: 0 },
    effects: null,
  };
}
function ud(e, t) {
  (e = e.updateQueue),
    t.updateQueue === e &&
      (t.updateQueue = {
        baseState: e.baseState,
        firstBaseUpdate: e.firstBaseUpdate,
        lastBaseUpdate: e.lastBaseUpdate,
        shared: e.shared,
        effects: e.effects,
      });
}
function at(e, t) {
  return {
    eventTime: e,
    lane: t,
    tag: 0,
    payload: null,
    callback: null,
    next: null,
  };
}
function jt(e, t, n) {
  var r = e.updateQueue;
  if (r === null) return null;
  if (((r = r.shared), I & 2)) {
    var a = r.pending;
    return (
      a === null ? (t.next = t) : ((t.next = a.next), (a.next = t)),
      (r.pending = t),
      lt(e, n)
    );
  }
  return (
    (a = r.interleaved),
    a === null ? ((t.next = t), Hl(r)) : ((t.next = a.next), (a.next = t)),
    (r.interleaved = t),
    lt(e, n)
  );
}
function ua(e, t, n) {
  if (
    ((t = t.updateQueue), t !== null && ((t = t.shared), (n & 4194240) !== 0))
  ) {
    var r = t.lanes;
    (r &= e.pendingLanes), (n |= r), (t.lanes = n), Pl(e, n);
  }
}
function ns(e, t) {
  var n = e.updateQueue,
    r = e.alternate;
  if (r !== null && ((r = r.updateQueue), n === r)) {
    var a = null,
      i = null;
    if (((n = n.firstBaseUpdate), n !== null)) {
      do {
        var o = {
          eventTime: n.eventTime,
          lane: n.lane,
          tag: n.tag,
          payload: n.payload,
          callback: n.callback,
          next: null,
        };
        i === null ? (a = i = o) : (i = i.next = o), (n = n.next);
      } while (n !== null);
      i === null ? (a = i = t) : (i = i.next = t);
    } else a = i = t;
    (n = {
      baseState: r.baseState,
      firstBaseUpdate: a,
      lastBaseUpdate: i,
      shared: r.shared,
      effects: r.effects,
    }),
      (e.updateQueue = n);
    return;
  }
  (e = n.lastBaseUpdate),
    e === null ? (n.firstBaseUpdate = t) : (e.next = t),
    (n.lastBaseUpdate = t);
}
function Ma(e, t, n, r) {
  var a = e.updateQueue;
  gt = !1;
  var i = a.firstBaseUpdate,
    o = a.lastBaseUpdate,
    l = a.shared.pending;
  if (l !== null) {
    a.shared.pending = null;
    var u = l,
      s = u.next;
    (u.next = null), o === null ? (i = s) : (o.next = s), (o = u);
    var p = e.alternate;
    p !== null &&
      ((p = p.updateQueue),
      (l = p.lastBaseUpdate),
      l !== o &&
        (l === null ? (p.firstBaseUpdate = s) : (l.next = s),
        (p.lastBaseUpdate = u)));
  }
  if (i !== null) {
    var f = a.baseState;
    (o = 0), (p = s = u = null), (l = i);
    do {
      var v = l.lane,
        _ = l.eventTime;
      if ((r & v) === v) {
        p !== null &&
          (p = p.next =
            {
              eventTime: _,
              lane: 0,
              tag: l.tag,
              payload: l.payload,
              callback: l.callback,
              next: null,
            });
        e: {
          var m = e,
            h = l;
          switch (((v = t), (_ = n), h.tag)) {
            case 1:
              if (((m = h.payload), typeof m == "function")) {
                f = m.call(_, f, v);
                break e;
              }
              f = m;
              break e;
            case 3:
              m.flags = (m.flags & -65537) | 128;
            case 0:
              if (
                ((m = h.payload),
                (v = typeof m == "function" ? m.call(_, f, v) : m),
                v == null)
              )
                break e;
              f = Q({}, f, v);
              break e;
            case 2:
              gt = !0;
          }
        }
        l.callback !== null &&
          l.lane !== 0 &&
          ((e.flags |= 64),
          (v = a.effects),
          v === null ? (a.effects = [l]) : v.push(l));
      } else
        (_ = {
          eventTime: _,
          lane: v,
          tag: l.tag,
          payload: l.payload,
          callback: l.callback,
          next: null,
        }),
          p === null ? ((s = p = _), (u = f)) : (p = p.next = _),
          (o |= v);
      if (((l = l.next), l === null)) {
        if (((l = a.shared.pending), l === null)) break;
        (v = l),
          (l = v.next),
          (v.next = null),
          (a.lastBaseUpdate = v),
          (a.shared.pending = null);
      }
    } while (1);
    if (
      (p === null && (u = f),
      (a.baseState = u),
      (a.firstBaseUpdate = s),
      (a.lastBaseUpdate = p),
      (t = a.shared.interleaved),
      t !== null)
    ) {
      a = t;
      do (o |= a.lane), (a = a.next);
      while (a !== t);
    } else i === null && (a.shared.lanes = 0);
    (qt |= o), (e.lanes = o), (e.memoizedState = f);
  }
}
function rs(e, t, n) {
  if (((e = t.effects), (t.effects = null), e !== null))
    for (t = 0; t < e.length; t++) {
      var r = e[t],
        a = r.callback;
      if (a !== null) {
        if (((r.callback = null), (r = n), typeof a != "function"))
          throw Error(k(191, a));
        a.call(r);
      }
    }
}
var sd = new lc.Component().refs;
function Go(e, t, n, r) {
  (t = e.memoizedState),
    (n = n(r, t)),
    (n = n == null ? t : Q({}, t, n)),
    (e.memoizedState = n),
    e.lanes === 0 && (e.updateQueue.baseState = n);
}
var ei = {
  isMounted: function (e) {
    return (e = e._reactInternals) ? nn(e) === e : !1;
  },
  enqueueSetState: function (e, t, n) {
    e = e._reactInternals;
    var r = he(),
      a = xt(e),
      i = at(r, a);
    (i.payload = t),
      n != null && (i.callback = n),
      (t = jt(e, i, a)),
      t !== null && (Ge(t, e, a, r), ua(t, e, a));
  },
  enqueueReplaceState: function (e, t, n) {
    e = e._reactInternals;
    var r = he(),
      a = xt(e),
      i = at(r, a);
    (i.tag = 1),
      (i.payload = t),
      n != null && (i.callback = n),
      (t = jt(e, i, a)),
      t !== null && (Ge(t, e, a, r), ua(t, e, a));
  },
  enqueueForceUpdate: function (e, t) {
    e = e._reactInternals;
    var n = he(),
      r = xt(e),
      a = at(n, r);
    (a.tag = 2),
      t != null && (a.callback = t),
      (t = jt(e, a, r)),
      t !== null && (Ge(t, e, r, n), ua(t, e, r));
  },
};
function as(e, t, n, r, a, i, o) {
  return (
    (e = e.stateNode),
    typeof e.shouldComponentUpdate == "function"
      ? e.shouldComponentUpdate(r, i, o)
      : t.prototype && t.prototype.isPureReactComponent
      ? !vr(n, r) || !vr(a, i)
      : !0
  );
}
function cd(e, t, n) {
  var r = !1,
    a = Ot,
    i = t.contextType;
  return (
    typeof i == "object" && i !== null
      ? (i = Ae(i))
      : ((a = Ee(t) ? Kt : fe.current),
        (r = t.contextTypes),
        (i = (r = r != null) ? xn(e, a) : Ot)),
    (t = new t(n, i)),
    (e.memoizedState = t.state !== null && t.state !== void 0 ? t.state : null),
    (t.updater = ei),
    (e.stateNode = t),
    (t._reactInternals = e),
    r &&
      ((e = e.stateNode),
      (e.__reactInternalMemoizedUnmaskedChildContext = a),
      (e.__reactInternalMemoizedMaskedChildContext = i)),
    t
  );
}
function is(e, t, n, r) {
  (e = t.state),
    typeof t.componentWillReceiveProps == "function" &&
      t.componentWillReceiveProps(n, r),
    typeof t.UNSAFE_componentWillReceiveProps == "function" &&
      t.UNSAFE_componentWillReceiveProps(n, r),
    t.state !== e && ei.enqueueReplaceState(t, t.state, null);
}
function Qo(e, t, n, r) {
  var a = e.stateNode;
  (a.props = n), (a.state = e.memoizedState), (a.refs = sd), bl(e);
  var i = t.contextType;
  typeof i == "object" && i !== null
    ? (a.context = Ae(i))
    : ((i = Ee(t) ? Kt : fe.current), (a.context = xn(e, i))),
    (a.state = e.memoizedState),
    (i = t.getDerivedStateFromProps),
    typeof i == "function" && (Go(e, t, i, n), (a.state = e.memoizedState)),
    typeof t.getDerivedStateFromProps == "function" ||
      typeof a.getSnapshotBeforeUpdate == "function" ||
      (typeof a.UNSAFE_componentWillMount != "function" &&
        typeof a.componentWillMount != "function") ||
      ((t = a.state),
      typeof a.componentWillMount == "function" && a.componentWillMount(),
      typeof a.UNSAFE_componentWillMount == "function" &&
        a.UNSAFE_componentWillMount(),
      t !== a.state && ei.enqueueReplaceState(a, a.state, null),
      Ma(e, n, a, r),
      (a.state = e.memoizedState)),
    typeof a.componentDidMount == "function" && (e.flags |= 4194308);
}
function bn(e, t, n) {
  if (
    ((e = n.ref), e !== null && typeof e != "function" && typeof e != "object")
  ) {
    if (n._owner) {
      if (((n = n._owner), n)) {
        if (n.tag !== 1) throw Error(k(309));
        var r = n.stateNode;
      }
      if (!r) throw Error(k(147, e));
      var a = r,
        i = "" + e;
      return t !== null &&
        t.ref !== null &&
        typeof t.ref == "function" &&
        t.ref._stringRef === i
        ? t.ref
        : ((t = function (o) {
            var l = a.refs;
            l === sd && (l = a.refs = {}),
              o === null ? delete l[i] : (l[i] = o);
          }),
          (t._stringRef = i),
          t);
    }
    if (typeof e != "string") throw Error(k(284));
    if (!n._owner) throw Error(k(290, e));
  }
  return e;
}
function qr(e, t) {
  throw (
    ((e = Object.prototype.toString.call(t)),
    Error(
      k(
        31,
        e === "[object Object]"
          ? "object with keys {" + Object.keys(t).join(", ") + "}"
          : e
      )
    ))
  );
}
function os(e) {
  var t = e._init;
  return t(e._payload);
}
function dd(e) {
  function t(d, c) {
    if (e) {
      var g = d.deletions;
      g === null ? ((d.deletions = [c]), (d.flags |= 16)) : g.push(c);
    }
  }
  function n(d, c) {
    if (!e) return null;
    for (; c !== null; ) t(d, c), (c = c.sibling);
    return null;
  }
  function r(d, c) {
    for (d = new Map(); c !== null; )
      c.key !== null ? d.set(c.key, c) : d.set(c.index, c), (c = c.sibling);
    return d;
  }
  function a(d, c) {
    return (d = Tt(d, c)), (d.index = 0), (d.sibling = null), d;
  }
  function i(d, c, g) {
    return (
      (d.index = g),
      e
        ? ((g = d.alternate),
          g !== null
            ? ((g = g.index), g < c ? ((d.flags |= 2), c) : g)
            : ((d.flags |= 2), c))
        : ((d.flags |= 1048576), c)
    );
  }
  function o(d) {
    return e && d.alternate === null && (d.flags |= 2), d;
  }
  function l(d, c, g, y) {
    return c === null || c.tag !== 6
      ? ((c = ao(g, d.mode, y)), (c.return = d), c)
      : ((c = a(c, g)), (c.return = d), c);
  }
  function u(d, c, g, y) {
    var j = g.type;
    return j === un
      ? p(d, c, g.props.children, y, g.key)
      : c !== null &&
        (c.elementType === j ||
          (typeof j == "object" &&
            j !== null &&
            j.$$typeof === ft &&
            os(j) === c.type))
      ? ((y = a(c, g.props)), (y.ref = bn(d, c, g)), (y.return = d), y)
      : ((y = ga(g.type, g.key, g.props, null, d.mode, y)),
        (y.ref = bn(d, c, g)),
        (y.return = d),
        y);
  }
  function s(d, c, g, y) {
    return c === null ||
      c.tag !== 4 ||
      c.stateNode.containerInfo !== g.containerInfo ||
      c.stateNode.implementation !== g.implementation
      ? ((c = io(g, d.mode, y)), (c.return = d), c)
      : ((c = a(c, g.children || [])), (c.return = d), c);
  }
  function p(d, c, g, y, j) {
    return c === null || c.tag !== 7
      ? ((c = $t(g, d.mode, y, j)), (c.return = d), c)
      : ((c = a(c, g)), (c.return = d), c);
  }
  function f(d, c, g) {
    if ((typeof c == "string" && c !== "") || typeof c == "number")
      return (c = ao("" + c, d.mode, g)), (c.return = d), c;
    if (typeof c == "object" && c !== null) {
      switch (c.$$typeof) {
        case Wr:
          return (
            (g = ga(c.type, c.key, c.props, null, d.mode, g)),
            (g.ref = bn(d, null, c)),
            (g.return = d),
            g
          );
        case ln:
          return (c = io(c, d.mode, g)), (c.return = d), c;
        case ft:
          var y = c._init;
          return f(d, y(c._payload), g);
      }
      if (Kn(c) || An(c))
        return (c = $t(c, d.mode, g, null)), (c.return = d), c;
      qr(d, c);
    }
    return null;
  }
  function v(d, c, g, y) {
    var j = c !== null ? c.key : null;
    if ((typeof g == "string" && g !== "") || typeof g == "number")
      return j !== null ? null : l(d, c, "" + g, y);
    if (typeof g == "object" && g !== null) {
      switch (g.$$typeof) {
        case Wr:
          return g.key === j ? u(d, c, g, y) : null;
        case ln:
          return g.key === j ? s(d, c, g, y) : null;
        case ft:
          return (j = g._init), v(d, c, j(g._payload), y);
      }
      if (Kn(g) || An(g)) return j !== null ? null : p(d, c, g, y, null);
      qr(d, g);
    }
    return null;
  }
  function _(d, c, g, y, j) {
    if ((typeof y == "string" && y !== "") || typeof y == "number")
      return (d = d.get(g) || null), l(c, d, "" + y, j);
    if (typeof y == "object" && y !== null) {
      switch (y.$$typeof) {
        case Wr:
          return (d = d.get(y.key === null ? g : y.key) || null), u(c, d, y, j);
        case ln:
          return (d = d.get(y.key === null ? g : y.key) || null), s(c, d, y, j);
        case ft:
          var x = y._init;
          return _(d, c, g, x(y._payload), j);
      }
      if (Kn(y) || An(y)) return (d = d.get(g) || null), p(c, d, y, j, null);
      qr(c, y);
    }
    return null;
  }
  function m(d, c, g, y) {
    for (
      var j = null, x = null, P = c, N = (c = 0), W = null;
      P !== null && N < g.length;
      N++
    ) {
      P.index > N ? ((W = P), (P = null)) : (W = P.sibling);
      var L = v(d, P, g[N], y);
      if (L === null) {
        P === null && (P = W);
        break;
      }
      e && P && L.alternate === null && t(d, P),
        (c = i(L, c, N)),
        x === null ? (j = L) : (x.sibling = L),
        (x = L),
        (P = W);
    }
    if (N === g.length) return n(d, P), V && Bt(d, N), j;
    if (P === null) {
      for (; N < g.length; N++)
        (P = f(d, g[N], y)),
          P !== null &&
            ((c = i(P, c, N)), x === null ? (j = P) : (x.sibling = P), (x = P));
      return V && Bt(d, N), j;
    }
    for (P = r(d, P); N < g.length; N++)
      (W = _(P, d, N, g[N], y)),
        W !== null &&
          (e && W.alternate !== null && P.delete(W.key === null ? N : W.key),
          (c = i(W, c, N)),
          x === null ? (j = W) : (x.sibling = W),
          (x = W));
    return (
      e &&
        P.forEach(function (Ce) {
          return t(d, Ce);
        }),
      V && Bt(d, N),
      j
    );
  }
  function h(d, c, g, y) {
    var j = An(g);
    if (typeof j != "function") throw Error(k(150));
    if (((g = j.call(g)), g == null)) throw Error(k(151));
    for (
      var x = (j = null), P = c, N = (c = 0), W = null, L = g.next();
      P !== null && !L.done;
      N++, L = g.next()
    ) {
      P.index > N ? ((W = P), (P = null)) : (W = P.sibling);
      var Ce = v(d, P, L.value, y);
      if (Ce === null) {
        P === null && (P = W);
        break;
      }
      e && P && Ce.alternate === null && t(d, P),
        (c = i(Ce, c, N)),
        x === null ? (j = Ce) : (x.sibling = Ce),
        (x = Ce),
        (P = W);
    }
    if (L.done) return n(d, P), V && Bt(d, N), j;
    if (P === null) {
      for (; !L.done; N++, L = g.next())
        (L = f(d, L.value, y)),
          L !== null &&
            ((c = i(L, c, N)), x === null ? (j = L) : (x.sibling = L), (x = L));
      return V && Bt(d, N), j;
    }
    for (P = r(d, P); !L.done; N++, L = g.next())
      (L = _(P, d, N, L.value, y)),
        L !== null &&
          (e && L.alternate !== null && P.delete(L.key === null ? N : L.key),
          (c = i(L, c, N)),
          x === null ? (j = L) : (x.sibling = L),
          (x = L));
    return (
      e &&
        P.forEach(function (It) {
          return t(d, It);
        }),
      V && Bt(d, N),
      j
    );
  }
  function E(d, c, g, y) {
    if (
      (typeof g == "object" &&
        g !== null &&
        g.type === un &&
        g.key === null &&
        (g = g.props.children),
      typeof g == "object" && g !== null)
    ) {
      switch (g.$$typeof) {
        case Wr:
          e: {
            for (var j = g.key, x = c; x !== null; ) {
              if (x.key === j) {
                if (((j = g.type), j === un)) {
                  if (x.tag === 7) {
                    n(d, x.sibling),
                      (c = a(x, g.props.children)),
                      (c.return = d),
                      (d = c);
                    break e;
                  }
                } else if (
                  x.elementType === j ||
                  (typeof j == "object" &&
                    j !== null &&
                    j.$$typeof === ft &&
                    os(j) === x.type)
                ) {
                  n(d, x.sibling),
                    (c = a(x, g.props)),
                    (c.ref = bn(d, x, g)),
                    (c.return = d),
                    (d = c);
                  break e;
                }
                n(d, x);
                break;
              } else t(d, x);
              x = x.sibling;
            }
            g.type === un
              ? ((c = $t(g.props.children, d.mode, y, g.key)),
                (c.return = d),
                (d = c))
              : ((y = ga(g.type, g.key, g.props, null, d.mode, y)),
                (y.ref = bn(d, c, g)),
                (y.return = d),
                (d = y));
          }
          return o(d);
        case ln:
          e: {
            for (x = g.key; c !== null; ) {
              if (c.key === x)
                if (
                  c.tag === 4 &&
                  c.stateNode.containerInfo === g.containerInfo &&
                  c.stateNode.implementation === g.implementation
                ) {
                  n(d, c.sibling),
                    (c = a(c, g.children || [])),
                    (c.return = d),
                    (d = c);
                  break e;
                } else {
                  n(d, c);
                  break;
                }
              else t(d, c);
              c = c.sibling;
            }
            (c = io(g, d.mode, y)), (c.return = d), (d = c);
          }
          return o(d);
        case ft:
          return (x = g._init), E(d, c, x(g._payload), y);
      }
      if (Kn(g)) return m(d, c, g, y);
      if (An(g)) return h(d, c, g, y);
      qr(d, g);
    }
    return (typeof g == "string" && g !== "") || typeof g == "number"
      ? ((g = "" + g),
        c !== null && c.tag === 6
          ? (n(d, c.sibling), (c = a(c, g)), (c.return = d), (d = c))
          : (n(d, c), (c = ao(g, d.mode, y)), (c.return = d), (d = c)),
        o(d))
      : n(d, c);
  }
  return E;
}
var Pn = dd(!0),
  pd = dd(!1),
  Mr = {},
  qe = Mt(Mr),
  yr = Mt(Mr),
  wr = Mt(Mr);
function Gt(e) {
  if (e === Mr) throw Error(k(174));
  return e;
}
function Vl(e, t) {
  switch ((B(wr, t), B(yr, e), B(qe, Mr), (e = t.nodeType), e)) {
    case 9:
    case 11:
      t = (t = t.documentElement) ? t.namespaceURI : Co(null, "");
      break;
    default:
      (e = e === 8 ? t.parentNode : t),
        (t = e.namespaceURI || null),
        (e = e.tagName),
        (t = Co(t, e));
  }
  b(qe), B(qe, t);
}
function Nn() {
  b(qe), b(yr), b(wr);
}
function fd(e) {
  Gt(wr.current);
  var t = Gt(qe.current),
    n = Co(t, e.type);
  t !== n && (B(yr, e), B(qe, n));
}
function Yl(e) {
  yr.current === e && (b(qe), b(yr));
}
var Y = Mt(0);
function Da(e) {
  for (var t = e; t !== null; ) {
    if (t.tag === 13) {
      var n = t.memoizedState;
      if (
        n !== null &&
        ((n = n.dehydrated), n === null || n.data === "$?" || n.data === "$!")
      )
        return t;
    } else if (t.tag === 19 && t.memoizedProps.revealOrder !== void 0) {
      if (t.flags & 128) return t;
    } else if (t.child !== null) {
      (t.child.return = t), (t = t.child);
      continue;
    }
    if (t === e) break;
    for (; t.sibling === null; ) {
      if (t.return === null || t.return === e) return null;
      t = t.return;
    }
    (t.sibling.return = t.return), (t = t.sibling);
  }
  return null;
}
var qi = [];
function Gl() {
  for (var e = 0; e < qi.length; e++)
    qi[e]._workInProgressVersionPrimary = null;
  qi.length = 0;
}
var sa = st.ReactCurrentDispatcher,
  Zi = st.ReactCurrentBatchConfig,
  Jt = 0,
  G = null,
  q = null,
  ne = null,
  Ia = !1,
  rr = !1,
  Sr = 0,
  Ng = 0;
function se() {
  throw Error(k(321));
}
function Ql(e, t) {
  if (t === null) return !1;
  for (var n = 0; n < t.length && n < e.length; n++)
    if (!Qe(e[n], t[n])) return !1;
  return !0;
}
function $l(e, t, n, r, a, i) {
  if (
    ((Jt = i),
    (G = t),
    (t.memoizedState = null),
    (t.updateQueue = null),
    (t.lanes = 0),
    (sa.current = e === null || e.memoizedState === null ? Mg : Dg),
    (e = n(r, a)),
    rr)
  ) {
    i = 0;
    do {
      if (((rr = !1), (Sr = 0), 25 <= i)) throw Error(k(301));
      (i += 1),
        (ne = q = null),
        (t.updateQueue = null),
        (sa.current = Ig),
        (e = n(r, a));
    } while (rr);
  }
  if (
    ((sa.current = za),
    (t = q !== null && q.next !== null),
    (Jt = 0),
    (ne = q = G = null),
    (Ia = !1),
    t)
  )
    throw Error(k(300));
  return e;
}
function Kl() {
  var e = Sr !== 0;
  return (Sr = 0), e;
}
function Ke() {
  var e = {
    memoizedState: null,
    baseState: null,
    baseQueue: null,
    queue: null,
    next: null,
  };
  return ne === null ? (G.memoizedState = ne = e) : (ne = ne.next = e), ne;
}
function Fe() {
  if (q === null) {
    var e = G.alternate;
    e = e !== null ? e.memoizedState : null;
  } else e = q.next;
  var t = ne === null ? G.memoizedState : ne.next;
  if (t !== null) (ne = t), (q = e);
  else {
    if (e === null) throw Error(k(310));
    (q = e),
      (e = {
        memoizedState: q.memoizedState,
        baseState: q.baseState,
        baseQueue: q.baseQueue,
        queue: q.queue,
        next: null,
      }),
      ne === null ? (G.memoizedState = ne = e) : (ne = ne.next = e);
  }
  return ne;
}
function kr(e, t) {
  return typeof t == "function" ? t(e) : t;
}
function eo(e) {
  var t = Fe(),
    n = t.queue;
  if (n === null) throw Error(k(311));
  n.lastRenderedReducer = e;
  var r = q,
    a = r.baseQueue,
    i = n.pending;
  if (i !== null) {
    if (a !== null) {
      var o = a.next;
      (a.next = i.next), (i.next = o);
    }
    (r.baseQueue = a = i), (n.pending = null);
  }
  if (a !== null) {
    (i = a.next), (r = r.baseState);
    var l = (o = null),
      u = null,
      s = i;
    do {
      var p = s.lane;
      if ((Jt & p) === p)
        u !== null &&
          (u = u.next =
            {
              lane: 0,
              action: s.action,
              hasEagerState: s.hasEagerState,
              eagerState: s.eagerState,
              next: null,
            }),
          (r = s.hasEagerState ? s.eagerState : e(r, s.action));
      else {
        var f = {
          lane: p,
          action: s.action,
          hasEagerState: s.hasEagerState,
          eagerState: s.eagerState,
          next: null,
        };
        u === null ? ((l = u = f), (o = r)) : (u = u.next = f),
          (G.lanes |= p),
          (qt |= p);
      }
      s = s.next;
    } while (s !== null && s !== i);
    u === null ? (o = r) : (u.next = l),
      Qe(r, t.memoizedState) || (Se = !0),
      (t.memoizedState = r),
      (t.baseState = o),
      (t.baseQueue = u),
      (n.lastRenderedState = r);
  }
  if (((e = n.interleaved), e !== null)) {
    a = e;
    do (i = a.lane), (G.lanes |= i), (qt |= i), (a = a.next);
    while (a !== e);
  } else a === null && (n.lanes = 0);
  return [t.memoizedState, n.dispatch];
}
function to(e) {
  var t = Fe(),
    n = t.queue;
  if (n === null) throw Error(k(311));
  n.lastRenderedReducer = e;
  var r = n.dispatch,
    a = n.pending,
    i = t.memoizedState;
  if (a !== null) {
    n.pending = null;
    var o = (a = a.next);
    do (i = e(i, o.action)), (o = o.next);
    while (o !== a);
    Qe(i, t.memoizedState) || (Se = !0),
      (t.memoizedState = i),
      t.baseQueue === null && (t.baseState = i),
      (n.lastRenderedState = i);
  }
  return [i, r];
}
function gd() {}
function vd(e, t) {
  var n = G,
    r = Fe(),
    a = t(),
    i = !Qe(r.memoizedState, a);
  if (
    (i && ((r.memoizedState = a), (Se = !0)),
    (r = r.queue),
    Xl(md.bind(null, n, r, e), [e]),
    r.getSnapshot !== t || i || (ne !== null && ne.memoizedState.tag & 1))
  ) {
    if (
      ((n.flags |= 2048),
      Er(9, _d.bind(null, n, r, a, t), void 0, null),
      re === null)
    )
      throw Error(k(349));
    Jt & 30 || hd(n, t, a);
  }
  return a;
}
function hd(e, t, n) {
  (e.flags |= 16384),
    (e = { getSnapshot: t, value: n }),
    (t = G.updateQueue),
    t === null
      ? ((t = { lastEffect: null, stores: null }),
        (G.updateQueue = t),
        (t.stores = [e]))
      : ((n = t.stores), n === null ? (t.stores = [e]) : n.push(e));
}
function _d(e, t, n, r) {
  (t.value = n), (t.getSnapshot = r), yd(t) && wd(e);
}
function md(e, t, n) {
  return n(function () {
    yd(t) && wd(e);
  });
}
function yd(e) {
  var t = e.getSnapshot;
  e = e.value;
  try {
    var n = t();
    return !Qe(e, n);
  } catch {
    return !0;
  }
}
function wd(e) {
  var t = lt(e, 1);
  t !== null && Ge(t, e, 1, -1);
}
function ls(e) {
  var t = Ke();
  return (
    typeof e == "function" && (e = e()),
    (t.memoizedState = t.baseState = e),
    (e = {
      pending: null,
      interleaved: null,
      lanes: 0,
      dispatch: null,
      lastRenderedReducer: kr,
      lastRenderedState: e,
    }),
    (t.queue = e),
    (e = e.dispatch = Lg.bind(null, G, e)),
    [t.memoizedState, e]
  );
}
function Er(e, t, n, r) {
  return (
    (e = { tag: e, create: t, destroy: n, deps: r, next: null }),
    (t = G.updateQueue),
    t === null
      ? ((t = { lastEffect: null, stores: null }),
        (G.updateQueue = t),
        (t.lastEffect = e.next = e))
      : ((n = t.lastEffect),
        n === null
          ? (t.lastEffect = e.next = e)
          : ((r = n.next), (n.next = e), (e.next = r), (t.lastEffect = e))),
    e
  );
}
function Sd() {
  return Fe().memoizedState;
}
function ca(e, t, n, r) {
  var a = Ke();
  (G.flags |= e),
    (a.memoizedState = Er(1 | t, n, void 0, r === void 0 ? null : r));
}
function ti(e, t, n, r) {
  var a = Fe();
  r = r === void 0 ? null : r;
  var i = void 0;
  if (q !== null) {
    var o = q.memoizedState;
    if (((i = o.destroy), r !== null && Ql(r, o.deps))) {
      a.memoizedState = Er(t, n, i, r);
      return;
    }
  }
  (G.flags |= e), (a.memoizedState = Er(1 | t, n, i, r));
}
function us(e, t) {
  return ca(8390656, 8, e, t);
}
function Xl(e, t) {
  return ti(2048, 8, e, t);
}
function kd(e, t) {
  return ti(4, 2, e, t);
}
function Ed(e, t) {
  return ti(4, 4, e, t);
}
function jd(e, t) {
  if (typeof t == "function")
    return (
      (e = e()),
      t(e),
      function () {
        t(null);
      }
    );
  if (t != null)
    return (
      (e = e()),
      (t.current = e),
      function () {
        t.current = null;
      }
    );
}
function Cd(e, t, n) {
  return (
    (n = n != null ? n.concat([e]) : null), ti(4, 4, jd.bind(null, t, e), n)
  );
}
function Jl() {}
function xd(e, t) {
  var n = Fe();
  t = t === void 0 ? null : t;
  var r = n.memoizedState;
  return r !== null && t !== null && Ql(t, r[1])
    ? r[0]
    : ((n.memoizedState = [e, t]), e);
}
function Td(e, t) {
  var n = Fe();
  t = t === void 0 ? null : t;
  var r = n.memoizedState;
  return r !== null && t !== null && Ql(t, r[1])
    ? r[0]
    : ((e = e()), (n.memoizedState = [e, t]), e);
}
function Pd(e, t, n) {
  return Jt & 21
    ? (Qe(n, t) || ((n = Rc()), (G.lanes |= n), (qt |= n), (e.baseState = !0)),
      t)
    : (e.baseState && ((e.baseState = !1), (Se = !0)), (e.memoizedState = n));
}
function Og(e, t) {
  var n = U;
  (U = n !== 0 && 4 > n ? n : 4), e(!0);
  var r = Zi.transition;
  Zi.transition = {};
  try {
    e(!1), t();
  } finally {
    (U = n), (Zi.transition = r);
  }
}
function Nd() {
  return Fe().memoizedState;
}
function Rg(e, t, n) {
  var r = xt(e);
  if (
    ((n = {
      lane: r,
      action: n,
      hasEagerState: !1,
      eagerState: null,
      next: null,
    }),
    Od(e))
  )
    Rd(t, n);
  else if (((n = ld(e, t, n, r)), n !== null)) {
    var a = he();
    Ge(n, e, r, a), Ld(n, t, r);
  }
}
function Lg(e, t, n) {
  var r = xt(e),
    a = { lane: r, action: n, hasEagerState: !1, eagerState: null, next: null };
  if (Od(e)) Rd(t, a);
  else {
    var i = e.alternate;
    if (
      e.lanes === 0 &&
      (i === null || i.lanes === 0) &&
      ((i = t.lastRenderedReducer), i !== null)
    )
      try {
        var o = t.lastRenderedState,
          l = i(o, n);
        if (((a.hasEagerState = !0), (a.eagerState = l), Qe(l, o))) {
          var u = t.interleaved;
          u === null
            ? ((a.next = a), Hl(t))
            : ((a.next = u.next), (u.next = a)),
            (t.interleaved = a);
          return;
        }
      } catch {
      } finally {
      }
    (n = ld(e, t, a, r)),
      n !== null && ((a = he()), Ge(n, e, r, a), Ld(n, t, r));
  }
}
function Od(e) {
  var t = e.alternate;
  return e === G || (t !== null && t === G);
}
function Rd(e, t) {
  rr = Ia = !0;
  var n = e.pending;
  n === null ? (t.next = t) : ((t.next = n.next), (n.next = t)),
    (e.pending = t);
}
function Ld(e, t, n) {
  if (n & 4194240) {
    var r = t.lanes;
    (r &= e.pendingLanes), (n |= r), (t.lanes = n), Pl(e, n);
  }
}
var za = {
    readContext: Ae,
    useCallback: se,
    useContext: se,
    useEffect: se,
    useImperativeHandle: se,
    useInsertionEffect: se,
    useLayoutEffect: se,
    useMemo: se,
    useReducer: se,
    useRef: se,
    useState: se,
    useDebugValue: se,
    useDeferredValue: se,
    useTransition: se,
    useMutableSource: se,
    useSyncExternalStore: se,
    useId: se,
    unstable_isNewReconciler: !1,
  },
  Mg = {
    readContext: Ae,
    useCallback: function (e, t) {
      return (Ke().memoizedState = [e, t === void 0 ? null : t]), e;
    },
    useContext: Ae,
    useEffect: us,
    useImperativeHandle: function (e, t, n) {
      return (
        (n = n != null ? n.concat([e]) : null),
        ca(4194308, 4, jd.bind(null, t, e), n)
      );
    },
    useLayoutEffect: function (e, t) {
      return ca(4194308, 4, e, t);
    },
    useInsertionEffect: function (e, t) {
      return ca(4, 2, e, t);
    },
    useMemo: function (e, t) {
      var n = Ke();
      return (
        (t = t === void 0 ? null : t), (e = e()), (n.memoizedState = [e, t]), e
      );
    },
    useReducer: function (e, t, n) {
      var r = Ke();
      return (
        (t = n !== void 0 ? n(t) : t),
        (r.memoizedState = r.baseState = t),
        (e = {
          pending: null,
          interleaved: null,
          lanes: 0,
          dispatch: null,
          lastRenderedReducer: e,
          lastRenderedState: t,
        }),
        (r.queue = e),
        (e = e.dispatch = Rg.bind(null, G, e)),
        [r.memoizedState, e]
      );
    },
    useRef: function (e) {
      var t = Ke();
      return (e = { current: e }), (t.memoizedState = e);
    },
    useState: ls,
    useDebugValue: Jl,
    useDeferredValue: function (e) {
      return (Ke().memoizedState = e);
    },
    useTransition: function () {
      var e = ls(!1),
        t = e[0];
      return (e = Og.bind(null, e[1])), (Ke().memoizedState = e), [t, e];
    },
    useMutableSource: function () {},
    useSyncExternalStore: function (e, t, n) {
      var r = G,
        a = Ke();
      if (V) {
        if (n === void 0) throw Error(k(407));
        n = n();
      } else {
        if (((n = t()), re === null)) throw Error(k(349));
        Jt & 30 || hd(r, t, n);
      }
      a.memoizedState = n;
      var i = { value: n, getSnapshot: t };
      return (
        (a.queue = i),
        us(md.bind(null, r, i, e), [e]),
        (r.flags |= 2048),
        Er(9, _d.bind(null, r, i, n, t), void 0, null),
        n
      );
    },
    useId: function () {
      var e = Ke(),
        t = re.identifierPrefix;
      if (V) {
        var n = rt,
          r = nt;
        (n = (r & ~(1 << (32 - Ye(r) - 1))).toString(32) + n),
          (t = ":" + t + "R" + n),
          (n = Sr++),
          0 < n && (t += "H" + n.toString(32)),
          (t += ":");
      } else (n = Ng++), (t = ":" + t + "r" + n.toString(32) + ":");
      return (e.memoizedState = t);
    },
    unstable_isNewReconciler: !1,
  },
  Dg = {
    readContext: Ae,
    useCallback: xd,
    useContext: Ae,
    useEffect: Xl,
    useImperativeHandle: Cd,
    useInsertionEffect: kd,
    useLayoutEffect: Ed,
    useMemo: Td,
    useReducer: eo,
    useRef: Sd,
    useState: function () {
      return eo(kr);
    },
    useDebugValue: Jl,
    useDeferredValue: function (e) {
      var t = Fe();
      return Pd(t, q.memoizedState, e);
    },
    useTransition: function () {
      var e = eo(kr)[0],
        t = Fe().memoizedState;
      return [e, t];
    },
    useMutableSource: gd,
    useSyncExternalStore: vd,
    useId: Nd,
    unstable_isNewReconciler: !1,
  },
  Ig = {
    readContext: Ae,
    useCallback: xd,
    useContext: Ae,
    useEffect: Xl,
    useImperativeHandle: Cd,
    useInsertionEffect: kd,
    useLayoutEffect: Ed,
    useMemo: Td,
    useReducer: to,
    useRef: Sd,
    useState: function () {
      return to(kr);
    },
    useDebugValue: Jl,
    useDeferredValue: function (e) {
      var t = Fe();
      return q === null ? (t.memoizedState = e) : Pd(t, q.memoizedState, e);
    },
    useTransition: function () {
      var e = to(kr)[0],
        t = Fe().memoizedState;
      return [e, t];
    },
    useMutableSource: gd,
    useSyncExternalStore: vd,
    useId: Nd,
    unstable_isNewReconciler: !1,
  };
function On(e, t) {
  try {
    var n = "",
      r = t;
    do (n += sf(r)), (r = r.return);
    while (r);
    var a = n;
  } catch (i) {
    a =
      `
Error generating stack: ` +
      i.message +
      `
` +
      i.stack;
  }
  return { value: e, source: t, stack: a, digest: null };
}
function no(e, t, n) {
  return { value: e, source: null, stack: n ?? null, digest: t ?? null };
}
function $o(e, t) {
  try {
    console.error(t.value);
  } catch (n) {
    setTimeout(function () {
      throw n;
    });
  }
}
var zg = typeof WeakMap == "function" ? WeakMap : Map;
function Md(e, t, n) {
  (n = at(-1, n)), (n.tag = 3), (n.payload = { element: null });
  var r = t.value;
  return (
    (n.callback = function () {
      Aa || ((Aa = !0), (al = r)), $o(e, t);
    }),
    n
  );
}
function Dd(e, t, n) {
  (n = at(-1, n)), (n.tag = 3);
  var r = e.type.getDerivedStateFromError;
  if (typeof r == "function") {
    var a = t.value;
    (n.payload = function () {
      return r(a);
    }),
      (n.callback = function () {
        $o(e, t);
      });
  }
  var i = e.stateNode;
  return (
    i !== null &&
      typeof i.componentDidCatch == "function" &&
      (n.callback = function () {
        $o(e, t),
          typeof r != "function" &&
            (Ct === null ? (Ct = new Set([this])) : Ct.add(this));
        var o = t.stack;
        this.componentDidCatch(t.value, {
          componentStack: o !== null ? o : "",
        });
      }),
    n
  );
}
function ss(e, t, n) {
  var r = e.pingCache;
  if (r === null) {
    r = e.pingCache = new zg();
    var a = new Set();
    r.set(t, a);
  } else (a = r.get(t)), a === void 0 && ((a = new Set()), r.set(t, a));
  a.has(n) || (a.add(n), (e = Xg.bind(null, e, t, n)), t.then(e, e));
}
function cs(e) {
  do {
    var t;
    if (
      ((t = e.tag === 13) &&
        ((t = e.memoizedState), (t = t !== null ? t.dehydrated !== null : !0)),
      t)
    )
      return e;
    e = e.return;
  } while (e !== null);
  return null;
}
function ds(e, t, n, r, a) {
  return e.mode & 1
    ? ((e.flags |= 65536), (e.lanes = a), e)
    : (e === t
        ? (e.flags |= 65536)
        : ((e.flags |= 128),
          (n.flags |= 131072),
          (n.flags &= -52805),
          n.tag === 1 &&
            (n.alternate === null
              ? (n.tag = 17)
              : ((t = at(-1, 1)), (t.tag = 2), jt(n, t, 1))),
          (n.lanes |= 1)),
      e);
}
var Ug = st.ReactCurrentOwner,
  Se = !1;
function ve(e, t, n, r) {
  t.child = e === null ? pd(t, null, n, r) : Pn(t, e.child, n, r);
}
function ps(e, t, n, r, a) {
  n = n.render;
  var i = t.ref;
  return (
    En(t, a),
    (r = $l(e, t, n, r, i, a)),
    (n = Kl()),
    e !== null && !Se
      ? ((t.updateQueue = e.updateQueue),
        (t.flags &= -2053),
        (e.lanes &= ~a),
        ut(e, t, a))
      : (V && n && zl(t), (t.flags |= 1), ve(e, t, r, a), t.child)
  );
}
function fs(e, t, n, r, a) {
  if (e === null) {
    var i = n.type;
    return typeof i == "function" &&
      !iu(i) &&
      i.defaultProps === void 0 &&
      n.compare === null &&
      n.defaultProps === void 0
      ? ((t.tag = 15), (t.type = i), Id(e, t, i, r, a))
      : ((e = ga(n.type, null, r, t, t.mode, a)),
        (e.ref = t.ref),
        (e.return = t),
        (t.child = e));
  }
  if (((i = e.child), !(e.lanes & a))) {
    var o = i.memoizedProps;
    if (
      ((n = n.compare), (n = n !== null ? n : vr), n(o, r) && e.ref === t.ref)
    )
      return ut(e, t, a);
  }
  return (
    (t.flags |= 1),
    (e = Tt(i, r)),
    (e.ref = t.ref),
    (e.return = t),
    (t.child = e)
  );
}
function Id(e, t, n, r, a) {
  if (e !== null) {
    var i = e.memoizedProps;
    if (vr(i, r) && e.ref === t.ref)
      if (((Se = !1), (t.pendingProps = r = i), (e.lanes & a) !== 0))
        e.flags & 131072 && (Se = !0);
      else return (t.lanes = e.lanes), ut(e, t, a);
  }
  return Ko(e, t, n, r, a);
}
function zd(e, t, n) {
  var r = t.pendingProps,
    a = r.children,
    i = e !== null ? e.memoizedState : null;
  if (r.mode === "hidden")
    if (!(t.mode & 1))
      (t.memoizedState = { baseLanes: 0, cachePool: null, transitions: null }),
        B(mn, xe),
        (xe |= n);
    else {
      if (!(n & 1073741824))
        return (
          (e = i !== null ? i.baseLanes | n : n),
          (t.lanes = t.childLanes = 1073741824),
          (t.memoizedState = {
            baseLanes: e,
            cachePool: null,
            transitions: null,
          }),
          (t.updateQueue = null),
          B(mn, xe),
          (xe |= e),
          null
        );
      (t.memoizedState = { baseLanes: 0, cachePool: null, transitions: null }),
        (r = i !== null ? i.baseLanes : n),
        B(mn, xe),
        (xe |= r);
    }
  else
    i !== null ? ((r = i.baseLanes | n), (t.memoizedState = null)) : (r = n),
      B(mn, xe),
      (xe |= r);
  return ve(e, t, a, n), t.child;
}
function Ud(e, t) {
  var n = t.ref;
  ((e === null && n !== null) || (e !== null && e.ref !== n)) &&
    ((t.flags |= 512), (t.flags |= 2097152));
}
function Ko(e, t, n, r, a) {
  var i = Ee(n) ? Kt : fe.current;
  return (
    (i = xn(t, i)),
    En(t, a),
    (n = $l(e, t, n, r, i, a)),
    (r = Kl()),
    e !== null && !Se
      ? ((t.updateQueue = e.updateQueue),
        (t.flags &= -2053),
        (e.lanes &= ~a),
        ut(e, t, a))
      : (V && r && zl(t), (t.flags |= 1), ve(e, t, n, a), t.child)
  );
}
function gs(e, t, n, r, a) {
  if (Ee(n)) {
    var i = !0;
    Pa(t);
  } else i = !1;
  if ((En(t, a), t.stateNode === null))
    da(e, t), cd(t, n, r), Qo(t, n, r, a), (r = !0);
  else if (e === null) {
    var o = t.stateNode,
      l = t.memoizedProps;
    o.props = l;
    var u = o.context,
      s = n.contextType;
    typeof s == "object" && s !== null
      ? (s = Ae(s))
      : ((s = Ee(n) ? Kt : fe.current), (s = xn(t, s)));
    var p = n.getDerivedStateFromProps,
      f =
        typeof p == "function" ||
        typeof o.getSnapshotBeforeUpdate == "function";
    f ||
      (typeof o.UNSAFE_componentWillReceiveProps != "function" &&
        typeof o.componentWillReceiveProps != "function") ||
      ((l !== r || u !== s) && is(t, o, r, s)),
      (gt = !1);
    var v = t.memoizedState;
    (o.state = v),
      Ma(t, r, o, a),
      (u = t.memoizedState),
      l !== r || v !== u || ke.current || gt
        ? (typeof p == "function" && (Go(t, n, p, r), (u = t.memoizedState)),
          (l = gt || as(t, n, l, r, v, u, s))
            ? (f ||
                (typeof o.UNSAFE_componentWillMount != "function" &&
                  typeof o.componentWillMount != "function") ||
                (typeof o.componentWillMount == "function" &&
                  o.componentWillMount(),
                typeof o.UNSAFE_componentWillMount == "function" &&
                  o.UNSAFE_componentWillMount()),
              typeof o.componentDidMount == "function" && (t.flags |= 4194308))
            : (typeof o.componentDidMount == "function" && (t.flags |= 4194308),
              (t.memoizedProps = r),
              (t.memoizedState = u)),
          (o.props = r),
          (o.state = u),
          (o.context = s),
          (r = l))
        : (typeof o.componentDidMount == "function" && (t.flags |= 4194308),
          (r = !1));
  } else {
    (o = t.stateNode),
      ud(e, t),
      (l = t.memoizedProps),
      (s = t.type === t.elementType ? l : He(t.type, l)),
      (o.props = s),
      (f = t.pendingProps),
      (v = o.context),
      (u = n.contextType),
      typeof u == "object" && u !== null
        ? (u = Ae(u))
        : ((u = Ee(n) ? Kt : fe.current), (u = xn(t, u)));
    var _ = n.getDerivedStateFromProps;
    (p =
      typeof _ == "function" ||
      typeof o.getSnapshotBeforeUpdate == "function") ||
      (typeof o.UNSAFE_componentWillReceiveProps != "function" &&
        typeof o.componentWillReceiveProps != "function") ||
      ((l !== f || v !== u) && is(t, o, r, u)),
      (gt = !1),
      (v = t.memoizedState),
      (o.state = v),
      Ma(t, r, o, a);
    var m = t.memoizedState;
    l !== f || v !== m || ke.current || gt
      ? (typeof _ == "function" && (Go(t, n, _, r), (m = t.memoizedState)),
        (s = gt || as(t, n, s, r, v, m, u) || !1)
          ? (p ||
              (typeof o.UNSAFE_componentWillUpdate != "function" &&
                typeof o.componentWillUpdate != "function") ||
              (typeof o.componentWillUpdate == "function" &&
                o.componentWillUpdate(r, m, u),
              typeof o.UNSAFE_componentWillUpdate == "function" &&
                o.UNSAFE_componentWillUpdate(r, m, u)),
            typeof o.componentDidUpdate == "function" && (t.flags |= 4),
            typeof o.getSnapshotBeforeUpdate == "function" && (t.flags |= 1024))
          : (typeof o.componentDidUpdate != "function" ||
              (l === e.memoizedProps && v === e.memoizedState) ||
              (t.flags |= 4),
            typeof o.getSnapshotBeforeUpdate != "function" ||
              (l === e.memoizedProps && v === e.memoizedState) ||
              (t.flags |= 1024),
            (t.memoizedProps = r),
            (t.memoizedState = m)),
        (o.props = r),
        (o.state = m),
        (o.context = u),
        (r = s))
      : (typeof o.componentDidUpdate != "function" ||
          (l === e.memoizedProps && v === e.memoizedState) ||
          (t.flags |= 4),
        typeof o.getSnapshotBeforeUpdate != "function" ||
          (l === e.memoizedProps && v === e.memoizedState) ||
          (t.flags |= 1024),
        (r = !1));
  }
  return Xo(e, t, n, r, i, a);
}
function Xo(e, t, n, r, a, i) {
  Ud(e, t);
  var o = (t.flags & 128) !== 0;
  if (!r && !o) return a && Zu(t, n, !1), ut(e, t, i);
  (r = t.stateNode), (Ug.current = t);
  var l =
    o && typeof n.getDerivedStateFromError != "function" ? null : r.render();
  return (
    (t.flags |= 1),
    e !== null && o
      ? ((t.child = Pn(t, e.child, null, i)), (t.child = Pn(t, null, l, i)))
      : ve(e, t, l, i),
    (t.memoizedState = r.state),
    a && Zu(t, n, !0),
    t.child
  );
}
function Ad(e) {
  var t = e.stateNode;
  t.pendingContext
    ? qu(e, t.pendingContext, t.pendingContext !== t.context)
    : t.context && qu(e, t.context, !1),
    Vl(e, t.containerInfo);
}
function vs(e, t, n, r, a) {
  return Tn(), Al(a), (t.flags |= 256), ve(e, t, n, r), t.child;
}
var Jo = { dehydrated: null, treeContext: null, retryLane: 0 };
function qo(e) {
  return { baseLanes: e, cachePool: null, transitions: null };
}
function Fd(e, t, n) {
  var r = t.pendingProps,
    a = Y.current,
    i = !1,
    o = (t.flags & 128) !== 0,
    l;
  if (
    ((l = o) ||
      (l = e !== null && e.memoizedState === null ? !1 : (a & 2) !== 0),
    l
      ? ((i = !0), (t.flags &= -129))
      : (e === null || e.memoizedState !== null) && (a |= 1),
    B(Y, a & 1),
    e === null)
  )
    return (
      Vo(t),
      (e = t.memoizedState),
      e !== null && ((e = e.dehydrated), e !== null)
        ? (t.mode & 1
            ? e.data === "$!"
              ? (t.lanes = 8)
              : (t.lanes = 1073741824)
            : (t.lanes = 1),
          null)
        : ((o = r.children),
          (e = r.fallback),
          i
            ? ((r = t.mode),
              (i = t.child),
              (o = { mode: "hidden", children: o }),
              !(r & 1) && i !== null
                ? ((i.childLanes = 0), (i.pendingProps = o))
                : (i = ai(o, r, 0, null)),
              (e = $t(e, r, n, null)),
              (i.return = t),
              (e.return = t),
              (i.sibling = e),
              (t.child = i),
              (t.child.memoizedState = qo(n)),
              (t.memoizedState = Jo),
              e)
            : ql(t, o))
    );
  if (((a = e.memoizedState), a !== null && ((l = a.dehydrated), l !== null)))
    return Ag(e, t, o, r, l, a, n);
  if (i) {
    (i = r.fallback), (o = t.mode), (a = e.child), (l = a.sibling);
    var u = { mode: "hidden", children: r.children };
    return (
      !(o & 1) && t.child !== a
        ? ((r = t.child),
          (r.childLanes = 0),
          (r.pendingProps = u),
          (t.deletions = null))
        : ((r = Tt(a, u)), (r.subtreeFlags = a.subtreeFlags & 14680064)),
      l !== null ? (i = Tt(l, i)) : ((i = $t(i, o, n, null)), (i.flags |= 2)),
      (i.return = t),
      (r.return = t),
      (r.sibling = i),
      (t.child = r),
      (r = i),
      (i = t.child),
      (o = e.child.memoizedState),
      (o =
        o === null
          ? qo(n)
          : {
              baseLanes: o.baseLanes | n,
              cachePool: null,
              transitions: o.transitions,
            }),
      (i.memoizedState = o),
      (i.childLanes = e.childLanes & ~n),
      (t.memoizedState = Jo),
      r
    );
  }
  return (
    (i = e.child),
    (e = i.sibling),
    (r = Tt(i, { mode: "visible", children: r.children })),
    !(t.mode & 1) && (r.lanes = n),
    (r.return = t),
    (r.sibling = null),
    e !== null &&
      ((n = t.deletions),
      n === null ? ((t.deletions = [e]), (t.flags |= 16)) : n.push(e)),
    (t.child = r),
    (t.memoizedState = null),
    r
  );
}
function ql(e, t) {
  return (
    (t = ai({ mode: "visible", children: t }, e.mode, 0, null)),
    (t.return = e),
    (e.child = t)
  );
}
function Zr(e, t, n, r) {
  return (
    r !== null && Al(r),
    Pn(t, e.child, null, n),
    (e = ql(t, t.pendingProps.children)),
    (e.flags |= 2),
    (t.memoizedState = null),
    e
  );
}
function Ag(e, t, n, r, a, i, o) {
  if (n)
    return t.flags & 256
      ? ((t.flags &= -257), (r = no(Error(k(422)))), Zr(e, t, o, r))
      : t.memoizedState !== null
      ? ((t.child = e.child), (t.flags |= 128), null)
      : ((i = r.fallback),
        (a = t.mode),
        (r = ai({ mode: "visible", children: r.children }, a, 0, null)),
        (i = $t(i, a, o, null)),
        (i.flags |= 2),
        (r.return = t),
        (i.return = t),
        (r.sibling = i),
        (t.child = r),
        t.mode & 1 && Pn(t, e.child, null, o),
        (t.child.memoizedState = qo(o)),
        (t.memoizedState = Jo),
        i);
  if (!(t.mode & 1)) return Zr(e, t, o, null);
  if (a.data === "$!") {
    if (((r = a.nextSibling && a.nextSibling.dataset), r)) var l = r.dgst;
    return (r = l), (i = Error(k(419))), (r = no(i, r, void 0)), Zr(e, t, o, r);
  }
  if (((l = (o & e.childLanes) !== 0), Se || l)) {
    if (((r = re), r !== null)) {
      switch (o & -o) {
        case 4:
          a = 2;
          break;
        case 16:
          a = 8;
          break;
        case 64:
        case 128:
        case 256:
        case 512:
        case 1024:
        case 2048:
        case 4096:
        case 8192:
        case 16384:
        case 32768:
        case 65536:
        case 131072:
        case 262144:
        case 524288:
        case 1048576:
        case 2097152:
        case 4194304:
        case 8388608:
        case 16777216:
        case 33554432:
        case 67108864:
          a = 32;
          break;
        case 536870912:
          a = 268435456;
          break;
        default:
          a = 0;
      }
      (a = a & (r.suspendedLanes | o) ? 0 : a),
        a !== 0 &&
          a !== i.retryLane &&
          ((i.retryLane = a), lt(e, a), Ge(r, e, a, -1));
    }
    return au(), (r = no(Error(k(421)))), Zr(e, t, o, r);
  }
  return a.data === "$?"
    ? ((t.flags |= 128),
      (t.child = e.child),
      (t = Jg.bind(null, e)),
      (a._reactRetry = t),
      null)
    : ((e = i.treeContext),
      (Te = Et(a.nextSibling)),
      (Pe = t),
      (V = !0),
      (Ve = null),
      e !== null &&
        ((De[Ie++] = nt),
        (De[Ie++] = rt),
        (De[Ie++] = Xt),
        (nt = e.id),
        (rt = e.overflow),
        (Xt = t)),
      (t = ql(t, r.children)),
      (t.flags |= 4096),
      t);
}
function hs(e, t, n) {
  e.lanes |= t;
  var r = e.alternate;
  r !== null && (r.lanes |= t), Yo(e.return, t, n);
}
function ro(e, t, n, r, a) {
  var i = e.memoizedState;
  i === null
    ? (e.memoizedState = {
        isBackwards: t,
        rendering: null,
        renderingStartTime: 0,
        last: r,
        tail: n,
        tailMode: a,
      })
    : ((i.isBackwards = t),
      (i.rendering = null),
      (i.renderingStartTime = 0),
      (i.last = r),
      (i.tail = n),
      (i.tailMode = a));
}
function Bd(e, t, n) {
  var r = t.pendingProps,
    a = r.revealOrder,
    i = r.tail;
  if ((ve(e, t, r.children, n), (r = Y.current), r & 2))
    (r = (r & 1) | 2), (t.flags |= 128);
  else {
    if (e !== null && e.flags & 128)
      e: for (e = t.child; e !== null; ) {
        if (e.tag === 13) e.memoizedState !== null && hs(e, n, t);
        else if (e.tag === 19) hs(e, n, t);
        else if (e.child !== null) {
          (e.child.return = e), (e = e.child);
          continue;
        }
        if (e === t) break e;
        for (; e.sibling === null; ) {
          if (e.return === null || e.return === t) break e;
          e = e.return;
        }
        (e.sibling.return = e.return), (e = e.sibling);
      }
    r &= 1;
  }
  if ((B(Y, r), !(t.mode & 1))) t.memoizedState = null;
  else
    switch (a) {
      case "forwards":
        for (n = t.child, a = null; n !== null; )
          (e = n.alternate),
            e !== null && Da(e) === null && (a = n),
            (n = n.sibling);
        (n = a),
          n === null
            ? ((a = t.child), (t.child = null))
            : ((a = n.sibling), (n.sibling = null)),
          ro(t, !1, a, n, i);
        break;
      case "backwards":
        for (n = null, a = t.child, t.child = null; a !== null; ) {
          if (((e = a.alternate), e !== null && Da(e) === null)) {
            t.child = a;
            break;
          }
          (e = a.sibling), (a.sibling = n), (n = a), (a = e);
        }
        ro(t, !0, n, null, i);
        break;
      case "together":
        ro(t, !1, null, null, void 0);
        break;
      default:
        t.memoizedState = null;
    }
  return t.child;
}
function da(e, t) {
  !(t.mode & 1) &&
    e !== null &&
    ((e.alternate = null), (t.alternate = null), (t.flags |= 2));
}
function ut(e, t, n) {
  if (
    (e !== null && (t.dependencies = e.dependencies),
    (qt |= t.lanes),
    !(n & t.childLanes))
  )
    return null;
  if (e !== null && t.child !== e.child) throw Error(k(153));
  if (t.child !== null) {
    for (
      e = t.child, n = Tt(e, e.pendingProps), t.child = n, n.return = t;
      e.sibling !== null;

    )
      (e = e.sibling), (n = n.sibling = Tt(e, e.pendingProps)), (n.return = t);
    n.sibling = null;
  }
  return t.child;
}
function Fg(e, t, n) {
  switch (t.tag) {
    case 3:
      Ad(t), Tn();
      break;
    case 5:
      fd(t);
      break;
    case 1:
      Ee(t.type) && Pa(t);
      break;
    case 4:
      Vl(t, t.stateNode.containerInfo);
      break;
    case 10:
      var r = t.type._context,
        a = t.memoizedProps.value;
      B(Ra, r._currentValue), (r._currentValue = a);
      break;
    case 13:
      if (((r = t.memoizedState), r !== null))
        return r.dehydrated !== null
          ? (B(Y, Y.current & 1), (t.flags |= 128), null)
          : n & t.child.childLanes
          ? Fd(e, t, n)
          : (B(Y, Y.current & 1),
            (e = ut(e, t, n)),
            e !== null ? e.sibling : null);
      B(Y, Y.current & 1);
      break;
    case 19:
      if (((r = (n & t.childLanes) !== 0), e.flags & 128)) {
        if (r) return Bd(e, t, n);
        t.flags |= 128;
      }
      if (
        ((a = t.memoizedState),
        a !== null &&
          ((a.rendering = null), (a.tail = null), (a.lastEffect = null)),
        B(Y, Y.current),
        r)
      )
        break;
      return null;
    case 22:
    case 23:
      return (t.lanes = 0), zd(e, t, n);
  }
  return ut(e, t, n);
}
var Wd, Zo, Hd, bd;
Wd = function (e, t) {
  for (var n = t.child; n !== null; ) {
    if (n.tag === 5 || n.tag === 6) e.appendChild(n.stateNode);
    else if (n.tag !== 4 && n.child !== null) {
      (n.child.return = n), (n = n.child);
      continue;
    }
    if (n === t) break;
    for (; n.sibling === null; ) {
      if (n.return === null || n.return === t) return;
      n = n.return;
    }
    (n.sibling.return = n.return), (n = n.sibling);
  }
};
Zo = function () {};
Hd = function (e, t, n, r) {
  var a = e.memoizedProps;
  if (a !== r) {
    (e = t.stateNode), Gt(qe.current);
    var i = null;
    switch (n) {
      case "input":
        (a = So(e, a)), (r = So(e, r)), (i = []);
        break;
      case "select":
        (a = Q({}, a, { value: void 0 })),
          (r = Q({}, r, { value: void 0 })),
          (i = []);
        break;
      case "textarea":
        (a = jo(e, a)), (r = jo(e, r)), (i = []);
        break;
      default:
        typeof a.onClick != "function" &&
          typeof r.onClick == "function" &&
          (e.onclick = xa);
    }
    xo(n, r);
    var o;
    n = null;
    for (s in a)
      if (!r.hasOwnProperty(s) && a.hasOwnProperty(s) && a[s] != null)
        if (s === "style") {
          var l = a[s];
          for (o in l) l.hasOwnProperty(o) && (n || (n = {}), (n[o] = ""));
        } else
          s !== "dangerouslySetInnerHTML" &&
            s !== "children" &&
            s !== "suppressContentEditableWarning" &&
            s !== "suppressHydrationWarning" &&
            s !== "autoFocus" &&
            (ur.hasOwnProperty(s)
              ? i || (i = [])
              : (i = i || []).push(s, null));
    for (s in r) {
      var u = r[s];
      if (
        ((l = a != null ? a[s] : void 0),
        r.hasOwnProperty(s) && u !== l && (u != null || l != null))
      )
        if (s === "style")
          if (l) {
            for (o in l)
              !l.hasOwnProperty(o) ||
                (u && u.hasOwnProperty(o)) ||
                (n || (n = {}), (n[o] = ""));
            for (o in u)
              u.hasOwnProperty(o) &&
                l[o] !== u[o] &&
                (n || (n = {}), (n[o] = u[o]));
          } else n || (i || (i = []), i.push(s, n)), (n = u);
        else
          s === "dangerouslySetInnerHTML"
            ? ((u = u ? u.__html : void 0),
              (l = l ? l.__html : void 0),
              u != null && l !== u && (i = i || []).push(s, u))
            : s === "children"
            ? (typeof u != "string" && typeof u != "number") ||
              (i = i || []).push(s, "" + u)
            : s !== "suppressContentEditableWarning" &&
              s !== "suppressHydrationWarning" &&
              (ur.hasOwnProperty(s)
                ? (u != null && s === "onScroll" && H("scroll", e),
                  i || l === u || (i = []))
                : (i = i || []).push(s, u));
    }
    n && (i = i || []).push("style", n);
    var s = i;
    (t.updateQueue = s) && (t.flags |= 4);
  }
};
bd = function (e, t, n, r) {
  n !== r && (t.flags |= 4);
};
function Vn(e, t) {
  if (!V)
    switch (e.tailMode) {
      case "hidden":
        t = e.tail;
        for (var n = null; t !== null; )
          t.alternate !== null && (n = t), (t = t.sibling);
        n === null ? (e.tail = null) : (n.sibling = null);
        break;
      case "collapsed":
        n = e.tail;
        for (var r = null; n !== null; )
          n.alternate !== null && (r = n), (n = n.sibling);
        r === null
          ? t || e.tail === null
            ? (e.tail = null)
            : (e.tail.sibling = null)
          : (r.sibling = null);
    }
}
function ce(e) {
  var t = e.alternate !== null && e.alternate.child === e.child,
    n = 0,
    r = 0;
  if (t)
    for (var a = e.child; a !== null; )
      (n |= a.lanes | a.childLanes),
        (r |= a.subtreeFlags & 14680064),
        (r |= a.flags & 14680064),
        (a.return = e),
        (a = a.sibling);
  else
    for (a = e.child; a !== null; )
      (n |= a.lanes | a.childLanes),
        (r |= a.subtreeFlags),
        (r |= a.flags),
        (a.return = e),
        (a = a.sibling);
  return (e.subtreeFlags |= r), (e.childLanes = n), t;
}
function Bg(e, t, n) {
  var r = t.pendingProps;
  switch ((Ul(t), t.tag)) {
    case 2:
    case 16:
    case 15:
    case 0:
    case 11:
    case 7:
    case 8:
    case 12:
    case 9:
    case 14:
      return ce(t), null;
    case 1:
      return Ee(t.type) && Ta(), ce(t), null;
    case 3:
      return (
        (r = t.stateNode),
        Nn(),
        b(ke),
        b(fe),
        Gl(),
        r.pendingContext &&
          ((r.context = r.pendingContext), (r.pendingContext = null)),
        (e === null || e.child === null) &&
          (Jr(t)
            ? (t.flags |= 4)
            : e === null ||
              (e.memoizedState.isDehydrated && !(t.flags & 256)) ||
              ((t.flags |= 1024), Ve !== null && (ll(Ve), (Ve = null)))),
        Zo(e, t),
        ce(t),
        null
      );
    case 5:
      Yl(t);
      var a = Gt(wr.current);
      if (((n = t.type), e !== null && t.stateNode != null))
        Hd(e, t, n, r, a),
          e.ref !== t.ref && ((t.flags |= 512), (t.flags |= 2097152));
      else {
        if (!r) {
          if (t.stateNode === null) throw Error(k(166));
          return ce(t), null;
        }
        if (((e = Gt(qe.current)), Jr(t))) {
          (r = t.stateNode), (n = t.type);
          var i = t.memoizedProps;
          switch (((r[Xe] = t), (r[mr] = i), (e = (t.mode & 1) !== 0), n)) {
            case "dialog":
              H("cancel", r), H("close", r);
              break;
            case "iframe":
            case "object":
            case "embed":
              H("load", r);
              break;
            case "video":
            case "audio":
              for (a = 0; a < Jn.length; a++) H(Jn[a], r);
              break;
            case "source":
              H("error", r);
              break;
            case "img":
            case "image":
            case "link":
              H("error", r), H("load", r);
              break;
            case "details":
              H("toggle", r);
              break;
            case "input":
              Cu(r, i), H("invalid", r);
              break;
            case "select":
              (r._wrapperState = { wasMultiple: !!i.multiple }),
                H("invalid", r);
              break;
            case "textarea":
              Tu(r, i), H("invalid", r);
          }
          xo(n, i), (a = null);
          for (var o in i)
            if (i.hasOwnProperty(o)) {
              var l = i[o];
              o === "children"
                ? typeof l == "string"
                  ? r.textContent !== l &&
                    (i.suppressHydrationWarning !== !0 &&
                      Xr(r.textContent, l, e),
                    (a = ["children", l]))
                  : typeof l == "number" &&
                    r.textContent !== "" + l &&
                    (i.suppressHydrationWarning !== !0 &&
                      Xr(r.textContent, l, e),
                    (a = ["children", "" + l]))
                : ur.hasOwnProperty(o) &&
                  l != null &&
                  o === "onScroll" &&
                  H("scroll", r);
            }
          switch (n) {
            case "input":
              Hr(r), xu(r, i, !0);
              break;
            case "textarea":
              Hr(r), Pu(r);
              break;
            case "select":
            case "option":
              break;
            default:
              typeof i.onClick == "function" && (r.onclick = xa);
          }
          (r = a), (t.updateQueue = r), r !== null && (t.flags |= 4);
        } else {
          (o = a.nodeType === 9 ? a : a.ownerDocument),
            e === "http://www.w3.org/1999/xhtml" && (e = hc(n)),
            e === "http://www.w3.org/1999/xhtml"
              ? n === "script"
                ? ((e = o.createElement("div")),
                  (e.innerHTML = "<script></script>"),
                  (e = e.removeChild(e.firstChild)))
                : typeof r.is == "string"
                ? (e = o.createElement(n, { is: r.is }))
                : ((e = o.createElement(n)),
                  n === "select" &&
                    ((o = e),
                    r.multiple
                      ? (o.multiple = !0)
                      : r.size && (o.size = r.size)))
              : (e = o.createElementNS(e, n)),
            (e[Xe] = t),
            (e[mr] = r),
            Wd(e, t, !1, !1),
            (t.stateNode = e);
          e: {
            switch (((o = To(n, r)), n)) {
              case "dialog":
                H("cancel", e), H("close", e), (a = r);
                break;
              case "iframe":
              case "object":
              case "embed":
                H("load", e), (a = r);
                break;
              case "video":
              case "audio":
                for (a = 0; a < Jn.length; a++) H(Jn[a], e);
                a = r;
                break;
              case "source":
                H("error", e), (a = r);
                break;
              case "img":
              case "image":
              case "link":
                H("error", e), H("load", e), (a = r);
                break;
              case "details":
                H("toggle", e), (a = r);
                break;
              case "input":
                Cu(e, r), (a = So(e, r)), H("invalid", e);
                break;
              case "option":
                a = r;
                break;
              case "select":
                (e._wrapperState = { wasMultiple: !!r.multiple }),
                  (a = Q({}, r, { value: void 0 })),
                  H("invalid", e);
                break;
              case "textarea":
                Tu(e, r), (a = jo(e, r)), H("invalid", e);
                break;
              default:
                a = r;
            }
            xo(n, a), (l = a);
            for (i in l)
              if (l.hasOwnProperty(i)) {
                var u = l[i];
                i === "style"
                  ? yc(e, u)
                  : i === "dangerouslySetInnerHTML"
                  ? ((u = u ? u.__html : void 0), u != null && _c(e, u))
                  : i === "children"
                  ? typeof u == "string"
                    ? (n !== "textarea" || u !== "") && sr(e, u)
                    : typeof u == "number" && sr(e, "" + u)
                  : i !== "suppressContentEditableWarning" &&
                    i !== "suppressHydrationWarning" &&
                    i !== "autoFocus" &&
                    (ur.hasOwnProperty(i)
                      ? u != null && i === "onScroll" && H("scroll", e)
                      : u != null && kl(e, i, u, o));
              }
            switch (n) {
              case "input":
                Hr(e), xu(e, r, !1);
                break;
              case "textarea":
                Hr(e), Pu(e);
                break;
              case "option":
                r.value != null && e.setAttribute("value", "" + Nt(r.value));
                break;
              case "select":
                (e.multiple = !!r.multiple),
                  (i = r.value),
                  i != null
                    ? yn(e, !!r.multiple, i, !1)
                    : r.defaultValue != null &&
                      yn(e, !!r.multiple, r.defaultValue, !0);
                break;
              default:
                typeof a.onClick == "function" && (e.onclick = xa);
            }
            switch (n) {
              case "button":
              case "input":
              case "select":
              case "textarea":
                r = !!r.autoFocus;
                break e;
              case "img":
                r = !0;
                break e;
              default:
                r = !1;
            }
          }
          r && (t.flags |= 4);
        }
        t.ref !== null && ((t.flags |= 512), (t.flags |= 2097152));
      }
      return ce(t), null;
    case 6:
      if (e && t.stateNode != null) bd(e, t, e.memoizedProps, r);
      else {
        if (typeof r != "string" && t.stateNode === null) throw Error(k(166));
        if (((n = Gt(wr.current)), Gt(qe.current), Jr(t))) {
          if (
            ((r = t.stateNode),
            (n = t.memoizedProps),
            (r[Xe] = t),
            (i = r.nodeValue !== n) && ((e = Pe), e !== null))
          )
            switch (e.tag) {
              case 3:
                Xr(r.nodeValue, n, (e.mode & 1) !== 0);
                break;
              case 5:
                e.memoizedProps.suppressHydrationWarning !== !0 &&
                  Xr(r.nodeValue, n, (e.mode & 1) !== 0);
            }
          i && (t.flags |= 4);
        } else
          (r = (n.nodeType === 9 ? n : n.ownerDocument).createTextNode(r)),
            (r[Xe] = t),
            (t.stateNode = r);
      }
      return ce(t), null;
    case 13:
      if (
        (b(Y),
        (r = t.memoizedState),
        e === null ||
          (e.memoizedState !== null && e.memoizedState.dehydrated !== null))
      ) {
        if (V && Te !== null && t.mode & 1 && !(t.flags & 128))
          od(), Tn(), (t.flags |= 98560), (i = !1);
        else if (((i = Jr(t)), r !== null && r.dehydrated !== null)) {
          if (e === null) {
            if (!i) throw Error(k(318));
            if (
              ((i = t.memoizedState),
              (i = i !== null ? i.dehydrated : null),
              !i)
            )
              throw Error(k(317));
            i[Xe] = t;
          } else
            Tn(), !(t.flags & 128) && (t.memoizedState = null), (t.flags |= 4);
          ce(t), (i = !1);
        } else Ve !== null && (ll(Ve), (Ve = null)), (i = !0);
        if (!i) return t.flags & 65536 ? t : null;
      }
      return t.flags & 128
        ? ((t.lanes = n), t)
        : ((r = r !== null),
          r !== (e !== null && e.memoizedState !== null) &&
            r &&
            ((t.child.flags |= 8192),
            t.mode & 1 &&
              (e === null || Y.current & 1 ? Z === 0 && (Z = 3) : au())),
          t.updateQueue !== null && (t.flags |= 4),
          ce(t),
          null);
    case 4:
      return (
        Nn(), Zo(e, t), e === null && hr(t.stateNode.containerInfo), ce(t), null
      );
    case 10:
      return Wl(t.type._context), ce(t), null;
    case 17:
      return Ee(t.type) && Ta(), ce(t), null;
    case 19:
      if ((b(Y), (i = t.memoizedState), i === null)) return ce(t), null;
      if (((r = (t.flags & 128) !== 0), (o = i.rendering), o === null))
        if (r) Vn(i, !1);
        else {
          if (Z !== 0 || (e !== null && e.flags & 128))
            for (e = t.child; e !== null; ) {
              if (((o = Da(e)), o !== null)) {
                for (
                  t.flags |= 128,
                    Vn(i, !1),
                    r = o.updateQueue,
                    r !== null && ((t.updateQueue = r), (t.flags |= 4)),
                    t.subtreeFlags = 0,
                    r = n,
                    n = t.child;
                  n !== null;

                )
                  (i = n),
                    (e = r),
                    (i.flags &= 14680066),
                    (o = i.alternate),
                    o === null
                      ? ((i.childLanes = 0),
                        (i.lanes = e),
                        (i.child = null),
                        (i.subtreeFlags = 0),
                        (i.memoizedProps = null),
                        (i.memoizedState = null),
                        (i.updateQueue = null),
                        (i.dependencies = null),
                        (i.stateNode = null))
                      : ((i.childLanes = o.childLanes),
                        (i.lanes = o.lanes),
                        (i.child = o.child),
                        (i.subtreeFlags = 0),
                        (i.deletions = null),
                        (i.memoizedProps = o.memoizedProps),
                        (i.memoizedState = o.memoizedState),
                        (i.updateQueue = o.updateQueue),
                        (i.type = o.type),
                        (e = o.dependencies),
                        (i.dependencies =
                          e === null
                            ? null
                            : {
                                lanes: e.lanes,
                                firstContext: e.firstContext,
                              })),
                    (n = n.sibling);
                return B(Y, (Y.current & 1) | 2), t.child;
              }
              e = e.sibling;
            }
          i.tail !== null &&
            X() > Rn &&
            ((t.flags |= 128), (r = !0), Vn(i, !1), (t.lanes = 4194304));
        }
      else {
        if (!r)
          if (((e = Da(o)), e !== null)) {
            if (
              ((t.flags |= 128),
              (r = !0),
              (n = e.updateQueue),
              n !== null && ((t.updateQueue = n), (t.flags |= 4)),
              Vn(i, !0),
              i.tail === null && i.tailMode === "hidden" && !o.alternate && !V)
            )
              return ce(t), null;
          } else
            2 * X() - i.renderingStartTime > Rn &&
              n !== 1073741824 &&
              ((t.flags |= 128), (r = !0), Vn(i, !1), (t.lanes = 4194304));
        i.isBackwards
          ? ((o.sibling = t.child), (t.child = o))
          : ((n = i.last),
            n !== null ? (n.sibling = o) : (t.child = o),
            (i.last = o));
      }
      return i.tail !== null
        ? ((t = i.tail),
          (i.rendering = t),
          (i.tail = t.sibling),
          (i.renderingStartTime = X()),
          (t.sibling = null),
          (n = Y.current),
          B(Y, r ? (n & 1) | 2 : n & 1),
          t)
        : (ce(t), null);
    case 22:
    case 23:
      return (
        ru(),
        (r = t.memoizedState !== null),
        e !== null && (e.memoizedState !== null) !== r && (t.flags |= 8192),
        r && t.mode & 1
          ? xe & 1073741824 && (ce(t), t.subtreeFlags & 6 && (t.flags |= 8192))
          : ce(t),
        null
      );
    case 24:
      return null;
    case 25:
      return null;
  }
  throw Error(k(156, t.tag));
}
function Wg(e, t) {
  switch ((Ul(t), t.tag)) {
    case 1:
      return (
        Ee(t.type) && Ta(),
        (e = t.flags),
        e & 65536 ? ((t.flags = (e & -65537) | 128), t) : null
      );
    case 3:
      return (
        Nn(),
        b(ke),
        b(fe),
        Gl(),
        (e = t.flags),
        e & 65536 && !(e & 128) ? ((t.flags = (e & -65537) | 128), t) : null
      );
    case 5:
      return Yl(t), null;
    case 13:
      if ((b(Y), (e = t.memoizedState), e !== null && e.dehydrated !== null)) {
        if (t.alternate === null) throw Error(k(340));
        Tn();
      }
      return (
        (e = t.flags), e & 65536 ? ((t.flags = (e & -65537) | 128), t) : null
      );
    case 19:
      return b(Y), null;
    case 4:
      return Nn(), null;
    case 10:
      return Wl(t.type._context), null;
    case 22:
    case 23:
      return ru(), null;
    case 24:
      return null;
    default:
      return null;
  }
}
var ea = !1,
  de = !1,
  Hg = typeof WeakSet == "function" ? WeakSet : Set,
  C = null;
function _n(e, t) {
  var n = e.ref;
  if (n !== null)
    if (typeof n == "function")
      try {
        n(null);
      } catch (r) {
        $(e, t, r);
      }
    else n.current = null;
}
function el(e, t, n) {
  try {
    n();
  } catch (r) {
    $(e, t, r);
  }
}
var _s = !1;
function bg(e, t) {
  if (((Uo = Ea), (e = Qc()), Il(e))) {
    if ("selectionStart" in e)
      var n = { start: e.selectionStart, end: e.selectionEnd };
    else
      e: {
        n = ((n = e.ownerDocument) && n.defaultView) || window;
        var r = n.getSelection && n.getSelection();
        if (r && r.rangeCount !== 0) {
          n = r.anchorNode;
          var a = r.anchorOffset,
            i = r.focusNode;
          r = r.focusOffset;
          try {
            n.nodeType, i.nodeType;
          } catch {
            n = null;
            break e;
          }
          var o = 0,
            l = -1,
            u = -1,
            s = 0,
            p = 0,
            f = e,
            v = null;
          t: for (;;) {
            for (
              var _;
              f !== n || (a !== 0 && f.nodeType !== 3) || (l = o + a),
                f !== i || (r !== 0 && f.nodeType !== 3) || (u = o + r),
                f.nodeType === 3 && (o += f.nodeValue.length),
                (_ = f.firstChild) !== null;

            )
              (v = f), (f = _);
            for (;;) {
              if (f === e) break t;
              if (
                (v === n && ++s === a && (l = o),
                v === i && ++p === r && (u = o),
                (_ = f.nextSibling) !== null)
              )
                break;
              (f = v), (v = f.parentNode);
            }
            f = _;
          }
          n = l === -1 || u === -1 ? null : { start: l, end: u };
        } else n = null;
      }
    n = n || { start: 0, end: 0 };
  } else n = null;
  for (Ao = { focusedElem: e, selectionRange: n }, Ea = !1, C = t; C !== null; )
    if (((t = C), (e = t.child), (t.subtreeFlags & 1028) !== 0 && e !== null))
      (e.return = t), (C = e);
    else
      for (; C !== null; ) {
        t = C;
        try {
          var m = t.alternate;
          if (t.flags & 1024)
            switch (t.tag) {
              case 0:
              case 11:
              case 15:
                break;
              case 1:
                if (m !== null) {
                  var h = m.memoizedProps,
                    E = m.memoizedState,
                    d = t.stateNode,
                    c = d.getSnapshotBeforeUpdate(
                      t.elementType === t.type ? h : He(t.type, h),
                      E
                    );
                  d.__reactInternalSnapshotBeforeUpdate = c;
                }
                break;
              case 3:
                var g = t.stateNode.containerInfo;
                g.nodeType === 1
                  ? (g.textContent = "")
                  : g.nodeType === 9 &&
                    g.documentElement &&
                    g.removeChild(g.documentElement);
                break;
              case 5:
              case 6:
              case 4:
              case 17:
                break;
              default:
                throw Error(k(163));
            }
        } catch (y) {
          $(t, t.return, y);
        }
        if (((e = t.sibling), e !== null)) {
          (e.return = t.return), (C = e);
          break;
        }
        C = t.return;
      }
  return (m = _s), (_s = !1), m;
}
function ar(e, t, n) {
  var r = t.updateQueue;
  if (((r = r !== null ? r.lastEffect : null), r !== null)) {
    var a = (r = r.next);
    do {
      if ((a.tag & e) === e) {
        var i = a.destroy;
        (a.destroy = void 0), i !== void 0 && el(t, n, i);
      }
      a = a.next;
    } while (a !== r);
  }
}
function ni(e, t) {
  if (
    ((t = t.updateQueue), (t = t !== null ? t.lastEffect : null), t !== null)
  ) {
    var n = (t = t.next);
    do {
      if ((n.tag & e) === e) {
        var r = n.create;
        n.destroy = r();
      }
      n = n.next;
    } while (n !== t);
  }
}
function tl(e) {
  var t = e.ref;
  if (t !== null) {
    var n = e.stateNode;
    switch (e.tag) {
      case 5:
        e = n;
        break;
      default:
        e = n;
    }
    typeof t == "function" ? t(e) : (t.current = e);
  }
}
function Vd(e) {
  var t = e.alternate;
  t !== null && ((e.alternate = null), Vd(t)),
    (e.child = null),
    (e.deletions = null),
    (e.sibling = null),
    e.tag === 5 &&
      ((t = e.stateNode),
      t !== null &&
        (delete t[Xe], delete t[mr], delete t[Wo], delete t[Cg], delete t[xg])),
    (e.stateNode = null),
    (e.return = null),
    (e.dependencies = null),
    (e.memoizedProps = null),
    (e.memoizedState = null),
    (e.pendingProps = null),
    (e.stateNode = null),
    (e.updateQueue = null);
}
function Yd(e) {
  return e.tag === 5 || e.tag === 3 || e.tag === 4;
}
function ms(e) {
  e: for (;;) {
    for (; e.sibling === null; ) {
      if (e.return === null || Yd(e.return)) return null;
      e = e.return;
    }
    for (
      e.sibling.return = e.return, e = e.sibling;
      e.tag !== 5 && e.tag !== 6 && e.tag !== 18;

    ) {
      if (e.flags & 2 || e.child === null || e.tag === 4) continue e;
      (e.child.return = e), (e = e.child);
    }
    if (!(e.flags & 2)) return e.stateNode;
  }
}
function nl(e, t, n) {
  var r = e.tag;
  if (r === 5 || r === 6)
    (e = e.stateNode),
      t
        ? n.nodeType === 8
          ? n.parentNode.insertBefore(e, t)
          : n.insertBefore(e, t)
        : (n.nodeType === 8
            ? ((t = n.parentNode), t.insertBefore(e, n))
            : ((t = n), t.appendChild(e)),
          (n = n._reactRootContainer),
          n != null || t.onclick !== null || (t.onclick = xa));
  else if (r !== 4 && ((e = e.child), e !== null))
    for (nl(e, t, n), e = e.sibling; e !== null; ) nl(e, t, n), (e = e.sibling);
}
function rl(e, t, n) {
  var r = e.tag;
  if (r === 5 || r === 6)
    (e = e.stateNode), t ? n.insertBefore(e, t) : n.appendChild(e);
  else if (r !== 4 && ((e = e.child), e !== null))
    for (rl(e, t, n), e = e.sibling; e !== null; ) rl(e, t, n), (e = e.sibling);
}
var ie = null,
  be = !1;
function dt(e, t, n) {
  for (n = n.child; n !== null; ) Gd(e, t, n), (n = n.sibling);
}
function Gd(e, t, n) {
  if (Je && typeof Je.onCommitFiberUnmount == "function")
    try {
      Je.onCommitFiberUnmount($a, n);
    } catch {}
  switch (n.tag) {
    case 5:
      de || _n(n, t);
    case 6:
      var r = ie,
        a = be;
      (ie = null),
        dt(e, t, n),
        (ie = r),
        (be = a),
        ie !== null &&
          (be
            ? ((e = ie),
              (n = n.stateNode),
              e.nodeType === 8 ? e.parentNode.removeChild(n) : e.removeChild(n))
            : ie.removeChild(n.stateNode));
      break;
    case 18:
      ie !== null &&
        (be
          ? ((e = ie),
            (n = n.stateNode),
            e.nodeType === 8
              ? Xi(e.parentNode, n)
              : e.nodeType === 1 && Xi(e, n),
            fr(e))
          : Xi(ie, n.stateNode));
      break;
    case 4:
      (r = ie),
        (a = be),
        (ie = n.stateNode.containerInfo),
        (be = !0),
        dt(e, t, n),
        (ie = r),
        (be = a);
      break;
    case 0:
    case 11:
    case 14:
    case 15:
      if (
        !de &&
        ((r = n.updateQueue), r !== null && ((r = r.lastEffect), r !== null))
      ) {
        a = r = r.next;
        do {
          var i = a,
            o = i.destroy;
          (i = i.tag),
            o !== void 0 && (i & 2 || i & 4) && el(n, t, o),
            (a = a.next);
        } while (a !== r);
      }
      dt(e, t, n);
      break;
    case 1:
      if (
        !de &&
        (_n(n, t),
        (r = n.stateNode),
        typeof r.componentWillUnmount == "function")
      )
        try {
          (r.props = n.memoizedProps),
            (r.state = n.memoizedState),
            r.componentWillUnmount();
        } catch (l) {
          $(n, t, l);
        }
      dt(e, t, n);
      break;
    case 21:
      dt(e, t, n);
      break;
    case 22:
      n.mode & 1
        ? ((de = (r = de) || n.memoizedState !== null), dt(e, t, n), (de = r))
        : dt(e, t, n);
      break;
    default:
      dt(e, t, n);
  }
}
function ys(e) {
  var t = e.updateQueue;
  if (t !== null) {
    e.updateQueue = null;
    var n = e.stateNode;
    n === null && (n = e.stateNode = new Hg()),
      t.forEach(function (r) {
        var a = qg.bind(null, e, r);
        n.has(r) || (n.add(r), r.then(a, a));
      });
  }
}
function We(e, t) {
  var n = t.deletions;
  if (n !== null)
    for (var r = 0; r < n.length; r++) {
      var a = n[r];
      try {
        var i = e,
          o = t,
          l = o;
        e: for (; l !== null; ) {
          switch (l.tag) {
            case 5:
              (ie = l.stateNode), (be = !1);
              break e;
            case 3:
              (ie = l.stateNode.containerInfo), (be = !0);
              break e;
            case 4:
              (ie = l.stateNode.containerInfo), (be = !0);
              break e;
          }
          l = l.return;
        }
        if (ie === null) throw Error(k(160));
        Gd(i, o, a), (ie = null), (be = !1);
        var u = a.alternate;
        u !== null && (u.return = null), (a.return = null);
      } catch (s) {
        $(a, t, s);
      }
    }
  if (t.subtreeFlags & 12854)
    for (t = t.child; t !== null; ) Qd(t, e), (t = t.sibling);
}
function Qd(e, t) {
  var n = e.alternate,
    r = e.flags;
  switch (e.tag) {
    case 0:
    case 11:
    case 14:
    case 15:
      if ((We(t, e), $e(e), r & 4)) {
        try {
          ar(3, e, e.return), ni(3, e);
        } catch (h) {
          $(e, e.return, h);
        }
        try {
          ar(5, e, e.return);
        } catch (h) {
          $(e, e.return, h);
        }
      }
      break;
    case 1:
      We(t, e), $e(e), r & 512 && n !== null && _n(n, n.return);
      break;
    case 5:
      if (
        (We(t, e),
        $e(e),
        r & 512 && n !== null && _n(n, n.return),
        e.flags & 32)
      ) {
        var a = e.stateNode;
        try {
          sr(a, "");
        } catch (h) {
          $(e, e.return, h);
        }
      }
      if (r & 4 && ((a = e.stateNode), a != null)) {
        var i = e.memoizedProps,
          o = n !== null ? n.memoizedProps : i,
          l = e.type,
          u = e.updateQueue;
        if (((e.updateQueue = null), u !== null))
          try {
            l === "input" && i.type === "radio" && i.name != null && gc(a, i),
              To(l, o);
            var s = To(l, i);
            for (o = 0; o < u.length; o += 2) {
              var p = u[o],
                f = u[o + 1];
              p === "style"
                ? yc(a, f)
                : p === "dangerouslySetInnerHTML"
                ? _c(a, f)
                : p === "children"
                ? sr(a, f)
                : kl(a, p, f, s);
            }
            switch (l) {
              case "input":
                ko(a, i);
                break;
              case "textarea":
                vc(a, i);
                break;
              case "select":
                var v = a._wrapperState.wasMultiple;
                a._wrapperState.wasMultiple = !!i.multiple;
                var _ = i.value;
                _ != null
                  ? yn(a, !!i.multiple, _, !1)
                  : v !== !!i.multiple &&
                    (i.defaultValue != null
                      ? yn(a, !!i.multiple, i.defaultValue, !0)
                      : yn(a, !!i.multiple, i.multiple ? [] : "", !1));
            }
            a[mr] = i;
          } catch (h) {
            $(e, e.return, h);
          }
      }
      break;
    case 6:
      if ((We(t, e), $e(e), r & 4)) {
        if (e.stateNode === null) throw Error(k(162));
        (a = e.stateNode), (i = e.memoizedProps);
        try {
          a.nodeValue = i;
        } catch (h) {
          $(e, e.return, h);
        }
      }
      break;
    case 3:
      if (
        (We(t, e), $e(e), r & 4 && n !== null && n.memoizedState.isDehydrated)
      )
        try {
          fr(t.containerInfo);
        } catch (h) {
          $(e, e.return, h);
        }
      break;
    case 4:
      We(t, e), $e(e);
      break;
    case 13:
      We(t, e),
        $e(e),
        (a = e.child),
        a.flags & 8192 &&
          ((i = a.memoizedState !== null),
          (a.stateNode.isHidden = i),
          !i ||
            (a.alternate !== null && a.alternate.memoizedState !== null) ||
            (tu = X())),
        r & 4 && ys(e);
      break;
    case 22:
      if (
        ((p = n !== null && n.memoizedState !== null),
        e.mode & 1 ? ((de = (s = de) || p), We(t, e), (de = s)) : We(t, e),
        $e(e),
        r & 8192)
      ) {
        if (
          ((s = e.memoizedState !== null),
          (e.stateNode.isHidden = s) && !p && e.mode & 1)
        )
          for (C = e, p = e.child; p !== null; ) {
            for (f = C = p; C !== null; ) {
              switch (((v = C), (_ = v.child), v.tag)) {
                case 0:
                case 11:
                case 14:
                case 15:
                  ar(4, v, v.return);
                  break;
                case 1:
                  _n(v, v.return);
                  var m = v.stateNode;
                  if (typeof m.componentWillUnmount == "function") {
                    (r = v), (n = v.return);
                    try {
                      (t = r),
                        (m.props = t.memoizedProps),
                        (m.state = t.memoizedState),
                        m.componentWillUnmount();
                    } catch (h) {
                      $(r, n, h);
                    }
                  }
                  break;
                case 5:
                  _n(v, v.return);
                  break;
                case 22:
                  if (v.memoizedState !== null) {
                    Ss(f);
                    continue;
                  }
              }
              _ !== null ? ((_.return = v), (C = _)) : Ss(f);
            }
            p = p.sibling;
          }
        e: for (p = null, f = e; ; ) {
          if (f.tag === 5) {
            if (p === null) {
              p = f;
              try {
                (a = f.stateNode),
                  s
                    ? ((i = a.style),
                      typeof i.setProperty == "function"
                        ? i.setProperty("display", "none", "important")
                        : (i.display = "none"))
                    : ((l = f.stateNode),
                      (u = f.memoizedProps.style),
                      (o =
                        u != null && u.hasOwnProperty("display")
                          ? u.display
                          : null),
                      (l.style.display = mc("display", o)));
              } catch (h) {
                $(e, e.return, h);
              }
            }
          } else if (f.tag === 6) {
            if (p === null)
              try {
                f.stateNode.nodeValue = s ? "" : f.memoizedProps;
              } catch (h) {
                $(e, e.return, h);
              }
          } else if (
            ((f.tag !== 22 && f.tag !== 23) ||
              f.memoizedState === null ||
              f === e) &&
            f.child !== null
          ) {
            (f.child.return = f), (f = f.child);
            continue;
          }
          if (f === e) break e;
          for (; f.sibling === null; ) {
            if (f.return === null || f.return === e) break e;
            p === f && (p = null), (f = f.return);
          }
          p === f && (p = null), (f.sibling.return = f.return), (f = f.sibling);
        }
      }
      break;
    case 19:
      We(t, e), $e(e), r & 4 && ys(e);
      break;
    case 21:
      break;
    default:
      We(t, e), $e(e);
  }
}
function $e(e) {
  var t = e.flags;
  if (t & 2) {
    try {
      e: {
        for (var n = e.return; n !== null; ) {
          if (Yd(n)) {
            var r = n;
            break e;
          }
          n = n.return;
        }
        throw Error(k(160));
      }
      switch (r.tag) {
        case 5:
          var a = r.stateNode;
          r.flags & 32 && (sr(a, ""), (r.flags &= -33));
          var i = ms(e);
          rl(e, i, a);
          break;
        case 3:
        case 4:
          var o = r.stateNode.containerInfo,
            l = ms(e);
          nl(e, l, o);
          break;
        default:
          throw Error(k(161));
      }
    } catch (u) {
      $(e, e.return, u);
    }
    e.flags &= -3;
  }
  t & 4096 && (e.flags &= -4097);
}
function Vg(e, t, n) {
  (C = e), $d(e);
}
function $d(e, t, n) {
  for (var r = (e.mode & 1) !== 0; C !== null; ) {
    var a = C,
      i = a.child;
    if (a.tag === 22 && r) {
      var o = a.memoizedState !== null || ea;
      if (!o) {
        var l = a.alternate,
          u = (l !== null && l.memoizedState !== null) || de;
        l = ea;
        var s = de;
        if (((ea = o), (de = u) && !s))
          for (C = a; C !== null; )
            (o = C),
              (u = o.child),
              o.tag === 22 && o.memoizedState !== null
                ? ks(a)
                : u !== null
                ? ((u.return = o), (C = u))
                : ks(a);
        for (; i !== null; ) (C = i), $d(i), (i = i.sibling);
        (C = a), (ea = l), (de = s);
      }
      ws(e);
    } else
      a.subtreeFlags & 8772 && i !== null ? ((i.return = a), (C = i)) : ws(e);
  }
}
function ws(e) {
  for (; C !== null; ) {
    var t = C;
    if (t.flags & 8772) {
      var n = t.alternate;
      try {
        if (t.flags & 8772)
          switch (t.tag) {
            case 0:
            case 11:
            case 15:
              de || ni(5, t);
              break;
            case 1:
              var r = t.stateNode;
              if (t.flags & 4 && !de)
                if (n === null) r.componentDidMount();
                else {
                  var a =
                    t.elementType === t.type
                      ? n.memoizedProps
                      : He(t.type, n.memoizedProps);
                  r.componentDidUpdate(
                    a,
                    n.memoizedState,
                    r.__reactInternalSnapshotBeforeUpdate
                  );
                }
              var i = t.updateQueue;
              i !== null && rs(t, i, r);
              break;
            case 3:
              var o = t.updateQueue;
              if (o !== null) {
                if (((n = null), t.child !== null))
                  switch (t.child.tag) {
                    case 5:
                      n = t.child.stateNode;
                      break;
                    case 1:
                      n = t.child.stateNode;
                  }
                rs(t, o, n);
              }
              break;
            case 5:
              var l = t.stateNode;
              if (n === null && t.flags & 4) {
                n = l;
                var u = t.memoizedProps;
                switch (t.type) {
                  case "button":
                  case "input":
                  case "select":
                  case "textarea":
                    u.autoFocus && n.focus();
                    break;
                  case "img":
                    u.src && (n.src = u.src);
                }
              }
              break;
            case 6:
              break;
            case 4:
              break;
            case 12:
              break;
            case 13:
              if (t.memoizedState === null) {
                var s = t.alternate;
                if (s !== null) {
                  var p = s.memoizedState;
                  if (p !== null) {
                    var f = p.dehydrated;
                    f !== null && fr(f);
                  }
                }
              }
              break;
            case 19:
            case 17:
            case 21:
            case 22:
            case 23:
            case 25:
              break;
            default:
              throw Error(k(163));
          }
        de || (t.flags & 512 && tl(t));
      } catch (v) {
        $(t, t.return, v);
      }
    }
    if (t === e) {
      C = null;
      break;
    }
    if (((n = t.sibling), n !== null)) {
      (n.return = t.return), (C = n);
      break;
    }
    C = t.return;
  }
}
function Ss(e) {
  for (; C !== null; ) {
    var t = C;
    if (t === e) {
      C = null;
      break;
    }
    var n = t.sibling;
    if (n !== null) {
      (n.return = t.return), (C = n);
      break;
    }
    C = t.return;
  }
}
function ks(e) {
  for (; C !== null; ) {
    var t = C;
    try {
      switch (t.tag) {
        case 0:
        case 11:
        case 15:
          var n = t.return;
          try {
            ni(4, t);
          } catch (u) {
            $(t, n, u);
          }
          break;
        case 1:
          var r = t.stateNode;
          if (typeof r.componentDidMount == "function") {
            var a = t.return;
            try {
              r.componentDidMount();
            } catch (u) {
              $(t, a, u);
            }
          }
          var i = t.return;
          try {
            tl(t);
          } catch (u) {
            $(t, i, u);
          }
          break;
        case 5:
          var o = t.return;
          try {
            tl(t);
          } catch (u) {
            $(t, o, u);
          }
      }
    } catch (u) {
      $(t, t.return, u);
    }
    if (t === e) {
      C = null;
      break;
    }
    var l = t.sibling;
    if (l !== null) {
      (l.return = t.return), (C = l);
      break;
    }
    C = t.return;
  }
}
var Yg = Math.ceil,
  Ua = st.ReactCurrentDispatcher,
  Zl = st.ReactCurrentOwner,
  Ue = st.ReactCurrentBatchConfig,
  I = 0,
  re = null,
  J = null,
  le = 0,
  xe = 0,
  mn = Mt(0),
  Z = 0,
  jr = null,
  qt = 0,
  ri = 0,
  eu = 0,
  ir = null,
  we = null,
  tu = 0,
  Rn = 1 / 0,
  et = null,
  Aa = !1,
  al = null,
  Ct = null,
  ta = !1,
  mt = null,
  Fa = 0,
  or = 0,
  il = null,
  pa = -1,
  fa = 0;
function he() {
  return I & 6 ? X() : pa !== -1 ? pa : (pa = X());
}
function xt(e) {
  return e.mode & 1
    ? I & 2 && le !== 0
      ? le & -le
      : Pg.transition !== null
      ? (fa === 0 && (fa = Rc()), fa)
      : ((e = U),
        e !== 0 || ((e = window.event), (e = e === void 0 ? 16 : Ac(e.type))),
        e)
    : 1;
}
function Ge(e, t, n, r) {
  if (50 < or) throw ((or = 0), (il = null), Error(k(185)));
  Or(e, n, r),
    (!(I & 2) || e !== re) &&
      (e === re && (!(I & 2) && (ri |= n), Z === 4 && ht(e, le)),
      je(e, r),
      n === 1 && I === 0 && !(t.mode & 1) && ((Rn = X() + 500), Za && Dt()));
}
function je(e, t) {
  var n = e.callbackNode;
  Pf(e, t);
  var r = ka(e, e === re ? le : 0);
  if (r === 0)
    n !== null && Ru(n), (e.callbackNode = null), (e.callbackPriority = 0);
  else if (((t = r & -r), e.callbackPriority !== t)) {
    if ((n != null && Ru(n), t === 1))
      e.tag === 0 ? Tg(Es.bind(null, e)) : rd(Es.bind(null, e)),
        Eg(function () {
          !(I & 6) && Dt();
        }),
        (n = null);
    else {
      switch (Lc(r)) {
        case 1:
          n = Tl;
          break;
        case 4:
          n = Nc;
          break;
        case 16:
          n = Sa;
          break;
        case 536870912:
          n = Oc;
          break;
        default:
          n = Sa;
      }
      n = np(n, Kd.bind(null, e));
    }
    (e.callbackPriority = t), (e.callbackNode = n);
  }
}
function Kd(e, t) {
  if (((pa = -1), (fa = 0), I & 6)) throw Error(k(327));
  var n = e.callbackNode;
  if (jn() && e.callbackNode !== n) return null;
  var r = ka(e, e === re ? le : 0);
  if (r === 0) return null;
  if (r & 30 || r & e.expiredLanes || t) t = Ba(e, r);
  else {
    t = r;
    var a = I;
    I |= 2;
    var i = Jd();
    (re !== e || le !== t) && ((et = null), (Rn = X() + 500), Qt(e, t));
    do
      try {
        $g();
        break;
      } catch (l) {
        Xd(e, l);
      }
    while (1);
    Bl(),
      (Ua.current = i),
      (I = a),
      J !== null ? (t = 0) : ((re = null), (le = 0), (t = Z));
  }
  if (t !== 0) {
    if (
      (t === 2 && ((a = Lo(e)), a !== 0 && ((r = a), (t = ol(e, a)))), t === 1)
    )
      throw ((n = jr), Qt(e, 0), ht(e, r), je(e, X()), n);
    if (t === 6) ht(e, r);
    else {
      if (
        ((a = e.current.alternate),
        !(r & 30) &&
          !Gg(a) &&
          ((t = Ba(e, r)),
          t === 2 && ((i = Lo(e)), i !== 0 && ((r = i), (t = ol(e, i)))),
          t === 1))
      )
        throw ((n = jr), Qt(e, 0), ht(e, r), je(e, X()), n);
      switch (((e.finishedWork = a), (e.finishedLanes = r), t)) {
        case 0:
        case 1:
          throw Error(k(345));
        case 2:
          Wt(e, we, et);
          break;
        case 3:
          if (
            (ht(e, r), (r & 130023424) === r && ((t = tu + 500 - X()), 10 < t))
          ) {
            if (ka(e, 0) !== 0) break;
            if (((a = e.suspendedLanes), (a & r) !== r)) {
              he(), (e.pingedLanes |= e.suspendedLanes & a);
              break;
            }
            e.timeoutHandle = Bo(Wt.bind(null, e, we, et), t);
            break;
          }
          Wt(e, we, et);
          break;
        case 4:
          if ((ht(e, r), (r & 4194240) === r)) break;
          for (t = e.eventTimes, a = -1; 0 < r; ) {
            var o = 31 - Ye(r);
            (i = 1 << o), (o = t[o]), o > a && (a = o), (r &= ~i);
          }
          if (
            ((r = a),
            (r = X() - r),
            (r =
              (120 > r
                ? 120
                : 480 > r
                ? 480
                : 1080 > r
                ? 1080
                : 1920 > r
                ? 1920
                : 3e3 > r
                ? 3e3
                : 4320 > r
                ? 4320
                : 1960 * Yg(r / 1960)) - r),
            10 < r)
          ) {
            e.timeoutHandle = Bo(Wt.bind(null, e, we, et), r);
            break;
          }
          Wt(e, we, et);
          break;
        case 5:
          Wt(e, we, et);
          break;
        default:
          throw Error(k(329));
      }
    }
  }
  return je(e, X()), e.callbackNode === n ? Kd.bind(null, e) : null;
}
function ol(e, t) {
  var n = ir;
  return (
    e.current.memoizedState.isDehydrated && (Qt(e, t).flags |= 256),
    (e = Ba(e, t)),
    e !== 2 && ((t = we), (we = n), t !== null && ll(t)),
    e
  );
}
function ll(e) {
  we === null ? (we = e) : we.push.apply(we, e);
}
function Gg(e) {
  for (var t = e; ; ) {
    if (t.flags & 16384) {
      var n = t.updateQueue;
      if (n !== null && ((n = n.stores), n !== null))
        for (var r = 0; r < n.length; r++) {
          var a = n[r],
            i = a.getSnapshot;
          a = a.value;
          try {
            if (!Qe(i(), a)) return !1;
          } catch {
            return !1;
          }
        }
    }
    if (((n = t.child), t.subtreeFlags & 16384 && n !== null))
      (n.return = t), (t = n);
    else {
      if (t === e) break;
      for (; t.sibling === null; ) {
        if (t.return === null || t.return === e) return !0;
        t = t.return;
      }
      (t.sibling.return = t.return), (t = t.sibling);
    }
  }
  return !0;
}
function ht(e, t) {
  for (
    t &= ~eu,
      t &= ~ri,
      e.suspendedLanes |= t,
      e.pingedLanes &= ~t,
      e = e.expirationTimes;
    0 < t;

  ) {
    var n = 31 - Ye(t),
      r = 1 << n;
    (e[n] = -1), (t &= ~r);
  }
}
function Es(e) {
  if (I & 6) throw Error(k(327));
  jn();
  var t = ka(e, 0);
  if (!(t & 1)) return je(e, X()), null;
  var n = Ba(e, t);
  if (e.tag !== 0 && n === 2) {
    var r = Lo(e);
    r !== 0 && ((t = r), (n = ol(e, r)));
  }
  if (n === 1) throw ((n = jr), Qt(e, 0), ht(e, t), je(e, X()), n);
  if (n === 6) throw Error(k(345));
  return (
    (e.finishedWork = e.current.alternate),
    (e.finishedLanes = t),
    Wt(e, we, et),
    je(e, X()),
    null
  );
}
function nu(e, t) {
  var n = I;
  I |= 1;
  try {
    return e(t);
  } finally {
    (I = n), I === 0 && ((Rn = X() + 500), Za && Dt());
  }
}
function Zt(e) {
  mt !== null && mt.tag === 0 && !(I & 6) && jn();
  var t = I;
  I |= 1;
  var n = Ue.transition,
    r = U;
  try {
    if (((Ue.transition = null), (U = 1), e)) return e();
  } finally {
    (U = r), (Ue.transition = n), (I = t), !(I & 6) && Dt();
  }
}
function ru() {
  (xe = mn.current), b(mn);
}
function Qt(e, t) {
  (e.finishedWork = null), (e.finishedLanes = 0);
  var n = e.timeoutHandle;
  if ((n !== -1 && ((e.timeoutHandle = -1), kg(n)), J !== null))
    for (n = J.return; n !== null; ) {
      var r = n;
      switch ((Ul(r), r.tag)) {
        case 1:
          (r = r.type.childContextTypes), r != null && Ta();
          break;
        case 3:
          Nn(), b(ke), b(fe), Gl();
          break;
        case 5:
          Yl(r);
          break;
        case 4:
          Nn();
          break;
        case 13:
          b(Y);
          break;
        case 19:
          b(Y);
          break;
        case 10:
          Wl(r.type._context);
          break;
        case 22:
        case 23:
          ru();
      }
      n = n.return;
    }
  if (
    ((re = e),
    (J = e = Tt(e.current, null)),
    (le = xe = t),
    (Z = 0),
    (jr = null),
    (eu = ri = qt = 0),
    (we = ir = null),
    Yt !== null)
  ) {
    for (t = 0; t < Yt.length; t++)
      if (((n = Yt[t]), (r = n.interleaved), r !== null)) {
        n.interleaved = null;
        var a = r.next,
          i = n.pending;
        if (i !== null) {
          var o = i.next;
          (i.next = a), (r.next = o);
        }
        n.pending = r;
      }
    Yt = null;
  }
  return e;
}
function Xd(e, t) {
  do {
    var n = J;
    try {
      if ((Bl(), (sa.current = za), Ia)) {
        for (var r = G.memoizedState; r !== null; ) {
          var a = r.queue;
          a !== null && (a.pending = null), (r = r.next);
        }
        Ia = !1;
      }
      if (
        ((Jt = 0),
        (ne = q = G = null),
        (rr = !1),
        (Sr = 0),
        (Zl.current = null),
        n === null || n.return === null)
      ) {
        (Z = 1), (jr = t), (J = null);
        break;
      }
      e: {
        var i = e,
          o = n.return,
          l = n,
          u = t;
        if (
          ((t = le),
          (l.flags |= 32768),
          u !== null && typeof u == "object" && typeof u.then == "function")
        ) {
          var s = u,
            p = l,
            f = p.tag;
          if (!(p.mode & 1) && (f === 0 || f === 11 || f === 15)) {
            var v = p.alternate;
            v
              ? ((p.updateQueue = v.updateQueue),
                (p.memoizedState = v.memoizedState),
                (p.lanes = v.lanes))
              : ((p.updateQueue = null), (p.memoizedState = null));
          }
          var _ = cs(o);
          if (_ !== null) {
            (_.flags &= -257),
              ds(_, o, l, i, t),
              _.mode & 1 && ss(i, s, t),
              (t = _),
              (u = s);
            var m = t.updateQueue;
            if (m === null) {
              var h = new Set();
              h.add(u), (t.updateQueue = h);
            } else m.add(u);
            break e;
          } else {
            if (!(t & 1)) {
              ss(i, s, t), au();
              break e;
            }
            u = Error(k(426));
          }
        } else if (V && l.mode & 1) {
          var E = cs(o);
          if (E !== null) {
            !(E.flags & 65536) && (E.flags |= 256),
              ds(E, o, l, i, t),
              Al(On(u, l));
            break e;
          }
        }
        (i = u = On(u, l)),
          Z !== 4 && (Z = 2),
          ir === null ? (ir = [i]) : ir.push(i),
          (i = o);
        do {
          switch (i.tag) {
            case 3:
              (i.flags |= 65536), (t &= -t), (i.lanes |= t);
              var d = Md(i, u, t);
              ns(i, d);
              break e;
            case 1:
              l = u;
              var c = i.type,
                g = i.stateNode;
              if (
                !(i.flags & 128) &&
                (typeof c.getDerivedStateFromError == "function" ||
                  (g !== null &&
                    typeof g.componentDidCatch == "function" &&
                    (Ct === null || !Ct.has(g))))
              ) {
                (i.flags |= 65536), (t &= -t), (i.lanes |= t);
                var y = Dd(i, l, t);
                ns(i, y);
                break e;
              }
          }
          i = i.return;
        } while (i !== null);
      }
      Zd(n);
    } catch (j) {
      (t = j), J === n && n !== null && (J = n = n.return);
      continue;
    }
    break;
  } while (1);
}
function Jd() {
  var e = Ua.current;
  return (Ua.current = za), e === null ? za : e;
}
function au() {
  (Z === 0 || Z === 3 || Z === 2) && (Z = 4),
    re === null || (!(qt & 268435455) && !(ri & 268435455)) || ht(re, le);
}
function Ba(e, t) {
  var n = I;
  I |= 2;
  var r = Jd();
  (re !== e || le !== t) && ((et = null), Qt(e, t));
  do
    try {
      Qg();
      break;
    } catch (a) {
      Xd(e, a);
    }
  while (1);
  if ((Bl(), (I = n), (Ua.current = r), J !== null)) throw Error(k(261));
  return (re = null), (le = 0), Z;
}
function Qg() {
  for (; J !== null; ) qd(J);
}
function $g() {
  for (; J !== null && !yf(); ) qd(J);
}
function qd(e) {
  var t = tp(e.alternate, e, xe);
  (e.memoizedProps = e.pendingProps),
    t === null ? Zd(e) : (J = t),
    (Zl.current = null);
}
function Zd(e) {
  var t = e;
  do {
    var n = t.alternate;
    if (((e = t.return), t.flags & 32768)) {
      if (((n = Wg(n, t)), n !== null)) {
        (n.flags &= 32767), (J = n);
        return;
      }
      if (e !== null)
        (e.flags |= 32768), (e.subtreeFlags = 0), (e.deletions = null);
      else {
        (Z = 6), (J = null);
        return;
      }
    } else if (((n = Bg(n, t, xe)), n !== null)) {
      J = n;
      return;
    }
    if (((t = t.sibling), t !== null)) {
      J = t;
      return;
    }
    J = t = e;
  } while (t !== null);
  Z === 0 && (Z = 5);
}
function Wt(e, t, n) {
  var r = U,
    a = Ue.transition;
  try {
    (Ue.transition = null), (U = 1), Kg(e, t, n, r);
  } finally {
    (Ue.transition = a), (U = r);
  }
  return null;
}
function Kg(e, t, n, r) {
  do jn();
  while (mt !== null);
  if (I & 6) throw Error(k(327));
  n = e.finishedWork;
  var a = e.finishedLanes;
  if (n === null) return null;
  if (((e.finishedWork = null), (e.finishedLanes = 0), n === e.current))
    throw Error(k(177));
  (e.callbackNode = null), (e.callbackPriority = 0);
  var i = n.lanes | n.childLanes;
  if (
    (Nf(e, i),
    e === re && ((J = re = null), (le = 0)),
    (!(n.subtreeFlags & 2064) && !(n.flags & 2064)) ||
      ta ||
      ((ta = !0),
      np(Sa, function () {
        return jn(), null;
      })),
    (i = (n.flags & 15990) !== 0),
    n.subtreeFlags & 15990 || i)
  ) {
    (i = Ue.transition), (Ue.transition = null);
    var o = U;
    U = 1;
    var l = I;
    (I |= 4),
      (Zl.current = null),
      bg(e, n),
      Qd(n, e),
      vg(Ao),
      (Ea = !!Uo),
      (Ao = Uo = null),
      (e.current = n),
      Vg(n),
      wf(),
      (I = l),
      (U = o),
      (Ue.transition = i);
  } else e.current = n;
  if (
    (ta && ((ta = !1), (mt = e), (Fa = a)),
    (i = e.pendingLanes),
    i === 0 && (Ct = null),
    Ef(n.stateNode),
    je(e, X()),
    t !== null)
  )
    for (r = e.onRecoverableError, n = 0; n < t.length; n++)
      (a = t[n]), r(a.value, { componentStack: a.stack, digest: a.digest });
  if (Aa) throw ((Aa = !1), (e = al), (al = null), e);
  return (
    Fa & 1 && e.tag !== 0 && jn(),
    (i = e.pendingLanes),
    i & 1 ? (e === il ? or++ : ((or = 0), (il = e))) : (or = 0),
    Dt(),
    null
  );
}
function jn() {
  if (mt !== null) {
    var e = Lc(Fa),
      t = Ue.transition,
      n = U;
    try {
      if (((Ue.transition = null), (U = 16 > e ? 16 : e), mt === null))
        var r = !1;
      else {
        if (((e = mt), (mt = null), (Fa = 0), I & 6)) throw Error(k(331));
        var a = I;
        for (I |= 4, C = e.current; C !== null; ) {
          var i = C,
            o = i.child;
          if (C.flags & 16) {
            var l = i.deletions;
            if (l !== null) {
              for (var u = 0; u < l.length; u++) {
                var s = l[u];
                for (C = s; C !== null; ) {
                  var p = C;
                  switch (p.tag) {
                    case 0:
                    case 11:
                    case 15:
                      ar(8, p, i);
                  }
                  var f = p.child;
                  if (f !== null) (f.return = p), (C = f);
                  else
                    for (; C !== null; ) {
                      p = C;
                      var v = p.sibling,
                        _ = p.return;
                      if ((Vd(p), p === s)) {
                        C = null;
                        break;
                      }
                      if (v !== null) {
                        (v.return = _), (C = v);
                        break;
                      }
                      C = _;
                    }
                }
              }
              var m = i.alternate;
              if (m !== null) {
                var h = m.child;
                if (h !== null) {
                  m.child = null;
                  do {
                    var E = h.sibling;
                    (h.sibling = null), (h = E);
                  } while (h !== null);
                }
              }
              C = i;
            }
          }
          if (i.subtreeFlags & 2064 && o !== null) (o.return = i), (C = o);
          else
            e: for (; C !== null; ) {
              if (((i = C), i.flags & 2048))
                switch (i.tag) {
                  case 0:
                  case 11:
                  case 15:
                    ar(9, i, i.return);
                }
              var d = i.sibling;
              if (d !== null) {
                (d.return = i.return), (C = d);
                break e;
              }
              C = i.return;
            }
        }
        var c = e.current;
        for (C = c; C !== null; ) {
          o = C;
          var g = o.child;
          if (o.subtreeFlags & 2064 && g !== null) (g.return = o), (C = g);
          else
            e: for (o = c; C !== null; ) {
              if (((l = C), l.flags & 2048))
                try {
                  switch (l.tag) {
                    case 0:
                    case 11:
                    case 15:
                      ni(9, l);
                  }
                } catch (j) {
                  $(l, l.return, j);
                }
              if (l === o) {
                C = null;
                break e;
              }
              var y = l.sibling;
              if (y !== null) {
                (y.return = l.return), (C = y);
                break e;
              }
              C = l.return;
            }
        }
        if (
          ((I = a), Dt(), Je && typeof Je.onPostCommitFiberRoot == "function")
        )
          try {
            Je.onPostCommitFiberRoot($a, e);
          } catch {}
        r = !0;
      }
      return r;
    } finally {
      (U = n), (Ue.transition = t);
    }
  }
  return !1;
}
function js(e, t, n) {
  (t = On(n, t)),
    (t = Md(e, t, 1)),
    (e = jt(e, t, 1)),
    (t = he()),
    e !== null && (Or(e, 1, t), je(e, t));
}
function $(e, t, n) {
  if (e.tag === 3) js(e, e, n);
  else
    for (; t !== null; ) {
      if (t.tag === 3) {
        js(t, e, n);
        break;
      } else if (t.tag === 1) {
        var r = t.stateNode;
        if (
          typeof t.type.getDerivedStateFromError == "function" ||
          (typeof r.componentDidCatch == "function" &&
            (Ct === null || !Ct.has(r)))
        ) {
          (e = On(n, e)),
            (e = Dd(t, e, 1)),
            (t = jt(t, e, 1)),
            (e = he()),
            t !== null && (Or(t, 1, e), je(t, e));
          break;
        }
      }
      t = t.return;
    }
}
function Xg(e, t, n) {
  var r = e.pingCache;
  r !== null && r.delete(t),
    (t = he()),
    (e.pingedLanes |= e.suspendedLanes & n),
    re === e &&
      (le & n) === n &&
      (Z === 4 || (Z === 3 && (le & 130023424) === le && 500 > X() - tu)
        ? Qt(e, 0)
        : (eu |= n)),
    je(e, t);
}
function ep(e, t) {
  t === 0 &&
    (e.mode & 1
      ? ((t = Yr), (Yr <<= 1), !(Yr & 130023424) && (Yr = 4194304))
      : (t = 1));
  var n = he();
  (e = lt(e, t)), e !== null && (Or(e, t, n), je(e, n));
}
function Jg(e) {
  var t = e.memoizedState,
    n = 0;
  t !== null && (n = t.retryLane), ep(e, n);
}
function qg(e, t) {
  var n = 0;
  switch (e.tag) {
    case 13:
      var r = e.stateNode,
        a = e.memoizedState;
      a !== null && (n = a.retryLane);
      break;
    case 19:
      r = e.stateNode;
      break;
    default:
      throw Error(k(314));
  }
  r !== null && r.delete(t), ep(e, n);
}
var tp;
tp = function (e, t, n) {
  if (e !== null)
    if (e.memoizedProps !== t.pendingProps || ke.current) Se = !0;
    else {
      if (!(e.lanes & n) && !(t.flags & 128)) return (Se = !1), Fg(e, t, n);
      Se = !!(e.flags & 131072);
    }
  else (Se = !1), V && t.flags & 1048576 && ad(t, Oa, t.index);
  switch (((t.lanes = 0), t.tag)) {
    case 2:
      var r = t.type;
      da(e, t), (e = t.pendingProps);
      var a = xn(t, fe.current);
      En(t, n), (a = $l(null, t, r, e, a, n));
      var i = Kl();
      return (
        (t.flags |= 1),
        typeof a == "object" &&
        a !== null &&
        typeof a.render == "function" &&
        a.$$typeof === void 0
          ? ((t.tag = 1),
            (t.memoizedState = null),
            (t.updateQueue = null),
            Ee(r) ? ((i = !0), Pa(t)) : (i = !1),
            (t.memoizedState =
              a.state !== null && a.state !== void 0 ? a.state : null),
            bl(t),
            (a.updater = ei),
            (t.stateNode = a),
            (a._reactInternals = t),
            Qo(t, r, e, n),
            (t = Xo(null, t, r, !0, i, n)))
          : ((t.tag = 0), V && i && zl(t), ve(null, t, a, n), (t = t.child)),
        t
      );
    case 16:
      r = t.elementType;
      e: {
        switch (
          (da(e, t),
          (e = t.pendingProps),
          (a = r._init),
          (r = a(r._payload)),
          (t.type = r),
          (a = t.tag = ev(r)),
          (e = He(r, e)),
          a)
        ) {
          case 0:
            t = Ko(null, t, r, e, n);
            break e;
          case 1:
            t = gs(null, t, r, e, n);
            break e;
          case 11:
            t = ps(null, t, r, e, n);
            break e;
          case 14:
            t = fs(null, t, r, He(r.type, e), n);
            break e;
        }
        throw Error(k(306, r, ""));
      }
      return t;
    case 0:
      return (
        (r = t.type),
        (a = t.pendingProps),
        (a = t.elementType === r ? a : He(r, a)),
        Ko(e, t, r, a, n)
      );
    case 1:
      return (
        (r = t.type),
        (a = t.pendingProps),
        (a = t.elementType === r ? a : He(r, a)),
        gs(e, t, r, a, n)
      );
    case 3:
      e: {
        if ((Ad(t), e === null)) throw Error(k(387));
        (r = t.pendingProps),
          (i = t.memoizedState),
          (a = i.element),
          ud(e, t),
          Ma(t, r, null, n);
        var o = t.memoizedState;
        if (((r = o.element), i.isDehydrated))
          if (
            ((i = {
              element: r,
              isDehydrated: !1,
              cache: o.cache,
              pendingSuspenseBoundaries: o.pendingSuspenseBoundaries,
              transitions: o.transitions,
            }),
            (t.updateQueue.baseState = i),
            (t.memoizedState = i),
            t.flags & 256)
          ) {
            (a = On(Error(k(423)), t)), (t = vs(e, t, r, n, a));
            break e;
          } else if (r !== a) {
            (a = On(Error(k(424)), t)), (t = vs(e, t, r, n, a));
            break e;
          } else
            for (
              Te = Et(t.stateNode.containerInfo.firstChild),
                Pe = t,
                V = !0,
                Ve = null,
                n = pd(t, null, r, n),
                t.child = n;
              n;

            )
              (n.flags = (n.flags & -3) | 4096), (n = n.sibling);
        else {
          if ((Tn(), r === a)) {
            t = ut(e, t, n);
            break e;
          }
          ve(e, t, r, n);
        }
        t = t.child;
      }
      return t;
    case 5:
      return (
        fd(t),
        e === null && Vo(t),
        (r = t.type),
        (a = t.pendingProps),
        (i = e !== null ? e.memoizedProps : null),
        (o = a.children),
        Fo(r, a) ? (o = null) : i !== null && Fo(r, i) && (t.flags |= 32),
        Ud(e, t),
        ve(e, t, o, n),
        t.child
      );
    case 6:
      return e === null && Vo(t), null;
    case 13:
      return Fd(e, t, n);
    case 4:
      return (
        Vl(t, t.stateNode.containerInfo),
        (r = t.pendingProps),
        e === null ? (t.child = Pn(t, null, r, n)) : ve(e, t, r, n),
        t.child
      );
    case 11:
      return (
        (r = t.type),
        (a = t.pendingProps),
        (a = t.elementType === r ? a : He(r, a)),
        ps(e, t, r, a, n)
      );
    case 7:
      return ve(e, t, t.pendingProps, n), t.child;
    case 8:
      return ve(e, t, t.pendingProps.children, n), t.child;
    case 12:
      return ve(e, t, t.pendingProps.children, n), t.child;
    case 10:
      e: {
        if (
          ((r = t.type._context),
          (a = t.pendingProps),
          (i = t.memoizedProps),
          (o = a.value),
          B(Ra, r._currentValue),
          (r._currentValue = o),
          i !== null)
        )
          if (Qe(i.value, o)) {
            if (i.children === a.children && !ke.current) {
              t = ut(e, t, n);
              break e;
            }
          } else
            for (i = t.child, i !== null && (i.return = t); i !== null; ) {
              var l = i.dependencies;
              if (l !== null) {
                o = i.child;
                for (var u = l.firstContext; u !== null; ) {
                  if (u.context === r) {
                    if (i.tag === 1) {
                      (u = at(-1, n & -n)), (u.tag = 2);
                      var s = i.updateQueue;
                      if (s !== null) {
                        s = s.shared;
                        var p = s.pending;
                        p === null
                          ? (u.next = u)
                          : ((u.next = p.next), (p.next = u)),
                          (s.pending = u);
                      }
                    }
                    (i.lanes |= n),
                      (u = i.alternate),
                      u !== null && (u.lanes |= n),
                      Yo(i.return, n, t),
                      (l.lanes |= n);
                    break;
                  }
                  u = u.next;
                }
              } else if (i.tag === 10) o = i.type === t.type ? null : i.child;
              else if (i.tag === 18) {
                if (((o = i.return), o === null)) throw Error(k(341));
                (o.lanes |= n),
                  (l = o.alternate),
                  l !== null && (l.lanes |= n),
                  Yo(o, n, t),
                  (o = i.sibling);
              } else o = i.child;
              if (o !== null) o.return = i;
              else
                for (o = i; o !== null; ) {
                  if (o === t) {
                    o = null;
                    break;
                  }
                  if (((i = o.sibling), i !== null)) {
                    (i.return = o.return), (o = i);
                    break;
                  }
                  o = o.return;
                }
              i = o;
            }
        ve(e, t, a.children, n), (t = t.child);
      }
      return t;
    case 9:
      return (
        (a = t.type),
        (r = t.pendingProps.children),
        En(t, n),
        (a = Ae(a)),
        (r = r(a)),
        (t.flags |= 1),
        ve(e, t, r, n),
        t.child
      );
    case 14:
      return (
        (r = t.type),
        (a = He(r, t.pendingProps)),
        (a = He(r.type, a)),
        fs(e, t, r, a, n)
      );
    case 15:
      return Id(e, t, t.type, t.pendingProps, n);
    case 17:
      return (
        (r = t.type),
        (a = t.pendingProps),
        (a = t.elementType === r ? a : He(r, a)),
        da(e, t),
        (t.tag = 1),
        Ee(r) ? ((e = !0), Pa(t)) : (e = !1),
        En(t, n),
        cd(t, r, a),
        Qo(t, r, a, n),
        Xo(null, t, r, !0, e, n)
      );
    case 19:
      return Bd(e, t, n);
    case 22:
      return zd(e, t, n);
  }
  throw Error(k(156, t.tag));
};
function np(e, t) {
  return Pc(e, t);
}
function Zg(e, t, n, r) {
  (this.tag = e),
    (this.key = n),
    (this.sibling =
      this.child =
      this.return =
      this.stateNode =
      this.type =
      this.elementType =
        null),
    (this.index = 0),
    (this.ref = null),
    (this.pendingProps = t),
    (this.dependencies =
      this.memoizedState =
      this.updateQueue =
      this.memoizedProps =
        null),
    (this.mode = r),
    (this.subtreeFlags = this.flags = 0),
    (this.deletions = null),
    (this.childLanes = this.lanes = 0),
    (this.alternate = null);
}
function ze(e, t, n, r) {
  return new Zg(e, t, n, r);
}
function iu(e) {
  return (e = e.prototype), !(!e || !e.isReactComponent);
}
function ev(e) {
  if (typeof e == "function") return iu(e) ? 1 : 0;
  if (e != null) {
    if (((e = e.$$typeof), e === jl)) return 11;
    if (e === Cl) return 14;
  }
  return 2;
}
function Tt(e, t) {
  var n = e.alternate;
  return (
    n === null
      ? ((n = ze(e.tag, t, e.key, e.mode)),
        (n.elementType = e.elementType),
        (n.type = e.type),
        (n.stateNode = e.stateNode),
        (n.alternate = e),
        (e.alternate = n))
      : ((n.pendingProps = t),
        (n.type = e.type),
        (n.flags = 0),
        (n.subtreeFlags = 0),
        (n.deletions = null)),
    (n.flags = e.flags & 14680064),
    (n.childLanes = e.childLanes),
    (n.lanes = e.lanes),
    (n.child = e.child),
    (n.memoizedProps = e.memoizedProps),
    (n.memoizedState = e.memoizedState),
    (n.updateQueue = e.updateQueue),
    (t = e.dependencies),
    (n.dependencies =
      t === null ? null : { lanes: t.lanes, firstContext: t.firstContext }),
    (n.sibling = e.sibling),
    (n.index = e.index),
    (n.ref = e.ref),
    n
  );
}
function ga(e, t, n, r, a, i) {
  var o = 2;
  if (((r = e), typeof e == "function")) iu(e) && (o = 1);
  else if (typeof e == "string") o = 5;
  else
    e: switch (e) {
      case un:
        return $t(n.children, a, i, t);
      case El:
        (o = 8), (a |= 8);
        break;
      case _o:
        return (
          (e = ze(12, n, t, a | 2)), (e.elementType = _o), (e.lanes = i), e
        );
      case mo:
        return (e = ze(13, n, t, a)), (e.elementType = mo), (e.lanes = i), e;
      case yo:
        return (e = ze(19, n, t, a)), (e.elementType = yo), (e.lanes = i), e;
      case dc:
        return ai(n, a, i, t);
      default:
        if (typeof e == "object" && e !== null)
          switch (e.$$typeof) {
            case sc:
              o = 10;
              break e;
            case cc:
              o = 9;
              break e;
            case jl:
              o = 11;
              break e;
            case Cl:
              o = 14;
              break e;
            case ft:
              (o = 16), (r = null);
              break e;
          }
        throw Error(k(130, e == null ? e : typeof e, ""));
    }
  return (
    (t = ze(o, n, t, a)), (t.elementType = e), (t.type = r), (t.lanes = i), t
  );
}
function $t(e, t, n, r) {
  return (e = ze(7, e, r, t)), (e.lanes = n), e;
}
function ai(e, t, n, r) {
  return (
    (e = ze(22, e, r, t)),
    (e.elementType = dc),
    (e.lanes = n),
    (e.stateNode = { isHidden: !1 }),
    e
  );
}
function ao(e, t, n) {
  return (e = ze(6, e, null, t)), (e.lanes = n), e;
}
function io(e, t, n) {
  return (
    (t = ze(4, e.children !== null ? e.children : [], e.key, t)),
    (t.lanes = n),
    (t.stateNode = {
      containerInfo: e.containerInfo,
      pendingChildren: null,
      implementation: e.implementation,
    }),
    t
  );
}
function tv(e, t, n, r, a) {
  (this.tag = t),
    (this.containerInfo = e),
    (this.finishedWork =
      this.pingCache =
      this.current =
      this.pendingChildren =
        null),
    (this.timeoutHandle = -1),
    (this.callbackNode = this.pendingContext = this.context = null),
    (this.callbackPriority = 0),
    (this.eventTimes = Fi(0)),
    (this.expirationTimes = Fi(-1)),
    (this.entangledLanes =
      this.finishedLanes =
      this.mutableReadLanes =
      this.expiredLanes =
      this.pingedLanes =
      this.suspendedLanes =
      this.pendingLanes =
        0),
    (this.entanglements = Fi(0)),
    (this.identifierPrefix = r),
    (this.onRecoverableError = a),
    (this.mutableSourceEagerHydrationData = null);
}
function ou(e, t, n, r, a, i, o, l, u) {
  return (
    (e = new tv(e, t, n, l, u)),
    t === 1 ? ((t = 1), i === !0 && (t |= 8)) : (t = 0),
    (i = ze(3, null, null, t)),
    (e.current = i),
    (i.stateNode = e),
    (i.memoizedState = {
      element: r,
      isDehydrated: n,
      cache: null,
      transitions: null,
      pendingSuspenseBoundaries: null,
    }),
    bl(i),
    e
  );
}
function nv(e, t, n) {
  var r = 3 < arguments.length && arguments[3] !== void 0 ? arguments[3] : null;
  return {
    $$typeof: ln,
    key: r == null ? null : "" + r,
    children: e,
    containerInfo: t,
    implementation: n,
  };
}
function rp(e) {
  if (!e) return Ot;
  e = e._reactInternals;
  e: {
    if (nn(e) !== e || e.tag !== 1) throw Error(k(170));
    var t = e;
    do {
      switch (t.tag) {
        case 3:
          t = t.stateNode.context;
          break e;
        case 1:
          if (Ee(t.type)) {
            t = t.stateNode.__reactInternalMemoizedMergedChildContext;
            break e;
          }
      }
      t = t.return;
    } while (t !== null);
    throw Error(k(171));
  }
  if (e.tag === 1) {
    var n = e.type;
    if (Ee(n)) return nd(e, n, t);
  }
  return t;
}
function ap(e, t, n, r, a, i, o, l, u) {
  return (
    (e = ou(n, r, !0, e, a, i, o, l, u)),
    (e.context = rp(null)),
    (n = e.current),
    (r = he()),
    (a = xt(n)),
    (i = at(r, a)),
    (i.callback = t ?? null),
    jt(n, i, a),
    (e.current.lanes = a),
    Or(e, a, r),
    je(e, r),
    e
  );
}
function ii(e, t, n, r) {
  var a = t.current,
    i = he(),
    o = xt(a);
  return (
    (n = rp(n)),
    t.context === null ? (t.context = n) : (t.pendingContext = n),
    (t = at(i, o)),
    (t.payload = { element: e }),
    (r = r === void 0 ? null : r),
    r !== null && (t.callback = r),
    (e = jt(a, t, o)),
    e !== null && (Ge(e, a, o, i), ua(e, a, o)),
    o
  );
}
function Wa(e) {
  if (((e = e.current), !e.child)) return null;
  switch (e.child.tag) {
    case 5:
      return e.child.stateNode;
    default:
      return e.child.stateNode;
  }
}
function Cs(e, t) {
  if (((e = e.memoizedState), e !== null && e.dehydrated !== null)) {
    var n = e.retryLane;
    e.retryLane = n !== 0 && n < t ? n : t;
  }
}
function lu(e, t) {
  Cs(e, t), (e = e.alternate) && Cs(e, t);
}
function rv() {
  return null;
}
var ip =
  typeof reportError == "function"
    ? reportError
    : function (e) {
        console.error(e);
      };
function uu(e) {
  this._internalRoot = e;
}
oi.prototype.render = uu.prototype.render = function (e) {
  var t = this._internalRoot;
  if (t === null) throw Error(k(409));
  ii(e, t, null, null);
};
oi.prototype.unmount = uu.prototype.unmount = function () {
  var e = this._internalRoot;
  if (e !== null) {
    this._internalRoot = null;
    var t = e.containerInfo;
    Zt(function () {
      ii(null, e, null, null);
    }),
      (t[ot] = null);
  }
};
function oi(e) {
  this._internalRoot = e;
}
oi.prototype.unstable_scheduleHydration = function (e) {
  if (e) {
    var t = Ic();
    e = { blockedOn: null, target: e, priority: t };
    for (var n = 0; n < vt.length && t !== 0 && t < vt[n].priority; n++);
    vt.splice(n, 0, e), n === 0 && Uc(e);
  }
};
function su(e) {
  return !(!e || (e.nodeType !== 1 && e.nodeType !== 9 && e.nodeType !== 11));
}
function li(e) {
  return !(
    !e ||
    (e.nodeType !== 1 &&
      e.nodeType !== 9 &&
      e.nodeType !== 11 &&
      (e.nodeType !== 8 || e.nodeValue !== " react-mount-point-unstable "))
  );
}
function xs() {}
function av(e, t, n, r, a) {
  if (a) {
    if (typeof r == "function") {
      var i = r;
      r = function () {
        var s = Wa(o);
        i.call(s);
      };
    }
    var o = ap(t, r, e, 0, null, !1, !1, "", xs);
    return (
      (e._reactRootContainer = o),
      (e[ot] = o.current),
      hr(e.nodeType === 8 ? e.parentNode : e),
      Zt(),
      o
    );
  }
  for (; (a = e.lastChild); ) e.removeChild(a);
  if (typeof r == "function") {
    var l = r;
    r = function () {
      var s = Wa(u);
      l.call(s);
    };
  }
  var u = ou(e, 0, !1, null, null, !1, !1, "", xs);
  return (
    (e._reactRootContainer = u),
    (e[ot] = u.current),
    hr(e.nodeType === 8 ? e.parentNode : e),
    Zt(function () {
      ii(t, u, n, r);
    }),
    u
  );
}
function ui(e, t, n, r, a) {
  var i = n._reactRootContainer;
  if (i) {
    var o = i;
    if (typeof a == "function") {
      var l = a;
      a = function () {
        var u = Wa(o);
        l.call(u);
      };
    }
    ii(t, o, e, a);
  } else o = av(n, t, e, a, r);
  return Wa(o);
}
Mc = function (e) {
  switch (e.tag) {
    case 3:
      var t = e.stateNode;
      if (t.current.memoizedState.isDehydrated) {
        var n = Xn(t.pendingLanes);
        n !== 0 &&
          (Pl(t, n | 1), je(t, X()), !(I & 6) && ((Rn = X() + 500), Dt()));
      }
      break;
    case 13:
      Zt(function () {
        var r = lt(e, 1);
        if (r !== null) {
          var a = he();
          Ge(r, e, 1, a);
        }
      }),
        lu(e, 1);
  }
};
Nl = function (e) {
  if (e.tag === 13) {
    var t = lt(e, 134217728);
    if (t !== null) {
      var n = he();
      Ge(t, e, 134217728, n);
    }
    lu(e, 134217728);
  }
};
Dc = function (e) {
  if (e.tag === 13) {
    var t = xt(e),
      n = lt(e, t);
    if (n !== null) {
      var r = he();
      Ge(n, e, t, r);
    }
    lu(e, t);
  }
};
Ic = function () {
  return U;
};
zc = function (e, t) {
  var n = U;
  try {
    return (U = e), t();
  } finally {
    U = n;
  }
};
No = function (e, t, n) {
  switch (t) {
    case "input":
      if ((ko(e, n), (t = n.name), n.type === "radio" && t != null)) {
        for (n = e; n.parentNode; ) n = n.parentNode;
        for (
          n = n.querySelectorAll(
            "input[name=" + JSON.stringify("" + t) + '][type="radio"]'
          ),
            t = 0;
          t < n.length;
          t++
        ) {
          var r = n[t];
          if (r !== e && r.form === e.form) {
            var a = qa(r);
            if (!a) throw Error(k(90));
            fc(r), ko(r, a);
          }
        }
      }
      break;
    case "textarea":
      vc(e, n);
      break;
    case "select":
      (t = n.value), t != null && yn(e, !!n.multiple, t, !1);
  }
};
kc = nu;
Ec = Zt;
var iv = { usingClientEntryPoint: !1, Events: [Lr, pn, qa, wc, Sc, nu] },
  Yn = {
    findFiberByHostInstance: Vt,
    bundleType: 0,
    version: "18.2.0",
    rendererPackageName: "react-dom",
  },
  ov = {
    bundleType: Yn.bundleType,
    version: Yn.version,
    rendererPackageName: Yn.rendererPackageName,
    rendererConfig: Yn.rendererConfig,
    overrideHookState: null,
    overrideHookStateDeletePath: null,
    overrideHookStateRenamePath: null,
    overrideProps: null,
    overridePropsDeletePath: null,
    overridePropsRenamePath: null,
    setErrorHandler: null,
    setSuspenseHandler: null,
    scheduleUpdate: null,
    currentDispatcherRef: st.ReactCurrentDispatcher,
    findHostInstanceByFiber: function (e) {
      return (e = xc(e)), e === null ? null : e.stateNode;
    },
    findFiberByHostInstance: Yn.findFiberByHostInstance || rv,
    findHostInstancesForRefresh: null,
    scheduleRefresh: null,
    scheduleRoot: null,
    setRefreshHandler: null,
    getCurrentFiber: null,
    reconcilerVersion: "18.2.0-next-9e3b772b8-20220608",
  };
if (typeof __REACT_DEVTOOLS_GLOBAL_HOOK__ < "u") {
  var na = __REACT_DEVTOOLS_GLOBAL_HOOK__;
  if (!na.isDisabled && na.supportsFiber)
    try {
      ($a = na.inject(ov)), (Je = na);
    } catch {}
}
Oe.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED = iv;
Oe.createPortal = function (e, t) {
  var n = 2 < arguments.length && arguments[2] !== void 0 ? arguments[2] : null;
  if (!su(t)) throw Error(k(200));
  return nv(e, t, null, n);
};
Oe.createRoot = function (e, t) {
  if (!su(e)) throw Error(k(299));
  var n = !1,
    r = "",
    a = ip;
  return (
    t != null &&
      (t.unstable_strictMode === !0 && (n = !0),
      t.identifierPrefix !== void 0 && (r = t.identifierPrefix),
      t.onRecoverableError !== void 0 && (a = t.onRecoverableError)),
    (t = ou(e, 1, !1, null, null, n, !1, r, a)),
    (e[ot] = t.current),
    hr(e.nodeType === 8 ? e.parentNode : e),
    new uu(t)
  );
};
Oe.findDOMNode = function (e) {
  if (e == null) return null;
  if (e.nodeType === 1) return e;
  var t = e._reactInternals;
  if (t === void 0)
    throw typeof e.render == "function"
      ? Error(k(188))
      : ((e = Object.keys(e).join(",")), Error(k(268, e)));
  return (e = xc(t)), (e = e === null ? null : e.stateNode), e;
};
Oe.flushSync = function (e) {
  return Zt(e);
};
Oe.hydrate = function (e, t, n) {
  if (!li(t)) throw Error(k(200));
  return ui(null, e, t, !0, n);
};
Oe.hydrateRoot = function (e, t, n) {
  if (!su(e)) throw Error(k(405));
  var r = (n != null && n.hydratedSources) || null,
    a = !1,
    i = "",
    o = ip;
  if (
    (n != null &&
      (n.unstable_strictMode === !0 && (a = !0),
      n.identifierPrefix !== void 0 && (i = n.identifierPrefix),
      n.onRecoverableError !== void 0 && (o = n.onRecoverableError)),
    (t = ap(t, null, e, 1, n ?? null, a, !1, i, o)),
    (e[ot] = t.current),
    hr(e),
    r)
  )
    for (e = 0; e < r.length; e++)
      (n = r[e]),
        (a = n._getVersion),
        (a = a(n._source)),
        t.mutableSourceEagerHydrationData == null
          ? (t.mutableSourceEagerHydrationData = [n, a])
          : t.mutableSourceEagerHydrationData.push(n, a);
  return new oi(t);
};
Oe.render = function (e, t, n) {
  if (!li(t)) throw Error(k(200));
  return ui(null, e, t, !1, n);
};
Oe.unmountComponentAtNode = function (e) {
  if (!li(e)) throw Error(k(40));
  return e._reactRootContainer
    ? (Zt(function () {
        ui(null, null, e, !1, function () {
          (e._reactRootContainer = null), (e[ot] = null);
        });
      }),
      !0)
    : !1;
};
Oe.unstable_batchedUpdates = nu;
Oe.unstable_renderSubtreeIntoContainer = function (e, t, n, r) {
  if (!li(n)) throw Error(k(200));
  if (e == null || e._reactInternals === void 0) throw Error(k(38));
  return ui(e, t, n, !1, r);
};
Oe.version = "18.2.0-next-9e3b772b8-20220608";
(function (e) {
  function t() {
    if (
      !(
        typeof __REACT_DEVTOOLS_GLOBAL_HOOK__ > "u" ||
        typeof __REACT_DEVTOOLS_GLOBAL_HOOK__.checkDCE != "function"
      )
    )
      try {
        __REACT_DEVTOOLS_GLOBAL_HOOK__.checkDCE(t);
      } catch (n) {
        console.error(n);
      }
  }
  t(), (e.exports = Oe);
})(nf);
var Ts = _a;
(go.createRoot = Ts.createRoot), (go.hydrateRoot = Ts.hydrateRoot);
var ul = {},
  lv = {
    get exports() {
      return ul;
    },
    set exports(e) {
      ul = e;
    },
  },
  op = {};
/**
 * @license React
 * use-sync-external-store-shim.production.min.js
 *
 * Copyright (c) Facebook, Inc. and its affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */ var Ln = S;
function uv(e, t) {
  return (e === t && (e !== 0 || 1 / e === 1 / t)) || (e !== e && t !== t);
}
var sv = typeof Object.is == "function" ? Object.is : uv,
  cv = Ln.useState,
  dv = Ln.useEffect,
  pv = Ln.useLayoutEffect,
  fv = Ln.useDebugValue;
function gv(e, t) {
  var n = t(),
    r = cv({ inst: { value: n, getSnapshot: t } }),
    a = r[0].inst,
    i = r[1];
  return (
    pv(
      function () {
        (a.value = n), (a.getSnapshot = t), oo(a) && i({ inst: a });
      },
      [e, n, t]
    ),
    dv(
      function () {
        return (
          oo(a) && i({ inst: a }),
          e(function () {
            oo(a) && i({ inst: a });
          })
        );
      },
      [e]
    ),
    fv(n),
    n
  );
}
function oo(e) {
  var t = e.getSnapshot;
  e = e.value;
  try {
    var n = t();
    return !sv(e, n);
  } catch {
    return !0;
  }
}
function vv(e, t) {
  return t();
}
var hv =
  typeof window > "u" ||
  typeof window.document > "u" ||
  typeof window.document.createElement > "u"
    ? vv
    : gv;
op.useSyncExternalStore =
  Ln.useSyncExternalStore !== void 0 ? Ln.useSyncExternalStore : hv;
(function (e) {
  e.exports = op;
})(lv);
var sl = {},
  _v = {
    get exports() {
      return sl;
    },
    set exports(e) {
      sl = e;
    },
  },
  lp = {};
/**
 * @license React
 * use-sync-external-store-shim/with-selector.production.min.js
 *
 * Copyright (c) Facebook, Inc. and its affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */ var si = S,
  mv = ul;
function yv(e, t) {
  return (e === t && (e !== 0 || 1 / e === 1 / t)) || (e !== e && t !== t);
}
var wv = typeof Object.is == "function" ? Object.is : yv,
  Sv = mv.useSyncExternalStore,
  kv = si.useRef,
  Ev = si.useEffect,
  jv = si.useMemo,
  Cv = si.useDebugValue;
lp.useSyncExternalStoreWithSelector = function (e, t, n, r, a) {
  var i = kv(null);
  if (i.current === null) {
    var o = { hasValue: !1, value: null };
    i.current = o;
  } else o = i.current;
  i = jv(
    function () {
      function u(_) {
        if (!s) {
          if (((s = !0), (p = _), (_ = r(_)), a !== void 0 && o.hasValue)) {
            var m = o.value;
            if (a(m, _)) return (f = m);
          }
          return (f = _);
        }
        if (((m = f), wv(p, _))) return m;
        var h = r(_);
        return a !== void 0 && a(m, h) ? m : ((p = _), (f = h));
      }
      var s = !1,
        p,
        f,
        v = n === void 0 ? null : n;
      return [
        function () {
          return u(t());
        },
        v === null
          ? void 0
          : function () {
              return u(v());
            },
      ];
    },
    [t, n, r, a]
  );
  var l = Sv(e, i[0], i[1]);
  return (
    Ev(
      function () {
        (o.hasValue = !0), (o.value = l);
      },
      [l]
    ),
    Cv(l),
    l
  );
};
(function (e) {
  e.exports = lp;
})(_v);
function xv(e) {
  e();
}
let up = xv;
const Tv = (e) => (up = e),
  Pv = () => up,
  Rt = S.createContext(null);
function sp() {
  return S.useContext(Rt);
}
const Nv = () => {
  throw new Error("uSES not initialized!");
};
let cp = Nv;
const Ov = (e) => {
    cp = e;
  },
  Rv = (e, t) => e === t;
function Lv(e = Rt) {
  const t = e === Rt ? sp : () => S.useContext(e);
  return function (r, a = Rv) {
    const { store: i, subscription: o, getServerState: l } = t(),
      u = cp(o.addNestedSub, i.getState, l || i.getState, r, a);
    return S.useDebugValue(u), u;
  };
}
const Mv = Lv();
var cl = {},
  Dv = {
    get exports() {
      return cl;
    },
    set exports(e) {
      cl = e;
    },
  },
  A = {};
/** @license React v16.13.1
 * react-is.production.min.js
 *
 * Copyright (c) Facebook, Inc. and its affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */ var ae = typeof Symbol == "function" && Symbol.for,
  cu = ae ? Symbol.for("react.element") : 60103,
  du = ae ? Symbol.for("react.portal") : 60106,
  ci = ae ? Symbol.for("react.fragment") : 60107,
  di = ae ? Symbol.for("react.strict_mode") : 60108,
  pi = ae ? Symbol.for("react.profiler") : 60114,
  fi = ae ? Symbol.for("react.provider") : 60109,
  gi = ae ? Symbol.for("react.context") : 60110,
  pu = ae ? Symbol.for("react.async_mode") : 60111,
  vi = ae ? Symbol.for("react.concurrent_mode") : 60111,
  hi = ae ? Symbol.for("react.forward_ref") : 60112,
  _i = ae ? Symbol.for("react.suspense") : 60113,
  Iv = ae ? Symbol.for("react.suspense_list") : 60120,
  mi = ae ? Symbol.for("react.memo") : 60115,
  yi = ae ? Symbol.for("react.lazy") : 60116,
  zv = ae ? Symbol.for("react.block") : 60121,
  Uv = ae ? Symbol.for("react.fundamental") : 60117,
  Av = ae ? Symbol.for("react.responder") : 60118,
  Fv = ae ? Symbol.for("react.scope") : 60119;
function Le(e) {
  if (typeof e == "object" && e !== null) {
    var t = e.$$typeof;
    switch (t) {
      case cu:
        switch (((e = e.type), e)) {
          case pu:
          case vi:
          case ci:
          case pi:
          case di:
          case _i:
            return e;
          default:
            switch (((e = e && e.$$typeof), e)) {
              case gi:
              case hi:
              case yi:
              case mi:
              case fi:
                return e;
              default:
                return t;
            }
        }
      case du:
        return t;
    }
  }
}
function dp(e) {
  return Le(e) === vi;
}
A.AsyncMode = pu;
A.ConcurrentMode = vi;
A.ContextConsumer = gi;
A.ContextProvider = fi;
A.Element = cu;
A.ForwardRef = hi;
A.Fragment = ci;
A.Lazy = yi;
A.Memo = mi;
A.Portal = du;
A.Profiler = pi;
A.StrictMode = di;
A.Suspense = _i;
A.isAsyncMode = function (e) {
  return dp(e) || Le(e) === pu;
};
A.isConcurrentMode = dp;
A.isContextConsumer = function (e) {
  return Le(e) === gi;
};
A.isContextProvider = function (e) {
  return Le(e) === fi;
};
A.isElement = function (e) {
  return typeof e == "object" && e !== null && e.$$typeof === cu;
};
A.isForwardRef = function (e) {
  return Le(e) === hi;
};
A.isFragment = function (e) {
  return Le(e) === ci;
};
A.isLazy = function (e) {
  return Le(e) === yi;
};
A.isMemo = function (e) {
  return Le(e) === mi;
};
A.isPortal = function (e) {
  return Le(e) === du;
};
A.isProfiler = function (e) {
  return Le(e) === pi;
};
A.isStrictMode = function (e) {
  return Le(e) === di;
};
A.isSuspense = function (e) {
  return Le(e) === _i;
};
A.isValidElementType = function (e) {
  return (
    typeof e == "string" ||
    typeof e == "function" ||
    e === ci ||
    e === vi ||
    e === pi ||
    e === di ||
    e === _i ||
    e === Iv ||
    (typeof e == "object" &&
      e !== null &&
      (e.$$typeof === yi ||
        e.$$typeof === mi ||
        e.$$typeof === fi ||
        e.$$typeof === gi ||
        e.$$typeof === hi ||
        e.$$typeof === Uv ||
        e.$$typeof === Av ||
        e.$$typeof === Fv ||
        e.$$typeof === zv))
  );
};
A.typeOf = Le;
(function (e) {
  e.exports = A;
})(Dv);
var pp = cl,
  Bv = {
    $$typeof: !0,
    render: !0,
    defaultProps: !0,
    displayName: !0,
    propTypes: !0,
  },
  Wv = {
    $$typeof: !0,
    compare: !0,
    defaultProps: !0,
    displayName: !0,
    propTypes: !0,
    type: !0,
  },
  fp = {};
fp[pp.ForwardRef] = Bv;
fp[pp.Memo] = Wv;
var Ps = {},
  Hv = {
    get exports() {
      return Ps;
    },
    set exports(e) {
      Ps = e;
    },
  },
  F = {};
/**
 * @license React
 * react-is.production.min.js
 *
 * Copyright (c) Facebook, Inc. and its affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */ var fu = Symbol.for("react.element"),
  gu = Symbol.for("react.portal"),
  wi = Symbol.for("react.fragment"),
  Si = Symbol.for("react.strict_mode"),
  ki = Symbol.for("react.profiler"),
  Ei = Symbol.for("react.provider"),
  ji = Symbol.for("react.context"),
  bv = Symbol.for("react.server_context"),
  Ci = Symbol.for("react.forward_ref"),
  xi = Symbol.for("react.suspense"),
  Ti = Symbol.for("react.suspense_list"),
  Pi = Symbol.for("react.memo"),
  Ni = Symbol.for("react.lazy"),
  Vv = Symbol.for("react.offscreen"),
  gp;
gp = Symbol.for("react.module.reference");
function Be(e) {
  if (typeof e == "object" && e !== null) {
    var t = e.$$typeof;
    switch (t) {
      case fu:
        switch (((e = e.type), e)) {
          case wi:
          case ki:
          case Si:
          case xi:
          case Ti:
            return e;
          default:
            switch (((e = e && e.$$typeof), e)) {
              case bv:
              case ji:
              case Ci:
              case Ni:
              case Pi:
              case Ei:
                return e;
              default:
                return t;
            }
        }
      case gu:
        return t;
    }
  }
}
F.ContextConsumer = ji;
F.ContextProvider = Ei;
F.Element = fu;
F.ForwardRef = Ci;
F.Fragment = wi;
F.Lazy = Ni;
F.Memo = Pi;
F.Portal = gu;
F.Profiler = ki;
F.StrictMode = Si;
F.Suspense = xi;
F.SuspenseList = Ti;
F.isAsyncMode = function () {
  return !1;
};
F.isConcurrentMode = function () {
  return !1;
};
F.isContextConsumer = function (e) {
  return Be(e) === ji;
};
F.isContextProvider = function (e) {
  return Be(e) === Ei;
};
F.isElement = function (e) {
  return typeof e == "object" && e !== null && e.$$typeof === fu;
};
F.isForwardRef = function (e) {
  return Be(e) === Ci;
};
F.isFragment = function (e) {
  return Be(e) === wi;
};
F.isLazy = function (e) {
  return Be(e) === Ni;
};
F.isMemo = function (e) {
  return Be(e) === Pi;
};
F.isPortal = function (e) {
  return Be(e) === gu;
};
F.isProfiler = function (e) {
  return Be(e) === ki;
};
F.isStrictMode = function (e) {
  return Be(e) === Si;
};
F.isSuspense = function (e) {
  return Be(e) === xi;
};
F.isSuspenseList = function (e) {
  return Be(e) === Ti;
};
F.isValidElementType = function (e) {
  return (
    typeof e == "string" ||
    typeof e == "function" ||
    e === wi ||
    e === ki ||
    e === Si ||
    e === xi ||
    e === Ti ||
    e === Vv ||
    (typeof e == "object" &&
      e !== null &&
      (e.$$typeof === Ni ||
        e.$$typeof === Pi ||
        e.$$typeof === Ei ||
        e.$$typeof === ji ||
        e.$$typeof === Ci ||
        e.$$typeof === gp ||
        e.getModuleId !== void 0))
  );
};
F.typeOf = Be;
(function (e) {
  e.exports = F;
})(Hv);
function Yv() {
  const e = Pv();
  let t = null,
    n = null;
  return {
    clear() {
      (t = null), (n = null);
    },
    notify() {
      e(() => {
        let r = t;
        for (; r; ) r.callback(), (r = r.next);
      });
    },
    get() {
      let r = [],
        a = t;
      for (; a; ) r.push(a), (a = a.next);
      return r;
    },
    subscribe(r) {
      let a = !0,
        i = (n = { callback: r, next: null, prev: n });
      return (
        i.prev ? (i.prev.next = i) : (t = i),
        function () {
          !a ||
            t === null ||
            ((a = !1),
            i.next ? (i.next.prev = i.prev) : (n = i.prev),
            i.prev ? (i.prev.next = i.next) : (t = i.next));
        }
      );
    },
  };
}
const Ns = { notify() {}, get: () => [] };
function Gv(e, t) {
  let n,
    r = Ns;
  function a(f) {
    return u(), r.subscribe(f);
  }
  function i() {
    r.notify();
  }
  function o() {
    p.onStateChange && p.onStateChange();
  }
  function l() {
    return Boolean(n);
  }
  function u() {
    n || ((n = t ? t.addNestedSub(o) : e.subscribe(o)), (r = Yv()));
  }
  function s() {
    n && (n(), (n = void 0), r.clear(), (r = Ns));
  }
  const p = {
    addNestedSub: a,
    notifyNestedSubs: i,
    handleChangeWrapper: o,
    isSubscribed: l,
    trySubscribe: u,
    tryUnsubscribe: s,
    getListeners: () => r,
  };
  return p;
}
const Qv =
    typeof window < "u" &&
    typeof window.document < "u" &&
    typeof window.document.createElement < "u",
  $v = Qv ? S.useLayoutEffect : S.useEffect;
function Kv({ store: e, context: t, children: n, serverState: r }) {
  const a = S.useMemo(() => {
      const l = Gv(e);
      return {
        store: e,
        subscription: l,
        getServerState: r ? () => r : void 0,
      };
    }, [e, r]),
    i = S.useMemo(() => e.getState(), [e]);
  $v(() => {
    const { subscription: l } = a;
    return (
      (l.onStateChange = l.notifyNestedSubs),
      l.trySubscribe(),
      i !== e.getState() && l.notifyNestedSubs(),
      () => {
        l.tryUnsubscribe(), (l.onStateChange = void 0);
      }
    );
  }, [a, i]);
  const o = t || Rt;
  return yl.createElement(o.Provider, { value: a }, n);
}
function vp(e = Rt) {
  const t = e === Rt ? sp : () => S.useContext(e);
  return function () {
    const { store: r } = t();
    return r;
  };
}
const Xv = vp();
function Jv(e = Rt) {
  const t = e === Rt ? Xv : vp(e);
  return function () {
    return t().dispatch;
  };
}
const zn = Jv();
Ov(sl.useSyncExternalStoreWithSelector);
Tv(_a.unstable_batchedUpdates);
function Cr(e) {
  return (
    (Cr =
      typeof Symbol == "function" && typeof Symbol.iterator == "symbol"
        ? function (t) {
            return typeof t;
          }
        : function (t) {
            return t &&
              typeof Symbol == "function" &&
              t.constructor === Symbol &&
              t !== Symbol.prototype
              ? "symbol"
              : typeof t;
          }),
    Cr(e)
  );
}
function qv(e, t) {
  if (Cr(e) !== "object" || e === null) return e;
  var n = e[Symbol.toPrimitive];
  if (n !== void 0) {
    var r = n.call(e, t || "default");
    if (Cr(r) !== "object") return r;
    throw new TypeError("@@toPrimitive must return a primitive value.");
  }
  return (t === "string" ? String : Number)(e);
}
function Zv(e) {
  var t = qv(e, "string");
  return Cr(t) === "symbol" ? t : String(t);
}
function eh(e, t, n) {
  return (
    (t = Zv(t)),
    t in e
      ? Object.defineProperty(e, t, {
          value: n,
          enumerable: !0,
          configurable: !0,
          writable: !0,
        })
      : (e[t] = n),
    e
  );
}
function Os(e, t) {
  var n = Object.keys(e);
  if (Object.getOwnPropertySymbols) {
    var r = Object.getOwnPropertySymbols(e);
    t &&
      (r = r.filter(function (a) {
        return Object.getOwnPropertyDescriptor(e, a).enumerable;
      })),
      n.push.apply(n, r);
  }
  return n;
}
function Rs(e) {
  for (var t = 1; t < arguments.length; t++) {
    var n = arguments[t] != null ? arguments[t] : {};
    t % 2
      ? Os(Object(n), !0).forEach(function (r) {
          eh(e, r, n[r]);
        })
      : Object.getOwnPropertyDescriptors
      ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(n))
      : Os(Object(n)).forEach(function (r) {
          Object.defineProperty(e, r, Object.getOwnPropertyDescriptor(n, r));
        });
  }
  return e;
}
function oe(e) {
  return (
    "Minified Redux error #" +
    e +
    "; visit https://redux.js.org/Errors?code=" +
    e +
    " for the full message or use the non-minified dev environment for full errors. "
  );
}
var Ls = (function () {
    return (typeof Symbol == "function" && Symbol.observable) || "@@observable";
  })(),
  lo = function () {
    return Math.random().toString(36).substring(7).split("").join(".");
  },
  xr = {
    INIT: "@@redux/INIT" + lo(),
    REPLACE: "@@redux/REPLACE" + lo(),
    PROBE_UNKNOWN_ACTION: function () {
      return "@@redux/PROBE_UNKNOWN_ACTION" + lo();
    },
  };
function th(e) {
  if (typeof e != "object" || e === null) return !1;
  for (var t = e; Object.getPrototypeOf(t) !== null; )
    t = Object.getPrototypeOf(t);
  return Object.getPrototypeOf(e) === t;
}
function Oi(e, t, n) {
  var r;
  if (
    (typeof t == "function" && typeof n == "function") ||
    (typeof n == "function" && typeof arguments[3] == "function")
  )
    throw new Error(oe(0));
  if (
    (typeof t == "function" && typeof n > "u" && ((n = t), (t = void 0)),
    typeof n < "u")
  ) {
    if (typeof n != "function") throw new Error(oe(1));
    return n(Oi)(e, t);
  }
  if (typeof e != "function") throw new Error(oe(2));
  var a = e,
    i = t,
    o = [],
    l = o,
    u = !1;
  function s() {
    l === o && (l = o.slice());
  }
  function p() {
    if (u) throw new Error(oe(3));
    return i;
  }
  function f(h) {
    if (typeof h != "function") throw new Error(oe(4));
    if (u) throw new Error(oe(5));
    var E = !0;
    return (
      s(),
      l.push(h),
      function () {
        if (E) {
          if (u) throw new Error(oe(6));
          (E = !1), s();
          var c = l.indexOf(h);
          l.splice(c, 1), (o = null);
        }
      }
    );
  }
  function v(h) {
    if (!th(h)) throw new Error(oe(7));
    if (typeof h.type > "u") throw new Error(oe(8));
    if (u) throw new Error(oe(9));
    try {
      (u = !0), (i = a(i, h));
    } finally {
      u = !1;
    }
    for (var E = (o = l), d = 0; d < E.length; d++) {
      var c = E[d];
      c();
    }
    return h;
  }
  function _(h) {
    if (typeof h != "function") throw new Error(oe(10));
    (a = h), v({ type: xr.REPLACE });
  }
  function m() {
    var h,
      E = f;
    return (
      (h = {
        subscribe: function (c) {
          if (typeof c != "object" || c === null) throw new Error(oe(11));
          function g() {
            c.next && c.next(p());
          }
          g();
          var y = E(g);
          return { unsubscribe: y };
        },
      }),
      (h[Ls] = function () {
        return this;
      }),
      h
    );
  }
  return (
    v({ type: xr.INIT }),
    (r = { dispatch: v, subscribe: f, getState: p, replaceReducer: _ }),
    (r[Ls] = m),
    r
  );
}
var nh = Oi;
function rh(e) {
  Object.keys(e).forEach(function (t) {
    var n = e[t],
      r = n(void 0, { type: xr.INIT });
    if (typeof r > "u") throw new Error(oe(12));
    if (typeof n(void 0, { type: xr.PROBE_UNKNOWN_ACTION() }) > "u")
      throw new Error(oe(13));
  });
}
function hp(e) {
  for (var t = Object.keys(e), n = {}, r = 0; r < t.length; r++) {
    var a = t[r];
    typeof e[a] == "function" && (n[a] = e[a]);
  }
  var i = Object.keys(n),
    o;
  try {
    rh(n);
  } catch (l) {
    o = l;
  }
  return function (u, s) {
    if ((u === void 0 && (u = {}), o)) throw o;
    for (var p = !1, f = {}, v = 0; v < i.length; v++) {
      var _ = i[v],
        m = n[_],
        h = u[_],
        E = m(h, s);
      if (typeof E > "u") throw (s && s.type, new Error(oe(14)));
      (f[_] = E), (p = p || E !== h);
    }
    return (p = p || i.length !== Object.keys(u).length), p ? f : u;
  };
}
function Ms(e, t) {
  return function () {
    return t(e.apply(this, arguments));
  };
}
function ah(e, t) {
  if (typeof e == "function") return Ms(e, t);
  if (typeof e != "object" || e === null) throw new Error(oe(16));
  var n = {};
  for (var r in e) {
    var a = e[r];
    typeof a == "function" && (n[r] = Ms(a, t));
  }
  return n;
}
function _p() {
  for (var e = arguments.length, t = new Array(e), n = 0; n < e; n++)
    t[n] = arguments[n];
  return t.length === 0
    ? function (r) {
        return r;
      }
    : t.length === 1
    ? t[0]
    : t.reduce(function (r, a) {
        return function () {
          return r(a.apply(void 0, arguments));
        };
      });
}
function ih() {
  for (var e = arguments.length, t = new Array(e), n = 0; n < e; n++)
    t[n] = arguments[n];
  return function (r) {
    return function () {
      var a = r.apply(void 0, arguments),
        i = function () {
          throw new Error(oe(15));
        },
        o = {
          getState: a.getState,
          dispatch: function () {
            return i.apply(void 0, arguments);
          },
        },
        l = t.map(function (u) {
          return u(o);
        });
      return (
        (i = _p.apply(void 0, l)(a.dispatch)),
        Rs(Rs({}, a), {}, { dispatch: i })
      );
    };
  };
}
const oh = Object.freeze(
    Object.defineProperty(
      {
        __proto__: null,
        __DO_NOT_USE__ActionTypes: xr,
        applyMiddleware: ih,
        bindActionCreators: ah,
        combineReducers: hp,
        compose: _p,
        createStore: Oi,
        legacy_createStore: nh,
      },
      Symbol.toStringTag,
      { value: "Module" }
    )
  ),
  lh = Rp(oh);
var Ds = lh.compose,
  uh =
    typeof window < "u" && window.__REDUX_DEVTOOLS_EXTENSION_COMPOSE__
      ? window.__REDUX_DEVTOOLS_EXTENSION_COMPOSE__
      : function () {
          if (arguments.length !== 0)
            return typeof arguments[0] == "object"
              ? Ds
              : Ds.apply(null, arguments);
        };
const vu = [
    {
      adult: !1,
      backdrop_path: "/qjGrUmKW78MCFG8PTLDBp67S27p.jpg",
      genre_ids: [16, 28, 12, 14],
      id: 635302,
      original_language: "ja",
      original_title: "劇場版「鬼滅の刃」無限列車編",
      overview:
        "Тандзиро с друзьями из отряда уничтожителей демонов расследует серию загадочных исчезновений внутри мчащегося поезда. Но компания ещё не знает, что в составе находится один из двенадцати Лунных демонов, и он приготовил для них ловушку.",
      popularity: 911.596,
      poster_path: "/sFXMvI0I5GEiXRCJDIuzBBXpkiZ.jpg",
      release_date: "2020-10-16",
      title: "Истребитель демонов: Поезд «Бесконечный»",
      video: !1,
      vote_average: 8.4,
      vote_count: 2488,
    },
    {
      adult: !1,
      backdrop_path: "/adlZgxAudw3ZPA0HUcDzSTKrH8B.jpg",
      genre_ids: [16, 10751, 35, 12, 10770],
      id: 755812,
      original_language: "fr",
      original_title: "Miraculous World : New York, les héros unis",
      overview:
        "В честь недели дружбы между Францией и США класс Маринет готовится к поездке в Нью-Йорк. Эдриан тоже хочет поехать, но поначалу его не отпускает отец, к тому же Леди Баг просит Супер-Кота последить за Парижем в ее отсутствие. Когда Габриель все-таки разрешает сыну полететь в Нью-Йорк, Эдриан оказывается перед сложным выбором. Желание посетить Нью-Йорк пересиливает, и он отправляется в долгожданное путешествие. Леди Баг и Супер-Кот не подозревают, что Бражник тоже собирается в город небоскребов, и что им предстоит объединить силы с американскими супергероями, чтобы дать отпор врагу.",
      popularity: 294.919,
      poster_path: "/kIHgjAkuzvKBnmdstpBOo4AfZah.jpg",
      release_date: "2020-09-25",
      title: "Леди Баг и Супер-Кот: Нью-Йорк. Союз героев",
      video: !1,
      vote_average: 8.2,
      vote_count: 899,
    },
    {
      adult: !1,
      backdrop_path: "/hdXQxlD4mep7qtC5TsvhIt840OV.jpg",
      genre_ids: [16, 28, 12, 878],
      id: 703771,
      original_language: "en",
      original_title: "Deathstroke: Knights & Dragons - The Movie",
      overview:
        "Десять лет назад Слейд Уилсон, также известный как супер-убийца по имени Детстроук, совершил трагическую ошибку, и его жена и сын заплатили ужасную цену. Теперь, десять лет спустя, семье Уилсона снова угрожают кровожадный Шакал и террористы из H.IV.E. Сможет ли Детстроук искупить грехи прошлого - или его семья заплатит самую высокую цену?",
      popularity: 266.621,
      poster_path: "/iTdtIZYvc9MHgnXf4ns8WDJoGzB.jpg",
      release_date: "2020-08-04",
      title: "Детстроук: Рыцари и Драконы",
      video: !1,
      vote_average: 6.9,
      vote_count: 350,
    },
    {
      adult: !1,
      backdrop_path: "/stmYfCUGd8Iy6kAMBr6AmWqx8Bq.jpg",
      genre_ids: [28, 878, 35, 10751],
      id: 454626,
      original_language: "en",
      original_title: "Sonic the Hedgehog",
      overview:
        "Отвязный ярко-синий ёжик Соник вместе с новообретённым — человеческим — лучшим другом Томом Вачовски знакомится со сложностями жизни на Земле и противостоит злодейскому доктору Роботнику, который хочет пленить Соника и использовать его безграничные суперсилы для завоевания мирового господства.",
      popularity: 228.436,
      poster_path: "/z7S7EX1YHOhCX9afC9aSyStJFEE.jpg",
      release_date: "2020-02-12",
      title: "Соник в кино",
      video: !1,
      vote_average: 7.4,
      vote_count: 8101,
    },
    {
      adult: !1,
      backdrop_path: "/fmIp40ev4VGquK2bMo52PQgaV2d.jpg",
      genre_ids: [16, 10751, 12, 14, 35],
      id: 529203,
      original_language: "en",
      original_title: "The Croods: A New Age",
      overview:
        "Такие харизматичные герои как члены клана Крудс просто не в силах усидеть на месте. Они смело идут навстречу самым головокружительным приключениям и готовы ответить непредсказуемостью и находчивостью на любой вызов судьбы.",
      popularity: 232.834,
      poster_path: "/gT8cDQklL3xoVuXPLGRdjDI3tRA.jpg",
      release_date: "2020-11-25",
      title: "Семейка Крудс: Новоселье",
      video: !1,
      vote_average: 7.6,
      vote_count: 2998,
    },
    {
      adult: !1,
      backdrop_path: "/z8TvnEVRenMSTemxYZwLGqFofgF.jpg",
      genre_ids: [28, 14, 12],
      id: 458576,
      original_language: "en",
      original_title: "Monster Hunter",
      overview:
        "Лейтенант Артемис и её солдаты с Земли случайно попадают в параллельный мир, где обитают разнообразные монстры. Закалённые бойцы оказываются в необычной ситуации и вынуждены использовать все свои навыки, чтобы пережить встречу с фантастическими тварями. Спасти их может только загадочный Охотник, который как никто умеет убивать монстров.",
      popularity: 223.39,
      poster_path: "/uHiil0BMdvGouYQv70MgYqho44n.jpg",
      release_date: "2020-12-03",
      title: "Охотник на монстров",
      video: !1,
      vote_average: 6.8,
      vote_count: 2592,
    },
    {
      adult: !1,
      backdrop_path: "/iDLtDgxLiYsarfdQ4msUhUqoNPp.jpg",
      genre_ids: [878],
      id: 725273,
      original_language: "en",
      original_title: "Solitary",
      overview:
        "Айзек Хэвлок — осужденный за убийство молодой человек, насильно отправленный в космос по программе реабилитации заключенных, которые должны создать первую земную колонию. Однако что-то идет не так, и на корабле, который дожидался его на орбите, происходит взрыв, в результате чего Айзек вместе со своей сокамерницей оказывается в ловушке в аварийной капсуле; начинается отчаянная борьба за жизнь.",
      popularity: 186.977,
      poster_path: "/ApwmbrMlsuOay5rXQA4Kbz7qJAl.jpg",
      release_date: "2020-08-31",
      title: "Одиночка",
      video: !1,
      vote_average: 5.6,
      vote_count: 76,
    },
    {
      adult: !1,
      backdrop_path: "/yjmqROw9fa9GahuUxvBFnoVYYMG.jpg",
      genre_ids: [16, 18, 10749],
      id: 719088,
      original_language: "ja",
      original_title: "イエスかノーか半分か",
      overview:
        "Куниеда Кэй - молодой популярный телеведущий, который поддерживает видимость идеала в своей профессиональной жизни, ему даже дали прозвище Принц. Но на самом деле презирает всех вокруг себя как некомпетентных дураков. Тем не менее, в силу обстоятельств, аниматор Цудзуки Усио сумел обнаружить потаенную сторону личности Кэя.",
      popularity: 191.354,
      poster_path: "/sH8xCXLEbihNNVDbewddzkYnNPF.jpg",
      release_date: "2020-12-11",
      title: "Да, нет или наверное?",
      video: !1,
      vote_average: 7.1,
      vote_count: 26,
    },
    {
      adult: !1,
      backdrop_path: "/6hgItrYQEG33y0I7yP2SRl2ei4w.jpg",
      genre_ids: [10749, 18],
      id: 613504,
      original_language: "en",
      original_title: "After We Collided",
      overview:
        "Встреча с Хардином разделила её жизнь на до и после. Молодые и такие разные – от их страсти буквально разлетаются искры. Однако внезапно в жизни девушки появляется новый знакомый, который готов положить к её ногам весь мир.",
      popularity: 230.647,
      poster_path: "/kWkFxHdXhKBM3NmncrX8JYhVXEe.jpg",
      release_date: "2020-09-02",
      title: "После. Глава 2",
      video: !1,
      vote_average: 7.3,
      vote_count: 4591,
    },
    {
      adult: !1,
      backdrop_path: "/wu1uilmhM4TdluKi2ytfz8gidHf.jpg",
      genre_ids: [10751, 16, 14, 12, 35],
      id: 400160,
      original_language: "en",
      original_title: "The SpongeBob Movie: Sponge on the Run",
      overview:
        "Гэри, лучший друг Губки Боба, пропал. Нужно найти его! Губка Боб и Патрик отправляются далеко за пределы Бикини Боттома, чтобы спасти своего друга в розовой раковине.",
      popularity: 205.016,
      poster_path: "/wEisOJUM7mLB27ekk8w3MzswUfI.jpg",
      release_date: "2020-08-14",
      title: "Губка Боб в бегах",
      video: !1,
      vote_average: 7.6,
      vote_count: 2476,
    },
    {
      adult: !1,
      backdrop_path: "/yzpCv8CCWondN7O5au1KGiqnC3A.jpg",
      genre_ids: [28, 12, 14],
      id: 464052,
      original_language: "en",
      original_title: "Wonder Woman 1984",
      overview:
        "1984 год. Диана всё ещё грустит по погибшему Стиву, борется с мелким криминалом и работает в музее Смитсоновского института. Однажды она знакомится с новой коллегой Барбарой, специалисткой широкого профиля, которой поручено изучить новые артефакты. Среди древних предметов оказывается загадочный кристалл, который исполняет желания. Так к Диане внезапно возвращается Стив, а застенчивая и неуклюжая Барбара обретает невероятную силу, сноровку и привлекательность. Но за волшебным кристаллом охотится бизнесмен Максвелл Лорд, и у него имеются очень коварные планы на этот артефакт.",
      popularity: 170.137,
      poster_path: "/1B2YJCYyRudISmfSWCRfc95gAtq.jpg",
      release_date: "2020-12-16",
      title: "Чудо-женщина: 1984",
      video: !1,
      vote_average: 6.6,
      vote_count: 6867,
    },
    {
      adult: !1,
      backdrop_path: "/rQaHA74pevnGsxcKGaoZVGWe9TC.jpg",
      genre_ids: [16, 35, 14, 10751],
      id: 508442,
      original_language: "en",
      original_title: "Soul",
      overview:
        "Уже немолодой школьный учитель музыки Джо Гарднер всю жизнь мечтал выступать на сцене в составе джазового ансамбля. Однажды он успешно проходит прослушивание у известной клубной певицы и, возвращаясь домой вне себя от счастья, падает в люк и умирает. Теперь у Джо одна дорога — в Великое После, но он сбегает с идущего в вечность эскалатора и случайно попадает в Великое До. Тут новенькие души обретают себя, и у будущих людей зарождаются увлечения, мечты и интересы. Джо становится наставником упрямой души 22, которая уже много веков не может найти свою искру и отправиться на Землю.",
      popularity: 172.215,
      poster_path: "/x4egXUupjQ8KkRzhD54fmPZuFvF.jpg",
      release_date: "2020-12-25",
      title: "Душа",
      video: !1,
      vote_average: 8.2,
      vote_count: 8112,
    },
    {
      adult: !1,
      backdrop_path: "/8rIoyM6zYXJNjzGseT3MRusMPWl.jpg",
      genre_ids: [10751, 14, 12, 35, 27],
      id: 531219,
      original_language: "en",
      original_title: "Roald Dahl's The Witches",
      overview:
        "В конце 1967 года после гибели родителей мальчик переезжает жить к бабушке. Та вскоре замечает, что в их городке отираются ведьмы, которые не прочь превратить её внука в какое-нибудь животное, поэтому решает спрятаться в самом шикарном отеле Алабамы, где работает её кузен. Бабуля думала, что там они встретят только скучающих богатых постояльцев, но именно в этом отеле решает устроить тайный шабаш Величайшая Самая Главная Ведьма.",
      popularity: 140.501,
      poster_path: "/1BUMAQIph7yzMHi2FRl2XgjONAs.jpg",
      release_date: "2020-10-25",
      title: "Ведьмы",
      video: !1,
      vote_average: 6.5,
      vote_count: 2209,
    },
    {
      adult: !1,
      backdrop_path: "/fRrpOILyXuWaWLmqF7kXeMVwITQ.jpg",
      genre_ids: [27, 53, 12, 9648],
      id: 522444,
      original_language: "en",
      original_title: "Black Water: Abyss",
      overview:
        "Пять любителей приключений отправляются на поиски тайной пещеры в Австралии. Друзья надеются на веселую вылазку, но разбушевавшаяся стихия преграждает обратный путь и приводит к ним нечто смертельно опасное – то, чего там не должно быть.",
      popularity: 165.705,
      poster_path: "/zBXMuJJfwswqeMj9s9ixPoJRNof.jpg",
      release_date: "2020-07-09",
      title: "Хищные Воды: Западня",
      video: !1,
      vote_average: 5.2,
      vote_count: 327,
    },
    {
      adult: !1,
      backdrop_path: "/54yOImQgj8i85u9hxxnaIQBRUuo.jpg",
      genre_ids: [28, 80, 18, 53],
      id: 539885,
      original_language: "en",
      original_title: "Ava",
      overview:
        "Она умна, красива и чрезвычайно опасна. Перемещаясь по миру словно тень, агент по имени Ева в одиночку выполняет самые сложные задания своего руководства. Внезапно, когда очередная спецоперация идёт не по плану, и Ева решает выйти из игры, один из влиятельных боссов секретной службы открывает на неё охоту.",
      popularity: 154.589,
      poster_path: "/xf92uk5zPY8aSofxdsRpWBPrkRi.jpg",
      release_date: "2020-07-02",
      title: "Агент Ева",
      video: !1,
      vote_average: 5.8,
      vote_count: 2005,
    },
    {
      adult: !1,
      backdrop_path: "/pQdZmXq0vvKP1RYUrDHQkzMlS4x.jpg",
      genre_ids: [16, 28, 53, 12, 14],
      id: 571265,
      original_language: "ja",
      original_title: "デジモンアドベンチャー LAST EVOLUTION 絆",
      overview:
        "Обычная подготовка подростков к летнему лагерю прервалась самым неожиданным образом. Вместе со своим соседом Тайчи узнаёт причину беспорядков. Вся электроника в мегаполисе вышла из строя. Мальчишка понимает, что его родственники, мать и сестра, теперь находятся в огромной опасности. Поезд, на который они сели, находится неизвестно где и, скорее всего, похищен неведомой силой. Необходимо остановить его, и они бегут на ближайшую железнодорожную станцию. Но именно там он внезапно переносится в параллельный, цифровой мир. Именно здесь, в виртуальном Токио, находится причина неприятностей. Кто-то захотел проникнуть в настоящий мир, чтобы обрести власть. Встреча со сражающимся дигимоном становится только началом новых приключений героев. Надежде на спокойную жизнь осуществиться не дано.",
      popularity: 164.177,
      poster_path: "/6C9fb7duBSzsXkkobfwuMgEW58F.jpg",
      release_date: "2020-02-21",
      title: "Приключения дигимонов: Последняя эволюция",
      video: !1,
      vote_average: 8.1,
      vote_count: 146,
    },
    {
      adult: !1,
      backdrop_path: "/ySHlkHDbXztjdjRJ0pZN7FXRnaW.jpg",
      genre_ids: [16, 12, 28, 14],
      id: 631132,
      original_language: "ja",
      original_title: "ゴブリンスレイヤー -GOBLIN'S CROWN-",
      overview:
        "Убийца Гоблинов вместе с товарищами по оружию продолжает выполнять свою миссию по спасению людей от гоблинов. В этот раз они направились в заснеженные края по просьбе Девы Меча, дабы спасти дочь дворянина, которая стала авантюристом. Однако за всем этим скрывается гораздо большая опасность, чем могло показаться на первый взгляд. Теперь главным героям предстоит не только выручить попавших в беду, но и вновь сразиться со свирепыми гоблинами.",
      popularity: 143.328,
      poster_path: "/cbwQ3KTcILbJh8wRYJbVEp10IRE.jpg",
      release_date: "2020-02-01",
      title: "Убийца гоблинов: Корона гоблина",
      video: !1,
      vote_average: 7.6,
      vote_count: 167,
    },
    {
      adult: !1,
      backdrop_path: "/zMrk2G3XsnfYKiIp1NEfdtvDyBH.jpg",
      genre_ids: [12, 14, 18],
      id: 337401,
      original_language: "en",
      original_title: "Mulan",
      overview:
        "История о бесстрашной молодой девушке, которая выдаёт себя за мужчину, чтобы вступить в ряды армии, противостоящей Северным захватчикам, надвигающимся на Китай. Старшая дочь храброго воина Хуа, Мулан — энергичная и решительная девушка. Когда Император издаёт указ о том, что один мужчина из каждой семьи должен вступить в ряды Имперской армии, Мулан занимает место своего больного отца, еще не зная, что ей предстоит прославиться как одному из величайших воинов в истории Китая.",
      popularity: 144.156,
      poster_path: "/uxPauLtzjl9oWZtS1Bwp9mCDRFh.jpg",
      release_date: "2020-09-04",
      title: "Мулан",
      video: !1,
      vote_average: 7,
      vote_count: 5687,
    },
    {
      adult: !1,
      backdrop_path: "/fX8e94MEWSuTJExndVYxKsmA4Hw.jpg",
      genre_ids: [28, 12, 80],
      id: 604822,
      original_language: "zh",
      original_title: "急先锋",
      overview:
        "Они берутся за задания, которые другим не под силу. Когда международный синдикат наемников похищает известного бизнесмена, единственным шансом на его спасение становится лихая команда «Авангард», не имеющая равных в мастерстве и дерзости.",
      popularity: 138.404,
      poster_path: "/rrHIr2HFbr3DPIwZHJUjEVxXEUQ.jpg",
      release_date: "2020-09-30",
      title: "Авангард: Арктические волки",
      video: !1,
      vote_average: 6.4,
      vote_count: 409,
    },
    {
      adult: !1,
      backdrop_path: "/jiqD14fg7UTZOT6qgvzTmfRYpWI.jpg",
      genre_ids: [28, 80],
      id: 495764,
      original_language: "en",
      original_title:
        "Birds of Prey (and the Fantabulous Emancipation of One Harley Quinn)",
      overview:
        "Харли Квинн рассталась с Джокером и через некоторое время сообщила об этом всему Готэму, взорвав тот самый химзавод. Девушка больше не тоскует по несостоявшимся отношениям - ей некогда, так как она также потеряла неприкосновенность, и теперь за ней охотятся разной степени криминализованности граждане, желающие отомстить. А в это время ставший крёстным отцом Готема Роман Сайонис очень жаждет вернуть пропавший бриллиант, который стащила у его подручного малолетняя карманница.",
      popularity: 121.282,
      poster_path: "/wh5FEKiNeirvD02T2Ui22duZGCZ.jpg",
      release_date: "2020-02-05",
      title: "Хищные птицы: Потрясающая история Харли Квинн",
      video: !1,
      vote_average: 7,
      vote_count: 8806,
    },
    {
      adult: !1,
      backdrop_path: "/u1wHUA0R48FH4WV3sGqjwx3aNZm.jpg",
      genre_ids: [16, 18, 10402, 10749],
      id: 632632,
      original_language: "ja",
      original_title: "映画 ギヴン",
      overview:
        "Продолжение одноименного сериала про любовную драму одного из участников группы - Акихико Кадзи.",
      popularity: 123.779,
      poster_path: "/x53z2oKgpz1twX1FuhSL0tyIc4w.jpg",
      release_date: "2020-08-22",
      title: "Дарованный. Фильм",
      video: !1,
      vote_average: 8.4,
      vote_count: 320,
    },
    {
      adult: !1,
      backdrop_path: "/iEIcBVnttD9KOnpxtN1adVqKQkv.jpg",
      genre_ids: [16, 10751, 35, 14],
      id: 741074,
      original_language: "en",
      original_title: "Once Upon a Snowman",
      overview:
        "Рассказ о появлении на свет всеми любимого снеговика Олафа, о его первых шагах, познании себя. И о цепи событий, которые приводят к судьбоносной встрече с Кристофом и Анной.",
      popularity: 123.303,
      poster_path: "/zfdhsR3Y3xw42OHrMpi0oBw0Uk8.jpg",
      release_date: "2020-10-23",
      title: "Жил-был снеговик",
      video: !1,
      vote_average: 6.9,
      vote_count: 432,
    },
    {
      adult: !1,
      backdrop_path: "/vwpxy4kmQyW5aBJ3uxhQM6koU6H.jpg",
      genre_ids: [10751, 16],
      id: 743439,
      original_language: "en",
      original_title: "PAW Patrol: Jet To The Rescue",
      overview:
        "Герцог Болтунский использует магические силы бриллианта принцессы, чтобы поднять Гавкенгбург в небо. Скай нужно будет преодолеть свои самые большие страхи и возглавить команду Улетной помощи.",
      popularity: 123.175,
      poster_path: "/84HyCM0NwGdpYuXdhGspEAajAV8.jpg",
      release_date: "2020-09-10",
      title: "Щенячий патруль: Улётная помощь",
      video: !1,
      vote_average: 7,
      vote_count: 136,
    },
    {
      adult: !1,
      backdrop_path: "/oj9pEWQq5higRzKWiE8f0d5hBSb.jpg",
      genre_ids: [53, 27, 18],
      id: 617708,
      original_language: "es",
      original_title: "La Llorona",
      overview:
        "Пожилого диктатора Гватемалы обвиняют в геноциде начала 1980-х годов, когда массово уничтожалось коренное население страны — индейцы кекчи. Небольшая группа выживших женщин выступает в суде, после чего выносится обвинительный приговор. В ожидании апелляции диктатор с семьёй, личной охраной и преданной экономкой запирается в своём особняке, а протестующие устраивают осаду. Остальная прислуга принципиально уволилась, поэтому экономка вызывает на подмогу девушку из своей деревни.",
      popularity: 112.37,
      poster_path: "/yVsINl4Aa9vvQ9lE2LF77qNj7AP.jpg",
      release_date: "2020-01-22",
      title: "Ла Йорона",
      video: !1,
      vote_average: 6.5,
      vote_count: 252,
    },
    {
      adult: !1,
      backdrop_path: "/buop8brXQ8gGiOPrVTxZZUqE8Oa.jpg",
      genre_ids: [27, 53, 9648],
      id: 723072,
      original_language: "en",
      original_title: "Host",
      overview:
        "Во время спиритического онлайн-сеанса группа друзей вызывает духов. С молодыми людьми начинают происходить странные и пугающие события. Они пытаются понять, это игра специального онлайн-приложения, или некто из потустороннего мира манипулирует ими, расставляя смертельные ловушки, и используя возможности сети для того, чтобы превратить их реальную жизнь в кошмар, более страшный, чем ужасы интернет-пространства.",
      popularity: 111.625,
      poster_path: "/p08dWpMqhGZD0Xw1HzmWhp9O4zo.jpg",
      release_date: "2020-12-04",
      title: "Астрал. Онлайн",
      video: !1,
      vote_average: 6.6,
      vote_count: 537,
    },
    {
      adult: !1,
      backdrop_path: "/mluxnTYUSTRh7GtpZmkGAh9L6zQ.jpg",
      genre_ids: [16, 878],
      id: 665251,
      original_language: "ja",
      original_title: "オルタード・カーボン：リスリーブド",
      overview:
        "Бывший боец сопротивления, а ныне — наёмник Такэси Ковач, на планете Латимер расследует убийство босса якудза и в результате будет вынужден защищать девушку-татуировщицу, обладающую загадочной силой.",
      popularity: 107.887,
      poster_path: "/dql1WvlOdEgzQ0Ub05NqHmW7D4y.jpg",
      release_date: "2020-03-19",
      title: "Видоизменённый углерод: Восстановленный",
      video: !1,
      vote_average: 6.5,
      vote_count: 209,
    },
    {
      adult: !1,
      backdrop_path: "/4KpNHvQIjyg1YFovRAoUXoFrGnR.jpg",
      genre_ids: [16, 12, 10751, 878, 28, 14],
      id: 662708,
      original_language: "ja",
      original_title: "劇場版ポケットモンスター ココ",
      overview:
        "Коко растет в джунглях под опекой Заруда-отшельника. Он знакомится с Эшем и Пикачу и узнает о мире людей, а также об опасности, которая грозит его дому.",
      popularity: 104.934,
      poster_path: "/HS659LwNNdFtcvD0m8FxAATNQW.jpg",
      release_date: "2020-12-25",
      title: "Покемон-фильм: Секреты джунглей",
      video: !1,
      vote_average: 7.1,
      vote_count: 90,
    },
    {
      adult: !1,
      backdrop_path: "/jeAQdDX9nguP6YOX6QSWKDPkbBo.jpg",
      genre_ids: [28, 878, 14],
      id: 590706,
      original_language: "en",
      original_title: "Jiu Jitsu",
      overview:
        "Тысячелетиями наш мир охранял древний Орден боевых искусств. Когда каждые шесть лет на Землю проникала инопланетная раса, он побеждал незваных гостей, но очередное вторжение может стать последним. Потерявший память морпех Джейк должен объединиться с новым поколением бойцов Ордена, чтобы вступить в схватку с инопланетным захватчиком и спасти человечество от уничтожения.",
      popularity: 103.775,
      poster_path: "/uQ2EQpbDLn1yl6OHrQCHwaDOM9k.jpg",
      release_date: "2020-11-20",
      title: "Джиу-джитсу: Битва за Землю",
      video: !1,
      vote_average: 5,
      vote_count: 489,
    },
    {
      adult: !1,
      backdrop_path: "/aiQICxiWNcOsJruYxdPuhb6WtWu.jpg",
      genre_ids: [9648, 18, 27, 12, 16],
      id: 658558,
      original_language: "ja",
      original_title: "劇場版 巨蟲列島",
      overview:
        "После крушения самолета во время школьной поездки Орибэ Митсуми и ее одноклассники попали на, казалось бы, необитаемый остров. Митсуми нашла выживших и использовала свои навыки ориентирования на местности, чтобы им помочь. Она ждет, что их спасут в течение трех дней, которые можно и перетерпеть. Однако, она не учла тот факт, что остров населен гигантскими насекомыми-убийцами. Может, единственное, что может помочь выжить ей и ее одноклассникам, это ее знания о бабочках, осах и других насекомых.",
      popularity: 97.142,
      poster_path: "/cFmuU6nibl4hJKbSCDfFymFWVTf.jpg",
      release_date: "2020-01-10",
      title: "Остров насекомых-гигантов",
      video: !1,
      vote_average: 4.5,
      vote_count: 14,
    },
    {
      adult: !1,
      backdrop_path: "/3N316jUSdhvPyYTW29G4v9ebbcS.jpg",
      genre_ids: [53, 28, 80],
      id: 38700,
      original_language: "en",
      original_title: "Bad Boys for Life",
      overview:
        "Детективы Майк Лоури и Маркус Бёрнетт снова в деле! Правда, их отстраняют от всех операций, но разве отчаянных друзей когда-нибудь что-либо останавливало? Ведь на этот раз на Майка открывает охоту некто из его прошлой жизни. Так что парням придется отжечь по полной!",
      popularity: 95.424,
      poster_path: "/7vyOB2wWvyild4ETMrW7EZOaWZx.jpg",
      release_date: "2020-01-15",
      title: "Плохие парни навсегда",
      video: !1,
      vote_average: 7.2,
      vote_count: 7045,
    },
    {
      adult: !1,
      backdrop_path: "/cFVgXPmF8UMGH67roYd4e3QaAHP.jpg",
      genre_ids: [878, 28, 80, 16],
      id: 655431,
      original_language: "ja",
      original_title: "PSYCHO-PASS サイコパス 3 FIRST INSPECTOR",
      overview:
        "Инспекторы первого отдела Бюро Общественной Безопасности Арата Синдо и Кэй Михаил Игнатов продолжают расследовать деятельность таинственной организации «Биврёст» и всё ближе подбираются к раскрытию тайны загадочной смерти своих родных. Однако члены «Биврёста» не собираются останавливаться и начинают финальный раунд своей безжалостной игры. Теперь под удар попадает само Бюро, а его сотрудникам предстоит пройти через смертельные испытания. Смогут ли они остановить преступников и остаться верными своим принципам, когда на кону жизни их товарищей?",
      popularity: 95.214,
      poster_path: "/m2RzGRMbdMUcp8S3PYtlPpHa83g.jpg",
      release_date: "2020-03-27",
      title: "Психопаспорт 3: Первый инспектор",
      video: !1,
      vote_average: 7.7,
      vote_count: 58,
    },
    {
      adult: !1,
      backdrop_path: "/yY76zq9XSuJ4nWyPDuwkdV7Wt0c.jpg",
      genre_ids: [28, 53, 878],
      id: 577922,
      original_language: "en",
      original_title: "Tenet",
      overview:
        "После теракта в киевском оперном театре агент ЦРУ объединяется с британской разведкой, чтобы противостоять русскому олигарху, который сколотил состояние на торговле оружием. Для этого агенты используют инверсию времени - технологию будущего, позволяющую времени идти вспять.",
      popularity: 86.419,
      poster_path: "/m96dj44zZJ8TxpaMZDJv63TldZh.jpg",
      release_date: "2020-08-22",
      title: "Довод",
      video: !1,
      vote_average: 7.2,
      vote_count: 7229,
    },
    {
      adult: !1,
      backdrop_path: "/obExOU9qDnMcxvWPumoD14oxup5.jpg",
      genre_ids: [53, 27],
      id: 509635,
      original_language: "en",
      original_title: "Alone",
      overview:
        "Собираясь в поездку к родителям, Джессика не могла представить, что по пути станет жертвой похищения безжалостным психопатом. Чудом высвободившись из заточения, девушка оказывается одна в дикой местности без еды и воды... с маньяком на хвосте. Её единственный шанс выжить – бежать, не оглядываясь.",
      popularity: 85.228,
      poster_path: "/yJ6soPrkOaPphelKhwMM9MNZyCP.jpg",
      release_date: "2020-09-10",
      title: "Пропавшая",
      video: !1,
      vote_average: 6.2,
      vote_count: 437,
    },
    {
      adult: !1,
      backdrop_path: "/3ombg55JQiIpoPnXYb2oYdr6DtP.jpg",
      genre_ids: [878, 28],
      id: 560144,
      original_language: "en",
      original_title: "Skylines",
      overview:
        "Через 15 лет после вторжения неизвестный вирус угрожает хрупкому сосуществованию дружественных инопланетных гибридов и людей. Родившаяся на космическом корабле мутант Роуз Корли сама испытывает трудности с управлением огневой рукой, когда на неё выходят члены сопротивления. Они предлагают девушке в составе хорошо подготовленного отряда, куда входит и её брат-инопланетянин Трент, отправиться на вражескую планету и захватить сердечник от разрушенного корабля, который поможет спасти человечество.",
      popularity: 85.03,
      poster_path: "/zMVvSXt3tyeOpQBAvqNQXheCymU.jpg",
      release_date: "2020-10-25",
      title: "Скайлайн 3",
      video: !1,
      vote_average: 5.6,
      vote_count: 426,
    },
    {
      adult: !1,
      backdrop_path: "/jxYROBerNLUExmqFb2yArglwIHd.jpg",
      genre_ids: [27],
      id: 720026,
      original_language: "en",
      original_title: "Butchers",
      overview:
        "Оуэн и Освальд Уотсоны живут уединенно в американской глубинке, они только что похоронили свою мать. Все местные знают, что они маньяки-садисты, но своих братья не трогают. И вот, на лесной дороге недалеко от их дома, ломается машина с молодой парочкой из городских. Выживут не все… а те, кто останутся в живых, позавидуют мертвым.",
      popularity: 84.502,
      poster_path: "/tZz6T84JIa8QZnDqSC12BODoVda.jpg",
      release_date: "2020-10-15",
      title: "Мясники",
      video: !1,
      vote_average: 4.9,
      vote_count: 146,
    },
    {
      adult: !1,
      backdrop_path: "/pO1SnM5a1fEsYrFaVZW78Wb0zRJ.jpg",
      genre_ids: [16, 12, 10751, 35, 10770],
      id: 677638,
      original_language: "en",
      original_title: "We Bare Bears: The Movie",
      overview:
        "Три брата-медведя Гризли, Панда и Белый отчаянно пытаются влиться в общество людей, однако покупая еду, борясь за интернет-славу или заводя друзей, им не всегда удаётся соответствовать людской природе. Но братья всегда готовы прийти друг другу на выручку.",
      popularity: 84.196,
      poster_path: "/36E4T9mJmCyW2hdyq47G0H69D1c.jpg",
      release_date: "2020-06-30",
      title: "Вся правда о медведях: Фильм",
      video: !1,
      vote_average: 7.9,
      vote_count: 721,
    },
    {
      adult: !1,
      backdrop_path: "/y6FYAhaeFMj9Lsk8OdF2NeWQkbd.jpg",
      genre_ids: [28, 14, 10751, 35],
      id: 615677,
      original_language: "en",
      original_title: "We Can Be Heroes",
      overview:
        "Группа детей пытается спасти своих родителей-супергероев, похищенных инопланетянами. Для этого они сбегают из правительственного убежища.",
      popularity: 83.437,
      poster_path: "/kZ3ulUJtNfaoZyVi7mT7B6hJlif.jpg",
      release_date: "2020-12-25",
      title: "Мы можем стать героями",
      video: !1,
      vote_average: 6.2,
      vote_count: 606,
    },
    {
      adult: !1,
      backdrop_path: "/cmZVWuXM1hpP2GKFABCUkoscKu4.jpg",
      genre_ids: [16, 10402],
      id: 738215,
      original_language: "en",
      original_title: "Barbie: Princess Adventure",
      overview:
        "Барби открывает для себя, что значит быть современной принцессой, когда она меняется местами со своим королевским двойником.",
      popularity: 83.245,
      poster_path: "/AwkmMTKJBAatbeAVhTwhcU3Pyp4.jpg",
      release_date: "2020-09-01",
      title: "Барби: Приключение принцессы",
      video: !1,
      vote_average: 7.3,
      vote_count: 185,
    },
    {
      adult: !1,
      backdrop_path: "/tcrNJfyNEIqaBR8Ogkgnq5xQJnf.jpg",
      genre_ids: [10749, 18],
      id: 342470,
      original_language: "en",
      original_title: "All the Bright Places",
      overview:
        "Жизни двух подростков, страдающих от травм прошлого, меняются, когда они находят друг друга.",
      popularity: 81.211,
      poster_path: "/5zD3v6Hb54gOqlKaF86JAC12DRW.jpg",
      release_date: "2020-02-28",
      title: "Все радостные места",
      video: !1,
      vote_average: 7.7,
      vote_count: 2416,
    },
    {
      adult: !1,
      backdrop_path: "/lrNbt21hRirjyTK0SeLA0L4RAVS.jpg",
      genre_ids: [27, 878, 9648, 28],
      id: 340102,
      original_language: "en",
      original_title: "The New Mutants",
      overview:
        "После нашествия на её резервацию загадочного торнадо и гибели отца Дэниель Мунстар приходит в себя связанной на больничной койке. Доктор Рейес сообщает девушке, что она - единственная выжившая, и теперь вместе с четырьмя подростками находится в специализированном учреждении, где их изучают и учат контролировать свои сверхспособности. Вскоре Дэни выясняет, что покинуть огороженную силовым полем территорию невозможно, а ребят начинают преследовать кажущиеся очень реальными кошмары.",
      popularity: 80.796,
      poster_path: "/2lyCa61EoF44vcQPXQfJzNvKT2D.jpg",
      release_date: "2020-08-26",
      title: "Новые мутанты",
      video: !1,
      vote_average: 6.2,
      vote_count: 2701,
    },
    {
      adult: !1,
      backdrop_path: "/2M2JxEv3HSpjnZWjY9NOdGgfUd.jpg",
      genre_ids: [53, 28, 80, 18],
      id: 553604,
      original_language: "en",
      original_title: "Honest Thief",
      overview:
        "Он – профессиональный грабитель банков, который решил отойти от дел. Но стать честным, будучи вором, не так просто: ему придется дать отпор продажным копам, чтобы спасти своё имя и любимую.",
      popularity: 80.463,
      poster_path: "/cjEO0gH8TnMzuWwNYdSDDXQKqbu.jpg",
      release_date: "2020-09-03",
      title: "Честный вор",
      video: !1,
      vote_average: 6.5,
      vote_count: 1292,
    },
    {
      adult: !1,
      backdrop_path: "/7unT3bKHwOXL9e646z9Ib17M9Pf.jpg",
      genre_ids: [27, 53],
      id: 681019,
      original_language: "pl",
      original_title: "W lesie dziś nie zaśnie nikt",
      overview:
        "Группа подростков, зависимых от гаджетов, отправляются в трудовой офлайн-лагерь, где им запрещено пользоваться прелестями мира технологий. Однако, отправившись в поход, они сталкиваются с кошмаром, которого они не видели даже в самых тёмных уголках интернета. Перед лицом смертельной опасности, скрывающейся в лесу, молодые люди поймут, что такое настоящие дружба и любовь.",
      popularity: 79.653,
      poster_path: "/5vFCHFVLPcU4M8uNXM1ba9y83Iu.jpg",
      release_date: "2020-03-13",
      title: "Никто сегодня не спит в лесу",
      video: !1,
      vote_average: 5,
      vote_count: 467,
    },
    {
      adult: !1,
      backdrop_path: "/ovwN1JDPvllWZaH3xtKp7txRWDj.jpg",
      genre_ids: [27],
      id: 715658,
      original_language: "en",
      original_title: "The Scientist",
      overview:
        "Захватывающий психологический триллер об ученом Стиве Унгере, который с трудом ухаживает за своей неизлечимо больной женой Дарлин, которой осталось жить всего несколько недель. Пытаясь оставаться сильным для своей жены и 10-летней дочери Лили, Стив поспешно начинает исследование, чтобы вылечить Дарлин и сохранить семью. Игнорирование всех юридических и моральных границ в поисках лекарства, а также его методы лечения и вмешательство в ДНК человека могут привести к исчезновению человечества.",
      popularity: 78.524,
      poster_path: "/hIkKM1nlzk8DThFT4vxgW1KoofL.jpg",
      release_date: "2020-09-21",
      title: "Учёный",
      video: !1,
      vote_average: 6.4,
      vote_count: 95,
    },
    {
      adult: !1,
      backdrop_path: "/lkeBhXGJFRlhI7cBWn8LQQAdZqK.jpg",
      genre_ids: [10751, 16, 878, 35, 10770],
      id: 594328,
      original_language: "en",
      original_title:
        "Phineas and Ferb: The Movie: Candace Against the Universe",
      overview:
        "Пускай Кэндис из «Финеса и Ферба» и хочет только лучшего для своей родни, кажется, будто весь мир настроен против неё. А тут ещё и инопланетяне похитили… полоса невезения не заканчивается. Но семья так просто не бросает своих! Финес, Ферб и их друзья отправляются в опасное космическое путешествие, чтобы спасти старшую сестрёнку и победить всех инопланетян.",
      popularity: 75.929,
      poster_path: "/s7b7PZA9SBImW0QSL0psu8uXq3Q.jpg",
      release_date: "2020-08-28",
      title: "Финес и Ферб: Кэндис против Вселенной",
      video: !1,
      vote_average: 7.4,
      vote_count: 300,
    },
    {
      adult: !1,
      backdrop_path: "/d9COll7H2LaS1GK4t6OnkpkycKf.jpg",
      genre_ids: [18, 10749],
      id: 610781,
      original_language: "en",
      original_title: "See You Soon",
      overview:
        "В преддверии Чемпионата мира по футболу-2018 американский игрок-суперзвезда получает серьезную травму, которая ставит под угрозу его дальнейшую карьеру. В поисках себя он отправляется в путешествие и встречает свою любовь. Невероятное чувство к девушке из Санкт-Петербурга меняет не только его жизнь, но и его самого.",
      popularity: 288.39,
      poster_path: "/agL8HPt0LFbd2mxd8s3FQAEhekG.jpg",
      release_date: "2019-07-25",
      title: "До скорой встречи",
      video: !1,
      vote_average: 6.8,
      vote_count: 18,
    },
    {
      adult: !1,
      backdrop_path: "/7RyHsO4yDXtBv1zUU3mTpHeQ0d5.jpg",
      genre_ids: [12, 878, 28],
      id: 299534,
      original_language: "en",
      original_title: "Avengers: Endgame",
      overview:
        'После разрушительных событий в "Мстители: Войны бесконечности", вселенная лежит в руинах. Оставшиеся в живых участники команды Мстителей снова объединяются в попытке восстановить баланс во вселенной.',
      popularity: 236.623,
      poster_path: "/zpUZ5qT89ByIPn21px8uBU7Lgsg.jpg",
      release_date: "2019-04-24",
      title: "Мстители: Финал",
      video: !1,
      vote_average: 8.3,
      vote_count: 21218,
    },
    {
      adult: !1,
      backdrop_path: "/997ToEZvF2Obp9zNZbY5ELVnmrW.jpg",
      genre_ids: [10749, 18],
      id: 537915,
      original_language: "en",
      original_title: "After",
      overview:
        "Случайная встреча перевернула их привычный мир. Она — прилежная студентка и образцовая дочь, а он — дьявольски притягательный бунтарь из богатой семьи. Живя в параллельных вселенных, они бы вряд ли даже взглянули друг на друга. Однако этому знакомству суждено разделить жизнь влюбленных на до и после…",
      popularity: 227.338,
      poster_path: "/7Prt3Le9H0EeRYVltAXFJqwDIig.jpg",
      release_date: "2019-04-11",
      title: "После",
      video: !1,
      vote_average: 7.2,
      vote_count: 7105,
    },
    {
      adult: !1,
      backdrop_path: "/9guoVF7zayiiUq5ulKQpt375VIy.jpg",
      genre_ids: [16, 28, 14, 12],
      id: 592350,
      original_language: "ja",
      original_title: "僕のヒーローアカデミア THE MOVIE ヒーローズ：ライジング",
      overview:
        "На очередном задании защитники преследовали грузовик с опасным преступником. Операция завершилась успешно, но в машине ребята не обнаружили ничего подозрительного. Вскоре голове отряда защитников удалось узнать о том, что существует наркотик, который может не только пробудить сверхсилу, но и отнять её у героев. Для лучших супергероев появилась нелёгкая задачка, но Мидория не собирается стоять в стороне. Волею судьбы смышлёный парень познакомился с девчонкой, которой известна секретная информация о новом наркотике. Изуку решает не только помочь новоиспечённой подруге с её проблемами, но и узнать, кто отвечает за производство загадочного наркотика. Что же ждёт напористого паренька без могущественных способностей впереди?",
      popularity: 226.425,
      poster_path: "/bbAlQRWEPOsk58TrfCCCYxAAZTX.jpg",
      release_date: "2019-12-20",
      title: "Моя геройская академия: Восхождение героев",
      video: !1,
      vote_average: 8.3,
      vote_count: 928,
    },
    {
      adult: !1,
      backdrop_path: "/8moTOzunF7p40oR5XhlDvJckOSW.jpg",
      genre_ids: [27, 14],
      id: 474350,
      original_language: "en",
      original_title: "It Chapter Two",
      overview:
        "Проходит 27 лет после первой встречи ребят с демоническим Пеннивайзом. Они уже выросли, и у каждого своя жизнь. Но неожиданно их спокойное существование нарушает странный телефонный звонок, который заставляет их вновь собраться вместе.",
      popularity: 199.974,
      poster_path: "/rjM13nwgzt80xAUKXVp9mzRYyHp.jpg",
      release_date: "2019-09-04",
      title: "Оно 2",
      video: !1,
      vote_average: 6.9,
      vote_count: 6977,
    },
    {
      adult: !1,
      backdrop_path: "/q62bpQ67qaXY0u6b2wFEnQYIbPd.jpg",
      genre_ids: [10751, 12, 16, 35, 14, 18],
      id: 301528,
      original_language: "en",
      original_title: "Toy Story 4",
      overview:
        "Космический рейнджер Баз Лайтер, ковбой Вуди, собака Спиралька, тиранозавр Рекс и вся команда игрушек снова вместе, и они как никогда готовы к приключениям. С тех пор как Энди поступил в колледж, игрушки поселились в доме своей новой хозяйки Бонни. В свой первый день в школе Бонни смастерила себе из отходов новую игрушку Вилкинса, что положит начало череде событий, в которой найдётся место и путешествиям с погонями, и знакомству с новыми игрушками, и встречам с некоторыми из старых друзей, и даже романтической истории.",
      popularity: 198.604,
      poster_path: "/lQBF6jzW2gsk3Kr7dMdqrkGaukV.jpg",
      release_date: "2019-06-19",
      title: "История игрушек 4",
      video: !1,
      vote_average: 7.6,
      vote_count: 8075,
    },
    {
      adult: !1,
      backdrop_path: "/6ufHj6uqyje1ogAlCvRB54hcGgn.jpg",
      genre_ids: [16, 12, 14, 28, 10751],
      id: 571891,
      original_language: "ja",
      original_title: "劇場版 ポケットモンスター ミュウツーの逆襲 EVOLUTION",
      overview:
        "Гениальный учёный работает над созданием живого оружия, используя для этого ресничку легендарного Мью. Получившееся создание он называет Мьюту. Узнав, что его планируют сделать оружием, Мьюту злится и вырывается из лаборатории. А позже начинает планировать свою месть человечеству.",
      popularity: 176.413,
      poster_path: "/xaSoSy7v6WTGpRd15TjBwTekqB2.jpg",
      release_date: "2019-07-12",
      title: "Покемон: Мьюту наносит ответный удар – Эволюция",
      video: !1,
      vote_average: 6.7,
      vote_count: 382,
    },
    {
      adult: !1,
      backdrop_path: "/eGa0w1EbNnb2wF696NVbF65IR4Y.jpg",
      genre_ids: [99],
      id: 638566,
      original_language: "ja",
      original_title: "Naruto to Boruto: The Live 2019",
      overview: "",
      popularity: 160.35,
      poster_path: "/8OkxtHotJnegiBbWQSz8fD60NEc.jpg",
      release_date: "2019-05-05",
      title: "Naruto to Boruto: The Live 2019",
      video: !1,
      vote_average: 6.9,
      vote_count: 22,
    },
    {
      adult: !1,
      backdrop_path: "/m5HPKCi7GdhKmxPTcOQmcLfEmZ9.jpg",
      genre_ids: [16, 12, 35, 14],
      id: 532067,
      original_language: "ja",
      original_title: "映画 この素晴らしい世界に祝福を！紅伝説",
      overview:
        "Приключения четырёх неудачливых героев продолжаются! На этот раз родная деревня Мэгумин и Юн-юн находится под угрозой уничтожения Владыкой демонов, который направляет туда одного из своих генералов. В отчаянии Юн-юн приходит к Казуме и просит его о помощи. Команда приключенцев направляется в деревню, чтобы предотвратить её уничтожение.",
      popularity: 160.145,
      poster_path: "/lnlEWSvfiHbQOgVyfnEqU1q31Yg.jpg",
      release_date: "2019-08-30",
      title: "Этот замечательный мир! Багровая легенда",
      video: !1,
      vote_average: 8.3,
      vote_count: 394,
    },
    {
      adult: !1,
      backdrop_path: "/8NEvnVKeGMLEXG45MMNqxJLcM4y.jpg",
      genre_ids: [28, 16, 14],
      id: 820232,
      original_language: "ja",
      original_title: "鬼滅の刃 兄妹の絆",
      overview:
        "Эпоха Тайсё. Ещё с древних времён ходят слухи, что в лесу обитают человекоподобные демоны, которые питаются людьми и выискивают по ночам новых жертв. Тандзиро Камадо — старший сын в семье, потерявший отца и взявший на себя заботу о родных. Однажды он уходит в соседний город, чтобы продать древесный уголь. Вернувшись утром, парень обнаруживает перед собой страшную картину: вся родня зверски убита, а единственная выжившая — младшая сестра Нэдзуко, обращённая в демона, но пока не потерявшая человечность. С этого момента начинается долгое и опасное путешествие Тандзиро и Нэдзуко, в котором мальчик намерен разыскать убийцу и узнать способ исцеления сестры.",
      popularity: 156.044,
      poster_path: "/99cJLZkNoIFmufJ4yap6ZABl4nk.jpg",
      release_date: "2019-03-29",
      title: "Клинок, рассекающий демонов: Узы брата и сестры",
      video: !1,
      vote_average: 8.7,
      vote_count: 35,
    },
    {
      adult: !1,
      backdrop_path: "/ng6SSB3JhbcpKTwbPDsRwUYK8Cq.jpg",
      genre_ids: [28, 12, 878],
      id: 429617,
      original_language: "en",
      original_title: "Spider-Man: Far From Home",
      overview:
        "Питер Паркер вместе с одноклассниками отправляется на летние каникулы в Европу. Однако отдохнуть приятелям вряд ли удастся — по прибытии в Венецию группа попадает в эпицентр сражения. Некое существо, состоящее из воды, появляется из ниоткуда и начинает крушить всё на своём пути, и теперь Питеру придётся спасать европейские архитектурные памятники вместе с загадочным супергероем Мистерио.",
      popularity: 154.983,
      poster_path: "/5NNAHfyG4Ttu6ED9PpVe9mrRthj.jpg",
      release_date: "2019-06-28",
      title: "Человек-паук: Вдали от дома",
      video: !1,
      vote_average: 7.5,
      vote_count: 12722,
    },
    {
      adult: !1,
      backdrop_path: null,
      genre_ids: [],
      id: 982790,
      original_language: "en",
      original_title: "The Boys - Behind the Scenes: Insider Look",
      overview: "",
      popularity: 151.223,
      poster_path: "/9LqTPqGKCAgD2Fn0EOJ456BY0rM.jpg",
      release_date: "2019-07-26",
      title: "The Boys - Behind the Scenes: Insider Look",
      video: !1,
      vote_average: 6.5,
      vote_count: 4,
    },
    {
      adult: !1,
      backdrop_path: "/qwgGtdlLMKYvT2tV8hLY22syRHN.jpg",
      genre_ids: [28, 35, 14],
      id: 287947,
      original_language: "en",
      original_title: "Shazam!",
      overview:
        "В каждом из нас живет супергерой, нужна лишь толика магии, чтобы он проявил себя. Билли Бэтсон должен прокричать всего лишь одно слово - ШАЗАМ - чтобы из сообразительного четырнадцатилетнего мальчишки превратиться во взрослого супергероя Шазама.",
      popularity: 146.396,
      poster_path: "/s4byZ8CP7NLfH54FV3QF6l0JLSh.jpg",
      release_date: "2019-03-29",
      title: "Шазам!",
      video: !1,
      vote_average: 7.1,
      vote_count: 7659,
    },
    {
      adult: !1,
      backdrop_path: "/AoSZyb37ljMAxw0RdeQEBHKtgcc.jpg",
      genre_ids: [10751, 16, 12, 35, 14],
      id: 330457,
      original_language: "en",
      original_title: "Frozen II",
      overview:
        "Анна, Эльза, Кристоф, его верный олень Свен и никогда не унывающий снеговик Олаф должны будут покинуть уютное королевство Эренделл и отправиться ещё дальше на север, в путешествие, которое приведет их к истокам древних легенд и поможет раскрыть тайны, касающиеся прошлого их родной страны.",
      popularity: 139.846,
      poster_path: "/p4XurAYUUxsJgHEnGsPSqNMskYr.jpg",
      release_date: "2019-11-20",
      title: "Холодное сердце 2",
      video: !1,
      vote_average: 7.3,
      vote_count: 8247,
    },
    {
      adult: !1,
      backdrop_path: "/1TUg5pO1VZ4B0Q1amk3OlXvlpXV.jpg",
      genre_ids: [12, 10751, 16, 18],
      id: 420818,
      original_language: "en",
      original_title: "The Lion King",
      overview:
        "История об отважном львенке по имени Симба. Знакомые с детства герои взрослеют, влюбляются, познают себя и окружающий мир, совершают ошибки и делают правильный выбор.",
      popularity: 132.56,
      poster_path: "/9NAGr8Z2OVURC4kjq31TIuNar4L.jpg",
      release_date: "2019-07-12",
      title: "Король Лев",
      video: !1,
      vote_average: 7.1,
      vote_count: 8479,
    },
    {
      adult: !1,
      backdrop_path: "/n6bUvigpRFqSwmPp1m2YADdbRBc.jpg",
      genre_ids: [80, 53, 18],
      id: 475557,
      original_language: "en",
      original_title: "Joker",
      overview:
        "Готэм, начало 1980-х годов. Комик Артур Флек живёт с больной матерью, которая с детства учит его «ходить с улыбкой». Пытаясь нести в мир хорошее и дарить людям радость, Артур сталкивается с человеческой жестокостью и постепенно приходит к выводу, что этот мир получит от него не добрую улыбку, а ухмылку злодея Джокера.",
      popularity: 132.296,
      poster_path: "/249zPw9NLgT73SCcDKfjKPCwfW5.jpg",
      release_date: "2019-10-02",
      title: "Джокер",
      video: !1,
      vote_average: 8.2,
      vote_count: 20739,
    },
    {
      adult: !1,
      backdrop_path: "/vVpEOvdxVBP2aV166j5Xlvb5Cdc.jpg",
      genre_ids: [28, 53, 80],
      id: 458156,
      original_language: "en",
      original_title: "John Wick: Chapter 3 - Parabellum",
      overview:
        "Джон Уик находится в бегах. За его голову объявили награду в 14 млн. долларов, а виной всему нарушение правила — никаких убийств на территории отеля «Continental». Администратор отеля Уинстон дал всего лишь час Джону, чтобы скрыться, после чего на него начнется самая настоящая охота. Джон будет вынужден сражаться и убивать, чтобы добраться до Нью-Йорка, где он сможет найти выход из сложившейся ситуации.",
      popularity: 128.153,
      poster_path: "/dIHfQmFvthJ5bvzYDi0GTbFbUjW.jpg",
      release_date: "2019-05-15",
      title: "Джон Уик 3",
      video: !1,
      vote_average: 7.4,
      vote_count: 7973,
    },
    {
      adult: !1,
      backdrop_path: "/eqVgQwv8HfDN2tv4XWhqW5GryV4.jpg",
      genre_ids: [27, 35],
      id: 607383,
      original_language: "en",
      original_title: "Aquaslash",
      overview:
        "Выпускники 2018 года собираются оторваться в аквапарке. Кто-то в большой шумной компании употребялет всё, до чего дотягивается, кто-то нарывается на неприятности, у кого-то намечаются романтические отношения, у кого-то расстраиваются, а коварный злоумышленник уже начал кровавую охоту на ничего неподозревающую молодёжь.",
      popularity: 127.311,
      poster_path: "/pmTWYNRerYhBpwENaEHknQE6lrt.jpg",
      release_date: "2019-07-29",
      title: "Пункт назначения. Аквапарк",
      video: !1,
      vote_average: 4.1,
      vote_count: 112,
    },
    {
      adult: !1,
      backdrop_path: "/wRDyTXw5j0RN9M03xZlHYkoNLCI.jpg",
      genre_ids: [16, 18, 14, 10749],
      id: 568160,
      original_language: "ja",
      original_title: "天気の子",
      overview:
        "Старшеклассник Ходака Морисима переезжает со своего родного острова в Токио, где у парня сразу кончаются деньги. Когда он устраивается на подработку в журнал, пишущий о сверхъестественном, погода портится, и начинаются бесконечные дожди. Однажды Ходака знакомится с девушкой по имени Хина Амано, которая обладает удивительной способностью разгонять тучи и останавливать дождь.",
      popularity: 121.54,
      poster_path: "/koEIzx6yaqrLkmmScO68s5oDAyc.jpg",
      release_date: "2019-06-19",
      title: "Дитя погоды",
      video: !1,
      vote_average: 8.1,
      vote_count: 1485,
    },
    {
      adult: !1,
      backdrop_path: "/2hhZZCbLllEwcED7S9yLIA6j6aZ.jpg",
      genre_ids: [35, 14, 10749],
      id: 526050,
      original_language: "en",
      original_title: "Little",
      overview:
        "Властная руководительница крупной компании однажды просыпается обычным подростком. И теперь ей надо заново научиться управлять коллективом и провести крупную сделку, от корой зависит дальнейшая жизнь компании. Но это очень тяжело сделать, ведь тебе сейчас всего четырнадцать...",
      popularity: 117.539,
      poster_path: "/3MZo6I4K1nKn9iTqDs7N57PCJez.jpg",
      release_date: "2019-04-04",
      title: "Мелкая",
      video: !1,
      vote_average: 6.9,
      vote_count: 567,
    },
    {
      adult: !1,
      backdrop_path: "/88J6waYVTta8Qz3iX3qUeWNA5d5.jpg",
      genre_ids: [18, 10749],
      id: 447362,
      original_language: "en",
      original_title: "Life in a Year",
      overview:
        "История 17-летнего Дэрина, который узнаёт, что его подруга смертельно больна. Он решает подарить девушке целую жизнь, уместив её в год, который ей остался.",
      popularity: 74.961,
      poster_path: "/q6ZdZVrYerWjOa1DASzCmcN9Lyg.jpg",
      release_date: "2020-11-27",
      title: "Жизнь за год",
      video: !1,
      vote_average: 8.3,
      vote_count: 1031,
    },
    {
      adult: !1,
      backdrop_path: "/oSSEcPDfwgZSv2i01Oqxdb9t8fI.jpg",
      genre_ids: [28, 12, 35, 14],
      id: 621870,
      original_language: "en",
      original_title: "Secret Society of Second Born Royals",
      overview:
        "Сэм - очаровательная молодая девушка, которая является принцессой. В будущем она может занять трон, поэтому с раннего детства изучает множество наук, способных помочь ей в правлении. У нее никогда не было времени на обычное веселье и беззаботную жизнь. Однако она не отчаивалась и верила в то, что полученные знания помогут ей стать справедливым правителем, способным улучшить благополучие своего народа. Но однажды Сэм обнаруживает в себе уникальную способность. С этого момента она становится ученицей засекреченной академии. В этом месте обучают наследников королевских семей, которые узнали о новых силах, которые им предстоит научиться контролировать...",
      popularity: 73.305,
      poster_path: "/xOmGTJtBgRVSAF4S5dZEUqHqyy5.jpg",
      release_date: "2020-09-25",
      title: "Тайное общество младших монарших особ",
      video: !1,
      vote_average: 6.8,
      vote_count: 434,
    },
    {
      adult: !1,
      backdrop_path: "/21Q8bzu10YF9i4O5amBkJBombYo.jpg",
      genre_ids: [10402, 18, 10749],
      id: 585244,
      original_language: "en",
      original_title: "I Still Believe",
      overview:
        "Подчас на пути к профессиональному успеху и личному счастью реальная жизнь предлагает преодолеть трагические обстоятельства, оставляя лишь крохи надежды и веру в любовь.",
      popularity: 72.832,
      poster_path: "/hDq0YRF4Aq92mJ9oinCBTJq3duU.jpg",
      release_date: "2020-03-12",
      title: "Верю в любовь",
      video: !1,
      vote_average: 7.6,
      vote_count: 978,
    },
    {
      adult: !1,
      backdrop_path: "/9R2FuGT0BeZl8usfsfqIcO7KK7v.jpg",
      genre_ids: [28, 53, 27],
      id: 514847,
      original_language: "en",
      original_title: "The Hunt",
      overview:
        "Двенадцать незнакомых между собой людей оказываются в неизвестном месте, и не понимают как туда попали. Они ещё не знают, что на них открыта охота. А они - дичь...",
      popularity: 72.782,
      poster_path: "/iX6tk4nQCPxk3eIs2ihtDOdAvsw.jpg",
      release_date: "2020-03-11",
      title: "Охота",
      video: !1,
      vote_average: 6.7,
      vote_count: 2349,
    },
    {
      adult: !1,
      backdrop_path: "/a9zFUuxzChmAlIybVge9IZt1hH0.jpg",
      genre_ids: [35],
      id: 578908,
      original_language: "en",
      original_title: "Bad Trip",
      overview:
        "В этой комедии, снятой скрытой камерой, лучших друзей ждет безумная поездка в Нью-Йорк. По дороге герои придумывают непристойные выходки и втягивают в них обычных людей.",
      popularity: 71.815,
      poster_path: "/ryVuohKC6Fwz1ror7lPlFYNOAke.jpg",
      release_date: "2020-09-26",
      title: "Приколисты в дороге",
      video: !1,
      vote_average: 6.1,
      vote_count: 373,
    },
    {
      adult: !1,
      backdrop_path: "/rc8WAwoTTZ5HDUDBiKhIohbl57A.jpg",
      genre_ids: [27],
      id: 575088,
      original_language: "ru",
      original_title: "Яга. Кошмар тёмного леса",
      overview:
        "Молодая семья переехала в новую квартиру на окраине города. Нанятая ими для новорожденной дочери няня быстро вошла в доверие. Однако старший сын Егор рассказывает о пугающем поведении женщины, но родители ему не верят. Камеры наблюдения, установленные отцом для успокоения, лишь подтверждают — всё в порядке. И вот однажды, вернувшись домой, Егор не обнаруживает ни няни, ни маленькой сестры, а родители пребывают в странном трансе и даже не помнят, что у них была дочь. Тогда мальчик вместе с друзьями отправляется на поиски, в ходе которых выяснится, что няня — древний славянский демон.",
      popularity: 69.026,
      poster_path: "/72r3FJH0Lv7M52lRcIhDy5wwkin.jpg",
      release_date: "2020-02-27",
      title: "Яга. Кошмар тёмного леса",
      video: !1,
      vote_average: 6,
      vote_count: 182,
    },
    {
      adult: !1,
      backdrop_path: "/kU7ZiyeUwcpTkYj3UcxSqGdZmxY.jpg",
      genre_ids: [16, 28],
      id: 618353,
      original_language: "en",
      original_title: "Batman: Death in the Family",
      overview:
        "«Смерть в семье» заново расскажет знаменитый сюжет, где Джокер безжалостно убивает Джейсона Тодда — второго Робина, который спустя годы воскрес и стал антигероем по прозвищу Красный Колпак. В определенные моменты зрителям позволят самим решать, в какую сторону будет двигаться история, так что протеже Темного рыцаря теперь не погибнет, а судьбы троих сложатся совершенно иначе.",
      popularity: 68.913,
      poster_path: "/k8Q9ulyRE8fkvZMkAM9LPYMKctb.jpg",
      release_date: "2020-10-13",
      title: "Бэтмен: Смерть в семье",
      video: !1,
      vote_average: 7.2,
      vote_count: 323,
    },
    {
      adult: !1,
      backdrop_path: "/isBjNHBblzxrzHrAtRbjgkYzAut.jpg",
      genre_ids: [10749, 18],
      id: 664413,
      original_language: "pl",
      original_title: "365 dni",
      overview:
        "Массимо Торичелли — молодой, красивый босс сицилийской мафиозной семьи. После убийства отца конкурентами он вынужден взять на себя его обязанности. Лаура Бель — директор по продажам в роскошном отеле. Она успешна в профессии, но в личной жизни ей не хватает страсти. Лаура делает последнюю попытку сохранить отношения. Вместе с партнёром и друзьями она летит на Сицилию, где у неё на пути встаёт Массимо — самый опасный мужчина на острове, который похитит её и даст ей 365 дней, чтобы она его полюбила.",
      popularity: 68.812,
      poster_path: "/ibaDBJBAvWd4J5lzfbpq1PJ6ZU1.jpg",
      release_date: "2020-02-07",
      title: "365 дней",
      video: !1,
      vote_average: 7,
      vote_count: 7034,
    },
    {
      adult: !1,
      backdrop_path: "/lIE7kfdLBRd0KENNtOaIqPPWNqh.jpg",
      genre_ids: [27, 18, 14],
      id: 590995,
      original_language: "en",
      original_title: "The Craft: Legacy",
      overview:
        "Новенькую старшеклассницу принимает в свою компанию группа из трёх девчонок и вскоре посвящает её в ведьмы.",
      popularity: 68.145,
      poster_path: "/57HLFLrvtlZLbo6q6nG53jk0bUy.jpg",
      release_date: "2020-10-28",
      title: "Колдовство: Новый ритуал",
      video: !1,
      vote_average: 6.3,
      vote_count: 565,
    },
    {
      adult: !1,
      backdrop_path: "/eHTZoXmB4vnDqANZXPZcdAiYQo5.jpg",
      genre_ids: [16, 18, 10749, 14],
      id: 667520,
      original_language: "ja",
      original_title: "泣きたい私は猫をかぶる",
      overview:
        "На фестивале фейерверков обычная японская старшеклассница Миё, распереживавшись из-за развода родителей и несправедливости мира, только в сердцах пожелала этому миру исчезнуть, как тут же встретила загадочного продавца. Незнакомец подарил девушке волшебную маску, с помощью которой Миё теперь может превращаться в белую кошечку. В таком виде она проводит время с одноклассником Кэнто, ведь он, несмотря на все её знаки внимания в школе, упорно игнорирует девушку. Но тот странный тип с фестиваля преследует Миё и пытается убедить её остаться в кошачьем обличье навсегда.",
      popularity: 67.954,
      poster_path: "/iNeeVDibeeJaBYT2NtTuKk8pUrI.jpg",
      release_date: "2020-06-18",
      title: "Сквозь слёзы я притворяюсь кошкой",
      video: !1,
      vote_average: 8,
      vote_count: 1007,
    },
    {
      adult: !1,
      backdrop_path: "/qsxhnirlp7y4Ae9bd11oYJSX59j.jpg",
      genre_ids: [10751, 16, 35, 14, 12, 10402],
      id: 446893,
      original_language: "en",
      original_title: "Trolls World Tour",
      overview:
        "Герои узнают, что за границами их поселения есть земли, на которых живут другие тролли с разными музыкальными предпочтениями. Внезапно объявляется королева Рокс, замышляющая уничтожить всё и устроить правление хард-рока. Друзья решают объединиться и отправиться в путешествие, чтобы спасти свою деревню от новой опасности.",
      popularity: 67.422,
      poster_path: "/8IhMh1qUKL1HerdnBvhqVYCF6dt.jpg",
      release_date: "2020-03-11",
      title: "Тролли. Мировой тур",
      video: !1,
      vote_average: 7.4,
      vote_count: 1761,
    },
    {
      adult: !1,
      backdrop_path: "/edXuLjxjYu1SkyMHd55hRasME8b.jpg",
      genre_ids: [27],
      id: 636879,
      original_language: "en",
      original_title: "A Nun's Curse",
      overview:
        "Во время поездки на выходные группа друзей вынуждена искать убежище в заброшенной тюрьме, где полвека назад работала монахиня по имени Сестра Мондэй. Ее подозревали в убийстве заключенных, но до того, как ее смогли допросить, монахиня исчезла. Во время исследования тюрьмы, герои узнают пугающую правду.",
      popularity: 67.164,
      poster_path: "/eutNRwSguV0NCx7BNAXOibWzjP6.jpg",
      release_date: "2020-05-12",
      title: "Проклятье монахини",
      video: !1,
      vote_average: 6.3,
      vote_count: 84,
    },
    {
      adult: !1,
      backdrop_path: "/yEBBtk1eyZltGgJt8Z2zi3KIvvX.jpg",
      genre_ids: [99],
      id: 689249,
      original_language: "es",
      original_title: "La Casa de Papel: el fenómeno",
      overview:
        "В 2017 году на испанском телеканале Antena 3 выходит в эфир многосерийная криминальная драма «Бумажный дом» про команду грабителей, запланировавших налет на Королевский монетный двор Испании. Два года спустя права на международную трансляцию выкупает американский стриминговый сервис Netflix, который превращает сериал в популярный мировой феномен, имеющий многомиллионную армию поклонников. Создатель шоу Алекс Пина, главный режиссер Хесус Кольменар, а также актеры Эрсула Корберо, Альваро Морте, Педро Алонсо, Альба Флорес, Мигель Херран, Хайме Лоренте, Ициар Итуньо и Эстер Асебо рассказывают о невероятной истории успеха «Бумажного дома».",
      popularity: 66.233,
      poster_path: "/xqwgc8TJnaMwssoN1XbXIPVOrWB.jpg",
      release_date: "2020-04-03",
      title: "Феномен «Бумажного дома»",
      video: !1,
      vote_average: 7.5,
      vote_count: 992,
    },
    {
      adult: !1,
      backdrop_path: "/d2UxKyaJ5GgzuHaSsWinFfv3g6L.jpg",
      genre_ids: [28, 27, 53],
      id: 581392,
      original_language: "ko",
      original_title: "반도",
      overview:
        "Через четыре года после вспышки страшного вируса, распространение которого Южная Корея сдержать не смогла, то, что осталось от страны, находится в блокаде, а полчища зомби разгуливают по всему полуострову. Бывшему военному предстоит путешествие по жуткому миру, где помимо означенной угрозы опасность представляют и люди, устраивающие жестокие игры на выживание.",
      popularity: 66.193,
      poster_path: "/bqmg1QyhPOsE3VCeFyfGIkKl1sJ.jpg",
      release_date: "2020-07-15",
      title: "Поезд в Пусан 2: Полуостров",
      video: !1,
      vote_average: 6.9,
      vote_count: 1900,
    },
    {
      adult: !1,
      backdrop_path: "/MocpW8Wrp8uireBg6bT6Y6TKWC.jpg",
      genre_ids: [80, 18, 53, 28],
      id: 606523,
      original_language: "ko",
      original_title: "낙원의 밤",
      overview:
        "Преступник переживает страшную трагедию и скрывается от врагов на острове Чеджу. Там он знакомится с женщиной, которая борется со своими собственными демонами.",
      popularity: 65.835,
      poster_path: "/mWZ2aujwBdFEONvRQv05zPBGy7J.jpg",
      release_date: "2020-09-03",
      title: "Ночь в раю",
      video: !1,
      vote_average: 7,
      vote_count: 180,
    },
    {
      adult: !1,
      backdrop_path: "/8K9w7juIRSEFM49ASyU2ROf30nP.jpg",
      genre_ids: [],
      id: 685833,
      original_language: "ko",
      original_title: "목공소에서 생긴 일",
      overview: "",
      popularity: 65.343,
      poster_path: "/bMhxD4DKRJM1dydluvsS5aL2Tci.jpg",
      release_date: "2020-03-05",
      title: "목공소에서 생긴 일",
      video: !1,
      vote_average: 2,
      vote_count: 1,
    },
    {
      adult: !1,
      backdrop_path: "/xYURC8jsY5F1EdTzzpurvwnhIis.jpg",
      genre_ids: [18, 10749],
      id: 729097,
      original_language: "ko",
      original_title: "젊은엄마 5",
      overview: "",
      popularity: 63.872,
      poster_path: "/ggvBofagO3rEs7bki3EqrNhBzNv.jpg",
      release_date: "2020-04-03",
      title: "젊은엄마 5",
      video: !1,
      vote_average: 6,
      vote_count: 3,
    },
    {
      adult: !1,
      backdrop_path: "/75ooojtgiKYm5LcCczbCexioZze.jpg",
      genre_ids: [35],
      id: 660982,
      original_language: "en",
      original_title: "American Pie Presents: Girls' Rules",
      overview:
        "Выпускницы Энни, Кайла, Мишель и Стефани объединяются, чтобы заполучить желаемое и как следует оторваться напоследок.",
      popularity: 62.843,
      poster_path: "/ocJ9zCkqNS9jT96O231ExK8TCSp.jpg",
      release_date: "2020-10-06",
      title: "Американский пирог представляет: Девчонки рулят",
      video: !1,
      vote_average: 6.3,
      vote_count: 455,
    },
    {
      adult: !1,
      backdrop_path: "/uztSaahK3bNVE5iZh4mRjHcCeii.jpg",
      genre_ids: [10751, 14, 35],
      id: 662546,
      original_language: "en",
      original_title: "Godmothered",
      overview:
        "Чтобы вернуть крёстным феям былую популярность, молодая и неопытная волшебница решает помочь маленькой девочке, чья просьба когда-то была проигнорирована другими феями. Вот только это было очень давно, и та девочка уже успела стать взрослой женщиной, не верящей ни в какие сказки",
      popularity: 61.082,
      poster_path: "/80tDCErk6ymHS7YfvqJcbnnTtqa.jpg",
      release_date: "2020-12-04",
      title: "Фея-крёстная",
      video: !1,
      vote_average: 6.9,
      vote_count: 490,
    },
    {
      adult: !1,
      backdrop_path: "/t22fWbzdnThPseipsdpwgdPOPCR.jpg",
      genre_ids: [35, 28],
      id: 726739,
      original_language: "en",
      original_title: "Cats & Dogs 3: Paws Unite",
      overview:
        "Кошка Гвен и Собака Роджер - секретные агенты, которые тайно защищают и спасают мир, и люди никогда не узнают об этом. Их партнерство связано с Великим перемирием, которое на десятилетие положило конец вражде между собаками и кошками. Но давний мир находится под угрозой, когда попугай-суперзлодей обнаруживает способ управления беспроводными частотами, который могут слышать только собаки и кошки. Смогут ли герои остановить грязную птицу, или он вызовет ссору между видами?",
      popularity: 60.736,
      poster_path: "/csZIYcwoxhRhLbw9NMa0YMX9d6s.jpg",
      release_date: "2020-10-02",
      title: "Кошки против собак 3: Лапы, объединяйтесь",
      video: !1,
      vote_average: 6.5,
      vote_count: 163,
    },
    {
      adult: !1,
      backdrop_path: "/rbONN3UloJa4syJn6pHlDqZsQqx.jpg",
      genre_ids: [35, 10751],
      id: 668742,
      original_language: "es",
      original_title: "Padre no hay más que uno 2: la llegada de la suegra",
      overview: "",
      popularity: 60.494,
      poster_path: "/7MjODwcPV5tAEa1zlje9N4gReGS.jpg",
      release_date: "2020-07-29",
      title: "Padre no hay más que uno 2: la llegada de la suegra",
      video: !1,
      vote_average: 7,
      vote_count: 128,
    },
    {
      adult: !1,
      backdrop_path: "/vam9VHLZl8tqNwkp1zjEAxIOrkk.jpg",
      genre_ids: [14, 10751, 10770],
      id: 671583,
      original_language: "en",
      original_title: "Upside-Down Magic",
      overview:
        "13-летняя Нори, которая умеет превращаться в животных, и её лучшая подруга Рейна, управляющая силой огня, поступают в Академию магии, чтобы развить свои волшебные способности. На вступительном экзамене Рейна показывает блестящий результат и зачисляется в огненный класс. А вот Нори, превратившаяся вместо котёнка  то ли в котекона, то ли в дратёнка - помесь кота и дракона, распределяют в класс шмагов - изгоев, дар которых нестабилен и непредсказуем. Считается, что такие ученики уязвимы для опасной и злой магии теней, и их способности подлежать подавлению. Однако Нори и её одноклассники стремятся доказать, что и они достойны быть волшебниками.",
      popularity: 60.487,
      poster_path: "/bbJXeOTpokY4LWRDq4Vh5wdpa2m.jpg",
      release_date: "2020-07-31",
      title: "Совсем другая магия",
      video: !1,
      vote_average: 7.3,
      vote_count: 197,
    },
    {
      adult: !1,
      backdrop_path: "/hIHtyIYgBqHybOgUdoAmveipuiO.jpg",
      genre_ids: [28, 27, 878],
      id: 703745,
      original_language: "en",
      original_title: "Deep Blue Sea 3",
      overview:
        "Большая любительница и исследовательница акул доктор Эмма Коллинз со своей командой изучает влияние глобального потепления на поведение больших белых акул и экосистему небольшого атолла. Внезапно к ней за помощью обращается давний приятель и бывший коллега Ричард Лоуэлл. Он просит Эмму помочь ему выследить и поймать двух бычьих акул, которые уже убили нескольких человек.",
      popularity: 60.465,
      poster_path: "/5wipsaOVJD6zi7XVlvfAeXlhrVs.jpg",
      release_date: "2020-08-25",
      title: "Глубокое синее море 3",
      video: !1,
      vote_average: 5.9,
      vote_count: 211,
    },
    {
      adult: !1,
      backdrop_path: "/5gTQmnGYKxDfmUWJ9GUWqrszRxN.jpg",
      genre_ids: [10751, 16, 9648, 35],
      id: 721656,
      original_language: "en",
      original_title: "Happy Halloween, Scooby-Doo!",
      overview:
        "Банда приезжает на Хэллоуин в город, который терроризирует Джонатан Крэйнь, переодевающийся в Пугало. В городе банда помогают их старые знакомые - Эльвира и Билл Най, но желательна ли помощь этих старых знакомых или эта помощь лишь будет мешать делу?",
      popularity: 59.212,
      poster_path: "/5aL71e0XBgHZ6zdWcWeuEhwD2Gw.jpg",
      release_date: "2020-10-06",
      title: "Счастливого Хэллоуина, Скуби-Ду!",
      video: !1,
      vote_average: 8,
      vote_count: 188,
    },
    {
      adult: !1,
      backdrop_path: "/hWpVSEMWDvG49ezvpksnS5BSnvB.jpg",
      genre_ids: [28, 27, 53],
      id: 614696,
      original_language: "ko",
      original_title: "#살아있다",
      overview:
        "Молодой человек оказывается в ловушке в жилом комплексе когда вокруг происходит прорыв зомбивирусной инфекции. Через некоторое время он замечает, что в доме напротив так же  прячется девушка.",
      popularity: 58.585,
      poster_path: "/2K3RjhGVoNyp0NKw06ydSQYEZhv.jpg",
      release_date: "2020-06-24",
      title: "#живой",
      video: !1,
      vote_average: 7.3,
      vote_count: 1297,
    },
    {
      adult: !1,
      backdrop_path: "/zogWnCSztU8xvabaepQnAwsOtOt.jpg",
      genre_ids: [28, 80, 18],
      id: 531499,
      original_language: "en",
      original_title: "The Tax Collector",
      overview:
        "Выбивая черный нал из должников, два коллектора перешли дорогу главе наркокартеля, практикующему шаманство. Спровоцировав конфликт между двумя бандами, который может обернуться кровавой бойней, коллекторы поставили под угрозу жизни своих семей.",
      popularity: 57.11,
      poster_path: "/fVR1vju4FJN0Zj4k6ajk5wsmTA9.jpg",
      release_date: "2020-08-07",
      title: "Выбивая долги",
      video: !1,
      vote_average: 5.7,
      vote_count: 354,
    },
    {
      adult: !1,
      backdrop_path: "/sizHX5VbwlBihaathTQHVGk1jdi.jpg",
      genre_ids: [878, 18, 10749, 28],
      id: 514207,
      original_language: "ru",
      original_title: "Вторжение",
      overview:
        "Падение инопланетного объекта разделило жизни на «до» и «после». Обычная девушка из московского Чертанова — Юлия Лебедева — вынуждена смириться с ролью подопытного кролика в лаборатории. Ведь она единственная была в контакте с пришельцем. Учёные и военные разбирают на атомы её чувства, эмоции и переживания, пытаясь разгадать природу растущей в ней силы. Но страшнее всего, что её сверхъестественные способности волнуют не только землян. Над планетой в буквальном смысле нависла угроза вторжения. И победить в грядущем столкновении можно только одним способом: найти в себе силы остаться людьми. Когда каждый ради общего спасения должен сделать выбор, от которого зависит жизнь и судьба миллионов, — смогут ли любовь, верность и милосердие стать сильнее безжалостной силы и инопланетных технологий?",
      popularity: 56.852,
      poster_path: "/agcNNguu18jZfbvBFRdhLvdWdf8.jpg",
      release_date: "2020-01-01",
      title: "Вторжение",
      video: !1,
      vote_average: 6.9,
      vote_count: 640,
    },
    {
      adult: !1,
      backdrop_path: "/aHYUj0hICtWZ5tPiCIm6pWUcjYK.jpg",
      genre_ids: [27, 28, 10752],
      id: 675327,
      original_language: "en",
      original_title: "Shadow in the Cloud",
      overview:
        "1943 год, разгар Второй мировой войны. Девушка-офицер Мод Гэррет с секретным грузом садится в американский бомбардировщик B-17 «Летающая крепость» и сразу же становится объектом насмешек. Капитан приказывает ей занять место в кабине пулемётчика под брюхом самолета, откуда девушка замечает японский истребитель и странное существо на фюзеляже. Если сексистские комментарии команды она до поры до времени решает потерпеть, то хозяйничать в небе вражеским самолетам и небесным гоблинам позволять не намерена.",
      popularity: 56.476,
      poster_path: "/4ig8j3M3kLBimwsYEAgEFFdhcGo.jpg",
      release_date: "2020-12-31",
      title: "Воздушный бой",
      video: !1,
      vote_average: 5.8,
      vote_count: 601,
    },
    {
      adult: !1,
      backdrop_path: "/7rnJpyHxNloqjLiRoLVnR4VzHyg.jpg",
      genre_ids: [53],
      id: 522406,
      original_language: "en",
      original_title: "Flashback",
      overview:
        "Приходит день, когда мы должны сделать выбор. Стабильность или воплощение заветной мечты? Работа в крупной корпорации, семья или свобода творчества? Как тут не ошибиться? Фред Фитцелл оказывается во вневременном пространстве своего прошлого и будущего, где ему придется сделать выбор.",
      popularity: 55.984,
      poster_path: "/dhwN9yz8JpaC2ZaViEW4VCSssFI.jpg",
      release_date: "2020-10-08",
      title: "Выбор Фредерика Фитцелла",
      video: !1,
      vote_average: 6.4,
      vote_count: 203,
    },
    {
      adult: !1,
      backdrop_path: "/kLsrE6q4ld144ECBkydZ4dV8t9T.jpg",
      genre_ids: [27],
      id: 648990,
      original_language: "es",
      original_title: "La casa",
      overview:
        "Приехав на срочный вызов, полицейский попадает в ловушку злых духов проклятого дома.",
      popularity: 55.75,
      poster_path: "/fvTGWaQejnHQBZbEOrdbb38NBTC.jpg",
      release_date: "2020-01-30",
      title: "Дом",
      video: !1,
      vote_average: 6.4,
      vote_count: 76,
    },
    {
      adult: !1,
      backdrop_path: "/htBUhLSS7FfHtydgYxUWjL3J1Q1.jpg",
      genre_ids: [16, 12, 10751, 14],
      id: 560050,
      original_language: "en",
      original_title: "Over the Moon",
      overview:
        "История о девочке, которая строит ракету, чтобы отправиться на Луну и доказать своему отцу, что легендарная богиня Луны существует.",
      popularity: 55.528,
      poster_path: "/lQfdytwN7eh0tXWjIiMceFdBBvD.jpg",
      release_date: "2020-10-16",
      title: "Путешествие на Луну",
      video: !1,
      vote_average: 7.3,
      vote_count: 1060,
    },
    {
      adult: !1,
      backdrop_path: "/7TxeZVg2evMG42p0uSbMJpWNQ8A.jpg",
      genre_ids: [16, 10751, 14],
      id: 520946,
      original_language: "en",
      original_title: "100% Wolf",
      overview:
        "Наследник семьи оборотней Фредди Люпин отчаянно желает стать как предки. Но на 13-й день рождения его ждёт шок, ведь он оборачивается не волком, а… пуделем.",
      popularity: 54.94,
      poster_path: "/yJj53kp126IUA1LlqB0BPjhc0Ux.jpg",
      release_date: "2020-06-26",
      title: "100% Волк",
      video: !1,
      vote_average: 5.9,
      vote_count: 228,
    },
    {
      adult: !1,
      backdrop_path: "/qVRBml5LwGEvI9JJWrTLiZ5BuNN.jpg",
      genre_ids: [18],
      id: 695568,
      original_language: "en",
      original_title: "Finding Grace",
      overview:
        "Проблемная семья на грани распада сталкивается с новыми проблемами, которые заставят их усомниться друг в друге и в Боге.",
      popularity: 54.212,
      poster_path: "/cE1pT8biarqm2Sjk094Sdygsu69.jpg",
      release_date: "2020-04-21",
      title: "Дорогой милосердия",
      video: !1,
      vote_average: 6.3,
      vote_count: 16,
    },
    {
      adult: !1,
      backdrop_path: "/v5CEt88iDsuoMaW1Q5Msu9UZdEt.jpg",
      genre_ids: [10749, 18],
      id: 730154,
      original_language: "ja",
      original_title: "きみの瞳が問いかけている",
      overview: "",
      popularity: 54.208,
      poster_path: "/8v0MtSxuvfEzVDtP4uiu1Z7r1Wl.jpg",
      release_date: "2020-10-23",
      title: "きみの瞳が問いかけている",
      video: !1,
      vote_average: 8.5,
      vote_count: 323,
    },
    {
      adult: !1,
      backdrop_path: "/wO5QSWZPBT71gMLvrRex0bVc0V9.jpg",
      genre_ids: [35, 10749],
      id: 583083,
      original_language: "en",
      original_title: "The Kissing Booth 2",
      overview:
        "Эль учится в выпускном классе и пытается найти баланс между своими отношениями на расстоянии с бойфрендом своей мечты Ноа, колледжем, куда она собиралась поступать со своим лучшим другом Ли, а также новыми отношениями с красавцем-одноклассником Марко, который активно с ней флиртует, что могут изменить всё.",
      popularity: 54.157,
      poster_path: "/cM6Kzs5f6N4Ld9SYb0ETFpbVbLJ.jpg",
      release_date: "2020-07-24",
      title: "Калейдоскоп поцелуев 2",
      video: !1,
      vote_average: 7.8,
      vote_count: 4510,
    },
    {
      adult: !1,
      backdrop_path: "/fKtYXUhX5fxMxzQfyUcQW9Shik6.jpg",
      genre_ids: [16, 10751, 35, 12],
      id: 385103,
      original_language: "en",
      original_title: "Scoob!",
      overview:
        "Умный пес Скуби-Ду знакомится с Шэгги, Фредом, Дафной и Вельмой. Вместе они основывают компанию Mistery Inc. Новые друзья столкнутся с проблемой – они должны разоблачить заговор, целью которого является вызов страшного призрака – пса Цербера.",
      popularity: 54.036,
      poster_path: "/t4VDqVusBNYIGWVP19RYyccDYpv.jpg",
      release_date: "2020-07-09",
      title: "Скуби-Ду",
      video: !1,
      vote_average: 7.2,
      vote_count: 1354,
    },
    {
      adult: !1,
      backdrop_path: "/3s9O5af2xWKWR5JzP2iJZpZeQQg.jpg",
      genre_ids: [28, 12, 878, 53],
      id: 351286,
      original_language: "en",
      original_title: "Jurassic World: Fallen Kingdom",
      overview:
        "Основатели парка Юрского периода решают восстановить его работу, пренебрегая действующим вулканом, который находится в центре острова. Огнедышащий исполин пробуждается и грозит утопить всё в лаве. Клэр Дэринг вынуждена снова обратиться за помощью к дрессировщику Оуэну Грэди, чтобы спасти животных от верной смерти.",
      popularity: 496.448,
      poster_path: "/4VEkflsi7zQmBN6vNpihNrmnRzj.jpg",
      release_date: "2018-06-06",
      title: "Мир Юрского периода 2",
      video: !1,
      vote_average: 6.6,
      vote_count: 9957,
    },
    {
      adult: !1,
      backdrop_path: "/lmZFxXgJE3vgrciwuDib0N8CfQo.jpg",
      genre_ids: [12, 28, 878],
      id: 299536,
      original_language: "en",
      original_title: "Avengers: Infinity War",
      overview:
        "Пока Мстители и их союзники продолжают защищать мир от различных опасностей, с которыми не смог бы справиться один супергерой, новая угроза возникает из космоса: Танос. Межгалактический тиран преследует цель собрать все шесть Камней Бесконечности — артефакты невероятной силы, с помощью которых можно менять реальность по своему желанию. Всё, с чем Мстители сталкивались ранее, вело к этому моменту — судьба Земли никогда ещё не была столь неопределённой.",
      popularity: 385.996,
      poster_path: "/qIUFg6tzKeK5bUDguonWCAFceNB.jpg",
      release_date: "2018-04-25",
      title: "Мстители: Война бесконечности",
      video: !1,
      vote_average: 8.3,
      vote_count: 24791,
    },
    {
      adult: !1,
      backdrop_path: "/3P52oz9HPQWxcwHOwxtyrVV1LKi.jpg",
      genre_ids: [28, 35, 12],
      id: 383498,
      original_language: "en",
      original_title: "Deadpool 2",
      overview:
        "Когда в его жизнь врывается  суперсолдат с убийственной миссией, Дэдпул вынужден задуматься о дружбе, семье и о том, что на самом деле значит быть героем, попутно надирая 50 оттенков задниц. Потому что иногда чтобы делать хорошие вещи, нужно использовать грязные приемчики.",
      popularity: 179.915,
      poster_path: "/ukbLruQbrchScy3jTXgeAz9IWDL.jpg",
      release_date: "2018-05-10",
      title: "Дэдпул 2",
      video: !1,
      vote_average: 7.5,
      vote_count: 14770,
    },
    {
      adult: !1,
      backdrop_path: "/9ywA15OAiwjSTvg3cBs9B7kOCBF.jpg",
      genre_ids: [18, 10749],
      id: 337167,
      original_language: "en",
      original_title: "Fifty Shades Freed",
      overview:
        "Кристиан и Анастейша поженились и живут в своё удовольствие, наслаждаясь обществом друг друга. Но жизнь новоиспечённой миссис Грей находится в опасности, поскольку объявляется недруг, который собирается мстить, используя свою богатую фантазию. Призраки прошлого Кристиана вновь вернулись, а тучи над супругами сгущаются всё сильнее.",
      popularity: 172.353,
      poster_path: "/uZ5ERtY6q8GvPZmJmuW6hQRQ1nO.jpg",
      release_date: "2018-01-17",
      title: "Пятьдесят оттенков свободы",
      video: !1,
      vote_average: 6.7,
      vote_count: 6908,
    },
    {
      adult: !1,
      backdrop_path: "/uicia399gyMGE1smatJ41M0CtFx.jpg",
      genre_ids: [16, 28, 12, 14],
      id: 505262,
      original_language: "ja",
      original_title: "僕のヒーローアカデミア THE MOVIE ～2人の英雄～",
      overview:
        "Идзуку и Всемогущий прилетают на остров, где проходит выставка I-Expo. Там они встречают старого друга Всемогущего Дэвида Шилда и его дочь Мелиссу. Вместе с другими учениками класса А-1 они отдыхают и развлекаются, а тем временем на острове появляется группа злодеев, с которой и придется сразиться ученикам геройской академии.",
      popularity: 157.037,
      poster_path: "/rwLuZhDcLbtC8VKW8Z2xnNKTsAJ.jpg",
      release_date: "2018-08-03",
      title: "Моя геройская академия: Два героя",
      video: !1,
      vote_average: 8,
      vote_count: 766,
    },
    {
      adult: !1,
      backdrop_path: "/gXSpNiP9a5uq0mo1hcfp2PVUsOV.jpg",
      genre_ids: [28, 878, 16],
      id: 503314,
      original_language: "ja",
      original_title: "ドラゴンボール超スーパー ブロリー",
      overview:
        "Тема этого замечательного нового фильма будет «Сайян», самая сильная раса во вселенной. Начиная с «Битвы богов», Гокуу претерпел новые формы от Бога Супер Сайяна до Супер Сайяна Блу в других эволюционированных формах, которые выросли против многих непобедимых воинов из нескольких вселенных. Эта новая история будет сфокусирована на происхождении силы сайян и что значит быть Сайаном.",
      popularity: 148.545,
      poster_path: "/qIqXrTKCKkfrmTakvTsjlya7OWw.jpg",
      release_date: "2018-12-14",
      title: "Драконий жемчуг: Супер (Броли)",
      video: !1,
      vote_average: 7.8,
      vote_count: 2280,
    },
    {
      adult: !1,
      backdrop_path: "/oMaj75G2WFw66xI1TREg1hrToSg.jpg",
      genre_ids: [28, 14, 878, 12],
      id: 268896,
      original_language: "en",
      original_title: "Pacific Rim: Uprising",
      overview:
        "Команда пилотируемых роботов-защитников остановила вторжение гигантских инопланетных монстров. Великая битва за Тихоокеанский рубеж ознаменовала новую главу в истории человечества. Однако война только начинается… Пришло время нового поколения отстаивать своё право на Землю.",
      popularity: 145.071,
      poster_path: "/hAR6AdEKMVQXrcTt1hnaEU7YvSX.jpg",
      release_date: "2018-03-21",
      title: "Тихоокеанский рубеж 2",
      video: !1,
      vote_average: 6.1,
      vote_count: 4030,
    },
    {
      adult: !1,
      backdrop_path: "/qrtRKRzoNkf5vemO9tJ2Y4DrHxQ.jpg",
      genre_ids: [14, 12, 28],
      id: 338952,
      original_language: "en",
      original_title: "Fantastic Beasts: The Crimes of Grindelwald",
      overview:
        "В конце первого фильма могущественный темный волшебник Геллерт Грин-де-Вальд был пойман сотрудниками МАКУСА (Магического Конгресса Управления по Северной Америке), не без помощи Ньюта Саламандера. Выполняя свое обещание, темный маг устраивает грандиозный побег и начинает собирать сторонников, большинство из которых не знают о его истинной цели: добиться превосходства волшебников над всеми немагическими существами на планете. Чтобы сорвать планы Грин-де-Вальда, Альбус Дамблдор обращается к своему бывшему студенту Ньюту Саламандеру, который соглашается помочь, не подозревая, какая опасность ему грозит. В раскалывающемся на части волшебном мире любовь и верность проверяются на прочность, а конфликт разделяет даже настоящих друзей и членов семей.",
      popularity: 132.645,
      poster_path: "/4wH16yc3Rmpx8Ob33QTJkFVNf5j.jpg",
      release_date: "2018-11-14",
      title: "Фантастические твари: Преступления Грин-де-Вальда",
      video: !1,
      vote_average: 6.9,
      vote_count: 9353,
    },
    {
      adult: !1,
      backdrop_path: "/VuukZLgaCrho2Ar8Scl9HtV3yD.jpg",
      genre_ids: [878, 28],
      id: 335983,
      original_language: "en",
      original_title: "Venom",
      overview:
        "Что если в один прекрасный день в тебя вселяется существо-симбиот, которое наделяет тебя сверхчеловеческими способностями? Вот только Веном – симбиот совсем недобрый, и договориться с ним невозможно. Хотя нужно ли договариваться?.. Ведь в какой-то момент ты понимаешь, что быть плохим вовсе не так уж и плохо. Так даже веселее. В мире и так слишком много супергероев! Мы – Веном!",
      popularity: 130.913,
      poster_path: "/8Gyl0fknqiZeCLm9XitxCXQmEL9.jpg",
      release_date: "2018-09-28",
      title: "Веном",
      video: !1,
      vote_average: 6.8,
      vote_count: 13420,
    },
    {
      adult: !1,
      backdrop_path: "/q7fXcrDPJcf6t3rzutaNwTzuKP1.jpg",
      genre_ids: [878, 12, 28],
      id: 333339,
      original_language: "en",
      original_title: "Ready Player One",
      overview:
        "Действие фильма происходит в 2045 году, мир погружается в хаос и находится на грани коллапса. Люди ищут спасения в игре OASIS — огромной вселенной виртуальной реальности. Ее создатель, гениальный и эксцентричный Джеймс Холлидэй, оставляет уникальное завещание. Все его колоссальное состояние получит игрок, первым обнаруживший цифровое «пасхальное яйцо», которое миллиардер спрятал где-то на просторах OASISа. Запущенный им квест охватывает весь мир. Совершенно негероический парень по имени Уэйд Уоттс решает принять участие в состязании, с головой бросаясь в головокружительную, искажающую реальность погоню за сокровищами по фантастической вселенной, полной загадок, открытий и опасностей.",
      popularity: 126.912,
      poster_path: "/lI4NVRow2Oe0x5frvRio5SV0SoB.jpg",
      release_date: "2018-03-28",
      title: "Первому игроку приготовиться",
      video: !1,
      vote_average: 7.6,
      vote_count: 12993,
    },
    {
      adult: !1,
      backdrop_path: "/qDQEQbgP3v7B9IYLAUcYexNrVYP.jpg",
      genre_ids: [10751, 16, 35, 12],
      id: 404368,
      original_language: "en",
      original_title: "Ralph Breaks the Internet",
      overview:
        "На этот раз Ральф и Ванилопа фон Кекс выйдут за пределы зала игровых автоматов и отправятся покорять бескрайние просторы Интернета, который может и не выдержать сокрушительного обаяния громилы.",
      popularity: 121.106,
      poster_path: "/tVmomifGhoJfijkOnSGDDZwRSH5.jpg",
      release_date: "2018-11-20",
      title: "Ральф против Интернета",
      video: !1,
      vote_average: 7.2,
      vote_count: 6279,
    },
    {
      adult: !1,
      backdrop_path: "/fgsHxz21B27hOOqQBiw9L6yWcM7.jpg",
      genre_ids: [27, 9648, 53],
      id: 439079,
      original_language: "en",
      original_title: "The Nun",
      overview:
        "Когда в уединенном монастыре в Румынии совершает самоубийство молодая монахиня, расследовать происшествие Ватикан отправляет священника с туманным прошлым и послушницу на пороге невозвратных обетов. Рискуя не только жизнями, но и верой, и даже своими душами, они сталкиваются со злобной силой, принявшей облик демонической монахини, а монастырь становится полем ужасающей битвы между живыми и проклятыми.",
      popularity: 115.534,
      poster_path: "/rheiph1J5YIAKhXaShVRYZGII8y.jpg",
      release_date: "2018-09-05",
      title: "Проклятие монахини",
      video: !1,
      vote_average: 5.8,
      vote_count: 5250,
    },
    {
      adult: !1,
      backdrop_path: "/x2IqsMlpbOhS8zIUJfyl1yO4gHF.jpg",
      genre_ids: [28, 12, 16, 878],
      id: 324857,
      original_language: "en",
      original_title: "Spider-Man: Into the Spider-Verse",
      overview:
        "В центре сюжета уникальной и инновационной в визуальном плане картины подросток из Нью-Йорка Майлз Моралес, который живет в мире безграничных возможностей вселенных Человека-паука, где костюм супергероя носит не только он.",
      popularity: 110.343,
      poster_path: "/dEh3ksfCPRp41Q6RPkIjI64Zfr1.jpg",
      release_date: "2018-12-06",
      title: "Человек-паук: Через вселенные",
      video: !1,
      vote_average: 8.4,
      vote_count: 11400,
    },
    {
      adult: !1,
      backdrop_path: "/hgESQ2sK21j22Yode4w99jfwj44.jpg",
      genre_ids: [35],
      id: 491418,
      original_language: "en",
      original_title: "Instant Family",
      overview:
        "Бездетные супруги Пит и Элли решают усыновить ребенка, но так уж получается, что в их семье появляется сразу трое детей — две сестры и брат. Теперь Пит и Элли оказываются перед настоящим испытанием.",
      popularity: 107.046,
      poster_path: "/yplNzepCZ8YWTGMqoZiwkzaZdZ1.jpg",
      release_date: "2018-11-13",
      title: "Семья по-быстрому",
      video: !1,
      vote_average: 7.6,
      vote_count: 2330,
    },
    {
      adult: !1,
      backdrop_path: "/mabuNsGJgRuCTuGqjFkWe1xdu19.jpg",
      genre_ids: [28, 12, 16, 10751],
      id: 260513,
      original_language: "en",
      original_title: "Incredibles 2",
      overview:
        "После событий первой части суперсемейка пользуется повышенным вниманием со стороны журналистов. Неожиданно оказалось, что обаятельная Миссис Исключительная гораздо лучше смотрится на экране телевизора, чем её муж. Мистеру Исключительному всё чаще приходится сидеть дома и приглядывать за детьми, у каждого из которых есть свои причины быть недовольным собственными суперспособностями. Однако семейным проблемам предстоит отступить на второй план, когда окажется, что спокойствию Суперсемейки угрожает новый могущественный враг.",
      popularity: 102.571,
      poster_path: "/vwer36JvbZxeZ6ZRBvzrsUm6tYt.jpg",
      release_date: "2018-06-14",
      title: "Суперсемейка 2",
      video: !1,
      vote_average: 7.5,
      vote_count: 11031,
    },
    {
      adult: !1,
      backdrop_path: "/rH79sB6Nkx4cMW3JzsUy7wK0rhX.jpg",
      genre_ids: [28, 878, 27],
      id: 345940,
      original_language: "en",
      original_title: "The Meg",
      overview:
        "Профессор палеонтологии Джонас Тэйлор, в прошлом элитный пилот глубоководных аппаратов, уже много лет пишет о меге и читает лекции, стараясь забыть незабываемое — трагически окончившуюся экспедицию, которая убедила его в том, что первобытное чудовище существует и в наши дни. При секретном погружении в самый глубокий океанский каньон Тэйлор едва спасся от огромных челюстей доисторического чудовища, способного поднять из воды даже тираннозавра. Тэйлору предлагали совершить еще более глубокое погружение. То, что он, возможно, найдет, должно заставить умолкнуть скептиков, но, когда эта находка будет поднята со дна, океанские приливы станут кроваво-красными до скончания времен.",
      popularity: 100.814,
      poster_path: "/hushjVvOcyNwBKAF4u0vSbNrc8T.jpg",
      release_date: "2018-08-09",
      title: "Мег: Монстр глубины",
      video: !1,
      vote_average: 6.2,
      vote_count: 5897,
    },
    {
      adult: !1,
      backdrop_path: "/bvbyidkMaBls1LTaIWYY6UmYTaL.jpg",
      genre_ids: [878, 28, 12, 53],
      id: 336843,
      original_language: "en",
      original_title: "Maze Runner: The Death Cure",
      overview:
        "Томас возглавляет отряд выживших глейдеров, чтобы выполнить последнюю и самую опасную миссию. Ради спасения своих друзей они должны проникнуть в легендарный Последний город, смертоносный лабиринт, контролируемый ПОРОКом. Каждый, кто выживет там, наконец-то получит ответы на все вопросы, которые мучили глейдеров с момента попадания в лабиринт.",
      popularity: 97.801,
      poster_path: "/6ps20wVKQghAx0KBTQvPcLPdLlc.jpg",
      release_date: "2018-01-10",
      title: "Бегущий в лабиринте: Лекарство от смерти",
      video: !1,
      vote_average: 7.1,
      vote_count: 6449,
    },
    {
      adult: !1,
      backdrop_path: "/6ELJEzQJ3Y45HczvreC3dg0GV5R.jpg",
      genre_ids: [28, 12, 878],
      id: 284054,
      original_language: "en",
      original_title: "Black Panther",
      overview:
        "С первого взгляда можно решить, что Ваканда — обычная территория дикой Африки, но это не так. Здесь, в недрах пустынных земель, скрываются залежи уникального металла, способного поглощать вибрацию. Многие пытались добраться до него, разоряя всё на своём пути и принося смерть аборигенам, но каждый раз таинственный дух саванны — Чёрная Пантера — вставал на защиту угнетённых.  Спустя много лет беда снова приходит в Ваканду, и в этот раз враг заручился поддержкой современных технологий. Когда шансов почти не остаётся, Т`Чалла, молодой принц Ваканды, узнаёт, что именно ему предстоит возродить легенду и продолжить вечную борьбу, надев маску Чёрной Пантеры.",
      popularity: 94.308,
      poster_path: "/ndpwgAANbZSMwgTcxmQ7o4SKDsd.jpg",
      release_date: "2018-02-13",
      title: "Чёрная Пантера",
      video: !1,
      vote_average: 7.4,
      vote_count: 19171,
    },
    {
      adult: !1,
      backdrop_path: "/f4E0ocYeToEuXvczZv6QArrMDJ.jpg",
      genre_ids: [878, 28, 12, 35, 27],
      id: 346910,
      original_language: "en",
      original_title: "The Predator",
      overview:
        "Главный герой, бывший морпех, обнаруживает злобную инопланетную расу на Земле, но никто не верит ему, что эти твари существуют. Его сын-аутист, которого все обижают в школе, становится ключевой фигурой в схватке с Хищниками, поскольку мальчик умеет невероятно быстро учить языки и, судя по всему, сможет понять пришельцев.",
      popularity: 92.617,
      poster_path: "/iFPCByJjLivNpfHWcF8M7aPNd62.jpg",
      release_date: "2018-09-05",
      title: "Хищник",
      video: !1,
      vote_average: 5.6,
      vote_count: 3769,
    },
    {
      adult: !1,
      backdrop_path: "/tnwMCH4yLBY4qpe6Nr4n66u4U3f.jpg",
      genre_ids: [28, 80, 18, 53],
      id: 400535,
      original_language: "en",
      original_title: "Sicario: Day of the Soldado",
      overview:
        "Фильм раскроет историю загадочного и мрачного мексиканца Алехандро Гиллике. Вместе с агентами ЦРУ он вскрывает «дыры» на границе США, которые дают путь для наркотрафика, нелегалов и даже для террористов.",
      popularity: 89.635,
      poster_path: "/8f1Dg0yU09NC9Wwv9f8IV5S56YF.jpg",
      release_date: "2018-06-27",
      title: "Убийца 2. Против всех",
      video: !1,
      vote_average: 6.9,
      vote_count: 2776,
    },
    {
      adult: !1,
      backdrop_path: "/53aUwtolUZxgzMr2KbDpOnD8jo3.jpg",
      genre_ids: [16, 10751],
      id: 718867,
      original_language: "en",
      original_title: "The Larva Island Movie",
      overview:
        "Две личинки, которые невероятно дружны между собой, по стечению обстоятельств оказываются на необитаемом острове. Любой бы на их месте испытывал истинный ужас. Но эти приятели находят в здешних безлюдных краях ни только еду и воду, но и новых друзей, представителей дикого, животного мира.",
      popularity: 51.606,
      poster_path: "/uHdxICUIRGoJjK67aOp3ykvL8ZW.jpg",
      release_date: "2020-07-23",
      title: "Остров личинок",
      video: !1,
      vote_average: 7.3,
      vote_count: 146,
    },
    {
      adult: !1,
      backdrop_path: "/fQq1FWp1rC89xDrRMuyFJdFUdMd.jpg",
      genre_ids: [10749, 35],
      id: 761053,
      original_language: "en",
      original_title: "Gabriel's Inferno: Part III",
      overview:
        "Некто из прошлого Джулии возвращается с плохими намерениями. У Габриэля есть секреты, которыми он может поделиться, но он боится потерять все, если он это сделает. Выдержат ли их отношения испытание раскрытыми секретами?",
      popularity: 51.527,
      poster_path: "/uqmTxOP3gNl5MWRt1wlrUnzTphM.jpg",
      release_date: "2020-11-19",
      title: "Инферно Габриэля 3",
      video: !1,
      vote_average: 8.5,
      vote_count: 966,
    },
    {
      adult: !1,
      backdrop_path: "/nz8xWrTKZzA5A7FgxaM4kfAoO1W.jpg",
      genre_ids: [878, 28],
      id: 651571,
      original_language: "en",
      original_title: "Breach",
      overview:
        "2242 год. Земля обречена. Среди 50 миллионов избранных, летящих к новой планете, — беременная дочь капитана корабля. Её парня Ноя нет в списках, поэтому ему приходится обманом проникнуть на борт и притворяться уборщиком. Пока большая часть пассажиров и команды находится в анабиозе, Ной в компании бывалого напарника Клэя занимается ежедневной рутиной и знакомится с коллегами. Но вскоре на борту обнаруживается агрессивная инопланетная форма жизни, которая угрожает самому существованию человечества.",
      popularity: 50.434,
      poster_path: "/mRPWDXdVe0mFuEnCLrLFPp68Jct.jpg",
      release_date: "2020-12-17",
      title: "Брешь",
      video: !1,
      vote_average: 4.3,
      vote_count: 519,
    },
    {
      adult: !1,
      backdrop_path: "/xUUtcxWC6H48UCrpRwwSPQz69XC.jpg",
      genre_ids: [53, 27],
      id: 659986,
      original_language: "en",
      original_title: "The Owners",
      overview:
        "Группа молодых людей затевает взлом дома богатой пожилой пары, однако загадочные хозяева хранят там не только сбережения, но и свои секреты.",
      popularity: 50.116,
      poster_path: "/3QILgMhfvFb3N4B9vDXW5td4Brw.jpg",
      release_date: "2020-08-27",
      title: "Не входи",
      video: !1,
      vote_average: 5.8,
      vote_count: 243,
    },
    {
      adult: !1,
      backdrop_path: "/hLtv6gYV9hIPWPFDniLSCodhmq.jpg",
      genre_ids: [27, 53],
      id: 652004,
      original_language: "en",
      original_title: "The Wolf of Snow Hollow",
      overview:
        "Ужас охватывает небольшой городок: жестоко растерзанные трупы появляются в каждую полную луну. Потрёпанный жизнью офицер Маршал берётся за расследование, даже не подозревая, что оно пошатнёт его представление о добре и зле.",
      popularity: 49.958,
      poster_path: "/4UOiXJGfEOYM4HTi1uXk442d19o.jpg",
      release_date: "2020-10-23",
      title: "Оборотень",
      video: !1,
      vote_average: 6.2,
      vote_count: 239,
    },
    {
      adult: !1,
      backdrop_path: "/cpTjuXuNTEDJc7qJEiUbUGcn85F.jpg",
      genre_ids: [27, 35],
      id: 664168,
      original_language: "en",
      original_title: "Fat Ass Zombies",
      overview:
        "Комедийный фильм рассказывает историю режиссера, который снимает фильмы ужасов. Главный герой живёт в маленьком городке и постоянно находится в ожидании момента, когда его жизнь наконец-то изменится. И вот однажды всё же наступил долгожданный момент, у него появился реальный шанс на минуту славы, но вот только далеко не Голливуд оказался на его пороге.",
      popularity: 49.808,
      poster_path: "/btrers8SNujMJhxjLvjh7AJpN3T.jpg",
      release_date: "2020-02-18",
      title: "Зомбиленд по-американски",
      video: !1,
      vote_average: 4.9,
      vote_count: 46,
    },
    {
      adult: !1,
      backdrop_path: "/fZHvSNUBKSgRAduB5k0fFST1xYn.jpg",
      genre_ids: [16, 10751],
      id: 749618,
      original_language: "es",
      original_title: "El camino de Xico",
      overview:
        "Мир и спокойствие маленького городка и его жителей находятся под угрозой, когда корпорация хочет разрушить гору, которая их защищает. Девушка по имени Копи вместе со своим лучшим другом и домашним животным Ксико, собакой Ксолоитцкуинтли, отправится в горы, чтобы спасти город. Но все будет зависеть от Ксико, когда он обнаружит, что он на самом деле волшебный пес и должен найти в себе мужество, чтобы спасти гору и всех жителей города.",
      popularity: 49.453,
      poster_path: "/ssk0Gd27ziryP2OUxprIVhAvr3e.jpg",
      release_date: "2020-11-12",
      title: "Путь Хико",
      video: !1,
      vote_average: 6.6,
      vote_count: 87,
    },
    {
      adult: !1,
      backdrop_path: "/yFRpUmsreYO5Bc0HVBTsJsHIIox.jpg",
      genre_ids: [12, 10751, 18],
      id: 481848,
      original_language: "en",
      original_title: "The Call of the Wild",
      overview:
        "История Бэка, дружелюбного пса, чья размеренная домашняя жизнь перевернулась с ног на голову во времена золотой лихорадки в 1880-х, когда его вырвали из дома в Калифорнии и перевезли в дикую и холодную Аляску. Будучи новичком в упряжке почтовой службы, а впоследствии лидером, Бак попадает в невероятное приключение, находит свое место в мире и становится хозяином своей жизни.",
      popularity: 49.17,
      poster_path: "/yzF53Gw27HiVNO9ykg8LOxt5q6m.jpg",
      release_date: "2020-02-19",
      title: "Зов предков",
      video: !1,
      vote_average: 7.6,
      vote_count: 2909,
    },
    {
      adult: !1,
      backdrop_path: "/jqz8FwISQfyum47PUqgPTGtmiMk.jpg",
      genre_ids: [18],
      id: 627725,
      original_language: "en",
      original_title: "The Banker",
      overview:
        "В центре сюжета оказываются два темнокожих товарища – Бернард и Джо, которые решают организовать первый в округе бизнес по продаже недвижимости. Основная проблема заключается в их цвете кожи – с ними попросту не захочет иметь дело ни один здравомыслящий белый. По этой причине главные герои нанимают белого сотрудника, а сами якобы становятся дворником и водителем. Со временем их дела стремительно идут верх, герои едва успевают обналичивать чеки, полученные от клиентов. Однако с ростом выручки стремительно приближается час расплаты, заключающийся в разоблачении серой схемы заработка.",
      popularity: 48.935,
      poster_path: "/wus6VjMU7QQyTutpz3bVTyRmbDh.jpg",
      release_date: "2020-03-06",
      title: "Банкир",
      video: !1,
      vote_average: 7.6,
      vote_count: 568,
    },
    {
      adult: !1,
      backdrop_path: "/naUJGskMlDg2HZ0FhsaMG5xPF2K.jpg",
      genre_ids: [35, 27],
      id: 623491,
      original_language: "en",
      original_title: "The Babysitter: Killer Queen",
      overview:
        "Спустя два года после победы Коула над сатанинским культом, возглавляемым его няней Би, парень пытается забыть своё прошлое и сосредоточиться на выживании в старшей школе. Но когда старые враги неожиданно вернутся, Коулу придётся снова перехитрить силы зла.",
      popularity: 48.773,
      poster_path: "/bfreoPO5aKNv2nJiHXHilXoIROO.jpg",
      release_date: "2020-09-10",
      title: "Няня. Королева проклятых",
      video: !1,
      vote_average: 6.4,
      vote_count: 1114,
    },
    {
      adult: !1,
      backdrop_path: "/1R6cvRtZgsYCkh8UFuWFN33xBP4.jpg",
      genre_ids: [18, 28, 53],
      id: 545609,
      original_language: "en",
      original_title: "Extraction",
      overview:
        "Группировка наёмников берётся освободить из заложников сына сидящего в тюрьме индийского наркобарона. Подростка похитили конкуренты и держат его в столице Бангладеш Дакке - городе, состоящем из трущоб и практически полностью окружённом реками. Ответственное задание поручатся бывшему военному Тайлеру Рейку. Он легко расправляется с вооружённой охраной и быстро освобождает пленника, но доставить парня в безопасное место и выбраться из города, где на тебя объявлена охота, оказывается не так-то просто.",
      popularity: 48.428,
      poster_path: "/zD9fW1g5gB6mirfX8Wk3FhdlzBn.jpg",
      release_date: "2020-04-24",
      title: "Тайлер Рейк: Операция по спасению",
      video: !1,
      vote_average: 7.4,
      vote_count: 4506,
    },
    {
      adult: !1,
      backdrop_path: "/yy85YPsJEVxCy97A89cAwdsCUXa.jpg",
      genre_ids: [18, 10749],
      id: 677115,
      original_language: "ko",
      original_title: "비뇨기과 여의사들 2",
      overview: "",
      popularity: 48.202,
      poster_path: "/uksxhYz0Xc7c5AIjoU205XXEVk8.jpg",
      release_date: "2020-02-20",
      title: "비뇨기과 여의사들 2",
      video: !1,
      vote_average: 7.3,
      vote_count: 5,
    },
    {
      adult: !1,
      backdrop_path: "/xFxk4vnirOtUxpOEWgA1MCRfy6J.jpg",
      genre_ids: [10751, 16, 12, 35, 14],
      id: 508439,
      original_language: "en",
      original_title: "Onward",
      overview:
        "Когда-то давно в сказочном мире царила магия, но после открытия электричества маги стали не нужны, и развитие пошло по техногенному пути. Теперь эльфы, тролли, гоблины, мантикоры, феи, кентавры и единороги летают на самолётах, пользуются автомобилями и мобильниками и ведут в целом прозаичную жизнь.  Братья-эльфы Иэн и Барли Лайтфуты воспитывались одной мамой, потому что их отец умер, когда они были ещё совсем маленькими. На 16-летие Иэну достаётся посох отца, и магическое заклинание, которое должно вернуть того к жизни на один день, но волшебный кристалл исчезает, воскресив только нижнюю половину папы. Барли, большой любитель квестов, невероятно рад — прихватив полупапу, братья отправляются в приключение на поиски другого кристалла.",
      popularity: 47.671,
      poster_path: "/xaJnPODeROjzvdiHotiF9YVdez2.jpg",
      release_date: "2020-02-29",
      title: "Вперёд",
      video: !1,
      vote_average: 7.8,
      vote_count: 5053,
    },
    {
      adult: !1,
      backdrop_path: "/tsGSTBl5H8DBLnHc24Teny0Ppc5.jpg",
      genre_ids: [14, 27],
      id: 524216,
      original_language: "en",
      original_title: "The Mortuary Collection",
      overview:
        "Девушка проходит собеседование перед приёмом на работу в местном похоронном бюро, и её просят рассказать самые страшные истории. Результатом является сборник из четырёх самых страшных историй, когда-либо рассказанных за четыре десятилетия, начиная с 50-х и вплоть до 80-х годов.",
      popularity: 47.633,
      poster_path: "/uPtpuLOs5MB1tBQ35UdmBWKOuw5.jpg",
      release_date: "2020-10-08",
      title: "Погребальные байки",
      video: !1,
      vote_average: 6.9,
      vote_count: 298,
    },
    {
      adult: !1,
      backdrop_path: "/2gBMBfmQvHeOQQ5PiKiuxVdGs77.jpg",
      genre_ids: [27],
      id: 587496,
      original_language: "en",
      original_title: "The Rental",
      overview:
        "Вы снимали жилье через Airbnb? Смотрели красивые картинки на сайте и представляли идеальный отпуск? Две молодые пары решили оторваться на берегу океана и арендовали роскошную виллу. Они не знали, что в каждой комнате установлены камеры, и за ними пристально следят. Их маленькие секреты, а вместе с ними и тайна всего дома станут явью, но это лишь начало игры. Кто не спрятался…",
      popularity: 47.395,
      poster_path: "/z1zttTXXhqV0ZczJ3ruY5cFzfgE.jpg",
      release_date: "2020-07-23",
      title: "Кто не спрятался",
      video: !1,
      vote_average: 5.6,
      vote_count: 632,
    },
    {
      adult: !1,
      backdrop_path: "/sCtJkYwBrJm8mygRrDGyluf1gSV.jpg",
      genre_ids: [27, 878, 53],
      id: 736509,
      original_language: "en",
      original_title: "Intersect",
      overview:
        "Молодые учёные из Мискатоникского университета совершают научный прорыв и изобретают машину времени. Но оказывается, что всё это время ими манипулировали невидимые силы из другого измерения.",
      popularity: 46.049,
      poster_path: "/e6hgmD0P133bW5EH1kfhscwXN2a.jpg",
      release_date: "2020-02-12",
      title: "Пересечение",
      video: !1,
      vote_average: 5.9,
      vote_count: 46,
    },
    {
      adult: !1,
      backdrop_path: "/bpHseZ8Vwh3nk7cowKxQ68Dm2Xq.jpg",
      genre_ids: [53, 18, 28],
      id: 616803,
      original_language: "sv",
      original_title: "Breaking Surface",
      overview:
        "Две сестры — любительницы дайвинга, решились на зимнее погружение. Опасное мероприятие окончилось для героинь непредвиденным поворотом. Неспокойный океан обрушил часть скалы и одна из девушек оказалась в ловушке на десятиметровой глубине. Напарнице придётся приложить максимум усилий для спасения близкого человека. Ведь её жизнь зависит от правильных действий без секунды промедления.",
      popularity: 45.85,
      poster_path: "/bYhA7lsoKiUGNOcPM4q4Iu7ED2x.jpg",
      release_date: "2020-02-14",
      title: "Глубокое погружение",
      video: !1,
      vote_average: 6.7,
      vote_count: 284,
    },
    {
      adult: !1,
      backdrop_path: "/2Fk3AB8E9dYIBc2ywJkxk8BTyhc.jpg",
      genre_ids: [28, 53, 18],
      id: 524047,
      original_language: "en",
      original_title: "Greenland",
      overview:
        "Огромная комета мчится к Земле и угрожает уничтожить планету. Конец света кажется неизбежным. Единственное безопасное место – бункер в далёкой Гренландии. Джон, его жена и их сын отправляются в опасное путешествие, которое осложняется не только разрушающими последствиями кометы, но и паникой обезумевших людей, ведь во время глобальной катастрофы никакие человеческие законы уже не действуют...",
      popularity: 45.264,
      poster_path: "/5Ko11P6zf8Wfe9IMFJEOAyNf0FK.jpg",
      release_date: "2020-07-29",
      title: "Гренландия",
      video: !1,
      vote_average: 7.2,
      vote_count: 3424,
    },
    {
      adult: !1,
      backdrop_path: "/mzMCO3dArUcg1OTmjxi8oZ0FQvK.jpg",
      genre_ids: [16, 10751, 10770],
      id: 743904,
      original_language: "en",
      original_title: "Curious George: Go West, Go Wild",
      overview:
        "Отправляйтесь вместе с Джорджем и Тедом в их путешествие к ферме кузины Джинни на расслабляющие выходные на свежем воздухе. История быстро меняет своё направление, когда животные сбегают с фермы.",
      popularity: 44.753,
      poster_path: "/7WWOiP8F6PHjXpJrnRZfvO2XNW.jpg",
      release_date: "2020-09-08",
      title: "Любопытный Джордж: Вперёд на запад",
      video: !1,
      vote_average: 6.2,
      vote_count: 84,
    },
    {
      adult: !1,
      backdrop_path: "/xNzlk4SAwwxo3rTRQD9xtU6ttEj.jpg",
      genre_ids: [16, 10770, 28, 35, 14],
      id: 852663,
      original_language: "en",
      original_title: "LEGO Monkie Kid: A Hero Is Born",
      overview: "",
      popularity: 44.267,
      poster_path: "/3LDHJRNu7er4QH1EJsFLuUmhOWB.jpg",
      release_date: "2020-06-13",
      title: "LEGO Monkie Kid: A Hero Is Born",
      video: !1,
      vote_average: 5.9,
      vote_count: 8,
    },
    {
      adult: !1,
      backdrop_path: "/pgBXz60Rf4zUaXleeIfR0ukIDhS.jpg",
      genre_ids: [18, 27, 53],
      id: 692969,
      original_language: "no",
      original_title: "Kadaver",
      overview:
        "После ядерной катастрофы голодающую семью под предлогом угощения завлекают для участия в спектакле в отеле, где зрители начинают исчезать.",
      popularity: 43.719,
      poster_path: "/jbYvBVXbNW3eVZqKNB4FxMpSvci.jpg",
      release_date: "2020-10-22",
      title: "Театр трупов",
      video: !1,
      vote_average: 5.6,
      vote_count: 484,
    },
    {
      adult: !1,
      backdrop_path: "/2sOzZQ1O87z9FTI0zukmWaJaGmH.jpg",
      genre_ids: [27, 53, 9648],
      id: 678489,
      original_language: "zh",
      original_title: "女鬼橋",
      overview:
        "Пять молодых людей загадочным образом расстались с жизнью на проклятом мосту. Это произошло четыре года назад в университете Донху, но до сих пор неясно, что подвигло студентов на такой шаг. Известно лишь то, что все они принимали участие в так называемой «игре на проверку храбрости». Забытую историю берутся изучить кинематографист и репортер, вернувшиеся к месту трагедии. Небольшой интерес со стороны героев со временем преобразовался в желание во что бы то ни стало выяснить правду. Выясненная героями информация, шокировала их, однако они не намерены молчать, напротив, они настроены серьёзно и хотят раскрыть все подробности мистического самоубийства…",
      popularity: 43.053,
      poster_path: "/81O7oVhi7FlyEHEhvTN93CLy7zG.jpg",
      release_date: "2020-02-27",
      title: "Проклятый мост",
      video: !1,
      vote_average: 6.5,
      vote_count: 58,
    },
    {
      adult: !1,
      backdrop_path: "/9pHxv7TX0jOKNgnGMDP6RJ2m1GL.jpg",
      genre_ids: [28, 53],
      id: 737568,
      original_language: "en",
      original_title: "The Doorman",
      overview:
        "В прошлом она – солдат спецподразделения, в настоящем - швейцар в роскошном отеле, где готовится дерзкое ограбление. Лишь она стоит между безжалостными преступниками и бесценными произведениями искусства, спрятанными в стенах отеля.",
      popularity: 42.773,
      poster_path: "/7ixe28EuO139fjuhBW5mvImMzNx.jpg",
      release_date: "2020-10-01",
      title: "Малышка с характером",
      video: !1,
      vote_average: 5.8,
      vote_count: 303,
    },
    {
      adult: !1,
      backdrop_path: "/mBbA77FyzhU0Tz9tmbKG8heGmh3.jpg",
      genre_ids: [878, 18, 27],
      id: 594718,
      original_language: "ru",
      original_title: "Спутник",
      overview:
        "СССР, 1983 год. Советский космонавт-герой Владимир Вешняков привозит на землю инопланетное существо... в собственном теле. В секретной лаборатории режимного города нейрофизиолог Татьяна Климова пытается спасти космонавта от чудовища, обнаруживая, что испытывает к своему пациенту нечто большее, чем профессиональный интерес.",
      popularity: 42.773,
      poster_path: "/tBi5xW9Rd0YoGB0aAggV0EN5LDP.jpg",
      release_date: "2020-09-08",
      title: "Спутник",
      video: !1,
      vote_average: 6.5,
      vote_count: 496,
    },
    {
      adult: !1,
      backdrop_path: "/h00iFXMpGuls2GEnXIGFzQUeaFr.jpg",
      genre_ids: [18, 27, 53],
      id: 726208,
      original_language: "es",
      original_title: "Voces",
      overview:
        "Даниэль переезжает в новый дом. После трагических событий он слышит мольбу о помощи от призрака и решает обратиться к знаменитому эксперту по паранормальным явлениям.",
      popularity: 41.419,
      poster_path: "/2Orw7EWUpI1LS2z5Dd6fpQ5if81.jpg",
      release_date: "2020-07-24",
      title: "Я слышу голоса",
      video: !1,
      vote_average: 6.5,
      vote_count: 585,
    },
    {
      adult: !1,
      backdrop_path: "/DnjwdbjdR2c5MFhKv5ZSRf8rDU.jpg",
      genre_ids: [53, 27, 18],
      id: 546121,
      original_language: "en",
      original_title: "Run",
      overview:
        "Гиперзаботливая мать Диана Шерман растит дочь Хлою в полной изоляции, контролируя каждый её шаг. Та прикована к инвалидному креслу, принимает множество таблеток, обучается дома и не общается со сверстниками. Разумеется, Хлоя растёт наивной девушкой, но однажды она начинает подозревать что-то неладное.",
      popularity: 41.19,
      poster_path: "/4Wem8UnVwwSXcyvDzG8u3RTd73U.jpg",
      release_date: "2020-11-20",
      title: "Взаперти",
      video: !1,
      vote_average: 7.4,
      vote_count: 1753,
    },
    {
      adult: !1,
      backdrop_path: "/h0IMyviDhX9QsRrhquuTejmjxPC.jpg",
      genre_ids: [53, 27],
      id: 675490,
      original_language: "en",
      original_title: "The Unfamiliar",
      overview:
        "Вернувшись из командировки, Иззи находит свой дом совершенно неузнаваемым, будто в нем поселилось какое-то жуткое зло. Обратившись за помощью, она узнает, что на ее дочерей наложено проклятье. И чтобы его снять, нужно провести новый обряд.",
      popularity: 40.83,
      poster_path: "/pxfjcLtj8zehBRMr1LJXFeieTS2.jpg",
      release_date: "2020-08-21",
      title: "Вуду",
      video: !1,
      vote_average: 6,
      vote_count: 70,
    },
    {
      adult: !1,
      backdrop_path: "/vw3zNfzvnVNF7nIjpiEgcdznfeC.jpg",
      genre_ids: [16, 28, 14],
      id: 664767,
      original_language: "en",
      original_title: "Mortal Kombat Legends: Scorpion's Revenge",
      overview:
        "Чернокнижник Шан Цзун устраивает межпространственный турнир по боевым искусствам, где победитель бросит ему вызов за судьбу их родного мира. В нем решает принять участие жаждущий отомстить за смерть семьи и клана Скорпион.",
      popularity: 40.772,
      poster_path: "/t4blfxA4rIqUD49gyU6txgeaRT9.jpg",
      release_date: "2020-04-12",
      title: "Легенды «Смертельной битвы»: Месть Скорпиона",
      video: !1,
      vote_average: 8.2,
      vote_count: 1106,
    },
    {
      adult: !1,
      backdrop_path: "/mxVpkHclXPgWxo62SJ0a0YcUzgC.jpg",
      genre_ids: [10751, 14, 12, 35],
      id: 654028,
      original_language: "en",
      original_title: "The Christmas Chronicles: Part Two",
      overview:
        "Кейт выросла и стала циничным подростком, но тем не менее ей придётся объединиться с Сантой, чтобы остановить таинственного волшебника по имени Белсникель, задумавшего отобрать у человечества Рождество.",
      popularity: 40.704,
      poster_path: "/iWa1b8gHSnA7CK9lp7ebnpmK3Kc.jpg",
      release_date: "2020-11-18",
      title: "Рождественские хроники 2",
      video: !1,
      vote_average: 6.6,
      vote_count: 767,
    },
    {
      adult: !1,
      backdrop_path: "/jW44Mb6zCPzjoMc29bbAC7vUIm8.jpg",
      genre_ids: [27],
      id: 732472,
      original_language: "en",
      original_title: "The Amityville Harvest",
      overview: "",
      popularity: 40.696,
      poster_path: "/lGYf7TKZPSvbI11IwoQBvbTCuq0.jpg",
      release_date: "2020-10-20",
      title: "The Amityville Harvest",
      video: !1,
      vote_average: 6.7,
      vote_count: 19,
    },
    {
      adult: !1,
      backdrop_path: "/lP5eKh8WOcPysfELrUpGhHJGZEH.jpg",
      genre_ids: [28, 878],
      id: 338762,
      original_language: "en",
      original_title: "Bloodshot",
      overview:
        "Военный Рэй Гаррисон возвращается из очередной горячей точки к любимой красавице-жене. Супруги проводят отпуск в Европе, но счастье длится недолго - террористы, не сумев вытянуть из Рэя нужную им информацию, убивают жену, а затем и его самого. Но вскоре корпорация RST возвращает его к жизни. Армия нанороботов в крови превратила Рэя в бессмертного Бладшота, и теперь солдат наделен сверхсилой и способностью мгновенно самоисцеляться, вот только память оставляет желать лучшего. Но вскоре воспоминания о последних мгновениях жизни возвращаются, и Бладшот отправляется мстить за смерть жены.",
      popularity: 39.92,
      poster_path: "/cCm0H9R61kKtN2f4OJ7uES4nmp4.jpg",
      release_date: "2020-03-05",
      title: "Бладшот",
      video: !1,
      vote_average: 6.8,
      vote_count: 4313,
    },
    {
      adult: !1,
      backdrop_path: "/pGTXJcZQIgqzFZlWy6KxlamlTHK.jpg",
      genre_ids: [18, 10749],
      id: 621013,
      original_language: "en",
      original_title: "Chemical Hearts",
      overview:
        "17-летний Генри Пейдж - самопровозглашённый романтик, который еще никогда не влюблялся. Он ждёт свою единственную, а пока всё свободное время посвящает школьной газете. Однажды в школе появляется новенькая Грейс Таун, и Генри понимает, что в скором будущем его жизнь изменится навсегда.",
      popularity: 39.883,
      poster_path: "/oHAStvkgyweQmSJUUB1vF57XwHP.jpg",
      release_date: "2020-08-21",
      title: "Химические сердца",
      video: !1,
      vote_average: 7.5,
      vote_count: 962,
    },
    {
      adult: !1,
      backdrop_path: "/xcUf6yIheo78btFqihlRLftdR3M.jpg",
      genre_ids: [10751, 14, 12],
      id: 448119,
      original_language: "en",
      original_title: "Dolittle",
      overview:
        "Семь лет назад доктор Дулиттл, прославленный врач-ветеринар, живущий в викторианской Англии, потерял свою жену. Теперь он ведёт затворнический образ жизни, скрывшись за высокими стенами своего поместья. Экзотические животные из его коллекции — его единственная компания. Но когда неизвестная болезнь становится угрозой для жизни юной королевы, доктору Дулиттлу приходится покинуть свое убежище и отправиться в невероятное путешествие к мифическому острову. В поисках лекарства для королевы он вступит в схватку с беспощадными врагами, познакомится с диковинными существами и будет вынужден проявить недюжинное мужество и смекалку.",
      popularity: 39.772,
      poster_path: "/AwsewPKXebJvtl4EU5Dx5uBCW6G.jpg",
      release_date: "2020-01-01",
      title: "Удивительное путешествие доктора Дулиттла",
      video: !1,
      vote_average: 6.7,
      vote_count: 3e3,
    },
    {
      adult: !1,
      backdrop_path: "/2LL1jHtNKJhTDc3tA1difsAnLui.jpg",
      genre_ids: [53, 27, 35, 80],
      id: 667141,
      original_language: "en",
      original_title: "12 Hour Shift",
      overview:
        "1999 год, штат Арканзас. Медсестра Мэнди заступает на 12-часовую смену в небольшую провинциальную больницу. Мало того, что женщина употребляет на рабочем месте наркотики, так ещё и приторговывает органами. Но в этот раз её кузина-подельница Реджина потеряла ценный пакет, и бандиты угрожают её саму пустить на органы. В отчаянии Реджина проникает в больницу и теперь будет пытаться любым способом достать замену утерянной почки, а Мэнди придётся разгребать последствия деятельности бестолковой родственницы.",
      popularity: 39.644,
      poster_path: "/c3SbZPZPOfGMHq7DB7FnTIMQKuY.jpg",
      release_date: "2020-10-02",
      title: "12-часовая смена",
      video: !1,
      vote_average: 5.4,
      vote_count: 163,
    },
    {
      adult: !1,
      backdrop_path: "/dKtkvL8tScUlh67vig2Ama74Jz1.jpg",
      genre_ids: [99, 10402],
      id: 740996,
      original_language: "ko",
      original_title: "블랙핑크: 세상을 밝혀라",
      overview:
        "Рассказ о каждодневной жизни участниц южнокорейской поп-группы Blackpink и их выступлении на музыкальном фестивале Coachella в 2019 году.",
      popularity: 39.078,
      poster_path: "/vXxOhPecWpISTUrj2EB7RIA3aCw.jpg",
      release_date: "2020-10-14",
      title: "BLACKPINK: Озаряя небо",
      video: !1,
      vote_average: 8.3,
      vote_count: 426,
    },
    {
      adult: !1,
      backdrop_path: "/3GTSbc85Xangv8JkBSsQZMXGyhh.jpg",
      genre_ids: [27, 9648],
      id: 595149,
      original_language: "en",
      original_title: "Death of Me",
      overview:
        "Супруги-американцы Кристин и Нейл проводят отпуск на одном из тайских островов. В день отъезда у них всё идет наперекосяк: сначала они просыпаются грязные в разгромленной комнате и ничего не могут вспомнить, потом чуть не опаздывают на паром, а после выясняется, что пропали их паспорта. А когда пара решает восстановить в памяти события предыдущего дня и просматривает сделанную накануне видеозапись, то видит, как Нейл душит Кристин и хоронит её бездыханное тело. Странный день продолжается.",
      popularity: 38.625,
      poster_path: "/w09iZiPwvaQ3fL20ZkhT5si0WGK.jpg",
      release_date: "2020-10-02",
      title: "С днём смерти",
      video: !1,
      vote_average: 5,
      vote_count: 185,
    },
    {
      adult: !1,
      backdrop_path: "/6mKAKhj8POVGqV1GsroS5mGIUe9.jpg",
      genre_ids: [14, 28, 12],
      id: 666750,
      original_language: "en",
      original_title: "Dragonheart: Vengeance",
      overview:
        "Лукас — молодой мужчина, который был абсолютно счастливым человеком. Он воспитывал детей вместе с любимой женой и имел неплохой доход от фермерского хозяйства. Но однажды на их ферму напали неизвестные люди, которые не только разгромили все вокруг, но и убили семью Лукаса. Горем убитый мужчина поклялся сделать все возможное, чтобы отомстить за смерть любимых людей. Он отправился в путешествие, надеясь отыскать убийц. По пути герой встречает новых друзей, готовых помочь ему в столь непростом деле. Среди них оказывается опытный наемник Дарий и настоящий дракон. Впереди их ждут очень опасные приключения...",
      popularity: 38.413,
      poster_path: "/1xXvGEpIPO8qCb9bYGmVn29ejzY.jpg",
      release_date: "2020-02-04",
      title: "Сердце дракона: Возмездие",
      video: !1,
      vote_average: 7,
      vote_count: 259,
    },
    {
      adult: !1,
      backdrop_path: "/edCKQA7LnN1R7fOPPj2nBIi6xeE.jpg",
      genre_ids: [53, 27, 14],
      id: 621151,
      original_language: "en",
      original_title: "Spell",
      overview:
        "Преуспевающий нью-йоркский адвокат Маркус узнаёт о кончине отца, с которым много лет не общался. Вместе с семьёй - женой и двумя детьми-подростками - он на собственном небольшом самолёте отправляется на похороны в Кентукки, но где-то над горами Аппалачи попадает в грозу и терпит крушение. Раненый Маркус приходит в себя в доме некой пожилой женщины Элоиз. Она утверждает, что выходила его, но вопросы о семье игнорирует, и что хуже всего - злоупотребляет магией худу.",
      popularity: 38.398,
      poster_path: "/rNLSebRD5N5M69kI6LffDMQaXBO.jpg",
      release_date: "2020-10-30",
      title: "Заклинание",
      video: !1,
      vote_average: 6.4,
      vote_count: 280,
    },
    {
      adult: !1,
      backdrop_path: "/1umKVgbjFG5Cho5ZKTpcvRFJjuJ.jpg",
      genre_ids: [35, 53, 80],
      id: 609242,
      original_language: "es",
      original_title: "El robo del siglo",
      overview:
        "В самом фешенебельном пригороде Буэнос-Айреса пятеро грабителей ворвались в здание банка Рио де Акассусо. Взяв в заложники всех сотрудников и посетителей банка, они начали переговоры с прибывшей полицией. Пока сотни полицейских окружали здание, благодаря хитроумному плану и тщательной подготовке преступникам удалось вынести из хранилища деньги и ценности на общую сумму в более чем 15 миллионов долларов.",
      popularity: 38.177,
      poster_path: "/d85lhqEdxFRUB8ECnztWQ10sIoj.jpg",
      release_date: "2020-01-16",
      title: "Афера века",
      video: !1,
      vote_average: 7.8,
      vote_count: 624,
    },
    {
      adult: !1,
      backdrop_path: "/hI1IQh3LKY100iQCEzXuHiqXqmH.jpg",
      genre_ids: [27, 9648, 53, 878],
      id: 716703,
      original_language: "en",
      original_title: "What Lies Below",
      overview:
        "Мать-одиночка с 16-летней дочерью отправляются в загородный дом, где девушку ждёт сюрприз: мама встречается с мужчиной и собирается за него замуж. Разумеется, в потенциальном отчиме дочь видит угрозу, и, как вскоре выяснится, не зря. Незнакомец оказывается вовсе не человеком, а загадочным монстром. Или девушке всего лишь так кажется?",
      popularity: 38.059,
      poster_path: "/9rVp8RJOwYMWg4m5WKReKTbjmGc.jpg",
      release_date: "2020-12-17",
      title: "Что скрывает вода",
      video: !1,
      vote_average: 5.3,
      vote_count: 116,
    },
    {
      adult: !1,
      backdrop_path: "/9LsJP9OuIBmBUxZpmVKtUUjF0PA.jpg",
      genre_ids: [10749, 35],
      id: 565426,
      original_language: "en",
      original_title: "To All the Boys: P.S. I Still Love You",
      overview:
        "Лара Джин и Питер – пара, а значит, все у них отлично, не правда ли? Но в жизни девушки снова появляется давний возмутитель спокойствия, и просыпаются старые чувства.",
      popularity: 37.485,
      poster_path: "/qLSgqErnIRfWlQaCkGmqE612u9T.jpg",
      release_date: "2020-02-12",
      title: "Всем парням: P.S. Я все еще люблю тебя",
      video: !1,
      vote_average: 6.9,
      vote_count: 2199,
    },
    {
      adult: !1,
      backdrop_path: "/wKAidKCypwuxtZfZ4iP584PsWXV.jpg",
      genre_ids: [27, 878, 35],
      id: 320846,
      original_language: "en",
      original_title: "Sky Sharks",
      overview:
        "Группа геологов во время экспедиции случайно натыкаются на подземную лабораторию нацистов. Там находится тайное оружие III Рейха — оснащённая ракетами эсэсовская армия полуразумных акул-зомби, способных не только плавать, но и летать. Оказавшись на свободе, бестии нажинают пожирать всех подряд. И даже армия и ВВС им не помеха. Остановить монстров способны только специально выведенные для этого мутанты — также летающие. Грядёт великая битва…",
      popularity: 36.59,
      poster_path: "/ne2xAzlgSaWvBrYGw2LkO8W5pHi.jpg",
      release_date: "2020-10-07",
      title: "Небесные акулы",
      video: !1,
      vote_average: 5.1,
      vote_count: 63,
    },
    {
      adult: !1,
      backdrop_path: "/qtq1mai9POAX2IRGcxhVpf8f6Aj.jpg",
      genre_ids: [27],
      id: 756278,
      original_language: "en",
      original_title: "Haunting of the Mary Celeste",
      overview:
        "Чтобы разобраться в сверхъестественном исчезновении семьи Рэйчел и всей команды с торгового судна, девушка нанимает группу смелых людей и отправляется на их поиски. Она считает, что весь экипаж корабля и пассажиры попали в другое измерение и если найти разлом, то их можно спасти. Ей мало кто верит, но когда их шхуна ломается в открытом океане, а люди из команды начинают пропадать один за другим, то оставшихся охватывает неподдельный ужас...",
      popularity: 36.529,
      poster_path: "/67ITWm7Me4ThtT7v3W9BYCcP2m.jpg",
      release_date: "2020-10-23",
      title: "Призраки «Марии Целесты»",
      video: !1,
      vote_average: 5.8,
      vote_count: 28,
    },
    {
      adult: !1,
      backdrop_path: "/hwpbTeZEaKKUfiUjc2FJZpv7uAI.jpg",
      genre_ids: [27, 9648, 53],
      id: 516632,
      original_language: "en",
      original_title: "The Empty Man",
      overview:
        "Несколько лет назад опытный страж правопорядка ежедневно сталкивался лицом к лицу с опасными бандитами. Мужчине удалось упечь за решетку не одного злоумышленника, но однажды его любимая жена и маленький сын скончались при весьма трагических обстоятельствах. Овдовевший главный герой картины спустя годы не смог смириться с потерей близких людей и теперь мечтает найти виновников, несмотря на то, что смерть женщины и ребенка списали на несчастный случай. Страж порядка уверен, что члены его семьи не могли умереть случайно. Тем временем в городе бесследно исчезает молодая девушка, после чего полицейский отправляется на ее поиски. Но чем ближе он подбирается к разгадке, тем ужаснее становится окружающая действительность. Герой находит следы таинственной секты, мечтающей погрузить человечество в сущий ад. Удастся ли ему остановить сектантов?",
      popularity: 36.297,
      poster_path: "/jXfnwBR0KiLcKAsB5taMeqORygk.jpg",
      release_date: "2020-10-22",
      title: "Пустой человек",
      video: !1,
      vote_average: 6.2,
      vote_count: 435,
    },
    {
      adult: !1,
      backdrop_path: "/4qdalDaJDy6LSBNapxmCc4Wvpls.jpg",
      genre_ids: [27, 9648],
      id: 611489,
      original_language: "en",
      original_title: "Initiation",
      overview:
        "После убийства местной спортивной звезды в Университете Уайтон раскрываются невероятные подробности жизни студентов, их секреты и тайные способы коммуникации. Один из них — комментарии, состоящие из одного лишь восклицательного знака, подтекст которого окажется намного страшнее, чем кажется.",
      popularity: 36.173,
      poster_path: "/fIPLIbsqE0cN53vsupLllQdmJHg.jpg",
      release_date: "2020-10-08",
      title: "Крик. Кровавое посвящение",
      video: !1,
      vote_average: 5.8,
      vote_count: 78,
    },
    {
      adult: !1,
      backdrop_path: "/lvNQKZUsw7pg79CROCFlIk8PT67.jpg",
      genre_ids: [12, 878, 16, 14, 27],
      id: 627661,
      original_language: "en",
      original_title: "Cranston Academy: Monster Zone",
      overview:
        "Школьнику Дэнни живется несладко: он не может подружиться с одноклассниками и не находит общего языка с преподавателями. И всему виной его гениальность. Но однажды он попадает в школу для одаренных детей, где случайно открывает портал в мир монстров.",
      popularity: 35.904,
      poster_path: "/ckW2pG4AP6iCm5YbANHWMFCxQcL.jpg",
      release_date: "2020-04-29",
      title: "Академия монстров",
      video: !1,
      vote_average: 6.2,
      vote_count: 84,
    },
    {
      adult: !1,
      backdrop_path: "/cxOJXigUvhJXNydR8il68PtHr8m.jpg",
      genre_ids: [27],
      id: 627290,
      original_language: "en",
      original_title: "Antebellum",
      overview:
        "Известная писательница Вероника Хенли оказывается в ловушке ужасающей реальности и должна раскрыть безумную тайну, пока не стало слишком поздно.",
      popularity: 35.675,
      poster_path: "/7VLlc53ReBe6iZWY9NX8UBcwWSe.jpg",
      release_date: "2020-09-02",
      title: "Антебеллум",
      video: !1,
      vote_average: 6.3,
      vote_count: 700,
    },
    {
      adult: !1,
      backdrop_path: "/sQkRiQo3nLrQYMXZodDjNUJKHZV.jpg",
      genre_ids: [16, 28, 12, 14, 878],
      id: 618344,
      original_language: "en",
      original_title: "Justice League Dark: Apokolips War",
      overview:
        "Мрачнейший и масштабнейший кроссовер, самый представительный в истории DC, в котором знаковые герои, включая Бэтмена, Супермена, Чудо-Женщину, Флэша, Киборга, Константина, Болотную Тварь и других, схлестнутся с могущественным Дарксайдом - дадут ему последний и решительный бой.",
      popularity: 35.211,
      poster_path: "/m19okK4LcbngHRQbWUOXxKnAzE1.jpg",
      release_date: "2020-05-05",
      title: "Тёмная Лига справедливости: Война Апокалипсиса",
      video: !1,
      vote_average: 8.3,
      vote_count: 1177,
    },
    {
      adult: !1,
      backdrop_path: "/ezLKohe4HKsHQbwQwhv0ARo83NC.jpg",
      genre_ids: [10751, 14, 12, 35],
      id: 624963,
      original_language: "en",
      original_title: "A Babysitter's Guide to Monster Hunting",
      overview:
        "Когда первокурсница средней школы Келли Фергюсон неохотно соглашается посидеть с ребенком на Хэллоуин, ее вербуют в тайное общество нянь, которые защищают от монстров детей с особыми способностями.",
      popularity: 35.196,
      poster_path: "/bkld8Me0WiLWipLORRNfF1yIPHu.jpg",
      release_date: "2020-10-14",
      title: "Руководство для нянь: Как поймать монстра",
      video: !1,
      vote_average: 6.2,
      vote_count: 237,
    },
    {
      adult: !1,
      backdrop_path: "/2it7SlRcp5UE0G38nL6oLMuqcAx.jpg",
      genre_ids: [27],
      id: 681429,
      original_language: "es",
      original_title: "Cuidado con lo que deseas",
      overview: "",
      popularity: 34.622,
      poster_path: "/lG6Wx5tWOEHkOuexEvNYha3ofvM.jpg",
      release_date: "2020-04-24",
      title: "Осторожнее с желаниями",
      video: !1,
      vote_average: 6.6,
      vote_count: 97,
    },
    {
      adult: !1,
      backdrop_path: null,
      genre_ids: [],
      id: 782116,
      original_language: "en",
      original_title: "Thor: End of Days",
      overview: "",
      popularity: 34.537,
      poster_path: "/f8Jn9wBMCmmhS5T3CkcmqwFhsQ.jpg",
      release_date: "2020-03-31",
      title: "Thor: End of Days",
      video: !1,
      vote_average: 2,
      vote_count: 1,
    },
    {
      adult: !1,
      backdrop_path: "/tYm0Yk9k9W0hb9Xy4coOXqoGfNT.jpg",
      genre_ids: [27, 53],
      id: 492611,
      original_language: "en",
      original_title: "The Turning",
      overview:
        "Начало 1990-х. Молодая учительница начальных классов по имени Кейт принимает заманчивое предложение и устраивается гувернанткой к 8-летней сиротке, предыдущая наставница которой, как ей сообщили, просто сбежала. На месте Кейт поражает шикарный особняк, в котором живёт маленькая Флора, и так же выясняется, что у девочки есть брат-подросток, которого на днях исключили из частной школы за драку. На этом сюрпризы не заканчиваются: строгая прислуга, пожилая миссис Гроз, запрещает Флоре покидать территорию поместья, а сама Кейт в первую же ночь начинает слышать странные звуки.",
      popularity: 34.403,
      poster_path: "/hsxF47PKVRtJTrQ65AT64dYcyI0.jpg",
      release_date: "2020-01-23",
      title: "Няня",
      video: !1,
      vote_average: 5.9,
      vote_count: 787,
    },
    {
      adult: !1,
      backdrop_path: "/n6rv1hySg2Ie0UyJENwnnJWb3no.jpg",
      genre_ids: [28, 16, 12],
      id: 575249,
      original_language: "ja",
      original_title: "Dragon Ball Z: The Real 4-D at 超天下一武道会",
      overview: "",
      popularity: 292.273,
      poster_path: "/bsML7r1iijCiGjowbcUfsgtrUWH.jpg",
      release_date: "2017-06-30",
      title: "Dragon Ball Z: The Real 4-D at 超天下一武道会",
      video: !1,
      vote_average: 6.8,
      vote_count: 13,
    },
    {
      adult: !1,
      backdrop_path: "/7C921eWK06n12c1miRXnYoEu5Yv.jpg",
      genre_ids: [12, 28, 14],
      id: 166426,
      original_language: "en",
      original_title: "Pirates of the Caribbean: Dead Men Tell No Tales",
      overview:
        "Исчерпавший свою удачу капитан Джек «Воробей» обнаруживает, что за ним охотится его старый неприятель, ужасный капитан Салазар и его призрачные пираты. Они только что сбежали из Дьявольского треугольника и намерены уничтожить всех пиратов, включая Джека. Поможет спастись лишь могущественный артефакт — трезубец Посейдона, который дарует своему обладателю полный контроль над морями.",
      popularity: 196.659,
      poster_path: "/jkrk8GyiEQ4heRRZivEHdM43BJ2.jpg",
      release_date: "2017-05-23",
      title: "Пираты Карибского моря: Мертвецы не рассказывают сказки",
      video: !1,
      vote_average: 6.6,
      vote_count: 10256,
    },
    {
      adult: !1,
      backdrop_path: "/askg3SMvhqEl4OL52YuvdtY40Yb.jpg",
      genre_ids: [10751, 16, 14, 10402, 35, 12],
      id: 354912,
      original_language: "en",
      original_title: "Coco",
      overview:
        "12-летний Мигель живет в мексиканской деревушке в семье сапожников и тайно мечтает стать музыкантом. Тайно — потому что в его семейном клане музыка считается проклятием. Когда-то его прадед оставил свою жену, прабабку Мигеля, ради мечты, которая теперь не дает спокойно жить Мигелю. С тех пор музыкальная тема в семье стала табу. Мигель обнаруживает, что между ним и его любимым певцом Эрнесто де ла Крусом, ныне покойным, существует некая — пока неназванная — связь. Паренек отправляется к своему кумиру в Страну Мертвых, где встречает души своих предков. Коко знакомится там с духом-трикстером по имени Эктор (в облике скелета), который становится его напарником-проводником. Вдвоем они отправляются на поиски де ла Круса.",
      popularity: 195.808,
      poster_path: "/lk1sAbyngNwiy0CcOzLRTRiOnaB.jpg",
      release_date: "2017-10-27",
      title: "Тайна Коко",
      video: !1,
      vote_average: 8.2,
      vote_count: 15785,
    },
    {
      adult: !1,
      backdrop_path: "/6G2fLCVm9fiLyHvBrccq6GSe2ih.jpg",
      genre_ids: [28, 12, 14, 878],
      id: 284053,
      original_language: "en",
      original_title: "Thor: Ragnarok",
      overview:
        "Вернувшись в Асгард в поисках таинственного врага, ведущего охоту на Камни Бесконечности, Тор обнаруживает, что действия его брата Локи, захватившего трон Асгарда, привели к приближению наиболее страшного события — Рагнарёка. По легенде, это ознаменует последнюю битву Асгарда, последствием которой станет его полное уничтожение. В попытке предотвратить это событие, Тору предстоит прибегнуть к помощи своего товарища из Мстителей — Халка. Вместе, им предстоит столкнуться лицом к лицу со злейшим врагом всех девяти миров — огненным демоном Суртуром, целью которого является исполнение пророчества о Рагнарёке и уничтожение девяти миров.",
      popularity: 176.616,
      poster_path: "/if5g9noo6QgMOIk6gU7bCzMNWUy.jpg",
      release_date: "2017-10-24",
      title: "Тор: Рагнарёк",
      video: !1,
      vote_average: 7.6,
      vote_count: 17781,
    },
    {
      adult: !1,
      backdrop_path: "/gYtXT7fJNBRZBbBulO11DoRveLT.jpg",
      genre_ids: [18, 35],
      id: 400928,
      original_language: "en",
      original_title: "Gifted",
      overview:
        "Фрэнк Адлер живёт в прибрежном городке во Флориде и воспитывает в одиночку свою необычайно одарённую племянницу Мэри. Но планы парня о спокойной школьной жизни для девочки рушатся, когда о математических способностях ребёнка узнаёт грозная мать Фрэнка Эвелин. У бабушки свои представления о будущем внучки, и ради этого она готова даже разлучить Мэри с Фрэнком.",
      popularity: 170.387,
      poster_path: "/l606JniLginmPRiUxsba1dY3atL.jpg",
      release_date: "2017-04-12",
      title: "Одарённая",
      video: !1,
      vote_average: 8,
      vote_count: 4398,
    },
    {
      adult: !1,
      backdrop_path: "/daOd8KBGDdsOhVdzXB3KU5fgBd2.jpg",
      genre_ids: [18],
      id: 453354,
      original_language: "fr",
      original_title: "Ava",
      overview:
        "Тринадцатилетняя Ава проводит каникулы на побережье океана. Она узнает о том, что потеряет зрение быстрее, чем предполагалось. Ее мама решает вести себя, как ни в чем не бывало, чтобы это лето стало незабываемым. Но Ава начинает вести себя непредсказуемо. Она крадет большую черную собаку, которая принадлежит молодому человеку, находящемуся в бегах...",
      popularity: 163.563,
      poster_path: "/cUI1PFsrQVJR3IZOpkfsOOOUhTO.jpg",
      release_date: "2017-06-21",
      title: "Ава",
      video: !1,
      vote_average: 6.2,
      vote_count: 152,
    },
    {
      adult: !1,
      backdrop_path: "/tTlAA0REGPXSZPBfWyTW9ipIv1I.jpg",
      genre_ids: [28, 12, 878, 18],
      id: 315635,
      original_language: "en",
      original_title: "Spider-Man: Homecoming",
      overview:
        "Казалось бы, Питер Паркер всего лишь обычный школьник, живущий в Нью-Йорке, и занимающийся абсолютно тем же, что и его многочисленные сверстники. Но это только обман зрения, ведь на самом деле, он обладает особыми способностями после того, как его укусил радиоактивный паук. Следуя кредо его дяди Бэна «с великой силой приходит великая ответственность», Питер становится Человеком-пауком — героем, который пытается помогать людям. Но каково это быть героем в таком юном возрасте, особенно, когда ты находишься в мире, где существуют Мстители? Преследуемого подобными мыслями Человека-паука ждёт встреча с новым могущественным врагом, чьи действия могут уничтожить весь город.",
      popularity: 155.039,
      poster_path: "/20KscwCTqRIC0fmh6oQog50PrJ1.jpg",
      release_date: "2017-07-05",
      title: "Человек-паук: Возвращение домой",
      video: !1,
      vote_average: 7.4,
      vote_count: 18722,
    },
    {
      adult: !1,
      backdrop_path: "/rAQcPrEaPzDRVNX7XX5TWyxCGFN.jpg",
      genre_ids: [27, 14],
      id: 346364,
      original_language: "en",
      original_title: "It",
      overview:
        "Когда в городке Дерри, штат Мэн, начинают пропадать дети, несколько ребят сталкиваются со своими величайшими страхами и вынуждены помериться силами со злобным клоуном Пеннивайзом, чьи проявления жестокости и список жертв уходят в глубь веков.",
      popularity: 143.518,
      poster_path: "/wEHpeWhH3zGYJbqz3OtUJ4tMMM6.jpg",
      release_date: "2017-09-06",
      title: "Оно",
      video: !1,
      vote_average: 7.2,
      vote_count: 16543,
    },
    {
      adult: !1,
      backdrop_path: "/uO6ZUez70Twan92pqoV60T9kiGH.jpg",
      genre_ids: [99],
      id: 440249,
      original_language: "en",
      original_title: "After Porn Ends 2",
      overview:
        "Фильм знакомит с одними из самых старых звезд индустрии для взрослых и рассказывает об их жизни по завершении карьеры. Кто-то принимает её с гордостью и без сожаления, как путь к обретению гармонии и счастья. Других карьера в порно привела к отчаянию и страданиям, и они всеми силами пытаются избавиться от последствий работы в индустрии.",
      popularity: 129.99,
      poster_path: "/rfItXrtDGILwsCdmgVxX79phFuI.jpg",
      release_date: "2017-03-28",
      title: "Жизнь после карьеры в порно 2",
      video: !1,
      vote_average: 5.4,
      vote_count: 197,
    },
    {
      adult: !1,
      backdrop_path: "/r17jFHAemzcWPPtoO0UxjIX0xas.jpg",
      genre_ids: [53, 28, 80],
      id: 324552,
      original_language: "en",
      original_title: "John Wick: Chapter 2",
      overview:
        "Когда бывший коллега Джона решает взять под свой контроль таинственную гильдию убийц, Уик вынужден выйти из отставки. Ведомый кровавой клятвой Джон отправляется в Рим, где ему придется сразиться с одними из самых опасных киллеров в мире.",
      popularity: 128.568,
      poster_path: "/jB5n93niK7bgaK3Dq4LbVpQN7RB.jpg",
      release_date: "2017-02-08",
      title: "Джон Уик 2",
      video: !1,
      vote_average: 7.3,
      vote_count: 10255,
    },
    {
      adult: !1,
      backdrop_path: "/9dWH18IZf0KdGx0kJaONzWcmD69.jpg",
      genre_ids: [18, 10749],
      id: 341174,
      original_language: "en",
      original_title: "Fifty Shades Darker",
      overview:
        "Пока Кристиан Грей борется со своими внутренними демонами, Анастейша Стил вынуждена противостоять гневу и зависти тех женщин, что были с Кристианом до нее.",
      popularity: 127.437,
      poster_path: "/5ZX9Y6biaDqpdz6vAzH46ydsGec.jpg",
      release_date: "2017-02-08",
      title: "На пятьдесят оттенков темнее",
      video: !1,
      vote_average: 6.5,
      vote_count: 6731,
    },
    {
      adult: !1,
      backdrop_path: "/jLS805N4QmLVPurQwuaCFTNEZss.jpg",
      genre_ids: [10751, 16, 12, 14],
      id: 436931,
      original_language: "ja",
      original_title: "劇場版 ポケットモンスター　キミにきめた！",
      overview:
        "Эш собирается посетить лабораторию профессора Оука, чтобы получить своего первого покемона. Но профессор спешит сообщить, что три стартовых покемона — Бульбазавр, Сквиртл и Чармандер — уже были выбраны другими тренерами, которые пришли раньше. Профессор предлагает Эшу оставшегося покемона электрического типа — Пикачу. Из-за упрямого характера Пикачу отказывается быть внутри покебола, но Эш с радостью принимает его и отправляется в далёкое путешествие.",
      popularity: 126.893,
      poster_path: "/mizn0hTHPuVridNt0GTGbEazg9Q.jpg",
      release_date: "2017-07-15",
      title: "Покемон: Я выбираю тебя!",
      video: !1,
      vote_average: 7,
      vote_count: 514,
    },
    {
      adult: !1,
      backdrop_path: "/4mlS1MitcOqdPLhxEFyk9Qwf5rr.jpg",
      genre_ids: [16, 12, 35, 10751],
      id: 260514,
      original_language: "en",
      original_title: "Cars 3",
      overview:
        "Встречайте самого знаменитого гонщика всех времен, легенду гоночного трека Молнию Маккуина! Герой все еще продолжает показывать лучшие результаты на всех турнирах, однако победы даются ему не так легко, а конкуренция становится все жестче. Молнию Маккуина ждут захватывающие приключения, где он встретит новых друзей, а также поймет, что не только скорость и мощность мотора делают гонщика чемпионом.",
      popularity: 123.822,
      poster_path: "/mkoUvZE7KEcrToftEEAmM1HoN6f.jpg",
      release_date: "2017-06-15",
      title: "Тачки 3",
      video: !1,
      vote_average: 6.8,
      vote_count: 4567,
    },
    {
      adult: !1,
      backdrop_path: "/bAup7FGUlmNVOuhZFlLgmDPo0Eg.jpg",
      genre_ids: [18, 14],
      id: 345938,
      original_language: "en",
      original_title: "The Shack",
      overview:
        "Мистические события происходят в судьбе главного героя, переживающего трагедию. Он получает личное приглашение встретиться с самим Всевышним в очень уединенном месте…",
      popularity: 123.524,
      poster_path: "/4ODDOXvBAMp8RMEJmChgBV0mrp2.jpg",
      release_date: "2017-03-03",
      title: "Хижина",
      video: !1,
      vote_average: 7.4,
      vote_count: 1690,
    },
    {
      adult: !1,
      backdrop_path: "/xpFIaHL7Rm5kKLcEPDEMpjh4Xq6.jpg",
      genre_ids: [12, 28, 878],
      id: 283995,
      original_language: "en",
      original_title: "Guardians of the Galaxy Vol. 2",
      overview:
        "Все в сборе: землянин Питер Квилл (Звездный Лорд), молчаливый громила Дракс, зеленокожая наемница Гамора, живое дерево Грут и говорящий енот. Герои не изменяют себе и с завидной регулярностью продолжают попадать в немыслимые ситуации, выпутываясь из них почти без ущерба (а иногда даже с пользой) для окружающих. На этот раз им предстоит раскрыть одну из самых главных тайн во всей Галактике: кто же на самом деле отец Питера Квилла?",
      popularity: 122.317,
      poster_path: "/miAFwzWPkv7l8VPwgH2sV5oQQKx.jpg",
      release_date: "2017-04-19",
      title: "Стражи Галактики. Часть 2",
      video: !1,
      vote_average: 7.6,
      vote_count: 18408,
    },
    {
      adult: !1,
      backdrop_path: "/t6TL7V9IsT4Ktx6YO8b1UYj3agd.jpg",
      genre_ids: [35, 28, 80],
      id: 339846,
      original_language: "en",
      original_title: "Baywatch",
      overview:
        "Группа пляжных спасателей работает на побережье Тихого океана. Они всегда находятся на своём посту и охраняют отдыхающих от несчастных случаев на воде. Главный герой, опытный спасатель Митч Бьюкеннон, и его молодой коллега Мэтт Броди случайно узнают, что их пляж находится под угрозой исчезновения. Большая нефтяная компания собирается разрушить побережье, преследуя личные интересы руководства, крепко завязанные на финансах.",
      popularity: 113.232,
      poster_path: "/s4T3R588ESvNTEuIizFyftzz8Kt.jpg",
      release_date: "2017-05-12",
      title: "Спасатели Малибу",
      video: !1,
      vote_average: 6.1,
      vote_count: 7020,
    },
    {
      adult: !1,
      backdrop_path: "/syFtR7W6eO7w9cK6PGjLsJhqBNz.jpg",
      genre_ids: [16],
      id: 699570,
      original_language: "ja",
      original_title: "Boruto: Jump Festa Special",
      overview: "",
      popularity: 112.202,
      poster_path: "/9AOtmiqHCH82TxCoCJmfkGcCz1s.jpg",
      release_date: "2017-07-27",
      title: "Boruto: Jump Festa Special",
      video: !1,
      vote_average: 5.9,
      vote_count: 12,
    },
    {
      adult: !1,
      backdrop_path: "/4QLdZ2A8mkDWp2rpfgDrwmeCtUW.jpg",
      genre_ids: [28, 12, 80],
      id: 47971,
      original_language: "en",
      original_title: "xXx: Return of Xander Cage",
      overview:
        "Все думали, что Ксандер вместе со своей девушкой Еленой погиб при таинственных обстоятельствах. Но это оказалось всего лишь прикрытие. Прошло 8 лет после того как любитель экстрима Ксандер Кейдж пропал из вида АНБ после якобы его убийства на Бора-Бора. Но теперь он возвращается из самоизгнания в секретную службу и готов выполнять новые сверхсекретные задания. Его новая миссия будет направлена на поиск секретного оружия невероятной мощности, которое несёт угрожающее название — «Ящик Пандоры». Так же Кейджу будет противостоять синдикат мировых преступников под руководством альфа-воина Сяна. У которого свои планы на этот экспонат. В добавок ко всему Ксандера не покидает мысль о кроте в организации и заговора на самом высоком уровне. От исхода их противоборства теперь вновь стоит судьба все обитателей Земли.",
      popularity: 105.397,
      poster_path: "/hnsmpmDM0NyODTbIy01Gv6l5L3D.jpg",
      release_date: "2017-01-13",
      title: "Три икса: Мировое господство",
      video: !1,
      vote_average: 5.9,
      vote_count: 6006,
    },
    {
      adult: !1,
      backdrop_path: "/jorgjEk6a0bed6jdR5wu4S6ZvRm.jpg",
      genre_ids: [28, 12, 878],
      id: 141052,
      original_language: "en",
      original_title: "Justice League",
      overview:
        "Вдохновившись самопожертвованием Супермена, Брюс Уэйн вновь обретает веру в человечество. Он заручается поддержкой новой союзницы Дианы Принс, чтобы сразиться с ещё более могущественным противником. Бэтмен и Чудо-Женщина быстро набирают команду сверхлюдей для борьбы с пробудившейся угрозой. Но, несмотря на уникальный состав отряда супергероев — Бэтмен, Чудо-Женщина, Аквамен, Киборг и Флэш, — быть может, спасать планету от вторжения катастрофических масштабов уже слишком поздно.",
      popularity: 102.037,
      poster_path: "/e2f1GaWLkk5Sj7cZMi38mUPXYdt.jpg",
      release_date: "2017-11-15",
      title: "Лига справедливости",
      video: !1,
      vote_average: 6.1,
      vote_count: 11397,
    },
    {
      adult: !1,
      backdrop_path: "/s1eUjN5hLpez9QoBK8t4rUoopgN.jpg",
      genre_ids: [28, 12, 35, 14, 16],
      id: 433422,
      original_language: "ja",
      original_title: "劇場版 FAIRY TAIL 『DRAGON CRY』",
      overview:
        'События второго по счёту фильма из вселенной "Сказка о Хвосте феи” вертятся вокруг некоего Плача дракона. Именно так называется таинственный артефакт, обладающий воистину нешуточной силой. Не преувеличением будет сказать, что Плач дракона настолько могущественен, что способен создать катаклизм, который уничтожит весь мир. Конечно, подобные вещи просто не могут лежать на своём законном месте, всегда найдётся тот, кто всеми фибрами души захочет прикарманить подобную игрушку себе. Так случилось и в нашей истории, когда предатель королевства Заш выкрал артефакт и доставил его Анимусу, монарху королевства Стеллы.',
      popularity: 95.685,
      poster_path: "/rTNk0cZ0CnTp7EMndtaJVLUUffh.jpg",
      release_date: "2017-05-06",
      title: "Сказка о хвосте феи - Плач дракона",
      video: !1,
      vote_average: 6.5,
      vote_count: 355,
    },
    {
      adult: !1,
      backdrop_path: "/yuxdzbQHEecWGOvaXeEYVtIyAfn.jpg",
      genre_ids: [878, 27],
      id: 515454,
      original_language: "en",
      original_title: "Come True",
      overview:
        "18-летняя Сара убежала из дома и теперь ночует под открытым небом на детской площадке. Каждую ночь ей снятся странные пугающие образы, поэтому когда девушка видит объявление института изучения сна о наборе добровольцев, она хватается за эту возможность. Тела испытуемых облепляют датчиками, и после первой ночи Сара чувствует себя отдохнувшей, но когда исследователи показывают ей смутно узнаваемые чёрно-белые снимки, у девушки начинается приступ.",
      popularity: 34.373,
      poster_path: "/ic6AlIbiMwRJFKjoRy3w5HSvqF9.jpg",
      release_date: "2020-09-24",
      title: "Кошмары",
      video: !1,
      vote_average: 6.1,
      vote_count: 155,
    },
    {
      adult: !1,
      backdrop_path: "/8LI4OEQGY2DIBTQQeRjYVKfL9Dl.jpg",
      genre_ids: [27, 53, 10752, 14],
      id: 608195,
      original_language: "en",
      original_title: "Ghosts of War",
      overview:
        "Французская провинция, 1944 год. Отряд из пяти американских солдат получают приказ охранять особняк, в котором ещё недавно располагалось нацистское верховное командование. Передав дежурство прибывшим, предыдущий отряд, несколько дней проживший в этом доме, подозрительно быстро ретируется, а новенькие надеются на долгожданную передышку и относительный комфорт. Но почти сразу они начинают слышать и видеть нечто, что заставляет содрогаться даже видавших ужасы войны.",
      popularity: 34.265,
      poster_path: "/iY1YX4XiyB9jhrZHBCGofCKJIQS.jpg",
      release_date: "2020-07-03",
      title: "Призраки войны",
      video: !1,
      vote_average: 6,
      vote_count: 312,
    },
    {
      adult: !1,
      backdrop_path: "/sdVZxrCFK6MB62Kxv3v9BUbCng3.jpg",
      genre_ids: [35, 18, 80],
      id: 725649,
      original_language: "es",
      original_title: "Lavaperros",
      overview: "",
      popularity: 34.199,
      poster_path: "/rtX6TPhoNACLQOTOZmTxc5r4R67.jpg",
      release_date: "2020-10-09",
      title: "Lavaperros",
      video: !1,
      vote_average: 6.5,
      vote_count: 31,
    },
    {
      adult: !1,
      backdrop_path: "/flNp7NdzBC1UGctzuubQveuVSF5.jpg",
      genre_ids: [27, 53, 9648],
      id: 748224,
      original_language: "en",
      original_title: "Reaptown",
      overview:
        "Керри Болдуин выходит из тюрьмы досрочно с условием отработки оставшейся части срока в железнодорожном депо. Но её главная цель - найти младшую сестру, которая недавно вышла на таких же условиях и пропала без вести.",
      popularity: 34.149,
      poster_path: "/wyN1R51euy3c2YHsyiSnc5ovF5t.jpg",
      release_date: "2020-09-28",
      title: "Риптаун",
      video: !1,
      vote_average: 4.6,
      vote_count: 10,
    },
    {
      adult: !1,
      backdrop_path: "/dqaobSdoRTEA3rRDhiWnrtNd7fB.jpg",
      genre_ids: [27],
      id: 758510,
      original_language: "pt",
      original_title: "A Gruta",
      overview: "",
      popularity: 33.956,
      poster_path: "/nBdoS8tjWubpEyQnqmM6tpZR3GU.jpg",
      release_date: "2020-10-29",
      title: "A Gruta",
      video: !1,
      vote_average: 6.4,
      vote_count: 65,
    },
    {
      adult: !1,
      backdrop_path: "/ynqCG7RxpH6jVPGIbGtzNFRAWEq.jpg",
      genre_ids: [16],
      id: 891657,
      original_language: "en",
      original_title: "LEGO Hidden Side: Night of the Harbinger",
      overview: "",
      popularity: 33.866,
      poster_path: "/yoDchepiE9Tcv36ivkzOF4z75ws.jpg",
      release_date: "2020-10-31",
      title: "LEGO Hidden Side: Night of the Harbinger",
      video: !1,
      vote_average: 6.1,
      vote_count: 5,
    },
    {
      adult: !1,
      backdrop_path: "/y7M0AC5D8A5wCuyYaN2Y4ErmMQU.jpg",
      genre_ids: [27],
      id: 733491,
      original_language: "en",
      original_title: "They Live Inside Us",
      overview:
        "После ухода жены, писатель Джейк вместе со своей дочерью-инвалидом Дэни решают провести Хэллоуин в печально известном доме с приведениями. Мужчина надеется, что в этот вечер к нему наконец придет вдохновение написать ту самую историю ужасов, но вскоре осознает, что оказался заложником своего же сценария.",
      popularity: 32.999,
      poster_path: "/hVGBsF3MvHuBxLsAaBVjNoU58X.jpg",
      release_date: "2020-10-06",
      title: "Они живут внутри нас",
      video: !1,
      vote_average: 5.7,
      vote_count: 26,
    },
    {
      adult: !1,
      backdrop_path: "/eShw0LB5CkoEfYtpUcXPD85oz5Q.jpg",
      genre_ids: [35, 14, 27, 53],
      id: 551804,
      original_language: "en",
      original_title: "Freaky",
      overview:
        "В результате странного инцидента старшеклассница оказывается в теле жестокого серийного убийцы. У неё есть меньше 24 часов, чтобы всё исправить, после чего ситуацию будет уже не обратить.",
      popularity: 32.957,
      poster_path: "/xEGrAhg9nCdCzL3ZzsoYSKXmk9K.jpg",
      release_date: "2020-11-12",
      title: "Дичь",
      video: !1,
      vote_average: 6.6,
      vote_count: 1066,
    },
    {
      adult: !1,
      backdrop_path: "/uu4L7SR5xpGyZZPIUlxKRTW1CUi.jpg",
      genre_ids: [27, 53],
      id: 728791,
      original_language: "en",
      original_title: "Playhouse",
      overview:
        "Известный писатель в компании дочери переезжает в удаленный замок для написания очередного шедевра в жанре ужасов. Он выбрал для этого очень подходящее, но очень опасное место, ведь в стенах постройки затаилось древнейшее зло.",
      popularity: 32.808,
      poster_path: "/mFzzCkJWcKw0Az0kMLOurSpbLB0.jpg",
      release_date: "2020-11-17",
      title: "Проклятый дом 4",
      video: !1,
      vote_average: 5.5,
      vote_count: 21,
    },
    {
      adult: !1,
      backdrop_path: "/1fOwb62XudDlevVyu1rqUlHUrES.jpg",
      genre_ids: [27, 878, 53],
      id: 716258,
      original_language: "en",
      original_title: "Black Box",
      overview:
        "Некоторое время назад Нолан потерял в автокатастрофе не только любимую жену, но собственные воспоминания. Теперь он страдает от амнезии и странных видений, в которых на него кто-то или что-то набрасывается. Забыв в очередной раз забрать маленькую дочь из школы и столкнувшись с перспективой общения с органами по защите детей, Нолан решается на экспериментальное лечение. Но существо из кошмаров никуда не пропадает и подкрадывается всё ближе.",
      popularity: 32.696,
      poster_path: "/ldlpTitpSTBINwgifzlRRXveXUF.jpg",
      release_date: "2020-10-06",
      title: "Чёрный ящик",
      video: !1,
      vote_average: 6.5,
      vote_count: 398,
    },
    {
      adult: !1,
      backdrop_path: "/g6qb2h3dT1tb0zkhktD5r7ZHbmp.jpg",
      genre_ids: [27, 878],
      id: 715123,
      original_language: "en",
      original_title: "Books of Blood",
      overview: "Три страшные истории, запутанные в пространстве и времени.",
      popularity: 32.576,
      poster_path: "/bXLVZGgLgd7OwbfP6fC5GCfrxuD.jpg",
      release_date: "2020-10-07",
      title: "Книги крови",
      video: !1,
      vote_average: 6.6,
      vote_count: 257,
    },
    {
      adult: !1,
      backdrop_path: "/tXUmghBMpKE2NvpNtMCYzofIazQ.jpg",
      genre_ids: [10749, 18],
      id: 602301,
      original_language: "ko",
      original_title: "조제",
      overview:
        "Студент Ён-сок помогает на улице девушке в инвалидном кресле и отвозит её домой. Та живет в ветхом доме со старенькой бабушкой и предпочитает реальному мир фантазий и книг, даже просит называть её именем героини романа Франсуазы Саган — Жозе. Ён-сок решает по мере сил помогать Жозе и вскоре понимает, что его тянет к этой странной девушке.",
      popularity: 32.355,
      poster_path: "/vZrFO6NvjOAGofKNn4S5bAC2eR5.jpg",
      release_date: "2020-12-10",
      title: "Жозе",
      video: !1,
      vote_average: 7.6,
      vote_count: 31,
    },
    {
      adult: !1,
      backdrop_path: null,
      genre_ids: [27, 18],
      id: 718022,
      original_language: "en",
      original_title: "Heks",
      overview:
        "Скорбящая молодая девушка возвращается из Англии в родную Южно-Африканскую глубинку, чтобы пролить свет на убийство своей матери, которое может быть как-то связано с держащей в страхе всю округу знахаркой.",
      popularity: 32.188,
      poster_path: "/yu7ruzNuhhlS9vuYGDwcqXomHIM.jpg",
      release_date: "2020-12-15",
      title: "Порча",
      video: !1,
      vote_average: 6.3,
      vote_count: 27,
    },
    {
      adult: !1,
      backdrop_path: "/1WDITmJAHz4rAnjVv1XYN4IfjKq.jpg",
      genre_ids: [27],
      id: 593655,
      original_language: "en",
      original_title: "Castle Freak",
      overview:
        "Недавно ослепшая Ребекка, ее парень Джон и их друзья едут в Албанию, чтобы управлять неожиданно доставшимся в наследство Ребекке замком от ее давно потерянной матери. Прибыв на место, они узнают, что у семьи Ребекки были мрачные тайны, последствия которых могут быть колоссальными.",
      popularity: 31.975,
      poster_path: "/HJEosQRIdBzAHEdZJCUF7e5FmF.jpg",
      release_date: "2020-12-04",
      title: "Урод в замке",
      video: !1,
      vote_average: 4.8,
      vote_count: 40,
    },
    {
      adult: !1,
      backdrop_path: "/iDw6J49rf7Xlu9QZBWpvmrMqSzI.jpg",
      genre_ids: [53, 9648, 878],
      id: 575604,
      original_language: "ko",
      original_title: "콜",
      overview:
        "Две женщины живут в одном доме и общаются по домашнему телефону, но их разделяет 20 лет: одна живёт в 2019 году, другая — в 1999. Сначала между ними завязывается дружба, и девушка из настоящего решает предотвратить трагическое событие в прошлом своей новой знакомой, но вскоре это странным образом начинает влиять на неё саму.",
      popularity: 31.893,
      poster_path: "/mBqqTUDBVNqSnJMd6N3W6YGFVNn.jpg",
      release_date: "2020-11-27",
      title: "Звонок из прошлого",
      video: !1,
      vote_average: 7.6,
      vote_count: 511,
    },
    {
      adult: !1,
      backdrop_path: "/gMRBMlcgp5yQeqJ0OAff0uyZfnw.jpg",
      genre_ids: [12, 10751],
      id: 657811,
      original_language: "de",
      original_title: "Lassie - Eine abenteuerliche Reise",
      overview:
        "Двенадцатилетний Флориан и его колли Лесси — неразлучные друзья. Они счастливо живут в немецкой деревеньке. Когда отец Флориана лишается работы, семье приходится переезжать в дом поменьше, т. к. с собаками туда нельзя. Флориану приходится расстаться с Лесси. Новым хозяином становится граф фон Шпренгель, который отправляется в Северное море вместе с ней и своенравной внучкой Присциллой. При первой возможности Лесси сбегает и ей предстоит долгий путь обратно к Флориану.",
      popularity: 31.791,
      poster_path: "/rtCNutCLezxHaSgELtpBFFGJ9Be.jpg",
      release_date: "2020-02-20",
      title: "Лесси. Возвращение домой",
      video: !1,
      vote_average: 6.6,
      vote_count: 86,
    },
    {
      adult: !1,
      backdrop_path: "/aLDQl8iUzrapHOob3VHgmyX1ijj.jpg",
      genre_ids: [878, 35, 27],
      id: 670428,
      original_language: "en",
      original_title: "Psycho Goreman",
      overview:
        "Брат с сестрой Люк и Мими невольно воскрешают древнего военачальника инопланетных пришельцев. Используя волшебный амулет, они заставляют монстра повиноваться своим детским прихотям и случайно вызывают целую армию межгалактических убийц в родной городок.",
      popularity: 31.761,
      poster_path: "/foQnDv051kQ7satMQkbkPsUxzkD.jpg",
      release_date: "2020-10-29",
      title: "Псих-расчленитель",
      video: !1,
      vote_average: 6.1,
      vote_count: 222,
    },
    {
      adult: !1,
      backdrop_path: "/6aVB2B2GDc4EuNinHgoBgtkuHQz.jpg",
      genre_ids: [18, 27, 53],
      id: 575774,
      original_language: "en",
      original_title: "His House",
      overview:
        "Бол и Риал с дочкой бегут из охваченного войной Южного Судана. Когда группа беженцев на хлипкой лодке пересекает море, происходит несчастный случай, и девочка тонет. Через некоторое время в Великобритании пара получает статус иммигрантов и возможность адаптироваться - им выдают жильё без возможности переехать. Но первоначальная радость быстро сменяется тревогой, так как по ночам в новом доме супруги замечают потустороннюю сущность.",
      popularity: 31.524,
      poster_path: "/s6XxJEe4ovVTMgmGmKeO87OFANU.jpg",
      release_date: "2020-01-27",
      title: "Его дом",
      video: !1,
      vote_average: 6.5,
      vote_count: 794,
    },
    {
      adult: !1,
      backdrop_path: "/brLBKS5vQfP1vjiupHqa3xg1XQC.jpg",
      genre_ids: [14, 12, 35],
      id: 520725,
      original_language: "fr",
      original_title: "Le prince oublié",
      overview:
        "Главный герой картины — Джиби, отец-одиночка, чья жизнь вращается вокруг его дочери Софии. Каждую ночь, когда она засыпает, Джиби приводит дочку на фантастическую киностудию «Страна историй». Там оживают невероятные и сказочные приключения, где Джиби играет героического Очаровательного Принца. С возрастом интересы Софии постепенно меняются, и она начинает придумывать собственные истории, отодвигая Джиби на второй план. Но отец не теряет надежд вновь стать для дочери полноценным героем.",
      popularity: 31.185,
      poster_path: "/9Ul4FeVDyZBNaoPPJIdX8RSDR7J.jpg",
      release_date: "2020-02-12",
      title: "Папина дочка",
      video: !1,
      vote_average: 5.8,
      vote_count: 285,
    },
    {
      adult: !1,
      backdrop_path: "/8JWfh0F9PE7nvg2S3HNnrxbKjuc.jpg",
      genre_ids: [27],
      id: 738542,
      original_language: "en",
      original_title: "The Haunting of Margam Castle",
      overview:
        "Группа американских парапсихологов едет в Уэльс, чтобы провести исследование замка Маргам, одного из самых густонаселенных призраками зданий в Великобритании.",
      popularity: 30.963,
      poster_path: "/r2BlIxjAaj0WERGrIehnczNE0On.jpg",
      release_date: "2020-09-01",
      title: "Призраки замка Маргам",
      video: !1,
      vote_average: 5.8,
      vote_count: 38,
    },
    {
      adult: !1,
      backdrop_path: "/hwBFGNabkGv2ZIyFUnCTSSmprf7.jpg",
      genre_ids: [27, 14],
      id: 714338,
      original_language: "hi",
      original_title: "Bulbbul",
      overview:
        "Как это принято в некоторых районах Индии, мальчик и девочка были помолвлены, но парень не спешил жениться, вырос и уехал. А вернувшись в родные края, обнаружил, что там свирепствует злобный ночной демон, убивающий только мужчин. Местные считают, что его суженая обернулась мифической сущностью и таким кровожадным способом скорбит о своей потере.",
      popularity: 30.829,
      poster_path: "/4XYEqHqvcf6vxFhNyeKZz5xbUfV.jpg",
      release_date: "2020-06-24",
      title: "Призрак в кронах деревьев",
      video: !1,
      vote_average: 6.7,
      vote_count: 86,
    },
    {
      adult: !1,
      backdrop_path: "/o0F8xAt8YuEm5mEZviX5pEFC12y.jpg",
      genre_ids: [12, 14, 878, 10751, 28],
      id: 475430,
      original_language: "en",
      original_title: "Artemis Fowl",
      overview:
        "Укрытый от посторонних глаз, глубоко под поверхностью земной коры таится целый мир, населенный гномами, эльфами, пикси, лепреконами, брауни и другими волшебными существами. Они спрятались от людей и создали свою собственную весьма развитую цивилизацию. Их тайна так и оставалась бы нераскрытой, если бы не юный Артемис Фаул, прирождённый мошенник и единственный наследник древнего рода криминальных гениев. Он сумел не только найти доказательства существования подземного мира, но и придумал дерзкий план — ограбить его обитателей.",
      popularity: 30.799,
      poster_path: "/d2HB9Egx0ODeFzCCMVU3Kvxas3B.jpg",
      release_date: "2020-06-12",
      title: "Артемис Фаул",
      video: !1,
      vote_average: 5.7,
      vote_count: 1374,
    },
    {
      adult: !1,
      backdrop_path: "/y6J8TOnSUscFeMgvWxc6xeEEXo7.jpg",
      genre_ids: [16, 35, 10751, 10749],
      id: 673595,
      original_language: "en",
      original_title: "Maggie Simpson in Playdate with Destiny",
      overview:
        "Когда Мэгги влюбляется в маленького мальчика, Мардж устраивает ей свидание в лучшем игровом парке для малышей Спрингфилда.",
      popularity: 30.547,
      poster_path: "/izToROD1OSsUIIuOerBIvflfcls.jpg",
      release_date: "2020-02-29",
      title: "Игра в судьбу",
      video: !1,
      vote_average: 7.5,
      vote_count: 299,
    },
    {
      adult: !1,
      backdrop_path: "/jeiVmutYPZyQRSYk7OvyWENGIrA.jpg",
      genre_ids: [27, 9648, 53],
      id: 555974,
      original_language: "en",
      original_title: "Brahms: The Boy II",
      overview:
        "Семейная пара Лиза и Шон вместе с маленьким сыном Джудом переезжает в большой дом, не подозревая о его страшном прошлом. Подозрения возникают, когда Джуд заводит дружбу с необычной фарфоровой куклой по имени Брамс, которая слишком похожа на живую.",
      popularity: 30.54,
      poster_path: "/aLT2dlTT8qOi69PwXUhpPpHr6Iy.jpg",
      release_date: "2020-02-20",
      title: "Кукла 2: Брамс",
      video: !1,
      vote_average: 6,
      vote_count: 1007,
    },
    {
      adult: !1,
      backdrop_path: "/qletQdagnYZ7StKZH209AkgKEVf.jpg",
      genre_ids: [28, 18, 14, 9648, 878],
      id: 757860,
      original_language: "zh",
      original_title: "晴雅集",
      overview:
        "После смерти наставника молодой мастер инь-ян отправляется в столицу, где вступает в конфронтацию с местным дворянином. Но вскоре оба выясняют, что общего у них гораздо больше, и лучше им держаться вместе. Парочка берётся расследовать загадочные дела и натыкается на вековую тайну, которая может повергнуть мир в хаос.",
      popularity: 30.251,
      poster_path: "/7yHG2F8Uep8j8ZRWKjtpSsuHdwg.jpg",
      release_date: "2020-12-24",
      title: "Колдун: мечта о вечности",
      video: !1,
      vote_average: 7.5,
      vote_count: 133,
    },
    {
      adult: !1,
      backdrop_path: "/2wKOO39vOvWK1vBDb72NuNfkE55.jpg",
      genre_ids: [27, 35, 80, 53],
      id: 653598,
      original_language: "en",
      original_title: "Spree",
      overview:
        "Курт Канкл - водитель, работающий в каршеринге - ведет собственный блог. В погоне за популярностью и увеличению числа подписчиков своего канала устраивает стримы, на которых подвергает своих клиентов смертельной опасности.",
      popularity: 30.011,
      poster_path: "/hJUA4wfvw0A6l3niOfxourqpERA.jpg",
      release_date: "2020-08-14",
      title: "Отрыв",
      video: !1,
      vote_average: 6.6,
      vote_count: 190,
    },
    {
      adult: !1,
      backdrop_path: "/aJK3XZGn7TaCnb1wbHBgCfXOxnc.jpg",
      genre_ids: [10402, 10749, 10770],
      id: 599521,
      original_language: "en",
      original_title: "Z-O-M-B-I-E-S 2",
      overview:
        "Чирлидер Эддисон и зомби-футболист Зед готовятся к школьному выпускному балу. Однако, когда неожиданно прибыла группа загадочных подростков-оборотней, испуганный городской совет пересматривает законы против монстров Сибрука, делая невозможным участие Зеда и Эддисон в выпускном вечере.",
      popularity: 29.792,
      poster_path: "/vAxuNfJQIIWIDhAPibRvGxLnKgt.jpg",
      release_date: "2020-06-13",
      title: "З-О-М-Б-И 2",
      video: !1,
      vote_average: 7.8,
      vote_count: 312,
    },
    {
      adult: !1,
      backdrop_path: null,
      genre_ids: [10749],
      id: 703120,
      original_language: "zh",
      original_title: "扶桑有狐",
      overview: "",
      popularity: 29.616,
      poster_path: "/2jSS1Iq1qw8fq2UNf29pF3F3xji.jpg",
      release_date: "2020-05-10",
      title: "扶桑有狐",
      video: !1,
      vote_average: 2,
      vote_count: 1,
    },
    {
      adult: !1,
      backdrop_path: "/tweDJNQzBGgsWVF5MC8JhSAk07p.jpg",
      genre_ids: [53, 878, 27],
      id: 570670,
      original_language: "en",
      original_title: "The Invisible Man",
      overview:
        "Когда отношения с богатым и гениальным учёным оборачиваются сплошным насилием и агрессией, Сесилия Касс сбегает под покровом ночи. Скрываться ей помогают сестра, друг детства и его дочь-подросток. Вскоре бывший бойфренд Сесилии совершает самоубийство и оставляет ей в наследство немалую долю своего огромного состояния. Однако девушка подозревает, что её бывший инсценировал свою смерть и на самом деле всё ещё жив. Столкнувшись с чередой жутких и смертельно опасных происшествий, Сесилия постепенно начинает сходить с ума, отчаянно пытаясь доказать, что её преследует некто невидимый.",
      popularity: 28.884,
      poster_path: "/dMfPWvQkETaB19fVQ50MQYeHCdQ.jpg",
      release_date: "2020-02-26",
      title: "Человек-невидимка",
      video: !1,
      vote_average: 7.2,
      vote_count: 4601,
    },
    {
      adult: !1,
      backdrop_path: "/ww7eC3BqSbFsyE5H5qMde8WkxJ2.jpg",
      genre_ids: [28, 27, 878, 53],
      id: 443791,
      original_language: "en",
      original_title: "Underwater",
      overview:
        "Команда ученых на подводной исследовательской станции переживает землетрясение и сталкивается с неведомым океанским ужасом.",
      popularity: 28.846,
      poster_path: "/qK8vBhQMzrreTNQcMYRRgRsvh23.jpg",
      release_date: "2020-01-08",
      title: "Под водой",
      video: !1,
      vote_average: 6.3,
      vote_count: 2457,
    },
    {
      adult: !1,
      backdrop_path: "/lA5fOBqTOQBQ1s9lEYYPmNXoYLi.jpg",
      genre_ids: [35, 28, 12, 14],
      id: 590223,
      original_language: "en",
      original_title: "Love and Monsters",
      overview:
        "Во время нападения на человечество ужасных монстров у Джоэла погибли родители. Выжившие теперь прячутся от новых хозяев планеты под землей, но парень не оставляет надежды найти свою девушку. И вот, однажды поймав её сигнал по радио, он отправляется на поиски любви всей своей жизни и встречает других выживальщиков.",
      popularity: 28.636,
      poster_path: "/pVfv1gjcTUxkdAZVrI6z3kySknq.jpg",
      release_date: "2020-10-16",
      title: "Любовь и монстры",
      video: !1,
      vote_average: 7.4,
      vote_count: 3099,
    },
    {
      adult: !1,
      backdrop_path: "/zphhP17YnyrihDBYdd1qxvI3ZSl.jpg",
      genre_ids: [27, 53, 18],
      id: 741998,
      original_language: "it",
      original_title: "Il legame",
      overview:
        "Находясь в гостях у матери своего жениха на юге Италии, женщина должна бороться с таинственным и злобным проклятием, направленным на то, чтобы забрать ее дочь.",
      popularity: 28.338,
      poster_path: "/ufbubTLEUvt2aEyFs2u9ow9XxWe.jpg",
      release_date: "2020-10-02",
      title: "Привязывание",
      video: !1,
      vote_average: 5.4,
      vote_count: 200,
    },
    {
      adult: !1,
      backdrop_path: "/6KG2KHBdAA6nDtGrWnOtno41hOR.jpg",
      genre_ids: [35, 18],
      id: 527660,
      original_language: "en",
      original_title: "Yes, God, Yes",
      overview:
        "Образцовая католичка Элис внезапно открыла для себя чувственный мир самоудовлетворения, и теперь боится угодить прямиком в ад. В поисках искупления девушка отправляется в весьма сомнительный религиозный лагерь-лечебницу, но там сдерживать низменные позывы оказывается ещё сложнее.",
      popularity: 27.941,
      poster_path: "/9Ln7tRlMgBTsTk7Bw8uo9ECnuKq.jpg",
      release_date: "2020-07-24",
      title: "Да, Боже, да",
      video: !1,
      vote_average: 6.4,
      vote_count: 378,
    },
    {
      adult: !1,
      backdrop_path: "/zrQrXkyNaYZvLmGqMyjlrVxPs6f.jpg",
      genre_ids: [27],
      id: 653615,
      original_language: "en",
      original_title: "Amulet",
      overview:
        "Бывшему военному с посттравматическим синдромом по имени Томас некуда податься, поэтому приходится бомжевать в Лондоне. Однажды он получает предложение пожить в отдалённом особняке в компании молодой женщины и её смертельно больной матери. Томас помогает с ремонтом дома, постепенно влюбляется в молодую хозяйку, и ему всё больше кажется, что тут творится что-то зловещее, кроме того, мужчина теряет ощущение реальности.",
      popularity: 27.48,
      poster_path: "/tTdGTX0tDYvS6PNYmKvhRd46VMu.jpg",
      release_date: "2020-07-24",
      title: "Амулет",
      video: !1,
      vote_average: 5.5,
      vote_count: 89,
    },
    {
      adult: !1,
      backdrop_path: "/bazlsLkNuhy3IuhviepqvlMm2hV.jpg",
      genre_ids: [16, 28, 878],
      id: 618354,
      original_language: "en",
      original_title: "Superman: Man of Tomorrow",
      overview:
        "Действия фильма разворачиваются в начале супергеройской карьеры Кларка Кента, который недавно устроился репортёром в Daily Planet. По сюжету на Землю нападают Лобо и Паразит, с которыми предстоит сразиться Супермену и Марсианскому Охотнику.",
      popularity: 27.307,
      poster_path: "/n9GtiJiBETVFayQy7YnVdF9AucU.jpg",
      release_date: "2020-08-23",
      title: "Супермен: Человек завтрашнего дня",
      video: !1,
      vote_average: 7.3,
      vote_count: 339,
    },
    {
      adult: !1,
      backdrop_path: "/3QZXtx2WUSucsQH2Uo7zCiTHzXT.jpg",
      genre_ids: [35, 10751, 16],
      id: 735364,
      original_language: "en",
      original_title: "The Boss Baby: Get That Baby!",
      overview:
        "Думаешь, у тебя есть все что нужно чтобы быть боссом? Этот интерактивный спэшл проверяет ваши управленческие навыки и подбирает для вас одну из 16 вакансий в Бэби Корп.",
      popularity: 27.121,
      poster_path: "/epvUyxGpiaacwojju2CMa0I3Y6T.jpg",
      release_date: "2020-09-01",
      title: "Босс-молокосос: Лови карапуза",
      video: !1,
      vote_average: 7,
      vote_count: 44,
    },
    {
      adult: !1,
      backdrop_path: "/azTsf28VYj0rw6ql67fB8WnxbZX.jpg",
      genre_ids: [53, 27],
      id: 554585,
      original_language: "es",
      original_title: "Sin origen",
      overview:
        "Педро решил завязать с наркобизнесом и, опасаясь за жизнь семьи, оборудовал свой дом как неприступную крепость. Однажды вечером на периметре срабатывает сигнализация, после чего охранники обнаруживают нарушительницу — с виду безобидную босую девочку. А вскоре за ней является отряд хорошо подготовленных убийц с мечами и арбалетами и, убив всю охрану, начинает осаждать успевшего закрыть дом бронёй Педро и его близких. У ассасинов всего одно требование — выдать им девочку, но Педро решает её защищать.",
      popularity: 26.707,
      poster_path: "/m9IfhfkdgCe148DdU1E9jgyb40z.jpg",
      release_date: "2020-10-25",
      title: "Происхождение неизвестно",
      video: !1,
      vote_average: 5.6,
      vote_count: 45,
    },
    {
      adult: !1,
      backdrop_path: "/x2lPPe2aH76uyYVhkVCtzTMYawC.jpg",
      genre_ids: [16, 10751, 14],
      id: 656561,
      original_language: "en",
      original_title: "Alien Xmas",
      overview:
        "Инопланетяне пытаются украсть гравитацию Земли, и только дух Рождества и маленький инопланетянин по имени Икс могут спасти мир.",
      popularity: 26.369,
      poster_path: "/3UGrAX6D1AMVra80jmUMUOLq8Qr.jpg",
      release_date: "2020-11-20",
      title: "ИКСтраординарное Рождество",
      video: !1,
      vote_average: 6.6,
      vote_count: 97,
    },
    {
      adult: !1,
      backdrop_path: "/7DMAH4LxHQMS4HL8teCHiuBlPWC.jpg",
      genre_ids: [10749, 14],
      id: 672955,
      original_language: "ko",
      original_title: "사랑하고 있습니까",
      overview:
        "Со Юн — талантливый кондитер, но из-за болезни матери ей приходится работать в кофейне Сын Дже — несдержанного и грубого боса. Однажды она угощает незнакомую старушку, промокшую под дождем, и та в благодарность дарит ей книгу, обложка которой гласит, что в ней можно найти ответы на все вопросы о любви. И когда предсказания книги начинают сбываться одно за другим, девушка уже сложно не прислушиваться к ее советам.",
      popularity: 26.305,
      poster_path: "/zd7OteG9GCwVLmRQFmGyp73R6uy.jpg",
      release_date: "2020-03-25",
      title: "Мы влюблены?",
      video: !1,
      vote_average: 8,
      vote_count: 37,
    },
    {
      adult: !1,
      backdrop_path: "/1Lhc32P0a62BgMFgd20wXR1osR3.jpg",
      genre_ids: [16, 10751, 12, 35, 878],
      id: 732670,
      original_language: "en",
      original_title: "The Lego Star Wars Holiday Special",
      overview:
        "В спецвыпуске Рей, Финн, По, Чуи, Роуз и дроиды воссоединятся на экране для празднования Дня жизни. Чтобы лучше познать силу, Рей вместе с BB-8 прилетает в таинственный храм джедаев, откуда с помощью загадочного артефакта отправляется в приключение по любимым моментам франшизы. Она встретится с Люком Скайуокером, Дартом Вейдером, Йодой, Оби-Ваном и другими знаковыми героями и злодеями.",
      popularity: 26.304,
      poster_path: "/zyzJSI7UZZzz5Toj10rYGF5689z.jpg",
      release_date: "2020-11-17",
      title: "ЛЕГО Звёздные войны: Праздничный спецвыпуск",
      video: !1,
      vote_average: 6.7,
      vote_count: 237,
    },
    {
      adult: !1,
      backdrop_path: "/eo6pahstCYRUm0Lk4MORmfCs2SH.jpg",
      genre_ids: [16, 10751],
      id: 752455,
      original_language: "en",
      original_title: "Angela's Christmas Wish",
      overview:
        "Решительная Анджела загадывает желание воссоединить свою семью к Рождеству, а затем запускает план, как найти дорогу из Ирландии в Австралию.",
      popularity: 26.06,
      poster_path: "/8ftutOnMM5JHl69mvkNbs8f1AUR.jpg",
      release_date: "2020-12-01",
      title: "Рождественское желание Анджелы",
      video: !1,
      vote_average: 6.4,
      vote_count: 82,
    },
    {
      adult: !1,
      backdrop_path: "/qak8tBFVVpUvFOf7L3a9rAfDMTS.jpg",
      genre_ids: [18],
      id: 636291,
      original_language: "ko",
      original_title: "야구소녀",
      overview: "",
      popularity: 26.008,
      poster_path: "/9IpuRZe3eMYLQjQRf641G2BM1Ok.jpg",
      release_date: "2020-06-18",
      title: "야구소녀",
      video: !1,
      vote_average: 6.6,
      vote_count: 11,
    },
    {
      adult: !1,
      backdrop_path: "/sZV8aQJlym7AiBcE1H3ISEGoVVw.jpg",
      genre_ids: [27, 9648, 53],
      id: 635199,
      original_language: "ja",
      original_title: "犬鳴村",
      overview:
        "Юма со своей девушкой Акиной попадают в давно несуществующую деревню Инунаки, после чего Акина становится сама не своя, постоянно напевает какую-то песню и в результате кончает с собой. Когда Юма решает вернуться в проклятую деревню, за ним увязывается младший брат Кота, и оба бесследно исчезают. Мать в истерике, отец заявляет, что больше не хочет иметь ничего общего с их грязной кровью, а сестре Канаэ, которая с детства пытается не замечать призраков, придётся спасать братьев и разгадывать тайну мистической деревни Инунаки.",
      popularity: 25.79,
      poster_path: "/iEoh3ZxyrGhiBA3ZGa79vb8w2xj.jpg",
      release_date: "2020-02-07",
      title: "Проклятие деревни Инунаки",
      video: !1,
      vote_average: 5.4,
      vote_count: 30,
    },
    {
      adult: !1,
      backdrop_path: "/chAGSv4DB9s3fsgULpSZQLN7LgN.jpg",
      genre_ids: [27],
      id: 514593,
      original_language: "en",
      original_title: "You Should Have Left",
      overview:
        "Тео Конрой - обеспеченный бывший банкир на пенсии, чей брак с намного более молодой женой-актрисой, Сюзанной, трещит по швам из-за его ревности и мрачных теней в его прошлом. Стремясь наладить отношения, Тео и Сюзанна отправляются вместе с шестилетней дочкой Эллой отдохнуть в съемном загородном доме в Уэльсе. Однако вскоре после приезда Тео начинает подозревать, что некая зловещая сила в доме знает их с Сюзанной секреты даже лучше, чем они сами знают друг друга.",
      popularity: 25.5,
      poster_path: "/4J9PACetIEY8KxJ9DXgLOhcAJvK.jpg",
      release_date: "2020-06-18",
      title: "Тебе стоило уйти",
      video: !1,
      vote_average: 5.9,
      vote_count: 489,
    },
    {
      adult: !1,
      backdrop_path: "/iCawkSvuiky8TbROdcvku7herrd.jpg",
      genre_ids: [12, 10751, 14],
      id: 743227,
      original_language: "en",
      original_title: "Wish Upon a Unicorn",
      overview:
        "Две сестры вместе с отцом переезжают из города в деревню, пожить к бабушке. На её ферме они обнаруживают маленького единорога по имени Рокко, с которым заводят дружбу. Рокко обладает магическими силами и дарит девочкам немного счастья, шуток и смеха, что облегчает приспособление к новому дому и школе. Но когда местные нищие ковбои узнают о волшебном единороге, они похищают Рокко, чтобы использовать его силы для себя. Сёстры отправляются спасать своего друга.",
      popularity: 25.194,
      poster_path: "/kcpEnSCmPDlTSwjibfpzfA3gCyn.jpg",
      release_date: "2020-09-15",
      title: "Коснись единорога и загадай желание",
      video: !1,
      vote_average: 7.1,
      vote_count: 38,
    },
    {
      adult: !1,
      backdrop_path: "/meIF4dCPCh80gsBY5cM4E3jSKRd.jpg",
      genre_ids: [27],
      id: 708336,
      original_language: "en",
      original_title: "Return of the Tooth Fairy",
      overview:
        "Через 15 лет после трагических событий, которые перевернули его жизнь, Кори, уже взрослый, но ещё не оправившийся от детской травмы юноша, едет на встречу выпускников. Однако, его планам хорошо провести время с друзьями и сблизиться с симпатичной бывшей однокурсницей снова мешает Зубная фея.",
      popularity: 24.96,
      poster_path: "/n8XFS2FVStaBPS2vLqUuH2bAjVf.jpg",
      release_date: "2020-06-15",
      title: "Зубная фея 2",
      video: !1,
      vote_average: 6.5,
      vote_count: 742,
    },
    {
      adult: !1,
      backdrop_path: "/23IeumCWwWE7u2d2FDNSGPsNRbl.jpg",
      genre_ids: [18, 27, 53],
      id: 474764,
      original_language: "en",
      original_title: "The Lodge",
      overview:
        "Молодая женщина Грейс выходит замуж за журналиста Ричарда, и теперь ей предстоит стать мачехой двоим детям. Чему сами Мия и Эйден не очень-то рады: в отличие от влюбчивого отца они ещё не забыли мать, из-за развода покончившую с собой. К тому же Грейс — дочь основателя секты, 12 лет назад совершившей массовое самоубийство, поэтому дети считают, что отец променял мать на психопатку. Чтобы Мия и Эйден ближе познакомились с Грейс, Ричард отправляет семейство провести пару дней перед Рождеством в отдаленном от цивилизации доме.",
      popularity: 24.63,
      poster_path: "/qVrEaP1H82Im0A26M5VM5PpdAWD.jpg",
      release_date: "2020-01-16",
      title: "Сторожка",
      video: !1,
      vote_average: 6.1,
      vote_count: 885,
    },
  ],
  sh = "POPULAR_ASCENDING",
  ch = "POPULAR_DESCENDING",
  dh = "RATING_ASCENDING",
  ph = "RATING_DESCENDING",
  fh = "2020",
  gh = "2019",
  vh = "2018",
  hh = "2017",
  _h = "GENRES",
  mh = "WATCH_LATER",
  yh = "FAVORITES",
  Ft = vu,
  wh = (e = Ft, t) => {
    switch (t.type) {
      case sh:
        return e
          .slice()
          .sort((n, r) =>
            n.popularity > r.popularity
              ? 1
              : n.popularity < r.popularity
              ? -1
              : 0
          );
      case ch:
        return e
          .slice()
          .sort((n, r) =>
            n.popularity > r.popularity
              ? -1
              : n.popularity < r.popularity
              ? 1
              : 0
          );
      case dh:
        return e
          .slice()
          .sort((n, r) =>
            n.vote_average > r.vote_average
              ? 1
              : n.vote_average < r.vote_average
              ? -1
              : 0
          );
      case ph:
        return e
          .slice()
          .sort((n, r) =>
            n.vote_average > r.vote_average
              ? -1
              : n.vote_average < r.vote_average
              ? 1
              : 0
          );
      case fh:
        return Ft.filter((n) => n.release_date.includes("2020"));
      case gh:
        return Ft.filter((n) => n.release_date.includes("2019"));
      case vh:
        return Ft.filter((n) => n.release_date.includes("2018"));
      case hh:
        return Ft.filter((n) => n.release_date.includes("2017"));
      case _h:
        return e.filter((n) => t.payload.every((r) => n.genre_ids.includes(r)));
      case mh:
        return Ft.filter((n) => t.payload.includes(n.id));
      case yh:
        return Ft.filter((n) => t.payload.includes(n.id));
      default:
        return e;
    }
  },
  Sh = "NEXT_CURRENT_PAGE",
  kh = "PREV_CURRENT_PAGE",
  Eh = "ADD_CURRENT_PAGE";
let jh = 1;
const Ch = (e = jh, t) => {
    switch (t.type) {
      case Sh:
        return e + 1;
      case kh:
        return e - 1;
      case Eh:
        return t.payload;
      default:
        return e;
    }
  },
  xh = "LOGIN",
  Th = "LOGOUT",
  Ph = [{ login: "PushToTalk", password: "123456789", isAuth: !1 }],
  Is = localStorage.getItem("user"),
  Nh = Is !== null ? JSON.parse(Is) : { login: "", password: "", isAuth: !1 },
  Oh = (e = Nh, t) => {
    switch (t.type) {
      case xh:
        return Ph.find((n) =>
          n.login === t.payload.login && n.password === t.payload.password
            ? ((n.isAuth = !0),
              localStorage.setItem("user", JSON.stringify(n)),
              n)
            : e
        );
      case Th:
        return localStorage.removeItem("user"), { ...e, isAuth: !1 };
      default:
        return e;
    }
  },
  Rh = "ADD_FAVORITE",
  Lh = "REMOVE_FAVORITE",
  zs = localStorage.getItem("favorites"),
  Mh = zs !== null ? JSON.parse(zs) : [],
  Dh = (e = Mh, t) => {
    switch (t.type) {
      case Rh:
        const n = [...e, t.payload];
        return localStorage.setItem("favorites", JSON.stringify(n)), n;
      case Lh:
        const r = e.filter((a) => a !== t.payload);
        return localStorage.setItem("favorites", JSON.stringify(r)), r;
      default:
        return e;
    }
  },
  Ih = "ADD_WATCH_LATER",
  zh = "REMOVE_WATCH_LATER",
  Us = localStorage.getItem("watchLater"),
  Uh = Us !== null ? JSON.parse(Us) : [],
  Ah = (e = Uh, t) => {
    switch (t.type) {
      case Ih:
        const n = [...e, t.payload];
        return localStorage.setItem("watchLater", JSON.stringify(n)), n;
      case zh:
        const r = e.filter((a) => a !== t.payload);
        return localStorage.setItem("watchLater", JSON.stringify(r)), r;
      default:
        return e;
    }
  },
  Fh = "ADD_FILM",
  As = localStorage.getItem("film"),
  Bh = As !== null ? JSON.parse(As) : [],
  Wh = (e = Bh, t) => {
    switch (t.type) {
      case Fh:
        const n = vu.find((r) => r.id === t.payload);
        return localStorage.setItem("film", JSON.stringify(n)), n;
      default:
        return e;
    }
  },
  Hh = "GENRE",
  bh = "HIGH",
  Vh = "LOW",
  Yh = "POPULAR",
  Gh = "UNKNOWN",
  Fs = vu,
  Qh = (e = Fs, t) => {
    switch (t.type) {
      case Hh:
        return Fs.filter((n) => n.genre_ids.includes(t.payload));
      case bh:
        return e.filter((n) => n.vote_average > 5);
      case Vh:
        return e.filter((n) => n.vote_average <= 5);
      case Yh:
        return e.filter((n) => n.popularity > 100 && n.vote_count > 200);
      case Gh:
        return e.filter((n) => n.popularity < 100 && n.vote_count <= 200);
      default:
        return e;
    }
  },
  $h = hp({
    listFilmsReducer: wh,
    currentPageReducer: Ch,
    authReducer: Oh,
    favoritesReducer: Dh,
    watchLaterReducer: Ah,
    filmReducer: Wh,
    searchReducer: Qh,
  }),
  Kh = Oi($h, uh());
/**
 * @remix-run/router v1.3.0
 *
 * Copyright (c) Remix Software Inc.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE.md file in the root directory of this source tree.
 *
 * @license MIT
 */ function Tr() {
  return (
    (Tr = Object.assign
      ? Object.assign.bind()
      : function (e) {
          for (var t = 1; t < arguments.length; t++) {
            var n = arguments[t];
            for (var r in n)
              Object.prototype.hasOwnProperty.call(n, r) && (e[r] = n[r]);
          }
          return e;
        }),
    Tr.apply(this, arguments)
  );
}
var yt;
(function (e) {
  (e.Pop = "POP"), (e.Push = "PUSH"), (e.Replace = "REPLACE");
})(yt || (yt = {}));
const Bs = "popstate";
function Xh(e) {
  e === void 0 && (e = {});
  function t(r, a) {
    let { pathname: i, search: o, hash: l } = r.location;
    return dl(
      "",
      { pathname: i, search: o, hash: l },
      (a.state && a.state.usr) || null,
      (a.state && a.state.key) || "default"
    );
  }
  function n(r, a) {
    return typeof a == "string" ? a : Ha(a);
  }
  return Zh(t, n, null, e);
}
function ee(e, t) {
  if (e === !1 || e === null || typeof e > "u") throw new Error(t);
}
function Jh(e, t) {
  if (!e) {
    typeof console < "u" && console.warn(t);
    try {
      throw new Error(t);
    } catch {}
  }
}
function qh() {
  return Math.random().toString(36).substr(2, 8);
}
function Ws(e, t) {
  return { usr: e.state, key: e.key, idx: t };
}
function dl(e, t, n, r) {
  return (
    n === void 0 && (n = null),
    Tr(
      { pathname: typeof e == "string" ? e : e.pathname, search: "", hash: "" },
      typeof t == "string" ? Un(t) : t,
      { state: n, key: (t && t.key) || r || qh() }
    )
  );
}
function Ha(e) {
  let { pathname: t = "/", search: n = "", hash: r = "" } = e;
  return (
    n && n !== "?" && (t += n.charAt(0) === "?" ? n : "?" + n),
    r && r !== "#" && (t += r.charAt(0) === "#" ? r : "#" + r),
    t
  );
}
function Un(e) {
  let t = {};
  if (e) {
    let n = e.indexOf("#");
    n >= 0 && ((t.hash = e.substr(n)), (e = e.substr(0, n)));
    let r = e.indexOf("?");
    r >= 0 && ((t.search = e.substr(r)), (e = e.substr(0, r))),
      e && (t.pathname = e);
  }
  return t;
}
function Zh(e, t, n, r) {
  r === void 0 && (r = {});
  let { window: a = document.defaultView, v5Compat: i = !1 } = r,
    o = a.history,
    l = yt.Pop,
    u = null,
    s = p();
  s == null && ((s = 0), o.replaceState(Tr({}, o.state, { idx: s }), ""));
  function p() {
    return (o.state || { idx: null }).idx;
  }
  function f() {
    let E = yt.Pop,
      d = p();
    if (d != null) {
      let c = d - s;
      (l = E), (s = d), u && u({ action: l, location: h.location, delta: c });
    } else
      Jh(
        !1,
        "You are trying to block a POP navigation to a location that was not created by @remix-run/router. The block will fail silently in production, but in general you should do all navigation with the router (instead of using window.history.pushState directly) to avoid this situation."
      );
  }
  function v(E, d) {
    l = yt.Push;
    let c = dl(h.location, E, d);
    n && n(c, E), (s = p() + 1);
    let g = Ws(c, s),
      y = h.createHref(c);
    try {
      o.pushState(g, "", y);
    } catch {
      a.location.assign(y);
    }
    i && u && u({ action: l, location: h.location, delta: 1 });
  }
  function _(E, d) {
    l = yt.Replace;
    let c = dl(h.location, E, d);
    n && n(c, E), (s = p());
    let g = Ws(c, s),
      y = h.createHref(c);
    o.replaceState(g, "", y),
      i && u && u({ action: l, location: h.location, delta: 0 });
  }
  function m(E) {
    let d = a.location.origin !== "null" ? a.location.origin : a.location.href,
      c = typeof E == "string" ? E : Ha(E);
    return (
      ee(
        d,
        "No window.location.(origin|href) available to create URL for href: " +
          c
      ),
      new URL(c, d)
    );
  }
  let h = {
    get action() {
      return l;
    },
    get location() {
      return e(a, o);
    },
    listen(E) {
      if (u) throw new Error("A history only accepts one active listener");
      return (
        a.addEventListener(Bs, f),
        (u = E),
        () => {
          a.removeEventListener(Bs, f), (u = null);
        }
      );
    },
    createHref(E) {
      return t(a, E);
    },
    createURL: m,
    encodeLocation(E) {
      let d = m(E);
      return { pathname: d.pathname, search: d.search, hash: d.hash };
    },
    push: v,
    replace: _,
    go(E) {
      return o.go(E);
    },
  };
  return h;
}
var Hs;
(function (e) {
  (e.data = "data"),
    (e.deferred = "deferred"),
    (e.redirect = "redirect"),
    (e.error = "error");
})(Hs || (Hs = {}));
function e0(e, t, n) {
  n === void 0 && (n = "/");
  let r = typeof t == "string" ? Un(t) : t,
    a = wp(r.pathname || "/", n);
  if (a == null) return null;
  let i = mp(e);
  t0(i);
  let o = null;
  for (let l = 0; o == null && l < i.length; ++l) o = c0(i[l], f0(a));
  return o;
}
function mp(e, t, n, r) {
  t === void 0 && (t = []), n === void 0 && (n = []), r === void 0 && (r = "");
  let a = (i, o, l) => {
    let u = {
      relativePath: l === void 0 ? i.path || "" : l,
      caseSensitive: i.caseSensitive === !0,
      childrenIndex: o,
      route: i,
    };
    u.relativePath.startsWith("/") &&
      (ee(
        u.relativePath.startsWith(r),
        'Absolute route path "' +
          u.relativePath +
          '" nested under path ' +
          ('"' + r + '" is not valid. An absolute child route path ') +
          "must start with the combined path of all its parent routes."
      ),
      (u.relativePath = u.relativePath.slice(r.length)));
    let s = Pt([r, u.relativePath]),
      p = n.concat(u);
    i.children &&
      i.children.length > 0 &&
      (ee(
        i.index !== !0,
        "Index routes must not have child routes. Please remove " +
          ('all child routes from route path "' + s + '".')
      ),
      mp(i.children, t, p, s)),
      !(i.path == null && !i.index) &&
        t.push({ path: s, score: u0(s, i.index), routesMeta: p });
  };
  return (
    e.forEach((i, o) => {
      var l;
      if (i.path === "" || !((l = i.path) != null && l.includes("?"))) a(i, o);
      else for (let u of yp(i.path)) a(i, o, u);
    }),
    t
  );
}
function yp(e) {
  let t = e.split("/");
  if (t.length === 0) return [];
  let [n, ...r] = t,
    a = n.endsWith("?"),
    i = n.replace(/\?$/, "");
  if (r.length === 0) return a ? [i, ""] : [i];
  let o = yp(r.join("/")),
    l = [];
  return (
    l.push(...o.map((u) => (u === "" ? i : [i, u].join("/")))),
    a && l.push(...o),
    l.map((u) => (e.startsWith("/") && u === "" ? "/" : u))
  );
}
function t0(e) {
  e.sort((t, n) =>
    t.score !== n.score
      ? n.score - t.score
      : s0(
          t.routesMeta.map((r) => r.childrenIndex),
          n.routesMeta.map((r) => r.childrenIndex)
        )
  );
}
const n0 = /^:\w+$/,
  r0 = 3,
  a0 = 2,
  i0 = 1,
  o0 = 10,
  l0 = -2,
  bs = (e) => e === "*";
function u0(e, t) {
  let n = e.split("/"),
    r = n.length;
  return (
    n.some(bs) && (r += l0),
    t && (r += a0),
    n
      .filter((a) => !bs(a))
      .reduce((a, i) => a + (n0.test(i) ? r0 : i === "" ? i0 : o0), r)
  );
}
function s0(e, t) {
  return e.length === t.length && e.slice(0, -1).every((r, a) => r === t[a])
    ? e[e.length - 1] - t[t.length - 1]
    : 0;
}
function c0(e, t) {
  let { routesMeta: n } = e,
    r = {},
    a = "/",
    i = [];
  for (let o = 0; o < n.length; ++o) {
    let l = n[o],
      u = o === n.length - 1,
      s = a === "/" ? t : t.slice(a.length) || "/",
      p = d0(
        { path: l.relativePath, caseSensitive: l.caseSensitive, end: u },
        s
      );
    if (!p) return null;
    Object.assign(r, p.params);
    let f = l.route;
    i.push({
      params: r,
      pathname: Pt([a, p.pathname]),
      pathnameBase: _0(Pt([a, p.pathnameBase])),
      route: f,
    }),
      p.pathnameBase !== "/" && (a = Pt([a, p.pathnameBase]));
  }
  return i;
}
function d0(e, t) {
  typeof e == "string" && (e = { path: e, caseSensitive: !1, end: !0 });
  let [n, r] = p0(e.path, e.caseSensitive, e.end),
    a = t.match(n);
  if (!a) return null;
  let i = a[0],
    o = i.replace(/(.)\/+$/, "$1"),
    l = a.slice(1);
  return {
    params: r.reduce((s, p, f) => {
      if (p === "*") {
        let v = l[f] || "";
        o = i.slice(0, i.length - v.length).replace(/(.)\/+$/, "$1");
      }
      return (s[p] = g0(l[f] || "", p)), s;
    }, {}),
    pathname: i,
    pathnameBase: o,
    pattern: e,
  };
}
function p0(e, t, n) {
  t === void 0 && (t = !1),
    n === void 0 && (n = !0),
    hu(
      e === "*" || !e.endsWith("*") || e.endsWith("/*"),
      'Route path "' +
        e +
        '" will be treated as if it were ' +
        ('"' + e.replace(/\*$/, "/*") + '" because the `*` character must ') +
        "always follow a `/` in the pattern. To get rid of this warning, " +
        ('please change the route path to "' + e.replace(/\*$/, "/*") + '".')
    );
  let r = [],
    a =
      "^" +
      e
        .replace(/\/*\*?$/, "")
        .replace(/^\/*/, "/")
        .replace(/[\\.*+^$?{}|()[\]]/g, "\\$&")
        .replace(/\/:(\w+)/g, (o, l) => (r.push(l), "/([^\\/]+)"));
  return (
    e.endsWith("*")
      ? (r.push("*"),
        (a += e === "*" || e === "/*" ? "(.*)$" : "(?:\\/(.+)|\\/*)$"))
      : n
      ? (a += "\\/*$")
      : e !== "" && e !== "/" && (a += "(?:(?=\\/|$))"),
    [new RegExp(a, t ? void 0 : "i"), r]
  );
}
function f0(e) {
  try {
    return decodeURI(e);
  } catch (t) {
    return (
      hu(
        !1,
        'The URL path "' +
          e +
          '" could not be decoded because it is is a malformed URL segment. This is probably due to a bad percent ' +
          ("encoding (" + t + ").")
      ),
      e
    );
  }
}
function g0(e, t) {
  try {
    return decodeURIComponent(e);
  } catch (n) {
    return (
      hu(
        !1,
        'The value for the URL param "' +
          t +
          '" will not be decoded because' +
          (' the string "' +
            e +
            '" is a malformed URL segment. This is probably') +
          (" due to a bad percent encoding (" + n + ").")
      ),
      e
    );
  }
}
function wp(e, t) {
  if (t === "/") return e;
  if (!e.toLowerCase().startsWith(t.toLowerCase())) return null;
  let n = t.endsWith("/") ? t.length - 1 : t.length,
    r = e.charAt(n);
  return r && r !== "/" ? null : e.slice(n) || "/";
}
function hu(e, t) {
  if (!e) {
    typeof console < "u" && console.warn(t);
    try {
      throw new Error(t);
    } catch {}
  }
}
function v0(e, t) {
  t === void 0 && (t = "/");
  let {
    pathname: n,
    search: r = "",
    hash: a = "",
  } = typeof e == "string" ? Un(e) : e;
  return {
    pathname: n ? (n.startsWith("/") ? n : h0(n, t)) : t,
    search: m0(r),
    hash: y0(a),
  };
}
function h0(e, t) {
  let n = t.replace(/\/+$/, "").split("/");
  return (
    e.split("/").forEach((a) => {
      a === ".." ? n.length > 1 && n.pop() : a !== "." && n.push(a);
    }),
    n.length > 1 ? n.join("/") : "/"
  );
}
function uo(e, t, n, r) {
  return (
    "Cannot include a '" +
    e +
    "' character in a manually specified " +
    ("`to." +
      t +
      "` field [" +
      JSON.stringify(r) +
      "].  Please separate it out to the ") +
    ("`to." + n + "` field. Alternatively you may provide the full path as ") +
    'a string in <Link to="..."> and the router will parse it for you.'
  );
}
function Sp(e) {
  return e.filter(
    (t, n) => n === 0 || (t.route.path && t.route.path.length > 0)
  );
}
function kp(e, t, n, r) {
  r === void 0 && (r = !1);
  let a;
  typeof e == "string"
    ? (a = Un(e))
    : ((a = Tr({}, e)),
      ee(
        !a.pathname || !a.pathname.includes("?"),
        uo("?", "pathname", "search", a)
      ),
      ee(
        !a.pathname || !a.pathname.includes("#"),
        uo("#", "pathname", "hash", a)
      ),
      ee(!a.search || !a.search.includes("#"), uo("#", "search", "hash", a)));
  let i = e === "" || a.pathname === "",
    o = i ? "/" : a.pathname,
    l;
  if (r || o == null) l = n;
  else {
    let f = t.length - 1;
    if (o.startsWith("..")) {
      let v = o.split("/");
      for (; v[0] === ".."; ) v.shift(), (f -= 1);
      a.pathname = v.join("/");
    }
    l = f >= 0 ? t[f] : "/";
  }
  let u = v0(a, l),
    s = o && o !== "/" && o.endsWith("/"),
    p = (i || o === ".") && n.endsWith("/");
  return !u.pathname.endsWith("/") && (s || p) && (u.pathname += "/"), u;
}
const Pt = (e) => e.join("/").replace(/\/\/+/g, "/"),
  _0 = (e) => e.replace(/\/+$/, "").replace(/^\/*/, "/"),
  m0 = (e) => (!e || e === "?" ? "" : e.startsWith("?") ? e : "?" + e),
  y0 = (e) => (!e || e === "#" ? "" : e.startsWith("#") ? e : "#" + e);
class w0 {
  constructor(t, n, r, a) {
    a === void 0 && (a = !1),
      (this.status = t),
      (this.statusText = n || ""),
      (this.internal = a),
      r instanceof Error
        ? ((this.data = r.toString()), (this.error = r))
        : (this.data = r);
  }
}
function S0(e) {
  return e instanceof w0;
}
const k0 = ["post", "put", "patch", "delete"];
[...k0];
/**
 * React Router v6.7.0
 *
 * Copyright (c) Remix Software Inc.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE.md file in the root directory of this source tree.
 *
 * @license MIT
 */ function pl() {
  return (
    (pl = Object.assign
      ? Object.assign.bind()
      : function (e) {
          for (var t = 1; t < arguments.length; t++) {
            var n = arguments[t];
            for (var r in n)
              Object.prototype.hasOwnProperty.call(n, r) && (e[r] = n[r]);
          }
          return e;
        }),
    pl.apply(this, arguments)
  );
}
function E0(e, t) {
  return (e === t && (e !== 0 || 1 / e === 1 / t)) || (e !== e && t !== t);
}
const j0 = typeof Object.is == "function" ? Object.is : E0,
  { useState: C0, useEffect: x0, useLayoutEffect: T0, useDebugValue: P0 } = fo;
function N0(e, t, n) {
  const r = t(),
    [{ inst: a }, i] = C0({ inst: { value: r, getSnapshot: t } });
  return (
    T0(() => {
      (a.value = r), (a.getSnapshot = t), so(a) && i({ inst: a });
    }, [e, r, t]),
    x0(
      () => (
        so(a) && i({ inst: a }),
        e(() => {
          so(a) && i({ inst: a });
        })
      ),
      [e]
    ),
    P0(r),
    r
  );
}
function so(e) {
  const t = e.getSnapshot,
    n = e.value;
  try {
    const r = t();
    return !j0(n, r);
  } catch {
    return !0;
  }
}
function O0(e, t, n) {
  return t();
}
const R0 =
    typeof window < "u" &&
    typeof window.document < "u" &&
    typeof window.document.createElement < "u",
  L0 = !R0,
  M0 = L0 ? O0 : N0;
"useSyncExternalStore" in fo && ((e) => e.useSyncExternalStore)(fo);
const Ep = S.createContext(null),
  _u = S.createContext(null),
  Dr = S.createContext(null),
  Ri = S.createContext(null),
  rn = S.createContext({ outlet: null, matches: [] }),
  jp = S.createContext(null);
function D0(e, t) {
  let { relative: n } = t === void 0 ? {} : t;
  Ir() || ee(!1);
  let { basename: r, navigator: a } = S.useContext(Dr),
    { hash: i, pathname: o, search: l } = mu(e, { relative: n }),
    u = o;
  return (
    r !== "/" && (u = o === "/" ? r : Pt([r, o])),
    a.createHref({ pathname: u, search: l, hash: i })
  );
}
function Ir() {
  return S.useContext(Ri) != null;
}
function zr() {
  return Ir() || ee(!1), S.useContext(Ri).location;
}
function I0() {
  Ir() || ee(!1);
  let { basename: e, navigator: t } = S.useContext(Dr),
    { matches: n } = S.useContext(rn),
    { pathname: r } = zr(),
    a = JSON.stringify(Sp(n).map((l) => l.pathnameBase)),
    i = S.useRef(!1);
  return (
    S.useEffect(() => {
      i.current = !0;
    }),
    S.useCallback(
      function (l, u) {
        if ((u === void 0 && (u = {}), !i.current)) return;
        if (typeof l == "number") {
          t.go(l);
          return;
        }
        let s = kp(l, JSON.parse(a), r, u.relative === "path");
        e !== "/" &&
          (s.pathname = s.pathname === "/" ? e : Pt([e, s.pathname])),
          (u.replace ? t.replace : t.push)(s, u.state, u);
      },
      [e, t, a, r]
    )
  );
}
const z0 = S.createContext(null);
function U0(e) {
  let t = S.useContext(rn).outlet;
  return t && S.createElement(z0.Provider, { value: e }, t);
}
function mu(e, t) {
  let { relative: n } = t === void 0 ? {} : t,
    { matches: r } = S.useContext(rn),
    { pathname: a } = zr(),
    i = JSON.stringify(Sp(r).map((o) => o.pathnameBase));
  return S.useMemo(() => kp(e, JSON.parse(i), a, n === "path"), [e, i, a, n]);
}
function A0(e, t) {
  Ir() || ee(!1);
  let { navigator: n } = S.useContext(Dr),
    r = S.useContext(_u),
    { matches: a } = S.useContext(rn),
    i = a[a.length - 1],
    o = i ? i.params : {};
  i && i.pathname;
  let l = i ? i.pathnameBase : "/";
  i && i.route;
  let u = zr(),
    s;
  if (t) {
    var p;
    let h = typeof t == "string" ? Un(t) : t;
    l === "/" || ((p = h.pathname) != null && p.startsWith(l)) || ee(!1),
      (s = h);
  } else s = u;
  let f = s.pathname || "/",
    v = l === "/" ? f : f.slice(l.length) || "/",
    _ = e0(e, { pathname: v }),
    m = H0(
      _ &&
        _.map((h) =>
          Object.assign({}, h, {
            params: Object.assign({}, o, h.params),
            pathname: Pt([
              l,
              n.encodeLocation
                ? n.encodeLocation(h.pathname).pathname
                : h.pathname,
            ]),
            pathnameBase:
              h.pathnameBase === "/"
                ? l
                : Pt([
                    l,
                    n.encodeLocation
                      ? n.encodeLocation(h.pathnameBase).pathname
                      : h.pathnameBase,
                  ]),
          })
        ),
      a,
      r || void 0
    );
  return t && m
    ? S.createElement(
        Ri.Provider,
        {
          value: {
            location: pl(
              {
                pathname: "/",
                search: "",
                hash: "",
                state: null,
                key: "default",
              },
              s
            ),
            navigationType: yt.Pop,
          },
        },
        m
      )
    : m;
}
function F0() {
  let e = G0(),
    t = S0(e)
      ? e.status + " " + e.statusText
      : e instanceof Error
      ? e.message
      : JSON.stringify(e),
    n = e instanceof Error ? e.stack : null,
    r = "rgba(200,200,200, 0.5)",
    a = { padding: "0.5rem", backgroundColor: r },
    i = { padding: "2px 4px", backgroundColor: r };
  return S.createElement(
    S.Fragment,
    null,
    S.createElement("h2", null, "Unhandled Thrown Error!"),
    S.createElement("h3", { style: { fontStyle: "italic" } }, t),
    n ? S.createElement("pre", { style: a }, n) : null,
    S.createElement("p", null, "💿 Hey developer 👋"),
    S.createElement(
      "p",
      null,
      "You can provide a way better UX than this when your app throws errors by providing your own ",
      S.createElement("code", { style: i }, "errorElement"),
      " props on ",
      S.createElement("code", { style: i }, "<Route>")
    )
  );
}
class B0 extends S.Component {
  constructor(t) {
    super(t), (this.state = { location: t.location, error: t.error });
  }
  static getDerivedStateFromError(t) {
    return { error: t };
  }
  static getDerivedStateFromProps(t, n) {
    return n.location !== t.location
      ? { error: t.error, location: t.location }
      : { error: t.error || n.error, location: n.location };
  }
  componentDidCatch(t, n) {
    console.error(
      "React Router caught the following error during render",
      t,
      n
    );
  }
  render() {
    return this.state.error
      ? S.createElement(
          rn.Provider,
          { value: this.props.routeContext },
          S.createElement(jp.Provider, {
            value: this.state.error,
            children: this.props.component,
          })
        )
      : this.props.children;
  }
}
function W0(e) {
  let { routeContext: t, match: n, children: r } = e,
    a = S.useContext(Ep);
  return (
    a &&
      a.static &&
      a.staticContext &&
      n.route.errorElement &&
      (a.staticContext._deepestRenderedBoundaryId = n.route.id),
    S.createElement(rn.Provider, { value: t }, r)
  );
}
function H0(e, t, n) {
  if ((t === void 0 && (t = []), e == null))
    if (n != null && n.errors) e = n.matches;
    else return null;
  let r = e,
    a = n == null ? void 0 : n.errors;
  if (a != null) {
    let i = r.findIndex(
      (o) => o.route.id && (a == null ? void 0 : a[o.route.id])
    );
    i >= 0 || ee(!1), (r = r.slice(0, Math.min(r.length, i + 1)));
  }
  return r.reduceRight((i, o, l) => {
    let u = o.route.id ? (a == null ? void 0 : a[o.route.id]) : null,
      s = n ? o.route.errorElement || S.createElement(F0, null) : null,
      p = t.concat(r.slice(0, l + 1)),
      f = () =>
        S.createElement(
          W0,
          { match: o, routeContext: { outlet: i, matches: p } },
          u ? s : o.route.element !== void 0 ? o.route.element : i
        );
    return n && (o.route.errorElement || l === 0)
      ? S.createElement(B0, {
          location: n.location,
          component: s,
          error: u,
          children: f(),
          routeContext: { outlet: null, matches: p },
        })
      : f();
  }, null);
}
var Vs;
(function (e) {
  (e.UseBlocker = "useBlocker"), (e.UseRevalidator = "useRevalidator");
})(Vs || (Vs = {}));
var ba;
(function (e) {
  (e.UseLoaderData = "useLoaderData"),
    (e.UseActionData = "useActionData"),
    (e.UseRouteError = "useRouteError"),
    (e.UseNavigation = "useNavigation"),
    (e.UseRouteLoaderData = "useRouteLoaderData"),
    (e.UseMatches = "useMatches"),
    (e.UseRevalidator = "useRevalidator");
})(ba || (ba = {}));
function b0(e) {
  let t = S.useContext(_u);
  return t || ee(!1), t;
}
function V0(e) {
  let t = S.useContext(rn);
  return t || ee(!1), t;
}
function Y0(e) {
  let t = V0(),
    n = t.matches[t.matches.length - 1];
  return n.route.id || ee(!1), n.route.id;
}
function G0() {
  var e;
  let t = S.useContext(jp),
    n = b0(ba.UseRouteError),
    r = Y0(ba.UseRouteError);
  return t || ((e = n.errors) == null ? void 0 : e[r]);
}
function Q0(e) {
  return U0(e.context);
}
function Ht(e) {
  ee(!1);
}
function $0(e) {
  let {
    basename: t = "/",
    children: n = null,
    location: r,
    navigationType: a = yt.Pop,
    navigator: i,
    static: o = !1,
  } = e;
  Ir() && ee(!1);
  let l = t.replace(/^\/*/, "/"),
    u = S.useMemo(() => ({ basename: l, navigator: i, static: o }), [l, i, o]);
  typeof r == "string" && (r = Un(r));
  let {
      pathname: s = "/",
      search: p = "",
      hash: f = "",
      state: v = null,
      key: _ = "default",
    } = r,
    m = S.useMemo(() => {
      let h = wp(s, l);
      return h == null
        ? null
        : { pathname: h, search: p, hash: f, state: v, key: _ };
    }, [l, s, p, f, v, _]);
  return m == null
    ? null
    : S.createElement(
        Dr.Provider,
        { value: u },
        S.createElement(Ri.Provider, {
          children: n,
          value: { location: m, navigationType: a },
        })
      );
}
function K0(e) {
  let { children: t, location: n } = e,
    r = S.useContext(Ep),
    a = r && !t ? r.router.routes : fl(t);
  return A0(a, n);
}
var Ys;
(function (e) {
  (e[(e.pending = 0)] = "pending"),
    (e[(e.success = 1)] = "success"),
    (e[(e.error = 2)] = "error");
})(Ys || (Ys = {}));
new Promise(() => {});
function fl(e, t) {
  t === void 0 && (t = []);
  let n = [];
  return (
    S.Children.forEach(e, (r, a) => {
      if (!S.isValidElement(r)) return;
      if (r.type === S.Fragment) {
        n.push.apply(n, fl(r.props.children, t));
        return;
      }
      r.type !== Ht && ee(!1), !r.props.index || !r.props.children || ee(!1);
      let i = [...t, a],
        o = {
          id: r.props.id || i.join("-"),
          caseSensitive: r.props.caseSensitive,
          element: r.props.element,
          index: r.props.index,
          path: r.props.path,
          loader: r.props.loader,
          action: r.props.action,
          errorElement: r.props.errorElement,
          hasErrorBoundary: r.props.errorElement != null,
          shouldRevalidate: r.props.shouldRevalidate,
          handle: r.props.handle,
        };
      r.props.children && (o.children = fl(r.props.children, i)), n.push(o);
    }),
    n
  );
}
/**
 * React Router DOM v6.7.0
 *
 * Copyright (c) Remix Software Inc.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE.md file in the root directory of this source tree.
 *
 * @license MIT
 */ function Va() {
  return (
    (Va = Object.assign
      ? Object.assign.bind()
      : function (e) {
          for (var t = 1; t < arguments.length; t++) {
            var n = arguments[t];
            for (var r in n)
              Object.prototype.hasOwnProperty.call(n, r) && (e[r] = n[r]);
          }
          return e;
        }),
    Va.apply(this, arguments)
  );
}
function Cp(e, t) {
  if (e == null) return {};
  var n = {},
    r = Object.keys(e),
    a,
    i;
  for (i = 0; i < r.length; i++)
    (a = r[i]), !(t.indexOf(a) >= 0) && (n[a] = e[a]);
  return n;
}
function X0(e) {
  return !!(e.metaKey || e.altKey || e.ctrlKey || e.shiftKey);
}
function J0(e, t) {
  return e.button === 0 && (!t || t === "_self") && !X0(e);
}
const q0 = [
    "onClick",
    "relative",
    "reloadDocument",
    "replace",
    "state",
    "target",
    "to",
    "preventScrollReset",
  ],
  Z0 = [
    "aria-current",
    "caseSensitive",
    "className",
    "end",
    "style",
    "to",
    "children",
  ];
function e1(e) {
  let { basename: t, children: n, window: r } = e,
    a = S.useRef();
  a.current == null && (a.current = Xh({ window: r, v5Compat: !0 }));
  let i = a.current,
    [o, l] = S.useState({ action: i.action, location: i.location });
  return (
    S.useLayoutEffect(() => i.listen(l), [i]),
    S.createElement($0, {
      basename: t,
      children: n,
      location: o.location,
      navigationType: o.action,
      navigator: i,
    })
  );
}
const Pr = S.forwardRef(function (t, n) {
    let {
        onClick: r,
        relative: a,
        reloadDocument: i,
        replace: o,
        state: l,
        target: u,
        to: s,
        preventScrollReset: p,
      } = t,
      f = Cp(t, q0),
      v = D0(s, { relative: a }),
      _ = t1(s, {
        replace: o,
        state: l,
        target: u,
        preventScrollReset: p,
        relative: a,
      });
    function m(h) {
      r && r(h), h.defaultPrevented || _(h);
    }
    return S.createElement(
      "a",
      Va({}, f, { href: v, onClick: i ? r : m, ref: n, target: u })
    );
  }),
  co = S.forwardRef(function (t, n) {
    let {
        "aria-current": r = "page",
        caseSensitive: a = !1,
        className: i = "",
        end: o = !1,
        style: l,
        to: u,
        children: s,
      } = t,
      p = Cp(t, Z0),
      f = mu(u, { relative: p.relative }),
      v = zr(),
      _ = S.useContext(_u),
      { navigator: m } = S.useContext(Dr),
      h = m.encodeLocation ? m.encodeLocation(f).pathname : f.pathname,
      E = v.pathname,
      d =
        _ && _.navigation && _.navigation.location
          ? _.navigation.location.pathname
          : null;
    a ||
      ((E = E.toLowerCase()),
      (d = d ? d.toLowerCase() : null),
      (h = h.toLowerCase()));
    let c = E === h || (!o && E.startsWith(h) && E.charAt(h.length) === "/"),
      g =
        d != null &&
        (d === h || (!o && d.startsWith(h) && d.charAt(h.length) === "/")),
      y = c ? r : void 0,
      j;
    typeof i == "function"
      ? (j = i({ isActive: c, isPending: g }))
      : (j = [i, c ? "active" : null, g ? "pending" : null]
          .filter(Boolean)
          .join(" "));
    let x = typeof l == "function" ? l({ isActive: c, isPending: g }) : l;
    return S.createElement(
      Pr,
      Va({}, p, { "aria-current": y, className: j, ref: n, style: x, to: u }),
      typeof s == "function" ? s({ isActive: c, isPending: g }) : s
    );
  });
var Gs;
(function (e) {
  (e.UseScrollRestoration = "useScrollRestoration"),
    (e.UseSubmitImpl = "useSubmitImpl"),
    (e.UseFetcher = "useFetcher");
})(Gs || (Gs = {}));
var Qs;
(function (e) {
  (e.UseFetchers = "useFetchers"),
    (e.UseScrollRestoration = "useScrollRestoration");
})(Qs || (Qs = {}));
function t1(e, t) {
  let {
      target: n,
      replace: r,
      state: a,
      preventScrollReset: i,
      relative: o,
    } = t === void 0 ? {} : t,
    l = I0(),
    u = zr(),
    s = mu(e, { relative: o });
  return S.useCallback(
    (p) => {
      if (J0(p, n)) {
        p.preventDefault();
        let f = r !== void 0 ? r : Ha(u) === Ha(s);
        l(e, { replace: f, state: a, preventScrollReset: i, relative: o });
      }
    },
    [u, l, s, r, a, n, e, i, o]
  );
}
const pe = Mv;
function n1({ setAuthActive: e }) {
  const t = pe((r) => r.authReducer.isAuth),
    n = zn();
  return O("div", {
    className: "header",
    children: [
      w(Pr, { to: "/KikiTime/", children: w("p", { children: "Домой" }) }),
      w(Pr, {
        to: "/KikiTime/search",
        children: w("p", { children: "Поиск" }),
      }),
      t
        ? w("button", {
            className: "login",
            onClick: () => {
              n({ type: "LOGOUT" });
            },
            children: "Выйти",
          })
        : w("button", {
            className: "login",
            onClick: () => {
              e(!0);
            },
            children: "Войти",
          }),
    ],
  });
}
function r1() {
  const e = pe((l) => l.listFilmsReducer),
    t = pe((l) => l.currentPageReducer),
    n = zn(),
    r = 12,
    a = Math.ceil(e.length / r),
    i = () => {
      if (t < a) n({ type: "NEXT_CURRENT_PAGE" });
      else return;
    },
    o = () => {
      if (t > 1) n({ type: "PREV_CURRENT_PAGE" });
      else return;
    };
  return (
    S.useEffect(() => {
      t > a
        ? n({ type: "ADD_CURRENT_PAGE", payload: a })
        : t === 0 && a > 0 && n({ type: "ADD_CURRENT_PAGE", payload: 1 });
    }),
    O("div", {
      className: "pagination",
      children: [
        O("div", {
          className: "directions",
          children: [
            w("button", {
              className: t > 1 ? "direction active" : "direction",
              onClick: () => o(),
              children: "Назад",
            }),
            w("button", {
              className: t < a ? "direction active" : "direction",
              onClick: () => i(),
              children: "Вперёд",
            }),
          ],
        }),
        O("div", { children: [t, " of ", a] }),
      ],
    })
  );
}
const a1 = (e, t, n) => {
    e.includes(n) || t([...e, n]);
  },
  i1 = (e, t, n) => {
    t(e.filter((r) => r !== n));
  };
function o1({ isReset: e, name: t, genreId: n, genres: r, setGenres: a }) {
  const [i, o] = S.useState(!1);
  return (
    S.useEffect(() => {
      o(!1);
    }, [e]),
    O("li", {
      className: "genre",
      children: [
        w("input", {
          type: "checkbox",
          id: `${n}`,
          checked: i,
          onChange: (l) => {
            i === !1 ? (a1(r, a, n), o(!0)) : (i1(r, a, n), o(!1));
          },
        }),
        w("label", { children: t }),
      ],
    })
  );
}
const gl = [
  { id: 28, name: "боевик" },
  { id: 12, name: "приключения" },
  { id: 16, name: "мультфильм" },
  { id: 35, name: "комедия" },
  { id: 80, name: "криминал" },
  { id: 99, name: "документальный" },
  { id: 18, name: "драма" },
  { id: 10751, name: "семейный" },
  { id: 14, name: "фэнтези" },
  { id: 36, name: "история" },
  { id: 27, name: "ужасы" },
  { id: 10402, name: "музыка" },
  { id: 9648, name: "детектив" },
  { id: 10749, name: "мелодрама" },
  { id: 878, name: "фантастика" },
  { id: 10770, name: "телевизионный фильм" },
  { id: 53, name: "триллер" },
  { id: 10752, name: "военный" },
  { id: 37, name: "вестерн" },
];
function l1() {
  const e = pe((m) => m.authReducer.isAuth),
    t = pe((m) => m.favoritesReducer),
    n = pe((m) => m.watchLaterReducer),
    [r, a] = S.useState([]),
    [i, o] = S.useState("POPULAR_DESCENDING"),
    [l, u] = S.useState("2020"),
    [s, p] = S.useState(""),
    [f, v] = S.useState(!1),
    _ = zn();
  return (
    S.useEffect(() => {
      r.length > 0
        ? (_({ type: l }),
          _({ type: "GENRES", payload: r }),
          _({ type: i }),
          _({ type: s, payload: s === "FAVORITES" ? t : n }))
        : r.length === 0 &&
          (_({ type: l }),
          _({ type: i }),
          _({ type: s, payload: s === "FAVORITES" ? t : n }));
    }),
    O("div", {
      className: "filter",
      children: [
        w("h1", { children: "Фильтры:" }),
        w("button", {
          className: "resetFilter",
          onClick: () => {
            o("POPULAR_DESCENDING"), u("2020"), a([]), p(""), v(!f);
          },
          children: "Сбросить все фильтры",
        }),
        e
          ? O("div", {
              children: [
                w("p", { children: "Сортировать по:" }),
                O("select", {
                  className: "select",
                  onChange: (m) => {
                    p(m.target.value);
                  },
                  value: s,
                  children: [
                    w("option", { value: "", children: "Не выбрано" }),
                    w("option", {
                      value: "WATCH_LATER",
                      children: "Смотреть позже",
                    }),
                    w("option", { value: "FAVORITES", children: "Избранные" }),
                  ],
                }),
              ],
            })
          : null,
        O("div", {
          children: [
            w("p", { children: "Сортировать по:" }),
            O("select", {
              className: "select",
              onChange: (m) => {
                o(m.target.value);
              },
              value: i,
              children: [
                w("option", {
                  value: "POPULAR_DESCENDING",
                  children: "Популярные по убыванию",
                }),
                w("option", {
                  value: "POPULAR_ASCENDING",
                  children: "Популярные по возрастанию",
                }),
                w("option", {
                  value: "RATING_DESCENDING",
                  children: "Рейтинг по убыванию",
                }),
                w("option", {
                  value: "RATING_ASCENDING",
                  children: "Рейтинг по возрастанию",
                }),
              ],
            }),
          ],
        }),
        O("div", {
          children: [
            w("p", { children: "Год релиза:" }),
            O("select", {
              className: "select",
              onChange: (m) => {
                u(m.target.value);
              },
              value: l,
              children: [
                w("option", { value: "2020", children: "2020" }),
                w("option", { value: "2019", children: "2019" }),
                w("option", { value: "2018", children: "2018" }),
                w("option", { value: "2017", children: "2017" }),
              ],
            }),
          ],
        }),
        w("ul", {
          className: "genres",
          children: gl.map((m) =>
            w(
              o1,
              {
                isReset: f,
                genreId: m.id,
                name: m.name,
                genres: r,
                setGenres: a,
              },
              m.id
            )
          ),
        }),
        w(r1, {}),
      ],
    })
  );
}
function u1({ setAuthActive: e }) {
  const t = zn();
  return w("div", {
    className: "popup",
    onClick: (n) => {
      n.target.className === "popup" && e(!1);
    },
    children: O("div", {
      className: "auth",
      children: [
        w("h1", { children: "Авторизация" }),
        O("form", {
          action: "",
          onSubmit: (n) => {
            n.preventDefault(),
              t({
                type: "LOGIN",
                payload: {
                  login: n.target[0].value,
                  password: n.target[1].value,
                },
              }),
              e(!1);
          },
          children: [
            w("input", { type: "text", placeholder: "Логин" }),
            w("input", { type: "password", placeholder: "Пароль" }),
            w("button", { className: "confirmButton", children: "Войти" }),
          ],
        }),
      ],
    }),
  });
}
function s1({ title: e, poster: t, vote: n, setAuthActive: r, filmId: a }) {
  const i = pe((_) => _.authReducer.isAuth),
    o = pe((_) => _.favoritesReducer),
    l = pe((_) => _.watchLaterReducer),
    u = zn(),
    s = () => {
      i
        ? o.includes(a)
          ? u({ type: "REMOVE_FAVORITE", payload: a })
          : u({ type: "ADD_FAVORITE", payload: a })
        : r(!0);
    },
    p = () => {
      i
        ? l.includes(a)
          ? u({ type: "REMOVE_WATCH_LATER", payload: a })
          : u({ type: "ADD_WATCH_LATER", payload: a })
        : r(!0);
    },
    f = () =>
      i
        ? o.includes(a)
          ? w("img", { src: "assets/img/star_active.png", alt: "star" })
          : w("img", { src: "assets/img/star.png", alt: "star" })
        : w("img", { src: "assets/img/star.png", alt: "star" }),
    v = () =>
      i
        ? l.includes(a)
          ? w("img", { src: "assets/img/mark_active.png", alt: "mark" })
          : w("img", { src: "assets/img/mark.png", alt: "mark" })
        : w("img", { src: "assets/img/mark.png", alt: "mark" });
  return O("div", {
    className: "card",
    children: [
      w("img", { src: `https://image.tmdb.org/t/p/w500${t}`, alt: "poster" }),
      O("div", {
        className: "cardContent",
        children: [
          O("div", {
            className: "cardHeader",
            children: [
              O("p", { children: ["Рейтинг: ", n] }),
              w("button", { onClick: () => s(), children: f() }),
              w("button", { onClick: () => p(), children: v() }),
            ],
          }),
          w("h4", { children: e }),
          w("div", {
            className: "cardFooter",
            children: w(Pr, {
              onClick: () => {
                u({ type: "ADD_FILM", payload: a });
              },
              to: `/KikiTime/${a}`,
              children: w("button", { children: "Подробнее" }),
            }),
          }),
        ],
      }),
    ],
  });
}
function c1({ setAuthActive: e }) {
  const t = pe((l) => l.listFilmsReducer),
    n = pe((l) => l.currentPageReducer),
    r = 12,
    a = n * r,
    i = a - r,
    o = t.slice(i, a);
  return w("div", {
    className: "listfilms",
    children: o.map((l) =>
      w(
        s1,
        {
          setAuthActive: e,
          title: l.title,
          filmId: l.id,
          poster: l.poster_path,
          vote: l.vote_average,
        },
        l.id
      )
    ),
  });
}
function d1() {
  const e = pe((l) => l.filmReducer),
    t = e.backdrop_path,
    n = e.poster_path,
    r = e.title,
    a = e.vote_average,
    i = e.overview,
    o = e.id;
  return O("div", {
    className: "film",
    children: [
      O("div", {
        className: "main_content",
        children: [
          w("img", {
            className: "backdrop",
            src: `https://image.tmdb.org/t/p/w500${t}`,
            alt: "backdrop",
          }),
          O("div", {
            className: "poster",
            children: [
              w("img", {
                src: `https://image.tmdb.org/t/p/w500${n}`,
                alt: "poster",
              }),
              O("div", {
                className: "overview",
                children: [
                  w("h1", { children: r }),
                  O("span", { children: ["Рейтинг: ", a] }),
                  w("p", { children: i }),
                ],
              }),
            ],
          }),
        ],
      }),
      O("div", {
        className: "alt_content",
        children: [
          O("div", {
            className: "tabs",
            children: [
              w(co, { to: `/KikiTime/${o}/details`, children: "Детали" }),
              w(co, { to: `/KikiTime/${o}/actors`, children: "Актеры" }),
              w(co, { to: `/KikiTime/${o}/video`, children: "Видео" }),
            ],
          }),
          w(Q0, {}),
        ],
      }),
    ],
  });
}
function en(e) {
  if (e === null || e === !0 || e === !1) return NaN;
  var t = Number(e);
  return isNaN(t) ? t : t < 0 ? Math.ceil(t) : Math.floor(t);
}
function ye(e, t) {
  if (t.length < e)
    throw new TypeError(
      e +
        " argument" +
        (e > 1 ? "s" : "") +
        " required, but only " +
        t.length +
        " present"
    );
}
function va(e) {
  return (
    typeof Symbol == "function" && typeof Symbol.iterator == "symbol"
      ? (va = function (n) {
          return typeof n;
        })
      : (va = function (n) {
          return n &&
            typeof Symbol == "function" &&
            n.constructor === Symbol &&
            n !== Symbol.prototype
            ? "symbol"
            : typeof n;
        }),
    va(e)
  );
}
function Ze(e) {
  ye(1, arguments);
  var t = Object.prototype.toString.call(e);
  return e instanceof Date || (va(e) === "object" && t === "[object Date]")
    ? new Date(e.getTime())
    : typeof e == "number" || t === "[object Number]"
    ? new Date(e)
    : ((typeof e == "string" || t === "[object String]") &&
        typeof console < "u" &&
        (console.warn(
          "Starting with v2.0.0-beta.1 date-fns doesn't accept strings as date arguments. Please use `parseISO` to parse strings. See: https://github.com/date-fns/date-fns/blob/master/docs/upgradeGuide.md#string-arguments"
        ),
        console.warn(new Error().stack)),
      new Date(NaN));
}
function p1(e, t) {
  ye(2, arguments);
  var n = Ze(e).getTime(),
    r = en(t);
  return new Date(n + r);
}
var f1 = {};
function Li() {
  return f1;
}
function g1(e) {
  var t = new Date(
    Date.UTC(
      e.getFullYear(),
      e.getMonth(),
      e.getDate(),
      e.getHours(),
      e.getMinutes(),
      e.getSeconds(),
      e.getMilliseconds()
    )
  );
  return t.setUTCFullYear(e.getFullYear()), e.getTime() - t.getTime();
}
function ha(e) {
  return (
    typeof Symbol == "function" && typeof Symbol.iterator == "symbol"
      ? (ha = function (n) {
          return typeof n;
        })
      : (ha = function (n) {
          return n &&
            typeof Symbol == "function" &&
            n.constructor === Symbol &&
            n !== Symbol.prototype
            ? "symbol"
            : typeof n;
        }),
    ha(e)
  );
}
function v1(e) {
  return (
    ye(1, arguments),
    e instanceof Date ||
      (ha(e) === "object" &&
        Object.prototype.toString.call(e) === "[object Date]")
  );
}
function h1(e) {
  if ((ye(1, arguments), !v1(e) && typeof e != "number")) return !1;
  var t = Ze(e);
  return !isNaN(Number(t));
}
function _1(e, t) {
  ye(2, arguments);
  var n = en(t);
  return p1(e, -n);
}
var m1 = 864e5;
function y1(e) {
  ye(1, arguments);
  var t = Ze(e),
    n = t.getTime();
  t.setUTCMonth(0, 1), t.setUTCHours(0, 0, 0, 0);
  var r = t.getTime(),
    a = n - r;
  return Math.floor(a / m1) + 1;
}
function Ya(e) {
  ye(1, arguments);
  var t = 1,
    n = Ze(e),
    r = n.getUTCDay(),
    a = (r < t ? 7 : 0) + r - t;
  return n.setUTCDate(n.getUTCDate() - a), n.setUTCHours(0, 0, 0, 0), n;
}
function xp(e) {
  ye(1, arguments);
  var t = Ze(e),
    n = t.getUTCFullYear(),
    r = new Date(0);
  r.setUTCFullYear(n + 1, 0, 4), r.setUTCHours(0, 0, 0, 0);
  var a = Ya(r),
    i = new Date(0);
  i.setUTCFullYear(n, 0, 4), i.setUTCHours(0, 0, 0, 0);
  var o = Ya(i);
  return t.getTime() >= a.getTime()
    ? n + 1
    : t.getTime() >= o.getTime()
    ? n
    : n - 1;
}
function w1(e) {
  ye(1, arguments);
  var t = xp(e),
    n = new Date(0);
  n.setUTCFullYear(t, 0, 4), n.setUTCHours(0, 0, 0, 0);
  var r = Ya(n);
  return r;
}
var S1 = 6048e5;
function k1(e) {
  ye(1, arguments);
  var t = Ze(e),
    n = Ya(t).getTime() - w1(t).getTime();
  return Math.round(n / S1) + 1;
}
function Ga(e, t) {
  var n, r, a, i, o, l, u, s;
  ye(1, arguments);
  var p = Li(),
    f = en(
      (n =
        (r =
          (a =
            (i = t == null ? void 0 : t.weekStartsOn) !== null && i !== void 0
              ? i
              : t == null ||
                (o = t.locale) === null ||
                o === void 0 ||
                (l = o.options) === null ||
                l === void 0
              ? void 0
              : l.weekStartsOn) !== null && a !== void 0
            ? a
            : p.weekStartsOn) !== null && r !== void 0
          ? r
          : (u = p.locale) === null ||
            u === void 0 ||
            (s = u.options) === null ||
            s === void 0
          ? void 0
          : s.weekStartsOn) !== null && n !== void 0
        ? n
        : 0
    );
  if (!(f >= 0 && f <= 6))
    throw new RangeError("weekStartsOn must be between 0 and 6 inclusively");
  var v = Ze(e),
    _ = v.getUTCDay(),
    m = (_ < f ? 7 : 0) + _ - f;
  return v.setUTCDate(v.getUTCDate() - m), v.setUTCHours(0, 0, 0, 0), v;
}
function Tp(e, t) {
  var n, r, a, i, o, l, u, s;
  ye(1, arguments);
  var p = Ze(e),
    f = p.getUTCFullYear(),
    v = Li(),
    _ = en(
      (n =
        (r =
          (a =
            (i = t == null ? void 0 : t.firstWeekContainsDate) !== null &&
            i !== void 0
              ? i
              : t == null ||
                (o = t.locale) === null ||
                o === void 0 ||
                (l = o.options) === null ||
                l === void 0
              ? void 0
              : l.firstWeekContainsDate) !== null && a !== void 0
            ? a
            : v.firstWeekContainsDate) !== null && r !== void 0
          ? r
          : (u = v.locale) === null ||
            u === void 0 ||
            (s = u.options) === null ||
            s === void 0
          ? void 0
          : s.firstWeekContainsDate) !== null && n !== void 0
        ? n
        : 1
    );
  if (!(_ >= 1 && _ <= 7))
    throw new RangeError(
      "firstWeekContainsDate must be between 1 and 7 inclusively"
    );
  var m = new Date(0);
  m.setUTCFullYear(f + 1, 0, _), m.setUTCHours(0, 0, 0, 0);
  var h = Ga(m, t),
    E = new Date(0);
  E.setUTCFullYear(f, 0, _), E.setUTCHours(0, 0, 0, 0);
  var d = Ga(E, t);
  return p.getTime() >= h.getTime()
    ? f + 1
    : p.getTime() >= d.getTime()
    ? f
    : f - 1;
}
function E1(e, t) {
  var n, r, a, i, o, l, u, s;
  ye(1, arguments);
  var p = Li(),
    f = en(
      (n =
        (r =
          (a =
            (i = t == null ? void 0 : t.firstWeekContainsDate) !== null &&
            i !== void 0
              ? i
              : t == null ||
                (o = t.locale) === null ||
                o === void 0 ||
                (l = o.options) === null ||
                l === void 0
              ? void 0
              : l.firstWeekContainsDate) !== null && a !== void 0
            ? a
            : p.firstWeekContainsDate) !== null && r !== void 0
          ? r
          : (u = p.locale) === null ||
            u === void 0 ||
            (s = u.options) === null ||
            s === void 0
          ? void 0
          : s.firstWeekContainsDate) !== null && n !== void 0
        ? n
        : 1
    ),
    v = Tp(e, t),
    _ = new Date(0);
  _.setUTCFullYear(v, 0, f), _.setUTCHours(0, 0, 0, 0);
  var m = Ga(_, t);
  return m;
}
var j1 = 6048e5;
function C1(e, t) {
  ye(1, arguments);
  var n = Ze(e),
    r = Ga(n, t).getTime() - E1(n, t).getTime();
  return Math.round(r / j1) + 1;
}
function z(e, t) {
  for (var n = e < 0 ? "-" : "", r = Math.abs(e).toString(); r.length < t; )
    r = "0" + r;
  return n + r;
}
var x1 = {
  y: function (t, n) {
    var r = t.getUTCFullYear(),
      a = r > 0 ? r : 1 - r;
    return z(n === "yy" ? a % 100 : a, n.length);
  },
  M: function (t, n) {
    var r = t.getUTCMonth();
    return n === "M" ? String(r + 1) : z(r + 1, 2);
  },
  d: function (t, n) {
    return z(t.getUTCDate(), n.length);
  },
  a: function (t, n) {
    var r = t.getUTCHours() / 12 >= 1 ? "pm" : "am";
    switch (n) {
      case "a":
      case "aa":
        return r.toUpperCase();
      case "aaa":
        return r;
      case "aaaaa":
        return r[0];
      case "aaaa":
      default:
        return r === "am" ? "a.m." : "p.m.";
    }
  },
  h: function (t, n) {
    return z(t.getUTCHours() % 12 || 12, n.length);
  },
  H: function (t, n) {
    return z(t.getUTCHours(), n.length);
  },
  m: function (t, n) {
    return z(t.getUTCMinutes(), n.length);
  },
  s: function (t, n) {
    return z(t.getUTCSeconds(), n.length);
  },
  S: function (t, n) {
    var r = n.length,
      a = t.getUTCMilliseconds(),
      i = Math.floor(a * Math.pow(10, r - 3));
    return z(i, n.length);
  },
};
const pt = x1;
var on = {
    am: "am",
    pm: "pm",
    midnight: "midnight",
    noon: "noon",
    morning: "morning",
    afternoon: "afternoon",
    evening: "evening",
    night: "night",
  },
  T1 = {
    G: function (t, n, r) {
      var a = t.getUTCFullYear() > 0 ? 1 : 0;
      switch (n) {
        case "G":
        case "GG":
        case "GGG":
          return r.era(a, { width: "abbreviated" });
        case "GGGGG":
          return r.era(a, { width: "narrow" });
        case "GGGG":
        default:
          return r.era(a, { width: "wide" });
      }
    },
    y: function (t, n, r) {
      if (n === "yo") {
        var a = t.getUTCFullYear(),
          i = a > 0 ? a : 1 - a;
        return r.ordinalNumber(i, { unit: "year" });
      }
      return pt.y(t, n);
    },
    Y: function (t, n, r, a) {
      var i = Tp(t, a),
        o = i > 0 ? i : 1 - i;
      if (n === "YY") {
        var l = o % 100;
        return z(l, 2);
      }
      return n === "Yo" ? r.ordinalNumber(o, { unit: "year" }) : z(o, n.length);
    },
    R: function (t, n) {
      var r = xp(t);
      return z(r, n.length);
    },
    u: function (t, n) {
      var r = t.getUTCFullYear();
      return z(r, n.length);
    },
    Q: function (t, n, r) {
      var a = Math.ceil((t.getUTCMonth() + 1) / 3);
      switch (n) {
        case "Q":
          return String(a);
        case "QQ":
          return z(a, 2);
        case "Qo":
          return r.ordinalNumber(a, { unit: "quarter" });
        case "QQQ":
          return r.quarter(a, { width: "abbreviated", context: "formatting" });
        case "QQQQQ":
          return r.quarter(a, { width: "narrow", context: "formatting" });
        case "QQQQ":
        default:
          return r.quarter(a, { width: "wide", context: "formatting" });
      }
    },
    q: function (t, n, r) {
      var a = Math.ceil((t.getUTCMonth() + 1) / 3);
      switch (n) {
        case "q":
          return String(a);
        case "qq":
          return z(a, 2);
        case "qo":
          return r.ordinalNumber(a, { unit: "quarter" });
        case "qqq":
          return r.quarter(a, { width: "abbreviated", context: "standalone" });
        case "qqqqq":
          return r.quarter(a, { width: "narrow", context: "standalone" });
        case "qqqq":
        default:
          return r.quarter(a, { width: "wide", context: "standalone" });
      }
    },
    M: function (t, n, r) {
      var a = t.getUTCMonth();
      switch (n) {
        case "M":
        case "MM":
          return pt.M(t, n);
        case "Mo":
          return r.ordinalNumber(a + 1, { unit: "month" });
        case "MMM":
          return r.month(a, { width: "abbreviated", context: "formatting" });
        case "MMMMM":
          return r.month(a, { width: "narrow", context: "formatting" });
        case "MMMM":
        default:
          return r.month(a, { width: "wide", context: "formatting" });
      }
    },
    L: function (t, n, r) {
      var a = t.getUTCMonth();
      switch (n) {
        case "L":
          return String(a + 1);
        case "LL":
          return z(a + 1, 2);
        case "Lo":
          return r.ordinalNumber(a + 1, { unit: "month" });
        case "LLL":
          return r.month(a, { width: "abbreviated", context: "standalone" });
        case "LLLLL":
          return r.month(a, { width: "narrow", context: "standalone" });
        case "LLLL":
        default:
          return r.month(a, { width: "wide", context: "standalone" });
      }
    },
    w: function (t, n, r, a) {
      var i = C1(t, a);
      return n === "wo" ? r.ordinalNumber(i, { unit: "week" }) : z(i, n.length);
    },
    I: function (t, n, r) {
      var a = k1(t);
      return n === "Io" ? r.ordinalNumber(a, { unit: "week" }) : z(a, n.length);
    },
    d: function (t, n, r) {
      return n === "do"
        ? r.ordinalNumber(t.getUTCDate(), { unit: "date" })
        : pt.d(t, n);
    },
    D: function (t, n, r) {
      var a = y1(t);
      return n === "Do"
        ? r.ordinalNumber(a, { unit: "dayOfYear" })
        : z(a, n.length);
    },
    E: function (t, n, r) {
      var a = t.getUTCDay();
      switch (n) {
        case "E":
        case "EE":
        case "EEE":
          return r.day(a, { width: "abbreviated", context: "formatting" });
        case "EEEEE":
          return r.day(a, { width: "narrow", context: "formatting" });
        case "EEEEEE":
          return r.day(a, { width: "short", context: "formatting" });
        case "EEEE":
        default:
          return r.day(a, { width: "wide", context: "formatting" });
      }
    },
    e: function (t, n, r, a) {
      var i = t.getUTCDay(),
        o = (i - a.weekStartsOn + 8) % 7 || 7;
      switch (n) {
        case "e":
          return String(o);
        case "ee":
          return z(o, 2);
        case "eo":
          return r.ordinalNumber(o, { unit: "day" });
        case "eee":
          return r.day(i, { width: "abbreviated", context: "formatting" });
        case "eeeee":
          return r.day(i, { width: "narrow", context: "formatting" });
        case "eeeeee":
          return r.day(i, { width: "short", context: "formatting" });
        case "eeee":
        default:
          return r.day(i, { width: "wide", context: "formatting" });
      }
    },
    c: function (t, n, r, a) {
      var i = t.getUTCDay(),
        o = (i - a.weekStartsOn + 8) % 7 || 7;
      switch (n) {
        case "c":
          return String(o);
        case "cc":
          return z(o, n.length);
        case "co":
          return r.ordinalNumber(o, { unit: "day" });
        case "ccc":
          return r.day(i, { width: "abbreviated", context: "standalone" });
        case "ccccc":
          return r.day(i, { width: "narrow", context: "standalone" });
        case "cccccc":
          return r.day(i, { width: "short", context: "standalone" });
        case "cccc":
        default:
          return r.day(i, { width: "wide", context: "standalone" });
      }
    },
    i: function (t, n, r) {
      var a = t.getUTCDay(),
        i = a === 0 ? 7 : a;
      switch (n) {
        case "i":
          return String(i);
        case "ii":
          return z(i, n.length);
        case "io":
          return r.ordinalNumber(i, { unit: "day" });
        case "iii":
          return r.day(a, { width: "abbreviated", context: "formatting" });
        case "iiiii":
          return r.day(a, { width: "narrow", context: "formatting" });
        case "iiiiii":
          return r.day(a, { width: "short", context: "formatting" });
        case "iiii":
        default:
          return r.day(a, { width: "wide", context: "formatting" });
      }
    },
    a: function (t, n, r) {
      var a = t.getUTCHours(),
        i = a / 12 >= 1 ? "pm" : "am";
      switch (n) {
        case "a":
        case "aa":
          return r.dayPeriod(i, {
            width: "abbreviated",
            context: "formatting",
          });
        case "aaa":
          return r
            .dayPeriod(i, { width: "abbreviated", context: "formatting" })
            .toLowerCase();
        case "aaaaa":
          return r.dayPeriod(i, { width: "narrow", context: "formatting" });
        case "aaaa":
        default:
          return r.dayPeriod(i, { width: "wide", context: "formatting" });
      }
    },
    b: function (t, n, r) {
      var a = t.getUTCHours(),
        i;
      switch (
        (a === 12
          ? (i = on.noon)
          : a === 0
          ? (i = on.midnight)
          : (i = a / 12 >= 1 ? "pm" : "am"),
        n)
      ) {
        case "b":
        case "bb":
          return r.dayPeriod(i, {
            width: "abbreviated",
            context: "formatting",
          });
        case "bbb":
          return r
            .dayPeriod(i, { width: "abbreviated", context: "formatting" })
            .toLowerCase();
        case "bbbbb":
          return r.dayPeriod(i, { width: "narrow", context: "formatting" });
        case "bbbb":
        default:
          return r.dayPeriod(i, { width: "wide", context: "formatting" });
      }
    },
    B: function (t, n, r) {
      var a = t.getUTCHours(),
        i;
      switch (
        (a >= 17
          ? (i = on.evening)
          : a >= 12
          ? (i = on.afternoon)
          : a >= 4
          ? (i = on.morning)
          : (i = on.night),
        n)
      ) {
        case "B":
        case "BB":
        case "BBB":
          return r.dayPeriod(i, {
            width: "abbreviated",
            context: "formatting",
          });
        case "BBBBB":
          return r.dayPeriod(i, { width: "narrow", context: "formatting" });
        case "BBBB":
        default:
          return r.dayPeriod(i, { width: "wide", context: "formatting" });
      }
    },
    h: function (t, n, r) {
      if (n === "ho") {
        var a = t.getUTCHours() % 12;
        return a === 0 && (a = 12), r.ordinalNumber(a, { unit: "hour" });
      }
      return pt.h(t, n);
    },
    H: function (t, n, r) {
      return n === "Ho"
        ? r.ordinalNumber(t.getUTCHours(), { unit: "hour" })
        : pt.H(t, n);
    },
    K: function (t, n, r) {
      var a = t.getUTCHours() % 12;
      return n === "Ko" ? r.ordinalNumber(a, { unit: "hour" }) : z(a, n.length);
    },
    k: function (t, n, r) {
      var a = t.getUTCHours();
      return (
        a === 0 && (a = 24),
        n === "ko" ? r.ordinalNumber(a, { unit: "hour" }) : z(a, n.length)
      );
    },
    m: function (t, n, r) {
      return n === "mo"
        ? r.ordinalNumber(t.getUTCMinutes(), { unit: "minute" })
        : pt.m(t, n);
    },
    s: function (t, n, r) {
      return n === "so"
        ? r.ordinalNumber(t.getUTCSeconds(), { unit: "second" })
        : pt.s(t, n);
    },
    S: function (t, n) {
      return pt.S(t, n);
    },
    X: function (t, n, r, a) {
      var i = a._originalDate || t,
        o = i.getTimezoneOffset();
      if (o === 0) return "Z";
      switch (n) {
        case "X":
          return Ks(o);
        case "XXXX":
        case "XX":
          return bt(o);
        case "XXXXX":
        case "XXX":
        default:
          return bt(o, ":");
      }
    },
    x: function (t, n, r, a) {
      var i = a._originalDate || t,
        o = i.getTimezoneOffset();
      switch (n) {
        case "x":
          return Ks(o);
        case "xxxx":
        case "xx":
          return bt(o);
        case "xxxxx":
        case "xxx":
        default:
          return bt(o, ":");
      }
    },
    O: function (t, n, r, a) {
      var i = a._originalDate || t,
        o = i.getTimezoneOffset();
      switch (n) {
        case "O":
        case "OO":
        case "OOO":
          return "GMT" + $s(o, ":");
        case "OOOO":
        default:
          return "GMT" + bt(o, ":");
      }
    },
    z: function (t, n, r, a) {
      var i = a._originalDate || t,
        o = i.getTimezoneOffset();
      switch (n) {
        case "z":
        case "zz":
        case "zzz":
          return "GMT" + $s(o, ":");
        case "zzzz":
        default:
          return "GMT" + bt(o, ":");
      }
    },
    t: function (t, n, r, a) {
      var i = a._originalDate || t,
        o = Math.floor(i.getTime() / 1e3);
      return z(o, n.length);
    },
    T: function (t, n, r, a) {
      var i = a._originalDate || t,
        o = i.getTime();
      return z(o, n.length);
    },
  };
function $s(e, t) {
  var n = e > 0 ? "-" : "+",
    r = Math.abs(e),
    a = Math.floor(r / 60),
    i = r % 60;
  if (i === 0) return n + String(a);
  var o = t || "";
  return n + String(a) + o + z(i, 2);
}
function Ks(e, t) {
  if (e % 60 === 0) {
    var n = e > 0 ? "-" : "+";
    return n + z(Math.abs(e) / 60, 2);
  }
  return bt(e, t);
}
function bt(e, t) {
  var n = t || "",
    r = e > 0 ? "-" : "+",
    a = Math.abs(e),
    i = z(Math.floor(a / 60), 2),
    o = z(a % 60, 2);
  return r + i + n + o;
}
const P1 = T1;
var Xs = function (t, n) {
    switch (t) {
      case "P":
        return n.date({ width: "short" });
      case "PP":
        return n.date({ width: "medium" });
      case "PPP":
        return n.date({ width: "long" });
      case "PPPP":
      default:
        return n.date({ width: "full" });
    }
  },
  Pp = function (t, n) {
    switch (t) {
      case "p":
        return n.time({ width: "short" });
      case "pp":
        return n.time({ width: "medium" });
      case "ppp":
        return n.time({ width: "long" });
      case "pppp":
      default:
        return n.time({ width: "full" });
    }
  },
  N1 = function (t, n) {
    var r = t.match(/(P+)(p+)?/) || [],
      a = r[1],
      i = r[2];
    if (!i) return Xs(t, n);
    var o;
    switch (a) {
      case "P":
        o = n.dateTime({ width: "short" });
        break;
      case "PP":
        o = n.dateTime({ width: "medium" });
        break;
      case "PPP":
        o = n.dateTime({ width: "long" });
        break;
      case "PPPP":
      default:
        o = n.dateTime({ width: "full" });
        break;
    }
    return o.replace("{{date}}", Xs(a, n)).replace("{{time}}", Pp(i, n));
  },
  O1 = { p: Pp, P: N1 };
const R1 = O1;
var L1 = ["D", "DD"],
  M1 = ["YY", "YYYY"];
function D1(e) {
  return L1.indexOf(e) !== -1;
}
function I1(e) {
  return M1.indexOf(e) !== -1;
}
function Js(e, t, n) {
  if (e === "YYYY")
    throw new RangeError(
      "Use `yyyy` instead of `YYYY` (in `"
        .concat(t, "`) for formatting years to the input `")
        .concat(
          n,
          "`; see: https://github.com/date-fns/date-fns/blob/master/docs/unicodeTokens.md"
        )
    );
  if (e === "YY")
    throw new RangeError(
      "Use `yy` instead of `YY` (in `"
        .concat(t, "`) for formatting years to the input `")
        .concat(
          n,
          "`; see: https://github.com/date-fns/date-fns/blob/master/docs/unicodeTokens.md"
        )
    );
  if (e === "D")
    throw new RangeError(
      "Use `d` instead of `D` (in `"
        .concat(t, "`) for formatting days of the month to the input `")
        .concat(
          n,
          "`; see: https://github.com/date-fns/date-fns/blob/master/docs/unicodeTokens.md"
        )
    );
  if (e === "DD")
    throw new RangeError(
      "Use `dd` instead of `DD` (in `"
        .concat(t, "`) for formatting days of the month to the input `")
        .concat(
          n,
          "`; see: https://github.com/date-fns/date-fns/blob/master/docs/unicodeTokens.md"
        )
    );
}
var z1 = {
    lessThanXSeconds: {
      one: "less than a second",
      other: "less than {{count}} seconds",
    },
    xSeconds: { one: "1 second", other: "{{count}} seconds" },
    halfAMinute: "half a minute",
    lessThanXMinutes: {
      one: "less than a minute",
      other: "less than {{count}} minutes",
    },
    xMinutes: { one: "1 minute", other: "{{count}} minutes" },
    aboutXHours: { one: "about 1 hour", other: "about {{count}} hours" },
    xHours: { one: "1 hour", other: "{{count}} hours" },
    xDays: { one: "1 day", other: "{{count}} days" },
    aboutXWeeks: { one: "about 1 week", other: "about {{count}} weeks" },
    xWeeks: { one: "1 week", other: "{{count}} weeks" },
    aboutXMonths: { one: "about 1 month", other: "about {{count}} months" },
    xMonths: { one: "1 month", other: "{{count}} months" },
    aboutXYears: { one: "about 1 year", other: "about {{count}} years" },
    xYears: { one: "1 year", other: "{{count}} years" },
    overXYears: { one: "over 1 year", other: "over {{count}} years" },
    almostXYears: { one: "almost 1 year", other: "almost {{count}} years" },
  },
  U1 = function (t, n, r) {
    var a,
      i = z1[t];
    return (
      typeof i == "string"
        ? (a = i)
        : n === 1
        ? (a = i.one)
        : (a = i.other.replace("{{count}}", n.toString())),
      r != null && r.addSuffix
        ? r.comparison && r.comparison > 0
          ? "in " + a
          : a + " ago"
        : a
    );
  };
const A1 = U1;
function po(e) {
  return function () {
    var t = arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : {},
      n = t.width ? String(t.width) : e.defaultWidth,
      r = e.formats[n] || e.formats[e.defaultWidth];
    return r;
  };
}
var F1 = {
    full: "EEEE, MMMM do, y",
    long: "MMMM do, y",
    medium: "MMM d, y",
    short: "MM/dd/yyyy",
  },
  B1 = {
    full: "h:mm:ss a zzzz",
    long: "h:mm:ss a z",
    medium: "h:mm:ss a",
    short: "h:mm a",
  },
  W1 = {
    full: "{{date}} 'at' {{time}}",
    long: "{{date}} 'at' {{time}}",
    medium: "{{date}}, {{time}}",
    short: "{{date}}, {{time}}",
  },
  H1 = {
    date: po({ formats: F1, defaultWidth: "full" }),
    time: po({ formats: B1, defaultWidth: "full" }),
    dateTime: po({ formats: W1, defaultWidth: "full" }),
  };
const b1 = H1;
var V1 = {
    lastWeek: "'last' eeee 'at' p",
    yesterday: "'yesterday at' p",
    today: "'today at' p",
    tomorrow: "'tomorrow at' p",
    nextWeek: "eeee 'at' p",
    other: "P",
  },
  Y1 = function (t, n, r, a) {
    return V1[t];
  };
const G1 = Y1;
function Gn(e) {
  return function (t, n) {
    var r = n != null && n.context ? String(n.context) : "standalone",
      a;
    if (r === "formatting" && e.formattingValues) {
      var i = e.defaultFormattingWidth || e.defaultWidth,
        o = n != null && n.width ? String(n.width) : i;
      a = e.formattingValues[o] || e.formattingValues[i];
    } else {
      var l = e.defaultWidth,
        u = n != null && n.width ? String(n.width) : e.defaultWidth;
      a = e.values[u] || e.values[l];
    }
    var s = e.argumentCallback ? e.argumentCallback(t) : t;
    return a[s];
  };
}
var Q1 = {
    narrow: ["B", "A"],
    abbreviated: ["BC", "AD"],
    wide: ["Before Christ", "Anno Domini"],
  },
  $1 = {
    narrow: ["1", "2", "3", "4"],
    abbreviated: ["Q1", "Q2", "Q3", "Q4"],
    wide: ["1st quarter", "2nd quarter", "3rd quarter", "4th quarter"],
  },
  K1 = {
    narrow: ["J", "F", "M", "A", "M", "J", "J", "A", "S", "O", "N", "D"],
    abbreviated: [
      "Jan",
      "Feb",
      "Mar",
      "Apr",
      "May",
      "Jun",
      "Jul",
      "Aug",
      "Sep",
      "Oct",
      "Nov",
      "Dec",
    ],
    wide: [
      "January",
      "February",
      "March",
      "April",
      "May",
      "June",
      "July",
      "August",
      "September",
      "October",
      "November",
      "December",
    ],
  },
  X1 = {
    narrow: ["S", "M", "T", "W", "T", "F", "S"],
    short: ["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"],
    abbreviated: ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"],
    wide: [
      "Sunday",
      "Monday",
      "Tuesday",
      "Wednesday",
      "Thursday",
      "Friday",
      "Saturday",
    ],
  },
  J1 = {
    narrow: {
      am: "a",
      pm: "p",
      midnight: "mi",
      noon: "n",
      morning: "morning",
      afternoon: "afternoon",
      evening: "evening",
      night: "night",
    },
    abbreviated: {
      am: "AM",
      pm: "PM",
      midnight: "midnight",
      noon: "noon",
      morning: "morning",
      afternoon: "afternoon",
      evening: "evening",
      night: "night",
    },
    wide: {
      am: "a.m.",
      pm: "p.m.",
      midnight: "midnight",
      noon: "noon",
      morning: "morning",
      afternoon: "afternoon",
      evening: "evening",
      night: "night",
    },
  },
  q1 = {
    narrow: {
      am: "a",
      pm: "p",
      midnight: "mi",
      noon: "n",
      morning: "in the morning",
      afternoon: "in the afternoon",
      evening: "in the evening",
      night: "at night",
    },
    abbreviated: {
      am: "AM",
      pm: "PM",
      midnight: "midnight",
      noon: "noon",
      morning: "in the morning",
      afternoon: "in the afternoon",
      evening: "in the evening",
      night: "at night",
    },
    wide: {
      am: "a.m.",
      pm: "p.m.",
      midnight: "midnight",
      noon: "noon",
      morning: "in the morning",
      afternoon: "in the afternoon",
      evening: "in the evening",
      night: "at night",
    },
  },
  Z1 = function (t, n) {
    var r = Number(t),
      a = r % 100;
    if (a > 20 || a < 10)
      switch (a % 10) {
        case 1:
          return r + "st";
        case 2:
          return r + "nd";
        case 3:
          return r + "rd";
      }
    return r + "th";
  },
  e_ = {
    ordinalNumber: Z1,
    era: Gn({ values: Q1, defaultWidth: "wide" }),
    quarter: Gn({
      values: $1,
      defaultWidth: "wide",
      argumentCallback: function (t) {
        return t - 1;
      },
    }),
    month: Gn({ values: K1, defaultWidth: "wide" }),
    day: Gn({ values: X1, defaultWidth: "wide" }),
    dayPeriod: Gn({
      values: J1,
      defaultWidth: "wide",
      formattingValues: q1,
      defaultFormattingWidth: "wide",
    }),
  };
const t_ = e_;
function Qn(e) {
  return function (t) {
    var n = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {},
      r = n.width,
      a = (r && e.matchPatterns[r]) || e.matchPatterns[e.defaultMatchWidth],
      i = t.match(a);
    if (!i) return null;
    var o = i[0],
      l = (r && e.parsePatterns[r]) || e.parsePatterns[e.defaultParseWidth],
      u = Array.isArray(l)
        ? r_(l, function (f) {
            return f.test(o);
          })
        : n_(l, function (f) {
            return f.test(o);
          }),
      s;
    (s = e.valueCallback ? e.valueCallback(u) : u),
      (s = n.valueCallback ? n.valueCallback(s) : s);
    var p = t.slice(o.length);
    return { value: s, rest: p };
  };
}
function n_(e, t) {
  for (var n in e) if (e.hasOwnProperty(n) && t(e[n])) return n;
}
function r_(e, t) {
  for (var n = 0; n < e.length; n++) if (t(e[n])) return n;
}
function a_(e) {
  return function (t) {
    var n = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {},
      r = t.match(e.matchPattern);
    if (!r) return null;
    var a = r[0],
      i = t.match(e.parsePattern);
    if (!i) return null;
    var o = e.valueCallback ? e.valueCallback(i[0]) : i[0];
    o = n.valueCallback ? n.valueCallback(o) : o;
    var l = t.slice(a.length);
    return { value: o, rest: l };
  };
}
var i_ = /^(\d+)(th|st|nd|rd)?/i,
  o_ = /\d+/i,
  l_ = {
    narrow: /^(b|a)/i,
    abbreviated: /^(b\.?\s?c\.?|b\.?\s?c\.?\s?e\.?|a\.?\s?d\.?|c\.?\s?e\.?)/i,
    wide: /^(before christ|before common era|anno domini|common era)/i,
  },
  u_ = { any: [/^b/i, /^(a|c)/i] },
  s_ = {
    narrow: /^[1234]/i,
    abbreviated: /^q[1234]/i,
    wide: /^[1234](th|st|nd|rd)? quarter/i,
  },
  c_ = { any: [/1/i, /2/i, /3/i, /4/i] },
  d_ = {
    narrow: /^[jfmasond]/i,
    abbreviated: /^(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)/i,
    wide: /^(january|february|march|april|may|june|july|august|september|october|november|december)/i,
  },
  p_ = {
    narrow: [
      /^j/i,
      /^f/i,
      /^m/i,
      /^a/i,
      /^m/i,
      /^j/i,
      /^j/i,
      /^a/i,
      /^s/i,
      /^o/i,
      /^n/i,
      /^d/i,
    ],
    any: [
      /^ja/i,
      /^f/i,
      /^mar/i,
      /^ap/i,
      /^may/i,
      /^jun/i,
      /^jul/i,
      /^au/i,
      /^s/i,
      /^o/i,
      /^n/i,
      /^d/i,
    ],
  },
  f_ = {
    narrow: /^[smtwf]/i,
    short: /^(su|mo|tu|we|th|fr|sa)/i,
    abbreviated: /^(sun|mon|tue|wed|thu|fri|sat)/i,
    wide: /^(sunday|monday|tuesday|wednesday|thursday|friday|saturday)/i,
  },
  g_ = {
    narrow: [/^s/i, /^m/i, /^t/i, /^w/i, /^t/i, /^f/i, /^s/i],
    any: [/^su/i, /^m/i, /^tu/i, /^w/i, /^th/i, /^f/i, /^sa/i],
  },
  v_ = {
    narrow: /^(a|p|mi|n|(in the|at) (morning|afternoon|evening|night))/i,
    any: /^([ap]\.?\s?m\.?|midnight|noon|(in the|at) (morning|afternoon|evening|night))/i,
  },
  h_ = {
    any: {
      am: /^a/i,
      pm: /^p/i,
      midnight: /^mi/i,
      noon: /^no/i,
      morning: /morning/i,
      afternoon: /afternoon/i,
      evening: /evening/i,
      night: /night/i,
    },
  },
  __ = {
    ordinalNumber: a_({
      matchPattern: i_,
      parsePattern: o_,
      valueCallback: function (t) {
        return parseInt(t, 10);
      },
    }),
    era: Qn({
      matchPatterns: l_,
      defaultMatchWidth: "wide",
      parsePatterns: u_,
      defaultParseWidth: "any",
    }),
    quarter: Qn({
      matchPatterns: s_,
      defaultMatchWidth: "wide",
      parsePatterns: c_,
      defaultParseWidth: "any",
      valueCallback: function (t) {
        return t + 1;
      },
    }),
    month: Qn({
      matchPatterns: d_,
      defaultMatchWidth: "wide",
      parsePatterns: p_,
      defaultParseWidth: "any",
    }),
    day: Qn({
      matchPatterns: f_,
      defaultMatchWidth: "wide",
      parsePatterns: g_,
      defaultParseWidth: "any",
    }),
    dayPeriod: Qn({
      matchPatterns: v_,
      defaultMatchWidth: "any",
      parsePatterns: h_,
      defaultParseWidth: "any",
    }),
  };
const m_ = __;
var y_ = {
  code: "en-US",
  formatDistance: A1,
  formatLong: b1,
  formatRelative: G1,
  localize: t_,
  match: m_,
  options: { weekStartsOn: 0, firstWeekContainsDate: 1 },
};
const w_ = y_;
var S_ = /[yYQqMLwIdDecihHKkms]o|(\w)\1*|''|'(''|[^'])+('|$)|./g,
  k_ = /P+p+|P+|p+|''|'(''|[^'])+('|$)|./g,
  E_ = /^'([^]*?)'?$/,
  j_ = /''/g,
  C_ = /[a-zA-Z]/;
function x_(e, t, n) {
  var r, a, i, o, l, u, s, p, f, v, _, m, h, E, d, c, g, y;
  ye(2, arguments);
  var j = String(t),
    x = Li(),
    P =
      (r =
        (a = n == null ? void 0 : n.locale) !== null && a !== void 0
          ? a
          : x.locale) !== null && r !== void 0
        ? r
        : w_,
    N = en(
      (i =
        (o =
          (l =
            (u = n == null ? void 0 : n.firstWeekContainsDate) !== null &&
            u !== void 0
              ? u
              : n == null ||
                (s = n.locale) === null ||
                s === void 0 ||
                (p = s.options) === null ||
                p === void 0
              ? void 0
              : p.firstWeekContainsDate) !== null && l !== void 0
            ? l
            : x.firstWeekContainsDate) !== null && o !== void 0
          ? o
          : (f = x.locale) === null ||
            f === void 0 ||
            (v = f.options) === null ||
            v === void 0
          ? void 0
          : v.firstWeekContainsDate) !== null && i !== void 0
        ? i
        : 1
    );
  if (!(N >= 1 && N <= 7))
    throw new RangeError(
      "firstWeekContainsDate must be between 1 and 7 inclusively"
    );
  var W = en(
    (_ =
      (m =
        (h =
          (E = n == null ? void 0 : n.weekStartsOn) !== null && E !== void 0
            ? E
            : n == null ||
              (d = n.locale) === null ||
              d === void 0 ||
              (c = d.options) === null ||
              c === void 0
            ? void 0
            : c.weekStartsOn) !== null && h !== void 0
          ? h
          : x.weekStartsOn) !== null && m !== void 0
        ? m
        : (g = x.locale) === null ||
          g === void 0 ||
          (y = g.options) === null ||
          y === void 0
        ? void 0
        : y.weekStartsOn) !== null && _ !== void 0
      ? _
      : 0
  );
  if (!(W >= 0 && W <= 6))
    throw new RangeError("weekStartsOn must be between 0 and 6 inclusively");
  if (!P.localize)
    throw new RangeError("locale must contain localize property");
  if (!P.formatLong)
    throw new RangeError("locale must contain formatLong property");
  var L = Ze(e);
  if (!h1(L)) throw new RangeError("Invalid time value");
  var Ce = g1(L),
    It = _1(L, Ce),
    zt = {
      firstWeekContainsDate: N,
      weekStartsOn: W,
      locale: P,
      _originalDate: L,
    },
    Ur = j
      .match(k_)
      .map(function (ge) {
        var Me = ge[0];
        if (Me === "p" || Me === "P") {
          var ct = R1[Me];
          return ct(ge, P.formatLong);
        }
        return ge;
      })
      .join("")
      .match(S_)
      .map(function (ge) {
        if (ge === "''") return "'";
        var Me = ge[0];
        if (Me === "'") return T_(ge);
        var ct = P1[Me];
        if (ct)
          return (
            !(n != null && n.useAdditionalWeekYearTokens) &&
              I1(ge) &&
              Js(ge, t, String(e)),
            !(n != null && n.useAdditionalDayOfYearTokens) &&
              D1(ge) &&
              Js(ge, t, String(e)),
            ct(It, ge, P.localize, zt)
          );
        if (Me.match(C_))
          throw new RangeError(
            "Format string contains an unescaped latin alphabet character `" +
              Me +
              "`"
          );
        return ge;
      })
      .join("");
  return Ur;
}
function T_(e) {
  var t = e.match(E_);
  return t ? t[1].replace(j_, "'") : e;
}
function P_() {
  const e = pe((i) => i.filmReducer),
    t = x_(new Date(), "yyyy-MM-dd"),
    n = e.release_date,
    r = t < n ? "No released" : "Released",
    a = e.original_language;
  return O("div", {
    className: "details",
    children: [
      O("div", {
        className: "status",
        children: [w("h3", { children: "Статус" }), w("span", { children: r })],
      }),
      O("div", {
        className: "status",
        children: [
          w("h3", { children: "Дата выхода" }),
          w("span", { children: n }),
        ],
      }),
      O("div", {
        className: "status",
        children: [
          w("h3", { children: "Язык оригинала" }),
          w("span", { children: a }),
        ],
      }),
    ],
  });
}
function N_() {
  return pe((e) => e.filmReducer), w("div", { className: "actors" });
}
function O_() {
  return pe((e) => e.filmReducer), w("div", { className: "video" });
}
function R_() {
  const e = pe((h) => h.searchReducer),
    [t, n] = S.useState(""),
    [r, a] = S.useState(""),
    [i, o] = S.useState(""),
    [l, u] = S.useState(0),
    [s, p] = S.useState(!1),
    [f, v] = S.useState(
      "Выберите жанр, оценку и популярность, и тогда предложим вам фильм, который соотвествует вашим желаниям!"
    ),
    _ = () =>
      gl
        .map((E) => {
          if (e[l].genre_ids.some((d) => d === E.id)) return E.name;
        })
        .filter((E) => E !== void 0),
    m = zn();
  return (
    S.useEffect(() => {
      t && r && i
        ? (m({ type: "GENRE", payload: +t }),
          m({ type: r }),
          m({ type: i }),
          u(0),
          v("К сожалению, больше фильмов не найдено"),
          p(!0))
        : (v(
            "Выберите жанр, оценку и популярность, и тогда предложим вам фильм, который соотвествует вашим желаниям!"
          ),
          u(0),
          p(!1));
    }, [t, r, i]),
    O("div", {
      className: "search",
      children: [
        O("div", {
          className: "form",
          children: [
            "Жанр",
            O("select", {
              className: "select",
              onChange: (h) => {
                n(h.target.value);
              },
              value: t,
              children: [
                w("option", { value: "", children: "Не выбрано" }),
                gl.map((h) =>
                  w("option", { value: h.id, children: h.name }, h.id)
                ),
              ],
            }),
            "Оценка",
            O("select", {
              className: "select",
              onChange: (h) => {
                a(h.target.value);
              },
              value: r,
              children: [
                w("option", { value: "", children: "Не выбрано" }),
                w("option", { value: "HIGH", children: "Высокая" }),
                w("option", { value: "LOW", children: "Низкая" }),
              ],
            }),
            "Популярность",
            O("select", {
              className: "select",
              onChange: (h) => {
                o(h.target.value);
              },
              value: i,
              children: [
                w("option", { value: "", children: "Не выбрано" }),
                w("option", { value: "POPULAR", children: "Популярный" }),
                w("option", { value: "UNKNOWN", children: "Неизвестный" }),
              ],
            }),
          ],
        }),
        s === !0 && l !== e.length
          ? O(tf, {
              children: [
                w(
                  "div",
                  {
                    className: "film",
                    children: O("div", {
                      className: "poster",
                      children: [
                        w("img", {
                          src: `https://image.tmdb.org/t/p/w500${e[l].poster_path}`,
                          alt: "poster",
                        }),
                        O("div", {
                          className: "overview",
                          children: [
                            w("h1", { children: e[l].title }),
                            O("div", {
                              className: "genres",
                              children: [
                                w("h4", {
                                  style: { flex: 0, padding: 1, margin: 0 },
                                  children: "Жанры:",
                                }),
                                _().map((h, E) =>
                                  E + 1 === _().length
                                    ? O("p", { children: [h, "."] }, E)
                                    : O("p", { children: [h, ","] }, E)
                                ),
                              ],
                            }),
                            w("p", { children: e[l].overview }),
                          ],
                        }),
                      ],
                    }),
                  },
                  e[l].id
                ),
                O("div", {
                  className: "choice",
                  children: [
                    w("button", {
                      className: "no",
                      onClick: () => {
                        u(l + 1);
                      },
                      children: "Не подходит",
                    }),
                    w(Pr, {
                      onClick: () => {
                        m({ type: "ADD_FILM", payload: e[l].id });
                      },
                      to: `/KikiTime/${e[l].id}`,
                      children: w("button", {
                        className: "yes",
                        children: "Подходит",
                      }),
                    }),
                  ],
                }),
              ],
            })
          : w("div", { className: "film", children: w("h1", { children: f }) }),
      ],
    })
  );
}
function L_() {
  const [e, t] = S.useState(!1);
  return w(e1, {
    children: O("div", {
      className: "App",
      children: [
        e ? w(u1, { setAuthActive: t }) : null,
        w(n1, { setAuthActive: t }),
        O(K0, {
          children: [
            w(Ht, {
              path: "/KikiTime/",
              element: O("div", {
                className: "container",
                children: [w(l1, {}), w(c1, { setAuthActive: t })],
              }),
            }),
            O(Ht, {
              path: "/KikiTime/:filmId",
              element: w(d1, {}),
              children: [
                w(Ht, {
                  path: "/KikiTime/:filmId/details",
                  element: w(P_, {}),
                }),
                w(Ht, { path: "/KikiTime/:filmId/actors", element: w(N_, {}) }),
                w(Ht, { path: "/KikiTime/:filmId/video", element: w(O_, {}) }),
              ],
            }),
            w(Ht, { path: "/KikiTime/search", element: w(R_, {}) }),
          ],
        }),
      ],
    }),
  });
}
go.createRoot(document.getElementById("root")).render(
  w(yl.StrictMode, { children: w(Kv, { store: Kh, children: w(L_, {}) }) })
);
